...
| M5 Docker pipeline | complete | 3 / 3 | M5 closed @ 174d88862; AC-8 PROVEN (tree-hash 6709428a…); NFR-3 RESOLVED by issue #2 via slim/full split (`:slim` 371 MB + `:latest` ~4.5 GB; see AGENTS.md § Image variants). Issue #2 review-repair pass wired s6-supervised dashboard + main-argo services, restored legacy `/init main-wrapper.sh` ENTRYPOINT, and added the runtime apt set (ripgrep / openssh-client / gcc / python3-dev / libffi-dev) that lazy_deps + ssh-clone profiles depend on. |
...
**Known GAP for M6** (parity suite): **RESOLVED by issue #2.**
- This M5.1 image was SLIM (371 MB, python:3.13-slim-bookworm + `git` + `ca-certificates` in the runtime stage). It did NOT match the legacy v0.14.0 image surface (4.71 GB with node/npm, playwright, ffmpeg, s6-overlay supervisor). M5's intent was zero-leakage + functional `argo --help` / `argo --version`, NOT full runtime parity.
- **Resolution (issue #2, branch `argo/issue-2-image-full-variant`):** the Dockerfile now ships two variants via a three-stage layout (`builder` → `runtime-slim` → `runtime-full`). `runtime-slim` is unchanged (the 371 MB CLI image, now tagged `:slim`). `runtime-full` extends slim with `nodejs`/`npm` (TUI dashboard), `ffmpeg` (voice mode), `npx playwright install --with-deps chromium --only-shell` (browser tools), and s6-overlay (multi-arch with checksum verification, mirrors legacy) — tagged `:latest` / `:<sha-short>` / `:v<X.Y.Z>` (the customer-facing defaults). `make image-full` builds the full variant locally. `.github/workflows/docker-publish.yml` publishes both variants on every event. `tools/parity_runner.py`'s `DEFAULT_NEW_IMAGE` now points at `:dev-full` so the parity gate runs apples-to-apples against legacy. **NFR-3 is RESOLVED.**
- **Review-repair commit (issue #2, follow-up).** The first issue-#2 commit (`1f8606a1`) installed s6-overlay but registered no services and left `ENTRYPOINT ["/init"]` bare — a leading-dash arg like `docker run <img> --tui` would be intercepted by /init's POSIX shell. This was caught in architect review. The repair pass:
    1. Wired the s6-rc.d/dashboard + s6-rc.d/main-argo services from the upstream-managed `docker/` tree (which the rename engine lifts into `dist/argo/docker/` as `main-argo` / `ARGO_DASHBOARD`). Dashboard is gated by `ARGO_DASHBOARD={1,true,yes}` via a `finish`-script-as-permanent-failure marker, mirroring legacy review item I3.
    2. Restored `ENTRYPOINT ["/init", "/opt/argo/docker/main-wrapper.sh"]` so leading-dash args (`--tui`, `--version`) reach argo via main-wrapper's `s6-setuidgid argo argo "$@"` instead of being intercepted by /init.
    3. Added cont-init.d/01-argo-setup (UID remap + volume chown + skills sync via stage2-hook.sh), 015-supervise-perms (chowns supervise/ trees for unprivileged argo-user s6 control), and 02-reconcile-profiles (rebuilds dynamic /run/service/ slots from $ARGO_HOME/profiles/ on restart).
    4. Added missing runtime apt: `ripgrep` (file_operations.py shells to `rg`), `openssh-client` (profile_distribution.py clones ssh:// URLs), `gcc` + `python3-dev` + `libffi-dev` (tools/lazy_deps.py compiles platform extras at first use).
    5. Documented the base-image divergence vs legacy (`python:3.13-slim-bookworm` vs legacy `debian:13.4` trixie) in the Dockerfile header — glibc/libssl skew is wheel-resolution surface only; behaviour is identical at runtime.
