...
- **NFR-3. Image size.** Final image MUST NOT exceed legacy image by >5%. Resolved by issue #2 via a slim/full split: `:slim` is the 371 MB CLI runtime (FR-16 surfaces only); `:latest` = `:full` matches legacy v0.14.0's customer surface (~4.5 GB) including node/npm, ffmpeg, playwright/chromium, s6-overlay supervision (dashboard + main-argo services), and the legacy ENTRYPOINT contract (`/init /opt/argo/docker/main-wrapper.sh`). **Base-image divergence vs legacy.** The slim+full stages build on `python:3.13-slim-bookworm` (Debian 12 + upstream CPython slim variant); legacy builds on `debian:13.4` (trixie) + apt's `python3`. This was retained intentionally for issue #2 — bookworm's stable LTS + python-slim posture lets pip install -e . resolve prebuilt wheels for psutil/pydantic-core/cryptography without the full build-essential toolchain in runtime. Expected effects: glibc 2.36 (bookworm) vs 2.39 (trixie) and OpenSSL 3.0 (bookworm) vs 3.2 (trixie) — wheel-resolution surface only, no behaviour delta for upstream's runtime deps. Documented in `Dockerfile`'s header comment. Re-evaluation triggers: a wheel-skew bug in the customer-parity surface; an upstream pin that requires glibc >2.36.
