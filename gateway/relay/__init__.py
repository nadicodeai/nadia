"""Relay/connector support package for the Nadia gateway.
...
def relay_connection_auth() -> tuple[Optional[str], Optional[str]]:
    ...
    Both come from enrollment (``nadia gateway enroll`` writes them to
    ``~/.nadia/.env``): ``GATEWAY_RELAY_ID`` identifies the enrolled instance,
...
def relay_endpoint() -> Optional[str]:
    ...
    ``NADIA_DASHBOARD_PUBLIC_URL``); a hosted/NAS container has the same var
...
def relay_wake_url() -> Optional[str]:
    ...
    sets it explicitly (or passes ``--wake-url`` to ``nadia gateway enroll``).
...
def self_provision_relay() -> bool:
    ...
    is already present, AND the agent can resolve its own Nadia access token. In
    that case the runtime resolves the agent's own Nadia access token (the same
    ...
    up. The creds live ONLY in process memory — never written to ``~/.nadia/.env``.
    ...
    sets neither ``NADIA_MANAGED`` nor a ``.managed`` marker), so gating on it
    ...
    self-guarding:
      ...
      - A self-hosted operator who ran ``nadia gateway enroll``: has a PINNED
    ...
    try:
        ...
        logger.warning("relay self-provision skipped: could not resolve Nadia token (%s)", exc)
        return False
    ...
    try:
        ...
        host = ""
    gateway_id = os.environ.get("GATEWAY_RELAY_ID", "").strip() or f"gw-{host or 'nadia'}"
