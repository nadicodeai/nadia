...
# ``NADIA_AUTO_CONTINUE_FRESHNESS`` at startup.
...
# ``sessions_dir / f"{session_id}.json"`` in nadia_state, request-dump
...
def _discord_tools_loaded() -> bool:
    ...
    Two conditions must hold:
      1. The `discord` or `discord_admin` toolset is enabled for the
         Discord platform via `nadia tools` (opt-in, default OFF).
...
def build_session_context_prompt(
    ...
    *,
    ...
    if context.source.platform == Platform.SLACK:
        ...
        # `discord` / `discord_admin` via `nadia tools` AND the bot
    ...
    lines.append(
        f"- `\"local\"` → Save to local files only ({display_nadia_home()}/cron/output/)"
    )
    ...
    def _save(self) -> None:
        ...
        data = {
            "_README": (
                ...
                "~/.nadia/state.db and are shown by `nadia sessions list` and "
    ...
    def get_or_create_session(
        ...
        with self._lock:
            ...
            if session_key in self._entries and not force_new:
                ...
                # (nadia_state.find_latest_gateway_session_for_peer) selects
