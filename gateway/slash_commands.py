...
Module-level run.py helpers a handler needs (``_nadia_home``,
...
from nadia_cli.config import cfg_get, clear_model_endpoint_credentials
from utils import (
    atomic_json_write,
    atomic_yaml_write,
...
def _model_switch_skew_guard() -> Optional[str]:
    ...
    return t(
        ...
        error=t(
            "gateway.model.stale_gateway_restart",
            boot_rev=boot_rev,
            disk_rev=disk_rev,
        ),
    )
...
class GatewaySlashCommandsMixin:
    ...
    def _typed_command_prefix_for(self, platform) -> str:
        """Return the prefix users can always type to reach Nadia commands.
    ...
    async def _handle_whoami_command(self, event: MessageEvent) -> str:
        ...
        scope = (
            t("gateway.whoami.scope_dm")
            if chat_type.lower() in {"dm", "direct", "private", ""}
            else t("gateway.whoami.scope_group")
        )
        user_id = (source.user_id if source else None) or "?"

        if not policy.enabled:
            return t(
                "gateway.whoami.unrestricted",
                platform=platform,
                scope=scope,
                user_id=user_id,
            )
        ...
        for c in floor + configured:
            if c not in seen:
                ...
                runnable.append(c)
        runnable_str = (
            ", ".join(f"/{c}" for c in runnable)
            if runnable
            else t("gateway.whoami.none")
        )
        return t(
            "gateway.whoami.user",
            platform=platform,
    ...
    async def _handle_kanban_command(self, event: MessageEvent) -> str:
        ...
        import re
        import shlex
        from nadia_cli.kanban import run_slash
    ...
    async def _handle_platform_command(self, event: MessageEvent) -> str:
        ...
        if action == "list":
            lines = [t("gateway.platform.header")]
            connected = sorted(p.value for p in self.adapters.keys())
            if connected:
                lines.append(t("gateway.platform.connected", platforms=", ".join(connected)))
            else:
                lines.append(t("gateway.platform.connected_none"))
            failed = getattr(self, "_failed_platforms", {}) or {}
            if failed:
                for p, info in failed.items():
                    if info.get("paused"):
                        reason = info.get("pause_reason") or t("gateway.platform.reason_paused")
                        lines.append(
                            t("gateway.platform.failed_paused", platform=p.value, reason=reason)
                        )
                    else:
                        attempts = info.get("attempts", 0)
                        lines.append(
                            t("gateway.platform.failed_retrying", platform=p.value, attempts=attempts)
                        )
            else:
                lines.append(t("gateway.platform.failed_none"))
            return "\n".join(lines)
        ...
        if action in {"pause", "resume"}:
            ...
            if action == "pause":
                if platform not in failed:
                    return t("gateway.platform.pause_not_in_retry_queue", platform=platform.value)
                ...
                return t("gateway.platform.paused", platform=platform.value)
            # action == "resume"
            if platform not in failed:
                return t("gateway.platform.resume_not_in_retry_queue", platform=platform.value)
            if not failed[platform].get("paused"):
                return t("gateway.platform.already_retrying", platform=platform.value)
            self._resume_paused_platform(platform)
            return t("gateway.platform.resumed", platform=platform.value)

        return t("gateway.platform.usage")

    async def _handle_restart_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        """Handle /restart command - drain active work, then restart the gateway."""
        from gateway.run import _nadia_home
        ...
        try:
            ...
            atomic_json_write(
                _nadia_home / ".restart_notify.json",
                notify_data,
                indent=None,
            )
        ...
        try:
            ...
            atomic_json_write(
                _nadia_home / ".restart_last_processed.json",
                dedup_data,
                indent=None,
            )
        except Exception as e:
    ...
    async def _handle_version_command(self, event: MessageEvent) -> str:
        """Handle /version — show the running Nadia Agent version."""
    ...
    async def _handle_help_command(self, event: MessageEvent) -> str:
        ...
        from nadia_cli.commands import gateway_help_lines
        lines = [
    ...
    async def _handle_model_command(self, event: MessageEvent) -> Optional[str]:
        ...
        """
        from gateway.run import _nadia_home, _load_gateway_config
        import yaml
            ...
            if has_picker:
                ...
                if providers:
                    ...
                    async def _on_model_selected(
                        ...
                        try:
                            from nadia_cli.context_switch_guard import (
                        ...
                        if cached_entry and cached_entry[0] is not None:
                            try:
                                ...
                                return t(
                                    "gateway.model.error_prefix",
                                    error=t(
                                        "gateway.model.switch_failed_staying",
        ...
        if _cost_warning is not None:
            async def _on_cost_confirm(choice: str) -> str:
                if choice == "cancel":
                    return t(
                        "gateway.model.expensive_cancelled",
            ...
            return await self._request_slash_confirm(
                event=event,
                command="model",
                title=t("gateway.model.expensive_title"),
                message=t(
                    "gateway.model.expensive_confirm_prompt",
    ...
    async def _handle_codex_runtime_command(self, event: MessageEvent) -> str:
        ...
        Same surface as the CLI handler in cli.py:
            ...
            /codex-runtime auto             — Nadia default runtime
        ...
        if errors:
            return t("gateway.codex_runtime.errors", errors="\n❌ ".join(errors))
        ...
        key = "gateway.codex_runtime.success" if result.success else "gateway.codex_runtime.failed"
        return t(key, message=result.message)
    ...
    async def _handle_personality_command(self, event: MessageEvent) -> str:
        ...
        if not personalities:
            return t("gateway.personality.none_configured", path=display_nadia_home())
    ...
    async def _handle_goal_command(self, event: "MessageEvent") -> str:
        ...
        if lower == "show":
            return t(
                "gateway.goal.completion_contract",
                status=mgr.status_line(),
                contract=mgr.render_contract(),
            )
        ...
        if lower == "wait" or lower.startswith("wait "):
            ...
            if not wait_arg:
                return t("gateway.goal.wait_usage")
            wtokens = wait_arg.split(None, 1)
            try:
                pid = int(wtokens[0])
            except ValueError:
                return t("gateway.goal.wait_pid_invalid")
            reason = wtokens[1].strip() if len(wtokens) > 1 else ""
            try:
                mgr.wait_on(pid, reason=reason)
            except (RuntimeError, ValueError) as exc:
                return t("gateway.goal.wait_error", error=exc)
            rtxt = f" ({reason})" if reason else ""
            return t("gateway.goal.wait_parked", pid=pid, reason=rtxt)

        # /goal unwait — clear the wait barrier.
        if lower == "unwait":
            if mgr.stop_waiting():
                return t("gateway.goal.wait_cleared")
            return t("gateway.goal.no_wait_barrier")
        ...
        if lower.startswith("draft"):
            ...
            if not objective:
                return t("gateway.goal.draft_usage")
            try:
                import asyncio
                from nadia_cli.goals import draft_contract
            ...
            from nadia_cli.goals import parse_contract
        ...
        if state.has_contract():
            return t(
                "gateway.goal.completion_contract",
                status=base,
                contract=state.contract.render_block(),
            )
        if lower.startswith("draft"):
            # Drafting was requested but the aux model couldn't produce one.
            return t("gateway.goal.draft_failed_freeform", base=base)
        return base
    ...
    async def _handle_subgoal_command(self, event: "MessageEvent") -> str:
        ...
        if mgr is None:
            return t("gateway.goal.unavailable")
        if not mgr.has_goal():
            return t("gateway.subgoal.no_active")

        # No args → list current subgoals.
        if not args:
            return t(
                "gateway.subgoal.list",
                status=mgr.status_line(),
                subgoals=mgr.render_subgoals(),
            )
        ...
        if verb == "remove":
            if not rest:
                return t("gateway.subgoal.remove_usage")
            try:
                idx = int(rest.split()[0])
            except ValueError:
                return t("gateway.subgoal.remove_invalid")
            try:
                removed = mgr.remove_subgoal(idx)
            except (IndexError, RuntimeError) as exc:
                return t("gateway.subgoal.remove_error", error=exc)
            return t("gateway.subgoal.removed", index=idx, text=removed)

        if verb == "clear":
            try:
                prev = mgr.clear_subgoals()
            except RuntimeError as exc:
                return t("gateway.subgoal.clear_error", error=exc)
            if prev:
                key = "gateway.subgoal.cleared_one" if prev == 1 else "gateway.subgoal.cleared_many"
        ...
        try:
            ...
            return t("gateway.subgoal.add_error", error=exc)
    ...
    async def _handle_rollback_command(self, event: MessageEvent) -> str:
        ...
        try:
            import yaml as _y
            _cfg_path = _nadia_home / "config.yaml"
            if _cfg_path.exists():
    ...
    async def _handle_reasoning_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _nadia_home, _platform_config_key
        import yaml
    ...
    async def _handle_memory_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _nadia_home
        from nadia_cli.write_approval_commands import handle_pending_subcommand
        ...
        if out is None:
            out = t("gateway.memory.unknown_subcommand")
        return out
    ...
    async def _handle_skills_command(self, event: MessageEvent) -> str:
        ...
        the pending JSON file under ``~/.nadia/pending/skills/``. (Note this is
        the write-approval ``diff <id>``; the CLI also has an unrelated
        ``nadia skills diff <name>`` that diffs a bundled skill vs stock.)
        """
        from gateway.run import _nadia_home
        from nadia_cli.write_approval_commands import handle_pending_subcommand
        from tools import write_approval as wa
        ...
        if not gate_on and not wants_toggle and wa.pending_count(wa.SKILLS) == 0:
            return t("gateway.skills.approval_off")
        ...
        # the real review surface. (Note: `nadia skills diff <name>` is a
        ...
        if args and args[0].lower() == "diff" and len(out) > 3000:
            ...
            out = (
                out[:3000]
                + t("gateway.skills.diff_truncated", pending_id=pending_id)
            )
        return out

    async def _handle_fast_command(self, event: MessageEvent) -> str:
        """Handle /fast — mirror the CLI Priority Processing toggle in gateway chats."""
        from gateway.run import _nadia_home, _load_gateway_config, _resolve_gateway_model
        import yaml
    ...
    async def _handle_verbose_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _nadia_home, _load_gateway_config, _platform_config_key
        ...
        try:
            ...
            atomic_yaml_write(config_path, user_config)
            return t(
                "gateway.verbose.saved",
                description=descriptions[new_mode],
                platform=platform_key,
            )
        except Exception as e:
            logger.warning("Failed to save tool_progress mode: %s", e)
            return t(
                "gateway.verbose.save_failed_with_description",
                description=descriptions[new_mode],
                error=e,
            )
    ...
    async def _handle_footer_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _nadia_home, _load_gateway_config, _platform_config_key, _resolve_gateway_model
    ...
    async def _handle_sessions_command(self, event: MessageEvent) -> str:
        ...
        from nadia_cli.session_listing import (
        ...
        return format_gateway_session_listing(
            ...
            title=(
                t("gateway.sessions.title_all")
                if include_unnamed
                else t("gateway.sessions.title_named")
            ),
        )
    ...
    async def _handle_credits_command(self, event: MessageEvent) -> str:
        """Handle /credits -- show Nadia credit balance and the top-up handoff.
        ...
        lines: list[str] = [t("gateway.credits.header")]
        ...
        if view.topup_url:
            lines.append("")
            lines.append(t("gateway.credits.top_up", url=view.topup_url))
            lines.append(t("gateway.credits.browser_completion"))
        return "\n".join(lines)
    ...
    async def _handle_usage_command(self, event: MessageEvent) -> str:
        ...
        # ── Nadia credits magnitudes + monthly-grant % gauge ─────────────
        ...
        # Run off the event loop. The helper gates on "a Nadia account is logged in"
        # — NOT the inference provider and NOT nested under `if provider:` — so a
        # Nadia-credentialled user running inference elsewhere (or with none resident)
    ...
    async def _handle_bundles_command(self, event: MessageEvent) -> str:
        ...
        try:
            ...
            return t("gateway.bundles.unavailable", error=exc)

        bundles = list_bundles()
        if not bundles:
            return t("gateway.bundles.none_installed", directory=_bundles_dir())

        lines = [t("gateway.bundles.header", count=len(bundles)), ""]
        for info in bundles:
            skill_count = len(info.get("skills", []))
            desc = info.get("description") or t("gateway.bundles.default_description", count=skill_count)
            lines.append(
                t(
                    "gateway.bundles.item",
                    slug=info["slug"],
                    description=desc,
                    count=skill_count,
                )
            ...
            for s in info.get("skills", []):
                lines.append(t("gateway.bundles.skill_item", skill=s))
        lines.append("")
        lines.append(t("gateway.bundles.invoke"))
        return "\n".join(lines)
    ...
    async def _handle_debug_command(self, event: MessageEvent) -> str:
        ...
        full log uploads should use ``nadia debug share`` from the CLI.
        """
        import asyncio
        from nadia_cli.debug import (
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        """Handle /update command — update Nadia Agent to the latest version.

        Spawns ``nadia update`` in a detached session (via ``setsid``) so it
        survives the gateway restart that ``nadia update`` may trigger. Marker
        files are written so either the current gateway process or the next one
        can notify the user when the update finishes.
        """
        from gateway.run import _nadia_home, _resolve_nadia_bin
        import json
        import shutil
        import subprocess
        from datetime import datetime
        from nadia_cli.config import is_managed, format_managed_message
        ...
        if is_managed():
            return t(
                "gateway.update.managed",
                message=format_managed_message("update Nadia Agent"),
            )
        ...
        if not nadia_cmd:
            return t("gateway.update.nadia_cmd_not_found")
        ...
        # Spawn `nadia update --gateway` detached so it survives gateway restart.
        ...
        # gateway can stream it to the messenger in near-real-time.
        #
        # Windows: no bash/setsid chain.  Run `nadia update --gateway`
        ...
        try:
            if sys.platform == "win32":
                ...
                # nadia_cmd is a list of argv parts we can pass directly
                ...
                nadia_cmd_str = " ".join(shlex.quote(part) for part in nadia_cmd)
                update_cmd = (
                    f"PYTHONUNBUFFERED=1 {nadia_cmd_str} update --gateway"
