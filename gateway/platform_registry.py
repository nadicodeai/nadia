...
class PlatformEntry:
    ...
    # Env vars this platform needs (for ``nadia setup`` display).
    ...
    # built-ins).  Used by ``nadia gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # plugin-discovery time added several seconds to *every* `nadia`
        # invocation -- including plain `nadia chat`, which never touches any
        ...
        # `nadia setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``nadia setup``/``gateway status``, channel
