...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    for nadia_root in (_NADIA_HOME, _NADIA_ROOT):
        denied.append(nadia_root / ".env")
        denied.append(nadia_root / "auth.json")
        denied.append(nadia_root / "credentials")
        denied.append(nadia_root / "config.yaml")
    return denied
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    # The denylist still blocks /etc, /proc, ~/.ssh, ~/.aws, ~/.nadia/.env,
    # ~/.nadia/auth.json, etc. — so the obvious prompt-injection sites
