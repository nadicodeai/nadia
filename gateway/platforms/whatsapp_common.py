...
class WhatsAppBehaviorMixin:
    ...
    DEFAULT_REPLY_PREFIX: str = "⚕ *Nadia Agent*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    """Resolve the WhatsApp bridge directory, mirroring to NADIA_HOME if needed.

    When the install tree is read-only (e.g., Docker /opt/nadia), this function
    mirrors the bridge source to a writable NADIA_HOME location and returns that
    ...
    # Try NADIA_HOME location first
    nadia_home = get_nadia_home()
    nadia_home_bridge = nadia_home / "scripts" / "whatsapp-bridge"
    ...
    # Install dir is read-only, mirror to NADIA_HOME if needed
    if nadia_home_bridge.exists():
        return nadia_home_bridge

    # Mirror the bridge source to NADIA_HOME
    try:
        nadia_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(
            install_bridge,
            nadia_home_bridge,
            dirs_exist_ok=False,
