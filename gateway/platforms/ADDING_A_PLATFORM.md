...
There are two ways to add a platform to the Nadia gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.nadia/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Nadia code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `nadia gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `nadia_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Nadia core.
...
And add it to the `nadia-gateway` composite:

```python
"nadia-gateway": {
    "includes": [..., "nadia-your-platform"]
}
```
...
## 12. Status Display (`nadia_cli/status.py`)
...
## 13. Gateway Setup Wizard (`nadia_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ nadia_cli/ toolsets.py \
  --include="*.py" -l | sort -u
