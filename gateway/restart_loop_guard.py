...
Defenses 1 and 2 (the ``_NADIA_GATEWAY`` guard on ``nadia gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.nadia.gateway")``,
...
State lives in ``<NADIA_HOME>/gateway/restart_loop.json`` so it is
...
def _state_path():
    return get_nadia_home() / "gateway" / "restart_loop.json"
