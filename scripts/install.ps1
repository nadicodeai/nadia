# ============================================================================
# Nadia Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [string]$Commit = "",
    [string]$Tag = "",
    [string]$NadiaHome = $(if ($env:NADIA_HOME) { $env:NADIA_HOME } else { "$env:LOCALAPPDATA\nadia" }),
    [string]$InstallDir = $(if ($env:NADIA_HOME) { "$env:NADIA_HOME\nadia-agent" } else { "$env:LOCALAPPDATA\nadia\nadia-agent" }),
    ...
    # builds apps/desktop into a launchable Nadia Agent.exe.
    #
    # Why opt-in:
    #   * Nadia-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Nadia Agent.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Nadia Agent.exe
    ...
    #     `nadia desktop` already builds on demand.
...
$RepoUrlSsh = "git@github.com:nadicodeai/nadia.git"
$RepoUrlHttps = "https://github.com/nadicodeai/nadia.git"
$PythonVersion = "3.11"
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $NadiaHome)) {
        New-Item -ItemType Directory -Force -Path $NadiaHome | Out-Null
    }
    $envFile = Join-Path $NadiaHome ".env"
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $NadiaHome "node"
...
function Install-Uv {
    # Nadia owns its own uv at $NadiaHome\bin\uv.exe.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (nadia_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `nadia update` stay in sync.
    $managedUv = Join-Path $NadiaHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $NadiaHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $NadiaHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $NadiaHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $NadiaHome "bin"
...
function Resolve-UvCmd {
    ...
    # sibling process and NADIA_HOME wasn't propagated).
...
function Resolve-AvailablePythonVersion {
    ...
    # ``$script:PythonVersion = $fallbackVer`` mutation.  Under Nadia-Setup.exe
...
function Install-Git {
    ...
    which Nadia uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\nadia\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Nadia needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Nadia copy of Git ourselves is predictable and
    ...
    ``NADIA_GIT_BASH_PATH`` (User scope) so Nadia can find it in a fresh
    ...
    # Download PortableGit into $NadiaHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $NadiaHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Nadia features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Nadia needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``NADIA_GIT_BASH_PATH`` (User env scope) so Nadia can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $NadiaHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $NadiaHome\git\usr\bin\bash.exe
    $candidates += "$NadiaHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$NadiaHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("NADIA_GIT_BASH_PATH", $candidate, "User")
            $env:NADIA_GIT_BASH_PATH = $candidate
            Write-Info "Set NADIA_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Nadia may not find Git Bash."
    Write-Info "If needed, set NADIA_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    $managedNode = "$NadiaHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$NadiaHome\node;$env:Path"
        Write-Success "Node.js $version found (Nadia-managed)"
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing Nadia-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $NadiaHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $NadiaHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            if ($extractedDir) {
                if (Test-Path "$NadiaHome\node") { Remove-Item -Recurse -Force "$NadiaHome\node" }
                Move-Item $extractedDir.FullName "$NadiaHome\node"
                ...
                $version = & "$NadiaHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $NadiaHome\node\ (portable, user-scoped)"
                $script:HasNode = $true
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\nadia-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `nadia
                    # update` path (#4735).
                    ...
                    $stashName = "nadia-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($Commit) {
                    ...
                    # ff-only pull cannot succeed — mirror ``nadia update`` and
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if ($LASTEXITCODE -eq 0) {
                            ...
                            Write-Warn "Review git diff / git status if Nadia behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running nadia processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/nadicodeai/nadia/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/nadicodeai/nadia/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/nadicodeai/nadia/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\nadia-agent-$zipLabel.zip"
    ...
    # next `nadia update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under Nadia-Setup.exe
    ...
    if (Test-Path "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running nadia process.
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running nadia processes before recreating venv..."
            # The launcher CLI (nadia.exe) plus its child tree.
            & taskkill /F /T /IM nadia.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM nadia.exe is NOT enough: the gateway/agent that a
            # scheduled task or watchdog autostarts runs as
            # `pythonw.exe -m nadia_cli.main gateway run` straight out of
            # venv\Scripts\, so its image name is python/pythonw, not nadia.exe.
            ...
            # user (see nadia_cli/gateway_windows.py), so the installer always
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `nadia.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install nadia-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and nadia.exe broken with
    ...
    # here, not 30 seconds later when the user runs `nadia`.
    if (-not $NoVenv) {
    ...
    if (-not $NoVenv) {
        # uv on Windows can register nadia.exe in dist-info/RECORD but fail to
        ...
        # `nadia` command while nadia-agent.exe / nadia-acp.exe exist
...
"@ 2>$null
            if ($LASTEXITCODE -eq 0 -and $scriptNames) {
                ...
                if ($missing.Count -gt 0) {
                    ...
                    if ($stillMissing.Count -gt 0) {
                        ...
                        Write-Info "Workaround: `"$pythonExe`" -m nadia_cli.main <command>"
                    } else {
    ...
    # users hit and lazy-import errors from `nadia dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `nadia dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `nadia dashboard` should now work."
            } else {
...
function Set-PathVariable {
    Write-Info "Setting up nadia command..."
    
    if ($NoVenv) {
        $nadiaBin = "$InstallDir"
    } else {
        $nadiaBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so nadia is globally available
    # On Windows, the nadia.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$nadiaBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$nadiaBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $nadiaBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set NADIA_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\nadia instead
    # of the Unix default ~/.nadia
    $currentNadiaHome = [Environment]::GetEnvironmentVariable("NADIA_HOME", "User")
    if (-not $currentNadiaHome -or $currentNadiaHome -ne $NadiaHome) {
        [Environment]::SetEnvironmentVariable("NADIA_HOME", $NadiaHome, "User")
        Write-Success "Set NADIA_HOME=$NadiaHome"
    }
    $env:NADIA_HOME = $NadiaHome
    
    # Update current session
    $env:Path = "$nadiaBin;$env:Path"
    
    Write-Success "nadia command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.nadia-bootstrap-complete which tells the Nadia
    ...
    $markerPath = Join-Path $InstallDir ".nadia-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the NADIA_HOME directory structure ($NadiaHome, default %LOCALAPPDATA%\nadia)
    New-Item -ItemType Directory -Force -Path "$NadiaHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$NadiaHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$NadiaHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$NadiaHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$NadiaHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$NadiaHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$NadiaHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$NadiaHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$NadiaHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$NadiaHome\.env"
    ...
    # Create config.yaml
    $configPath = "$NadiaHome\config.yaml"
    ...
    # (the default PS5 behaviour), and Nadia's prompt-injection scanner
    ...
    # PowerShell version.
    $soulPath = "$NadiaHome\SOUL.md"
    if (-not (Test-Path $soulPath)) {
        # MUST match DEFAULT_SOUL_MD in nadia_cli/default_soul.py. The runtime
...
"@
    ...
    Write-Success "Configuration directory ready: $NadiaHome"
    
    # Seed bundled skills into $NadiaHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $NadiaHome\skills ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to $NadiaHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$NadiaHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $NadiaHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Nadia-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'nadia setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\nadia-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\nadia-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
...
# the final `electron` -> `Nadia` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable Nadia Agent.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_nadia_desktop command
    # resolves apps/desktop/release/win-unpacked/Nadia Agent.exe directly,
    ...
    # enforces the build floor (^20.19 || >=22.12) and prepends the Nadia-managed
    ...
    # The --dir mode produces an unpacked Nadia Agent.exe in
    ...
    # are NOT trying to work around). The Nadia icon + product name are
    # stamped onto Nadia Agent.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\nadia-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Nadia Agent.exe",
        "$desktopDir\release\win-arm64-unpacked\Nadia Agent.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Nadia Agent.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Nadia icon + identity are stamped onto Nadia Agent.exe by the
    ...
    #    Nadia Agent.exe. We deliberately do NOT point them at `nadia desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Nadia.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Nadia.lnk')
        )
        ...
        # with the Nadia icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.nadia/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'nadia setup'."
        return
    }
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m nadia_cli.main setup
    } else {
        python -m nadia_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $nadiaCmd = "$InstallDir\venv\Scripts\nadia.exe"
    if (-not (Test-Path $nadiaCmd)) {
        $nadiaCmd = "nadia"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$NadiaHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'nadia whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: nadia gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$NadiaHome\logs\gateway.log"
            Start-Process -FilePath $nadiaCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$NadiaHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: nadia gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: nadia gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$NadiaHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$NadiaHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$NadiaHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$NadiaHome\nadia-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   nadia              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   nadia setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   nadia config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   nadia config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   nadia gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   nadia update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Nadia repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
if ($IncludeDesktop) {
    ...
    # (Nadia-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Nadia to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/nadicodeai/nadia/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
