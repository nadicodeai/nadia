#!/usr/bin/env python3
"""Live test harness for Nadia Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any nadia imports.
ORIGINAL_HOME = os.environ.get("NADIA_HOME")
ORIGINAL_AUTH = Path.home() / ".nadia" / "auth.json"
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.nadia/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.nadia/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="nadia_ts_live_"))
    nadia_home = home_dir / ".nadia"
    nadia_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, nadia_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".nadia" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, nadia_home / ".env")
        ...
        load_nadia_dotenv(nadia_home=str(Path.home() / ".nadia"))
    ...
    (nadia_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return nadia_home
...
def reset_module_state():
    """Drop cached modules so the new NADIA_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "nadia_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["NADIA_HOME"] = str(home)
...
def main():
    ...
    # Restore original NADIA_HOME
    if ORIGINAL_HOME is not None:
        os.environ["NADIA_HOME"] = ORIGINAL_HOME
    else:
        os.environ.pop("NADIA_HOME", None)
