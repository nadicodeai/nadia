...
# Bootstrap Open WebUI against Nadia Agent's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.nadia/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-nadia.sh
...
#   NADIA_API_PORT=8642
#   NADIA_API_HOST=127.0.0.1
#   NADIA_API_MODEL_NAME='Nadia Agent'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-Nadia Agent WebUI}"
...
NADIA_ENV_FILE="${NADIA_ENV_FILE:-$HOME/.nadia/.env}"
NADIA_API_PORT="${NADIA_API_PORT:-8642}"
NADIA_API_HOST="${NADIA_API_HOST:-127.0.0.1}"
NADIA_API_CONNECT_HOST="${NADIA_API_CONNECT_HOST:-127.0.0.1}"
NADIA_API_MODEL_NAME="${NADIA_API_MODEL_NAME:-Nadia Agent}"
NADIA_API_BASE_URL="http://${NADIA_API_CONNECT_HOST}:${NADIA_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-nadia.sh"
LOG_DIR="$HOME/.nadia/logs"
...
write_launcher() {
  ...
  quoted_base_url="$(shell_quote "$NADIA_API_BASE_URL")"
...
ensure_env_permissions() {
  chmod 600 "$NADIA_ENV_FILE" 2>/dev/null || true
}

install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.nadia.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.nadia</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.nadia"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.nadia"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-nadia.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to Nadia Agent
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-nadia.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.nadia/logs/openwebui.log
StandardError=append:%h/.nadia/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-nadia.service
}
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$NADIA_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
    api_key="$(generate_secret)"
  fi

  log 'Ensuring Nadia API server is configured...'
  upsert_env API_SERVER_ENABLED true "$NADIA_ENV_FILE"
  upsert_env API_SERVER_HOST "$NADIA_API_HOST" "$NADIA_ENV_FILE"
  upsert_env API_SERVER_PORT "$NADIA_API_PORT" "$NADIA_ENV_FILE"
  upsert_env API_SERVER_MODEL_NAME "$NADIA_API_MODEL_NAME" "$NADIA_ENV_FILE"
  upsert_env API_SERVER_KEY "$api_key" "$NADIA_ENV_FILE"
  ensure_env_permissions

  log 'Restarting Nadia gateway so API server settings take effect...'
  nadia gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${NADIA_API_CONNECT_HOST}:${NADIA_API_PORT}/health" >/dev/null; then
    log 'Nadia API server did not answer on the first check. Trying to start gateway in the background...'
    nohup nadia gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  curl -fsS "http://${NADIA_API_CONNECT_HOST}:${NADIA_API_PORT}/health" >/dev/null
  ...
  log "Nadia API endpoint: ${NADIA_API_BASE_URL}"
