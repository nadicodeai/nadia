#!/usr/bin/env bash
# Canonical test runner for nadia-agent. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.nadia/...)
#
# Usage:
...
# ── Activate venv ───────────────────────────────────────────────────────────
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.nadia/nadia-agent/venv"; do
...
if [ -f "$HOME/.nadia/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.nadia"
...
exec env -i \
  ...
  ${NADIA_RUN_SLOW_PET_TESTS:+NADIA_RUN_SLOW_PET_TESTS="$NADIA_RUN_SLOW_PET_TESTS"} \
