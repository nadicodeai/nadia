...
(``hermes-cli/<version>``). The blanket hermes->argo rebrand rewrote them to
``argo-*``, which the Nous portal REJECTS — device-code login then fails in the
field with the reported symptom "cannot join the Nous portal, it says argo-cli".
...
the test's EXPECTED literal in lock-step with the source — both became argo-* and
...
_WIRE_PRESERVED = [
    ...
    ('_HERMES_USER_AGENT = f"hermes-cli/{_HERMES_VERSION}"',
     '_ARGO_USER_AGENT = f"hermes-cli/{_ARGO_VERSION}"'),
...
def test_nous_wire_identifiers_survive_the_rebrand(rename, wire_in: str, expected: str) -> None:
    assert rename(wire_in) == expected, (
        ...
        f"  want: {expected!r}\nThe Nous backend keys on the literal value; argo-* is rejected "
        "(this is the 'cannot join the Nous portal, it says argo-cli' bug)."
    )


# --- The OVERLOADED toolset key MUST still rebrand (no over-preserve) ----------
# `hermes-cli` is ALSO the internal default toolset key. It must become argo-cli
# so the product is internally consistent; a too-broad skip_context that preserved
# it would leave a "hermes-cli" toolset in `argo tools` — the hermes/argo mixture.

@pytest.mark.parametrize(
    "toolset_in, expected_out",
    [
        ('    "hermes-cli": {', '    "argo-cli": {'),                        # toolsets.py registry key
        ('default_toolset="hermes-cli"', 'default_toolset="argo-cli"'),       # platforms.py
        ('config.get("toolsets", ["hermes-cli"])', 'config.get("toolsets", ["argo-cli"])'),  # config/dump default
        ('if toolset == "hermes-cli":', 'if toolset == "argo-cli":'),          # delegate_tool comparison
        ('"cli": "hermes-cli",', '"cli": "argo-cli",'),                        # nous_subscription mapping
    ],
)
