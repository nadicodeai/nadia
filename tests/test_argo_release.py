"""tests/test_argo_release.py — pure-function coverage for tools/argo_release.py.

Mirrors the upstream pure helpers cited in the module docstring of
``tools/argo_release.py`` (``upstream/scripts/release.py:1390-1436``):
...
Run with::
    ...
    pytest tests/test_argo_release.py -v
"""
...
# Load tools/argo_release.py by absolute path. Register in sys.modules under
...
_MODULE_PATH = _REPO_ROOT / "tools" / "argo_release.py"
_MODULE_NAME = "argo_release_under_test"
_spec = importlib.util.spec_from_file_location(_MODULE_NAME, _MODULE_PATH)
assert _spec is not None and _spec.loader is not None
argo_release = importlib.util.module_from_spec(_spec)
sys.modules[_MODULE_NAME] = argo_release
_spec.loader.exec_module(argo_release)
...
class TestBumpSemver:
    def test_patch_increments_z(self) -> None:
        assert argo_release._bump_semver("0.14.0", "patch") == "0.14.1"

    def test_patch_from_nonzero_z(self) -> None:
        assert argo_release._bump_semver("0.14.7", "patch") == "0.14.8"

    def test_minor_zeros_z_and_increments_y(self) -> None:
        assert argo_release._bump_semver("0.14.7", "minor") == "0.15.0"

    def test_major_zeros_y_z_and_increments_x(self) -> None:
        assert argo_release._bump_semver("0.14.7", "major") == "1.0.0"

    def test_unknown_part_raises(self) -> None:
        with pytest.raises(argo_release.VersionParseError):
            argo_release._bump_semver("0.14.0", "build")

    def test_non_triple_raises(self) -> None:
        with pytest.raises(argo_release.VersionParseError):
            argo_release._bump_semver("0.14", "patch")

    def test_non_integer_component_raises(self) -> None:
        with pytest.raises(argo_release.VersionParseError):
            argo_release._bump_semver("0.14.x", "patch")
...
class TestFormatTodayCalver:
    def test_no_zero_padding_single_digit_month(self) -> None:
        assert argo_release._format_today_calver(date(2026, 5, 28)) == "2026.5.28"

    def test_no_zero_padding_single_digit_day(self) -> None:
        assert argo_release._format_today_calver(date(2026, 5, 1)) == "2026.5.1"

    def test_double_digit_month_and_day(self) -> None:
        assert argo_release._format_today_calver(date(2026, 12, 31)) == "2026.12.31"

    def test_default_calls_date_today(self) -> None:
        # Just shape — current year / month / day are non-zero-padded.
        out = argo_release._format_today_calver()
        parts = out.split(".")
        assert len(parts) == 3
...
class TestValidators:
    ...
    def test_calver_accepts_valid(self, value: str) -> None:
        argo_release._validate_calver(value)  # no raise
    ...
    def test_semver_rejects_invalid(self, value: str) -> None:
        with pytest.raises(argo_release.VersionParseError):
            argo_release._validate_semver(value)
...
_INIT_FIXTURE = '''"""Argo CLI - Unified command-line interface for Argo Agent."""
...
class TestRewriteVersionFile:
    def test_substitutes_both_strings(self) -> None:
        out = argo_release._rewrite_version_file(
            _INIT_FIXTURE,
            new_version="0.14.1",
...
class TestRewritePyprojectVersion:
    def test_rewrites_top_level_version_only(self) -> None:
        out = argo_release._rewrite_pyproject_version(
    ...
    def test_preserves_other_content(self) -> None:
        ...
        assert 'name = "argo-agent"' in out
        assert "[tool.uv]" in out

    def test_no_match_returns_unchanged(self) -> None:
        text = "[project]\nname = \"argo-agent\"\n"
        out = argo_release._rewrite_pyproject_version(text, new_version="9.9.9")
        assert out == text
...
class TestResolveRepoRoot:
    def test_raises_outside_git_repo(self, tmp_path: Path) -> None:
        with pytest.raises(argo_release.NotAGitRepoError):
            argo_release._resolve_repo_root(tmp_path)
