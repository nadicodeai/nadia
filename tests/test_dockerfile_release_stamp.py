...
`make build` regenerates dist/nadia/ from pristine upstream/, so the image's
nadia_cli/__init__.py carries UPSTREAM's __release_date__ unless the build
re-stamps the Nadia release date. build.py._stamp_release_date does that from
$NADIA_RELEASE_DATE (the one value a hermetic docker build can't read off the git
...
def fake_dist(tmp_path, monkeypatch):
    ...
    init_path = tmp_path / "nadia_cli" / "__init__.py"
...
def test_stamp_overwrites_upstream_date_when_env_set(fake_dist, monkeypatch):
    monkeypatch.setenv("NADIA_RELEASE_DATE", "2026.6.5")
...
def test_stamp_is_noop_when_env_unset(fake_dist, monkeypatch):
    monkeypatch.delenv("NADIA_RELEASE_DATE", raising=False)
...
def test_stamp_is_noop_when_env_blank(fake_dist, monkeypatch):
    monkeypatch.setenv("NADIA_RELEASE_DATE", "   ")
    assert build._stamp_release_date() is None
    assert '__release_date__ = "2026.5.29"' in fake_dist.read_text(encoding="utf-8")


def test_stamp_rejects_non_calver_date(fake_dist, monkeypatch):
    monkeypatch.setenv("NADIA_RELEASE_DATE", "not-a-date")
...
def test_stamp_accepts_four_segment_calver(fake_dist, monkeypatch):
    ...
    monkeypatch.setenv("NADIA_RELEASE_DATE", "2026.6.5.2")
...
def test_dockerfile_passes_release_date_into_make_build():
    ...
    assert "ARG NADIA_RELEASE_DATE" in text, (
        "Dockerfile must declare ARG NADIA_RELEASE_DATE"
    )
    assert "ENV NADIA_RELEASE_DATE" in text, (
        "Dockerfile must export NADIA_RELEASE_DATE so `make build` (build.py) sees it"
    )
...
def test_publish_workflow_threads_release_date_into_both_variants():
    ...
    assert text.count("--build-arg NADIA_RELEASE_DATE=") >= 2, (
        "both the slim and full buildx invocations must pass NADIA_RELEASE_DATE"
    )
