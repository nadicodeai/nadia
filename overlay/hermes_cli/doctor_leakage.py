"""argo_cli.doctor_leakage — leakage detection modes for `argo doctor`.
...
--live
    ...
    fallback set: `argo --help`, `argo --version`, `argo doctor --static`),
    captures its combined output, and greps for the same identifier.

Both modes read their configuration from argo-rename.yaml (located in the
repo root by default, overridable via --rename-yaml) and use the same
skip_contexts / exceptions logic as the argo_sync rename engine.

Runtime fallback (issue #4)
---------------------------
When no yaml is reachable — the published ``ghcr.io/nadicodeai/argo`` image
intentionally does NOT ship ``argo-rename.yaml`` (build-time config, spec
FR-7) — the same constants are loaded from the build-time-generated module
``hermes_cli._rename_defaults`` (renamed to ``argo_cli._rename_defaults``
in the runtime image). ``argo-rename.yaml`` remains the single source of
...
#: Directory basenames excluded from the static walk (mirrors argo_sync).
...
def _load_baked_defaults() -> tuple[
    ...
    (``argo_cli._rename_defaults`` in the runtime image), which is
    generated by ``tools/generate_rename_defaults.py`` from
    ``argo-rename.yaml``. This is the fallback when no yaml is reachable
...
def _load_rename_config(rename_yaml: Path | None) -> tuple[
    ...
    ----------
    rename_yaml:
        Path to argo-rename.yaml. When ``None``, the build-time baked
    ...
    if not isinstance(data, dict):
        raise ValueError(f"argo-rename.yaml must be a YAML mapping, got {type(data).__name__!r}")
...
def run_static(
    ...
    *,
    ...
    rename_yaml:
        Path to argo-rename.yaml.  This file itself is always excluded from
