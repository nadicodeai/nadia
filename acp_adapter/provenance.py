...
This is an additive Nadia extension surfaced under ACP ``_meta.nadia`` so
...
compression rotates the internal Nadia head, ``build_session_provenance`` lets
...
def build_session_provenance(
    ...
    *,
    ...
    """Build ``_meta.nadia.sessionProvenance`` for an ACP session.

    Args:
        db: A ``SessionDB`` (must expose ``get_session``).
        acp_session_id: The stable ACP/editor-facing session handle.
        current_nadia_session_id: The live internal Nadia DB session id
            (``state.agent.session_id``).
        previous_nadia_session_id: The internal id from before the most recent
            turn, when known. Supplied by ``prompt()`` to flag a rotation.

    Returns:
        A dict suitable for ``{"nadia": {"sessionProvenance": <dict>}}`` under
        ACP ``_meta``, or ``None`` if the session can't be read.
    """
    try:
        row = db.get_session(current_nadia_session_id)
    except Exception:
        return None
    if not row:
        return None
    ...
    root_id = current_nadia_session_id
    compression_depth = 0
    cursor_parent = parent_id
    seen = {current_nadia_session_id}
    ...
    rotated = bool(
        ...
        and previous_nadia_session_id != current_nadia_session_id
    )

    provenance: Dict[str, Any] = {
        "acpSessionId": acp_session_id,
        "currentNadiaSessionId": current_nadia_session_id,
        "rootNadiaSessionId": root_id,
        "parentNadiaSessionId": parent_id,
        "sessionKind": "continuation" if is_continuation else "root",
        "compressionDepth": compression_depth,
    }
    if previous_nadia_session_id:
        provenance["previousNadiaSessionId"] = previous_nadia_session_id
    if rotated:
...
def session_provenance_meta(
    ...
    *,
    ...
    """Return a ready ``_meta`` payload: ``{"nadia": {"sessionProvenance": ...}}``."""
    ...
    if prov is None:
        return None
    return {"nadia": {"sessionProvenance": prov}}
