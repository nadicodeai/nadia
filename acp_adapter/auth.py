"""ACP auth helpers — detect and advertise Nadia authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "nadia-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Nadia runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Nadia can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Nadia.
    ...
    not have Nadia provider credentials configured yet, so Nadia always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Nadia using the currently configured "
    ...
    methods.append(
        TerminalAuthMethod(
            ...
            name="Configure Nadia provider",
            description=(
                "Open Nadia's interactive model/provider setup in a terminal. "
                "Use this when Nadia has not been configured on this machine yet."
            ),
            type="terminal",
            args=["--setup"],
        )
