"""CLI entry point for the nadia-agent ACP adapter.

Loads environment variables from ``~/.nadia/.env``, configures logging
...
try:
    ...
    # shadowing Nadia's own modules — ``nadia acp`` can be started from any
    # cwd, including a project that has same-named packages on its path.
    nadia_bootstrap.harden_import_path()
...
def _load_env() -> None:
    """Load .env from NADIA_HOME (default ``~/.nadia``)."""
    from nadia_cli.env_loader import load_nadia_dotenv

    nadia_home = get_nadia_home()
    loaded = load_nadia_dotenv(nadia_home=nadia_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", nadia_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="nadia-acp",
        description="Run Nadia Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Nadia version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Nadia provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.nadia/node/ "
...
def _run_check() -> None:
    ...
    from acp_adapter.server import NadiaACPAgent  # noqa: F401

    print("Nadia ACP check OK")


def _run_setup() -> None:
    from nadia_cli.main import main as nadia_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "nadia", "model"]
        nadia_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``nadia postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting nadia-agent ACP adapter")
    ...
    import acp
    from .server import NadiaACPAgent
