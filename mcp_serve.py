"""
Nadia MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Nadia-specific extra)

Usage:
    nadia mcp serve
    nadia mcp serve --verbose
...
logger = logging.getLogger("nadia.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using NADIA_HOME."""
    try:
        from nadia_constants import get_nadia_home
        return get_nadia_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("NADIA_HOME", Path.home() / ".nadia")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from nadia_constants import get_nadia_home
        directory_file = get_nadia_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("NADIA_HOME", Path.home() / ".nadia")
...
class EventBridge:
    ...
    This is the Nadia equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from nadia_constants import get_nadia_home
            db_file = get_nadia_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("NADIA_HOME", Path.home() / ".nadia")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Nadia MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "nadia",
        instructions=(
            "Nadia Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Nadia MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
