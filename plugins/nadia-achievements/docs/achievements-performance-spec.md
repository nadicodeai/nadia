# Nadia Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: nadia-achievements plugin
...
### `GET /api/plugins/nadia-achievements/achievements`
...
### `POST /api/plugins/nadia-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/nadia-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/nadia-achievements/overview`
...
- Persist plugin data under:
  - `~/.nadia/plugins/nadia-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.nadia/plugins/nadia-achievements/scan_checkpoint.json`
...
- `~/.nadia/plugins/nadia-achievements/`
