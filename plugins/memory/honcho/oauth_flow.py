...
``nadia://`` handler. Endpoints are env-overridable with local-dev defaults
...
# The loopback redirect registered for the Nadia OAuth client. IP-literal so
...
_DEFAULT_CLIENT_ID = "nadia-agent"
...
def begin_authorization(
    ...
    *,
    ...
    (``nadia-desktop`` / ``nadia-cli``) so the consent side can attribute
...
_CALLBACK_HTML = (
    ...
    b"<div>Connected to Honcho. You can close this tab and return to Nadia.</div>"
)
...
def start_loopback_flow_background(
    *,
    ...
    # the request, where a context-local NADIA_HOME override can't reach.
