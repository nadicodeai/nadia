...
```bash
nadia memory setup honcho   # configure Honcho directly (works on a fresh install)
nadia memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
nadia config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.nadia/.env
```

> `nadia honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `nadia memory setup honcho`.
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$NADIA_HOME/honcho.json` | Profile-local (isolated Nadia instances) |
| 2 | `~/.nadia/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Nadia profile: `nadia` (default) or `nadia_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `nadia honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 4 | `per-session` strategy | Nadia session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`nadia-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`nadia`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-nadia-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening nadia in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Nadia session ID (timestamp + hex). Every `nadia` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Nadia profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`nadia`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Nadia profile: `nadia` (default) or `nadia_<profile>` (e.g. `nadia -p coder` -> host key `nadia_coder`). Older `nadia.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `NADIA_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `nadia-agent`) |
...
|---------|-------------|
| `nadia memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `nadia honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `nadia memory setup`) |
| `nadia honcho status` | Show resolved config for active profile |
| `nadia honcho enable` / `disable` | Toggle Honcho for active profile |
| `nadia honcho mode <mode>` | Change recall or observation mode |
| `nadia honcho peer --user <name>` | Update user peer name |
| `nadia honcho peer --ai <name>` | Update AI peer name |
| `nadia honcho tokens --context <N>` | Set context token budget |
| `nadia honcho tokens --dialectic <N>` | Set dialectic max chars |
| `nadia honcho map <name>` | Map current directory to a session name |
| `nadia honcho sync` | Create host blocks for all Nadia profiles |
