...
def cmd_setup(args) -> None:
    ...
    if new_shape == "single":
        ...
        nadia_host["pinPeerName"] = True
        print(f"  pinPeerName=true → all gateway users route to '{nadia_host.get('peerName', '?')}'.")
        ...
        if _new_prefix and not (prefix_from_root and _new_prefix == current_prefix):
            nadia_host["runtimePeerPrefix"] = _new_prefix
        print("  Multi-user mode: each runtime ID → own peer. Use 'nadia honcho status' to inspect.")
        ...
        if existing_aliases:
            nadia_host["userPeerAliases"] = existing_aliases
        _prefix_default = current_prefix or ""
        _new_prefix = _prompt(
