...
- Supermemory API key from [app.supermemory.ai/integrations?connect=nadia](http://app.supermemory.ai/integrations?connect=nadia)
...
Config file: `$NADIA_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `nadia` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `nadia-{identity}` → `nadia-coder`). |
...
All Supermemory API calls send `x-sm-source: nadia`, and document writes stamp
`metadata.sm_source: nadia`. This is a **functional routing key, not telemetry**:
it groups Nadia-written memories into a dedicated "Nadia" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Nadia profile:

```json
{
  "container_tag": "nadia-{identity}"
}
```

For a profile named `coder`, this resolves to `nadia-coder`. The default profile resolves to `nadia-default`. Without `{identity}`, all profiles share the same container.
