...
Behavioral settings live in `$NADIA_HOME/mem0.json` (set them via `nadia memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.nadia/.env`.

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `platform` | `platform` (Mem0 Cloud) or `oss` (self-hosted) |
| `user_id` | `nadia-user` | User identifier on Mem0 |
...
```bash
nadia memory setup mem0 --mode oss \
...
```bash
nadia memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$NADIA_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.nadia/mem0_qdrant"}}
  }
}
```
...
```bash
nadia memory setup mem0 --mode platform --api-key sk-...
```
...
- Check `user_id` matches between sessions (`$NADIA_HOME/mem0.json`).
