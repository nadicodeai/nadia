...
Chronos (the Greek god of time, alongside Nadia) is the first non-default
...
NAS's ``agent-cron`` endpoints with the agent's existing Nadia token.
...
class ChronosCronScheduler(CronScheduler):
    ...
    def is_available(self) -> bool:
        ...
        callback URL (for NAS→agent fires), and a usable Nadia token (the agent
