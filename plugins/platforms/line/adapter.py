"""
LINE Messaging API platform adapter for Nadia Agent.
...
to Nadia Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``NADIA_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            nadia_home = Path.home().joinpath(".nadia").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``nadia status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``nadia status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``nadia setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.nadia/.env`` via ``nadia_cli.config``.
    """
    print()
    ...
    try:
        from nadia_cli.config import get_env_var, set_env_var
    except ImportError:
        print("nadia_cli.config not available; set LINE_* vars manually in ~/.nadia/.env")
        return
