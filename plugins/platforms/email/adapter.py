"""
Email platform adapter for the Nadia gateway.

Allows users to interact with Nadia by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "nadia-agent" "version" "{_nadia_version}" '
            '"vendor" "NadicodeAI" '
            '"support-email" "noreply@nadicode.ai")',
        )
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `nadia config show` already read. Without the
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `nadia gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Nadia Agent")
        ...
        msg_id = f"<nadia-{uuid.uuid4().hex[:12]}@{self._address.split('@')[1]}>"
...
# nadia_cli/gateway.py, and the _send_email dispatch in
