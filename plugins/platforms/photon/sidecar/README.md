...
Small Node helper that bridges Nadia Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Nadia is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Nadia-Sidecar-Token`)
...
The Nadia plugin's `nadia photon setup` command runs `npm install`
here automatically.
