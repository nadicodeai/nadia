// Nadia Agent — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Nadia): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Nadia -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Nadia-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `nadia photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so Nadia can restart the Photon adapter"
    );
    process.exit(75);
...
// instead of a cryptic module-resolution error during import. Apply Nadia'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local Nadia allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of Nadia, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-nadia-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Nadia supplied them so a known-good
