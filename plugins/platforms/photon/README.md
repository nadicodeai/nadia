...
This plugin connects Nadia Agent to iMessage (and other Spectrum
...
TypeScript-only, Nadia runs it inside a small supervised Node sidecar and
...
- **Outbound**: `send` / `send_typing` / reaction tapbacks are loopback POSTs
  ...
  `/unreact`), authenticated with a shared `X-Nadia-Sidecar-Token`.
...
There is no separate `login` command; like every other Nadia channel,
...
Run `nadia photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.nadia/.env` (the same place every other
...
Management metadata lives in `~/.nadia/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `nadia photon telemetry on\|off` (restart the gateway to apply) |
...
6. Verify end-to-end: `nadia photon status`, a DM and a group roundtrip,
