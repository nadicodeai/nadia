...
Credential storage mirrors every other Nadia channel:

    * runtime SDK creds  -> ``~/.nadia/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.nadia/auth.json`` under
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a nadia dependency
...
# registers Nadia as its own client_id.
...
# Default name of the project Nadia provisions for the operator.
DEFAULT_PROJECT_NAME = "Nadia Agent"
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of nadia-agent.

def _auth_json_path() -> Path:
    """Resolve ``~/.nadia/auth.json`` honouring the active Nadia profile."""
    try:
        from nadia_constants import get_nadia_home
        return Path(get_nadia_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.nadia")) / "auth.json"
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.nadia/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.nadia/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.nadia/.env`` (canonical runtime store).
    ...
    try:
        ...
        logger.warning("photon: nadia_cli.config unavailable — skipping .env write")
        return
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `nadia photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `nadia photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `nadia photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `nadia photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `nadia photon setup`)"
