"""ntfy platform adapter (Nadia plugin).
...
``plugins/platforms/ntfy/``. The Nadia plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "nadia-in"              # subscribe topic (incoming)
          publish_topic: "nadia-out"     # optional — defaults to topic
...
_ECHO_TAG = "nadia-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``nadia cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Nadia dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `nadia gateway status` without
