...
# ``model_tools`` import time, so ``nadia status``, ``nadia chat``, etc.
...
# ``nadia_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Nadia is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a nadia MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${NADIA_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _nadia_home = _get_nadia_home()
        except (ModuleNotFoundError, ImportError):
            _nadia_home = _Path.home() / ".nadia"
        self._thread_count_store = _ThreadCountStore(
            _nadia_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Nadia is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("NADIA_HOME", str(_Path.home() / ".nadia"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${NADIA_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a nadia MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Nadia is thinking…' marker message.
        ...
        "Nadia is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Nadia is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Nadia is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Nadia is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Nadia is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``nadia setup``.

    The setup wizard at ``nadia_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.nadia/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. nadia-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.nadia/.env")
    print_info("Restart the gateway: nadia gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``nadia cron`` running as a
    separate process from ``nadia gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Nadia plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'nadia-agent[google_chat]'",
        ...
        # the "Nadia is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Nadia is thinking…' marker message "
