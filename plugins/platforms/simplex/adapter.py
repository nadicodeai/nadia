"""SimpleX Chat platform adapter (Nadia plugin).
...
This adapter ships as a Nadia platform plugin under
``plugins/platforms/simplex/``. The Nadia plugin loader scans the
...
Optional environment variables:
    ...
    NADIA_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``nadia setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Nadia — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("NADIA_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``nadia setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.nadia/.env`` via
    ``nadia_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "nadia_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.nadia/.env"
        )
        return
