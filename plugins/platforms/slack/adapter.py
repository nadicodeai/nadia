...
class SlackAdapter(BasePlatformAdapter):
        ...
        try:
            ...
            if "im:history" in granted and "mpim:history" not in granted:
                ...
                logger.warning(
                    ...
                    "Regenerating the app from `nadia slack` produces a "
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        tokens_file = get_nadia_home() / "slack_tokens.json"
        ...
        try:
            ...
            # Nadia does not currently need to route them into the agent loop.
            ...
            # /model work directly without /nadia prefix). A single regex
            ...
            # manifest (see `nadia slack manifest`). In Socket Mode, Slack
            ...
            from nadia_cli.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/nadia$")

            @self._app.command(_slash_pattern)
            async def handle_nadia_command(ack, command):
            ...
            for _action_id in (
                "nadia_approve_once",
                "nadia_approve_session",
                "nadia_approve_always",
                "nadia_deny",
            ):
            ...
            for _action_id in (
                "nadia_confirm_once",
                "nadia_confirm_always",
                "nadia_confirm_cancel",
            ):
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = (
                f":thread: Nadia handoff — *{(name or 'session').strip()[:80]}*"
            )
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own Nadia session — matching the per-thread behavior channels already
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "nadia_approve_once",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Allow Session"},
                            "action_id": "nadia_approve_session",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Allow"},
                            "action_id": "nadia_approve_always",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "nadia_confirm_once",
                            "value": value,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Approve"},
                            "action_id": "nadia_confirm_always",
                            "value": value,
                        },
                        {
                            "type": "button",
                            ...
                            "style": "danger",
                            "action_id": "nadia_confirm_cancel",
                            "value": value,
                        },
                    ],
                },
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "nadia_confirm_once": "once",
            "nadia_confirm_always": "always",
            "nadia_confirm_cancel": "cancel",
        }
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "nadia_approve_once": "once",
            "nadia_approve_session": "session",
            "nadia_approve_always": "always",
            "nadia_deny": "deny",
        }
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/nadia <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/nadia
        ...
        if slash_name in {"nadia", ""}:
            # Legacy /nadia <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            from nadia_cli.commands import slack_subcommand_map

            subcommand_map = slack_subcommand_map()
            subcommand_map["compact"] = "/compress"
            # Guard against whitespace-only text where ``text`` is truthy but
            # ``text.split()`` returns ``[]`` (e.g. user sends ``/nadia   ``).
        ...
        # "Running /cmd…" ack shown by handle_nadia_command).
        # Only stash for COMMAND events (text starts with "/") — free-form
        # questions via "/nadia <question>" must produce public replies so
...
# ``nadia_cli/{setup,gateway}.py``, and the ``_send_slack`` dispatch in
...
def interactive_setup() -> None:
    ...
    offers to set a home channel. Replaces ``nadia_cli/setup.py::_setup_slack``.
    """
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``nadia_cli.gateway.get_env_value`` at call time (not via the
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'nadia-agent[slack]'",
        # Interactive setup wizard — replaces nadia_cli/setup.py::_setup_slack
        # and the static _PLATFORMS["slack"] dict in nadia_cli/gateway.py.
