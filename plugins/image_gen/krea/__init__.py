...
# Nadia uses 3 abstract aspect ratios. Map to Krea's enum (which is wider).
...
def _resolve_managed_krea_gateway():
    ...
    the Nadia-hosted Krea gateway wins when it is resolvable AND either no direct
        ...
        **kwargs: Any,
        ...
        if managed is not None:
            ...
            if not auth_token:
                return error_response(
                    error=(
                        "KREA_API_KEY not set. Run `nadia tools` → Image "
                        "Generation → Krea to configure, get a key at "
                        "https://www.krea.ai/settings/api-tokens, or sign in to "
                        "a Nadia account with the managed Krea gateway enabled "
                        "(`nadia setup`)."
                    ),
        ...
        headers = {
            ...
            "User-Agent": "Nadia-Agent/1.0 (krea-image-gen)",
        }
        if managed is not None:
        ...
        # ``/jobs/{id}`` with the Nadia token (404 on cross-user/unknown jobs).
