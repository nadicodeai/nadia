...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under NADIA_HOME (but leave NADIA_HOME itself and
    ...
    empty_removed = 0
    try:
        for dirpath in sorted(nadia_home.rglob("*"), reverse=True):
            if not dirpath.is_dir() or dirpath == nadia_home:
                continue
            try:
                rel_parts = dirpath.relative_to(nadia_home).parts
            except ValueError:
                continue
