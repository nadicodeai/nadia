"""disk_cleanup — ephemeral file cleanup for Nadia Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under NADIA_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly NADIA_HOME and /tmp/nadia-*
Never touches: ~/.nadia/logs/ or any system directory.
"""
...
try:
    ...
    def get_nadia_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("NADIA_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".nadia").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$NADIA_HOME/logs/``."""
    return get_nadia_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$NADIA_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under NADIA_HOME or ``/tmp/nadia-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    nadia_home = get_nadia_home()
    try:
        path.resolve().relative_to(nadia_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/nadia-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("nadia-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "nadia-agent", "backups", "profiles", ".worktrees",
})
...
# Paths under $NADIA_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    """
    # Lazily build the set once per process so NADIA_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        nadia_home = get_nadia_home()
        for parent in ("cron", "cronjobs"):
            base = nadia_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside NADIA_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under NADIA_HOME, but never recurse into known
    # durable state trees.  Some installs place the Nadia checkout, venv,
    # and desktop build under NADIA_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    nadia_home = get_nadia_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in nadia_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    nadia_home = get_nadia_home()
    try:
        rel = path.resolve().relative_to(nadia_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "nadia-agent",
        }:
            return None
        ...
        # Path isn't under NADIA_HOME (e.g. /tmp/nadia-*) — fall through.
        pass
