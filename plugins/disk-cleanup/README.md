...
Auto-tracks and cleans up ephemeral files created during Nadia Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$NADIA_HOME` and `/tmp/nadia-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `NADIA_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `NADIA_HOME` or `/tmp/nadia-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$NADIA_HOME/disk-cleanup/` is itself excluded
- `$NADIA_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
