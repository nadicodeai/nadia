...
Authentication via ``FAL_KEY`` or the managed Nadia gateway. Output is an
        ...
        if not _check_fal_video_available():
            return error_response(
                error=(
                    ...
                    "(run `nadia tools` → Video Generation → FAL to configure) "
                    "or sign in to Nadia (`nadia setup`) for managed gateway access."
                ),
            ...
            if not endpoint:
                return error_response(
                    error=(
                        ...
                        f"via `nadia tools` → Video Generation."
                    ),
