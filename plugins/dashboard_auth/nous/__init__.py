...
Key contract points encoded here:
  ...
  - audience claim is the bare ``client_id`` (no ``nadia-cli:`` prefix).
...
"state": …}`` and the route serializes those into the ``nadia_session_pkce``
cookie.
...
Skip reasons:
  ...
  ("Set NADIA_DASHBOARD_OAUTH_CLIENT_ID …") instead of the bare "no
...
class NousDashboardAuthProvider(DashboardAuthProvider):
    ...
    def start_login(self, *, redirect_uri: str) -> LoginStart:
        ...
        # The auth-route layer expects ``cookie_payload[\"nadia_session_pkce\"]``
    ...
    def _verify_jwt(self, access_token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # between NADIA_DASHBOARD_PORTAL_URL / NADIA_DASHBOARD_OAUTH_CLIENT_ID
...
def _load_config_oauth_section() -> dict:
    ...
    try:
        ...
        logger.debug(
            "dashboard-auth-nadia: load_config() raised %s; "
...
def _resolve_client_id() -> str:
    ...
    Order:
      1. ``NADIA_DASHBOARD_OAUTH_CLIENT_ID`` env var (when non-empty
    ...
    """
    env = os.environ.get("NADIA_DASHBOARD_OAUTH_CLIENT_ID", "").strip()
    if env:
        return env
...
def _resolve_portal_url() -> str:
    ...
    Order:
      1. ``NADIA_DASHBOARD_PORTAL_URL`` env var (non-empty after strip).
      2. ``dashboard.oauth.portal_url`` in ``config.yaml``.
      3. :data:`_DEFAULT_PORTAL_URL` (production Portal).
    """
    env = os.environ.get("NADIA_DASHBOARD_PORTAL_URL", "").strip()
    if env:
        return env
    cfg_value = str(
...
def register(ctx) -> None:
    ...
    configured (either via ``NADIA_DASHBOARD_OAUTH_CLIENT_ID`` env var
    ...
    can surface "Set NADIA_DASHBOARD_OAUTH_CLIENT_ID …" instead of the
    ...
    engagement layer (``nadia_cli.web_server.should_require_auth`` +
    ...
    if not client_id:
        ...
        logger.debug("dashboard-auth-nadia: %s", LAST_SKIP_REASON)
        return
    ...
    if not client_id.startswith("agent:"):
        ...
        logger.warning("dashboard-auth-nadia: %s", LAST_SKIP_REASON)
        return
    ...
    logger.info(
        "dashboard-auth-nadia: registered provider (client_id=%s, portal=%s)",
        client_id,
        portal_url,
    )
