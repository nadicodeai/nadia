...
OAuth gate. Unlike the bundled ``nous`` provider (which encodes Nadia Agents Portal's
...
provider verifies its *access* token because Nadia Agents Portal mints a custom JWT
...
class SelfHostedOIDCProvider(DashboardAuthProvider):
    ...
    def _validate_redirect_uri(self, redirect_uri: str) -> None:
        ...
        operator-error case with a clear message. Mirrors the nadia provider.
        """
