...
A standards-compliant OpenID Connect Relying Party for the ``nadia dashboard``
OAuth gate. Unlike the bundled ``nous`` provider (which encodes NadicodeAI Portal's
...
:class:`~nadia_cli.dashboard_auth.DashboardAuthProvider` methods and touches
...
``nadia_cli/dashboard_auth/routes.py``; this provider only:
  ...
  4. verifies the **ID token** (RS256/ES256) against the discovered
     ...
     onto a :class:`~nadia_cli.dashboard_auth.Session`.
...
provider verifies its *access* token because NadicodeAI Portal mints a custom JWT
    ...
    dashboard:
      oauth:
        ...
        self_hosted:
          ...
          # credential — prefer the env var / ~/.nadia/.env over config.yaml.

    # Environment overrides (Docker/Fly secret injection)
    NADIA_DASHBOARD_OIDC_ISSUER
    NADIA_DASHBOARD_OIDC_CLIENT_ID
    NADIA_DASHBOARD_OIDC_SCOPES        # optional; defaults to "openid profile email"
    NADIA_DASHBOARD_OIDC_CLIENT_SECRET # optional; set for a confidential client
...
# JWKS cache (PyJWKClient handles its own caching; this mirrors the nadia
# provider's 5-minute lifespan so key rotation is picked up promptly).
_JWKS_CACHE_SECONDS = 300


# ---------------------------------------------------------------------------
# Skip-reason channel (mirrors the nadia plugin)
# ---------------------------------------------------------------------------
...
class SelfHostedOIDCProvider(DashboardAuthProvider):
    ...
    def start_login(self, *, redirect_uri: str) -> LoginStart:
        ...
        cookie_payload = {
            "nadia_session_pkce": f"state={state};verifier={code_verifier}",
        }
    ...
    def _session_from_tokens(
        ...
        *,
        ...
        OAuth access token is intentionally NOT stored — Nadia does not call
    ...
    def _validate_redirect_uri(self, redirect_uri: str) -> None:
        ...
        redirect_uris are permitted. Mirrors the nadia provider.
        """
...
def register(ctx) -> None:
    ...
    client_id are configured (via ``NADIA_DASHBOARD_OIDC_*`` env vars or the
    ...
    issuer = _resolve_setting(
        "NADIA_DASHBOARD_OIDC_ISSUER", oidc_cfg.get("issuer")
    )
    client_id = _resolve_setting(
        "NADIA_DASHBOARD_OIDC_CLIENT_ID", oidc_cfg.get("client_id")
    )
    scopes = (
        _resolve_setting("NADIA_DASHBOARD_OIDC_SCOPES", oidc_cfg.get("scopes"))
        or _DEFAULT_SCOPES
    )
    # Optional — set only for a confidential client. A credential, so the
    # canonical home is the env var / ~/.nadia/.env; config.yaml is supported
    # for precedence symmetry. Empty ⇒ public client (unchanged behaviour).
    client_secret = _resolve_setting(
        "NADIA_DASHBOARD_OIDC_CLIENT_SECRET", oidc_cfg.get("client_secret")
    )
    ...
    if not issuer or not client_id:
        LAST_SKIP_REASON = (
            ...
            "(NADIA_DASHBOARD_OIDC_ISSUER + NADIA_DASHBOARD_OIDC_CLIENT_ID) "
