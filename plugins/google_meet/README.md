...
Let the nadia agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where nadia runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `nadia meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `nadia meet` CLI |
...
| `cli.py` | `nadia meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$NADIA_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `nadia meet node {run,list,approve,remove,status,ping}` |
...
nadia meet install                                      # pip + Chromium
nadia meet setup                                        # preflight
nadia meet auth                                         # optional
nadia meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
nadia meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.nadia/.env
nadia meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
nadia meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
nadia meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.nadia/.env
nadia meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, nadia will **not** switch your system audio input automatically — the
...
nadia plugins enable google_meet
nadia meet node run --display-name my-mac --host 0.0.0.0 --port 18789
