...
session-scoped state file under ``$NADIA_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $NADIA_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_nadia_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this nadia install, leave it first —
    ...
    env = os.environ.copy()
    env["NADIA_MEET_URL"] = url
    env["NADIA_MEET_OUT_DIR"] = str(out)
    env["NADIA_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["NADIA_MEET_HEADED"] = "1"
    if auth_state:
        env["NADIA_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["NADIA_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if NADIA_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["NADIA_MEET_MODE"] = mode
    if realtime_model:
        env["NADIA_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["NADIA_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["NADIA_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["NADIA_MEET_REALTIME_KEY"] = realtime_api_key
