...
``{title, url, description, position}`` rows every other Nadia web
provider produces.
...
prefers Nadia-managed xAI Grok OAuth (via ``nadia auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.nadia/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``nadia tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `nadia auth` to sign in with "
