...
Optional Nadia observability plugin that maps Nadia observer hooks to
...
not replace Nadia Agent's planner, tools, memory, model provider routing, or
CLI UX. Instead, this plugin lets Nadia emit NeMo Relay lifecycle events for
the work Nadia already owns: sessions, turns, provider/API calls, tool calls,
approval prompts, and delegated subagents.

With this plugin enabled, Nadia Agent can:

- Preserve Nadia execution as NeMo Relay scopes, LLM spans, tool spans, and
  mark events.
...
```bash
nadia plugins enable observability/nemo_relay
```

The `NADIA_NEMO_RELAY_*` environment variables below only configure an
already-enabled plugin. They do not enable plugin discovery by themselves.

For isolated test homes, enable the plugin in the same `NADIA_HOME` that the
agent run will use:

```bash
env NADIA_HOME=/tmp/nadia-nemo-relay-test \
  nadia plugins enable observability/nemo_relay
```

Runs started with `--ignore_user_config` skip the enabled-plugin state from
`NADIA_HOME`, so local E2E tests should omit that flag unless the test harness
loads `observability/nemo_relay` explicitly another way.

`NADIA_HOME` is the Nadia profile/config home used by both
`nadia plugins enable ...` and the later `nadia chat ...` run. If unset,
Nadia uses the user's default home, usually `~/.nadia`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export NADIA_HOME=/tmp/nadia-nemo-relay-test
nadia plugins enable observability/nemo_relay
nadia chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `nadia` command you run is built from the
...
uv run nadia plugins enable observability/nemo_relay
uv run nadia chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
```bash
uv build --wheel
python -m pip install --force-reinstall dist/nadia_agent-*.whl
...
The plugin can configure exporters directly from `NADIA_NEMO_RELAY_*`
...
```bash
export NADIA_NEMO_RELAY_ATOF_ENABLED=1
export NADIA_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=.nemo-relay/atof
export NADIA_NEMO_RELAY_ATIF_ENABLED=1
export NADIA_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=.nemo-relay/atif
```

Optional overrides:

- `NADIA_NEMO_RELAY_ATOF_FILENAME`
- `NADIA_NEMO_RELAY_ATOF_MODE` (`append` or `overwrite`)
- `NADIA_NEMO_RELAY_ATIF_FILENAME_TEMPLATE`
- `NADIA_NEMO_RELAY_ATIF_AGENT_NAME`
- `NADIA_NEMO_RELAY_ATIF_AGENT_VERSION`
- `NADIA_NEMO_RELAY_ATIF_MODEL_NAME`
- `NADIA_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE` (`embedded` by default; set `all` to also write standalone child files)

### NeMo Relay Component Config

To initialize NeMo Relay from a component config, create a `plugins.toml` file
and point Nadia at it:

```bash
export NADIA_NEMO_RELAY_PLUGINS_TOML=.nemo-relay/plugins.toml
```
...
agent_name = "Nadia Agent"
agent_version = "local"
```

When `NADIA_NEMO_RELAY_PLUGINS_TOML` is set and initializes successfully, NeMo
Relay owns exporter lifecycle through that config. The direct
`NADIA_NEMO_RELAY_ATOF_*` fallback setup is skipped. If the same
`plugins.toml` observability config enables `atif`, the direct
`NADIA_NEMO_RELAY_ATIF_*` fallback setup is also skipped so Nadia does not
double-export trajectories on teardown. If `plugins.toml` initialization fails,
Nadia keeps the direct env-var fallbacks active for that run.
...
exposes `llm.execute(...)` / `tools.execute(...)`, Nadia routes LLM and tool
...
For the full generic Nadia middleware contract, see
...
export NADIA_HOME=/tmp/nadia-nemo-relay-docs/nadia-home
mkdir -p "$NADIA_HOME"

cat > "$NADIA_HOME/config.yaml" <<'YAML'
model:
  provider: custom
  default: qwen3.6:35b
...
This run starts a parent Nadia session, delegates to a child subagent, has the
child call `terminal`, and writes both ATOF and ATIF.

```bash
export NADIA_NEMO_RELAY_ATOF_ENABLED=1
export NADIA_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/nadia-nemo-relay-docs/subagent/atof
export NADIA_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export NADIA_NEMO_RELAY_ATOF_MODE=overwrite
export NADIA_NEMO_RELAY_ATIF_ENABLED=1
export NADIA_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/nadia-nemo-relay-docs/subagent/atif
export NADIA_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='nested-subagent-atif-{session_id}.json'
export NADIA_NEMO_RELAY_ATIF_AGENT_NAME='Nadia Agent E2E'
export NADIA_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
export NADIA_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
...
{"kind":"mark","name":"nadia.subagent.start","metadata":{"parent_session_id":"docs-parent-session","session_id":"docs-child-session","subagent_id":"sa-0-docs","child_role":"leaf"}}
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "Nadia Agent E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. Nadia dispatches the read-only tools as one batch, and NeMo Relay
records both tool invocations.

```bash
mkdir -p /tmp/nadia-nemo-relay-docs/workdir
printf 'docs_parallel_alpha_function\n' > /tmp/nadia-nemo-relay-docs/workdir/alpha.txt
printf 'docs_parallel_beta_function\n' > /tmp/nadia-nemo-relay-docs/workdir/beta.txt
cd /tmp/nadia-nemo-relay-docs/workdir

export NADIA_NEMO_RELAY_ATOF_ENABLED=1
export NADIA_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/nadia-nemo-relay-docs/parallel/atof
export NADIA_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export NADIA_NEMO_RELAY_ATOF_MODE=overwrite
export NADIA_NEMO_RELAY_ATIF_ENABLED=1
export NADIA_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/nadia-nemo-relay-docs/parallel/atif
export NADIA_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='parallel-tools-atif-{session_id}.json'
export NADIA_NEMO_RELAY_ATIF_AGENT_NAME='Nadia Agent E2E'
export NADIA_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
...
- Nadia sessions map to `agent` scopes.
- Nadia API request hooks map to `llm` scope start/end events.
- Nadia tool hooks map to `tool` scope start/end events.
...
The `observability/nemo_relay` plugin uses Nadia execution middleware to hand
...
```bash
export NADIA_NEMO_RELAY_PLUGINS_TOML=/tmp/nadia-middleware-test/plugins.toml
```

When the adaptive component is enabled and the installed NeMo Relay runtime
exposes `llm.execute(...)` and `tools.execute(...)`, Nadia routes execution
...
Nadia provider call
  -> llm_execution middleware
    -> nemo_relay.llm.execute(...)
      -> Nadia provider adapter next_call(...)

Nadia tool call
  -> tool_execution middleware
    -> nemo_relay.tools.execute(...)
      -> Nadia tool dispatcher next_call(...)
```
...
middleware for a local Nadia run. This path requires a NeMo Relay runtime that
...
```bash
export NADIA_HOME=/tmp/nadia-middleware-test/nadia-home
mkdir -p "$NADIA_HOME" /tmp/nadia-middleware-test/nemo-relay
...
cat > /tmp/nadia-middleware-test/nemo-relay/plugins.toml <<'TOML'
version = 1
...
[components.config.atof]
enabled = true
output_directory = "/tmp/nadia-middleware-test/atof"
filename = "middleware-events.jsonl"
mode = "overwrite"

[components.config.atif]
enabled = true
output_directory = "/tmp/nadia-middleware-test/atif"
filename_template = "middleware-trajectory-{session_id}.json"
agent_name = "Nadia Middleware E2E"
agent_version = "local"
...
export NADIA_NEMO_RELAY_PLUGINS_TOML=/tmp/nadia-middleware-test/nemo-relay/plugins.toml
