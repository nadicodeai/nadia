"""Slash command definitions and autocomplete for the Nadia CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of Nadia config/state", "Session",
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal Nadia works on across turns until achieved", "Session",
    ...
    CommandDef("busy", "Control what Enter does while Nadia is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.nadia/skills/ for newly installed or removed skills",
    ...
    CommandDef("credits", "Show Nadia credit balance and top up", "Info"),
    CommandDef("billing", "Manage Nadia terminal billing — buy credits, auto-reload, limits", "Info",
               cli_only=True),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`nadia_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/nadia`` subcommand mapping, and
...
# Telegram allows up to 100 BotCommands. Nadia ships ~50 built-in commands;
...
Telegram only displays a small BotCommand menu in practice.  The full Nadia
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``nadia skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``nadia skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``nadia skills list`` and
...
# unrelated command landed. These claim their slots right after /nadia,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /nadia <command>).
...
# native slot, the alias spelling stays reachable via /nadia reset).
...
# ``/nadia <command>`` on Slack only. They remain native on every other
# surface (CLI, TUI, Telegram, Discord). Keep this list TIGHT and intentional —
# the telegram-parity test reads it so an entry here is a deliberate
# "Slack-via-/nadia" decision, not a silent clamp.
#   - credits: the billing/top-up surface; reached via /nadia credits on Slack.
#   - billing: the terminal-billing surface (buy/auto-reload/limit); /nadia billing.
#   - moa: high-cost slash mode, available through /nadia moa to avoid
#     displacing existing native Slack slash commands at the 50-command cap.
#   - debug: the log/report upload surface; reached via /nadia debug on Slack.
_SLACK_VIA_NADIA_ONLY = frozenset({"credits", "billing", "moa", "debug"})
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/nadia <verb>`` subcommand.
    ...
    can still reach them via ``/nadia <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/nadia`` is always reserved as the first entry so the
    legacy ``/nadia <subcommand>`` form keeps working for anything that
    ...
    # Reserve /nadia as the catch-all top-level command.
    entries.append(("nadia", "Talk to Nadia or run a subcommand", "[subcommand] [args]"))
    seen.add("nadia")
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_RESERVED_COMMANDS:
            return
        if slack_name in _SLACK_VIA_NADIA_ONLY:
            # Intentionally Slack-via-/nadia only (see _SLACK_VIA_NADIA_ONLY).
            return
    ...
    # everything except /nadia, so a new canonical command can never silently
...
def slack_app_manifest(request_url: str = "https://nadia-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /nadia handler.

    Maps both canonical names and aliases so /nadia bg do stuff works
    the same as /nadia background do stuff.

    Plugin-registered slash commands are included so ``/nadia <plugin-cmd>``
