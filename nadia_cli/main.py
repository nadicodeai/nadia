...
def cmd_gui(args: argparse.Namespace):
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            if build_result.returncode != 0 and not source_mode:
                ...
                # on the final `electron` -> `Nadia` rename: unpack-electron
    ...
    print(f"→ Launching packaged Nadia Desktop: {packaged_executable}")
...
def _find_stale_dashboard_pids(
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    ...
    backend child process: when the desktop spawns ``nadia dashboard`` as
...
def _nadia_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    if not _is_windows():
        return []
    return [
        scripts_dir / "nadia.exe",
        scripts_dir / "nadia-gateway.exe",
    ]
...
def _resume_windows_gateways_after_update(token: dict | None) -> None:
    ...
    try:
        from nadia_cli.gateway import launch_detached_profile_gateway_restart
    except Exception as exc:
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        # Tying the refresh to ``nadia update`` gives users a predictable
...
def cmd_profile(args):
    ...
    if action == "list":
        ...
        from nadia_cli.profiles import profile_exists
...
def cmd_dashboard(args):
    ...
    if (
        ...
        env = os.environ.copy()
        # Drop the profile NADIA_HOME so the child binds the machine root.
