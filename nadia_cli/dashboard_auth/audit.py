...
Profile-aware location: ``$NADIA_HOME/logs/dashboard-auth.log``.
...
from ``nadia_constants`` or other nadia_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$NADIA_HOME/logs/dashboard-auth.log`` with the standard fallback.

    Mirrors ``nadia_constants.get_nadia_home`` semantics: env var wins,
    else ``~/.nadia``. A local copy avoids an import cycle with the
    middleware which lives below ``nadia_cli``.
    """
    home = os.environ.get("NADIA_HOME") or str(Path.home() / ".nadia")
