...
Three cookies in play:
  - nadia_session_at:   the OAuth access token
  ...
  - nadia_session_pkce: short-lived PKCE state + CSRF nonce + provider
...
https://datatracker.ietf.org/doc/html/draft-west-cookie-prefixes):
  ...
  * Gated HTTPS, direct deploy (Path=/) — ``__Host-`` prefix. Binds the
    ...
    guarantee.
  * Gated HTTPS, behind a reverse-proxy prefix (Path=/nadia) —
    ``__Secure-`` prefix. ``__Host-`` is disallowed when Path != "/";
    ``__Secure-`` keeps the Secure-required hardening without the
    Path constraint, and the explicit ``Path=/nadia`` covers
...
setter wrote ``__Secure-nadia_session_at`` would never find the value.
...
SESSION_AT_COOKIE = "nadia_session_at"
SESSION_RT_COOKIE = "nadia_session_rt"
PKCE_COOKIE = "nadia_session_pkce"
...
SSO_ATTEMPT_COOKIE = "nadia_sso_attempt"
...
def _cookie_path(prefix: str) -> str:
    ...
    Under ``X-Forwarded-Prefix: /nadia`` we want ``Path=/nadia`` so:
...
def set_session_cookies(
    ...
    *,
    ...
    ``prefix`` is the normalised X-Forwarded-Prefix value (e.g. ``/nadia``)
