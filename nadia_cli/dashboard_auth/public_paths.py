...
* ``nadia_cli.web_server.auth_middleware`` — loopback / ``--insecure``
  mode, gates on the ephemeral ``_SESSION_TOKEN``.
* ``nadia_cli.dashboard_auth.middleware.gated_auth_middleware`` —
...
liveness probe (``nadia-account-service`` ``fly-provider.ts``
