"""
Dump command for nadia CLI.

Outputs a compact, plain-text summary of the user's Nadia setup
...
from nadia_cli.config import get_nadia_home, get_env_path, get_project_root, load_config
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.nadia_build_sha`` by the Dockerfile's
    ``NADIA_GIT_SHA`` build-arg (see ``nadia_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``nadia dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from nadia_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(nadia_home: Path) -> int:
...
def _cron_summary(nadia_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = nadia_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"nadia_home:      {display_nadia_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # A credential added via `nadia auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `nadia auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["nadia-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(nadia_home)}")
    lines.append(f"  skills:             {_count_skills(nadia_home)}")
