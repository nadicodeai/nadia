"""nadia hooks — inspect and manage shell-script hooks.

Usage::

    nadia hooks list
    nadia hooks test <event> [--for-tool X] [--payload-file F]
    nadia hooks revoke <command>
    nadia hooks doctor

Consent records live under ``~/.nadia/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.nadia/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``nadia hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: nadia hooks {list|test|revoke|doctor}")
        print("Run 'nadia hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.nadia/config.yaml.")
        print("See `nadia hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `nadia hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `nadia hooks test` and `nadia hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Nadia wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `nadia hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `nadia hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `nadia hooks doctor`.")
