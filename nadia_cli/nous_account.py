...
# Portal's TOOL_COVERAGE_CATEGORIES (nadia-account-service
...
def nous_portal_billing_url(account_info: Optional[NousPortalAccountInfo] = None) -> str:
    """Return the billing URL for a normalized Nadia account snapshot."""
    try:
...
def format_nous_portal_entitlement_message(
    ...
    *,
    ...
    """Return user-facing guidance for a missing Nadia tool-gateway entitlement.
    ...
    if account_info.paid_service_access is None:
        ...
        if include_refresh_hint:
            detail += " Run `nadia model` to refresh your session."
    ...
    if reason == "no_usable_credits" or account_info.paid_service_access is False:
        ...
        if include_refresh_hint and not account_info.fresh:
            message += " If you recently bought credits, run `nadia model` to refresh Nadia."
        return message
...
def _fresh_account_info(
    *,
    ...
    try:
        from nadia_cli.auth import get_provider_auth_state, resolve_nous_access_token
...
def _info_from_inference_key_pool(
    ...
    """Return an explicit unknown-entitlement snapshot for opaque Nadia keys."""
    try:
...
def _info_from_valid_jwt(
    ...
    *,
    ...
    try:
        from nadia_cli.auth import _decode_jwt_claims
    except Exception:
        return None
