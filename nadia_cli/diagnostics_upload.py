"""Client for uploading ``nadia debug share`` bundles to Nadia-internal S3.

This is the opt-in (``--nadia``) destination for ``nadia debug share``.
Unlike the public paste.rs path, bundles uploaded here go to a Nadia-owned
S3 bucket via a short-lived signed URL minted by the Nadia account service
(NAS).  The bucket auto-expires objects after 14 days, and the contents are
only viewable by Nadia staff (and allowlisted Discord mods) through a
...
# Base URL of the Nadia account service that mints the signed upload URL.
...
NAS_BASE = os.environ.get(
    "NADIA_DIAGNOSTICS_BASE_URL", "https://portal.nadicodeai.com"
)
...
_USER_AGENT = "nadia-agent/debug-share"
...
def share_to_nous(report_bundle: bytes) -> dict:
    """Orchestrate the full Nadia-S3 upload of a gzipped *report_bundle*.
