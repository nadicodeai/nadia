"""
Nadia Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.nadia/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Nadia PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing nadia-agent or nadia PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Nadia Agent" comment and following line
                if '# Nadia Agent' in line or '# nadia-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('nadia' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing nadia
                if 'nadia' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the nadia wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "nadia",
        Path("/usr/local/bin/nadia"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains nadia_cli reference)
                content = wrapper.read_text()
                if 'nadia_cli' in content or 'nadia-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(nadia_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``nadia`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Nadia home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (nadia_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``nadia gateway run`` processes
    ...
    try:
        from nadia_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from nadia_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``NADIA_HOME`` and ``NADIA_GIT_BASH_PATH``
...
def _nadia_path_markers(nadia_home: Path) -> list[str]:
    """Path-entry substrings that identify Nadia-owned User-PATH entries."""
    root = str(nadia_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare nadia-agent install dir.
    markers = [root + "\\nadia-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if NADIA_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "nadia".  We don't want to catch
    # unrelated entries like "cnadia-foo" or "ephermeral", so we look for
    # backslash-nadia as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(nadia_home: Path) -> list[str]:
    """Strip Nadia-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _nadia_path_markers(nadia_home)
            kept: list[str] = []
...
def remove_nadia_env_vars_windows() -> list[str]:
    """Delete NADIA_HOME and NADIA_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("NADIA_HOME", "NADIA_GIT_BASH_PATH"):
                try:
...
def _is_default_nadia_home(nadia_home: Path) -> bool:
    """Return True when ``nadia_home`` points at the default (non-profile) root."""
    try:
        from nadia_constants import get_default_nadia_root
        return nadia_home.resolve() == get_default_nadia_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its NADIA_HOME directory.

    We shell out to ``nadia -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current NADIA_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m nadia_cli.main` so we don't depend on a `nadia`
    #    wrapper that may be half-removed mid-uninstall.
    nadia_invocation = [_sys.executable, "-m", "nadia_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                nadia_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's NADIA_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``nadia uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$NADIA_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from nadia_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(nadia_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         ⚕ Nadia Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Nadia Chat GUI installation was found.")
        print(f"  Checked: {nadia_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Nadia agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(nadia_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Nadia agent at {nadia_home / 'nadia-agent'}")
        print(f"  • Your config, sessions, and secrets under {nadia_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Nadia agent is still installed. Run 'nadia' to use the CLI,")
    print("or 'nadia uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.nadia/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.nadia/ for future reinstall
    """
    project_root = get_project_root()
    nadia_home = get_nadia_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie NADIA_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_nadia_home(nadia_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.nadia data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Nadia Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {nadia_home / 'config.yaml'}")
    print(f"  Secrets: {nadia_home / '.env'}")
    print(f"  Data:    {nadia_home / 'cron/'}, {nadia_home / 'sessions/'}, {nadia_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their NADIA_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Nadia data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Nadia code but keep your configuration and data.")
...
def _perform_uninstall(
    *,
    ...
    ``nadia`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$NADIA_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in nadia_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\nadia\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(nadia_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Nadia-owned PATH entries in User environment")

        log_info("Removing NADIA_HOME / NADIA_GIT_BASH_PATH User env vars...")
        removed_env = remove_nadia_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Nadia-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing nadia command...")
    ...
    #     (only when they still point into this Nadia home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Nadia-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(nadia_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Nadia-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(nadia_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE NADIA_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from nadia_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(nadia_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.nadia/, just remove the nadia-agent subdir
            if nadia_home in project_root.parents or project_root.parent == nadia_home:
    ...
    #     under NADIA_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(nadia_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(nadia_home)
    ...
    # 5. Optionally remove ~/.nadia/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile NADIA_HOME dirs live under
        ...
        try:
            if nadia_home.exists():
                shutil.rmtree(nadia_home)
                log_success(f"Removed {nadia_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {nadia_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {nadia_home}")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://raw.githubusercontent.com/nadicodeai/nadia/main/scripts/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://raw.githubusercontent.com/nadicodeai/nadia/main/scripts/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Nadia Agent! ⚕")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m nadia_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import nadia_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``nadia_constants`` + ``nadia_cli.colors``
    (and lazily ``nadia_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m nadia_cli.uninstall")
    parser.add_argument(
        "--mode",
