"""
Security advisory checker for Nadia Agent.
...
- **Acknowledgeable.** Once the user has read and acted on an advisory they
  can dismiss it via ``nadia doctor --ack <id>``; the ack is persisted to
...
1. ``nadia doctor`` (and ``nadia doctor --ack <id>``)
...
run in environments where the rest of Nadia failed to import.
"""
...
ADVISORIES: tuple[Advisory, ...] = (
    Advisory(
        ...
        summary=(
            ...
            "imported mistralai 2.4.6 — including nadia when configured "
        ...
        remediation=(
            ...
            "Rotate API keys in ~/.nadia/.env (OpenRouter, Anthropic, OpenAI, "
            "Nadia, GitHub, AWS, Google, Mistral, etc.).",
            ...
            "After cleanup: nadia doctor --ack shai-hulud-2026-05  to dismiss "
            "this warning.",
        ),
...
def _installed_version(pkg_name: str) -> Optional[str]:
    ...
    except ImportError:  # py<3.8 — Nadia requires 3.10+ but defensive.
        return None
    try:
...
def short_banner_lines(hits: list[AdvisoryHit]) -> list[str]:
    ...
    lines = [
        ...
        "  Run 'nadia doctor' for remediation steps.",
    ]
    if len(hits) > 1:
...
# ``~/.nadia/cache/advisory_banner_seen`` (a single line per advisory ID:
...
def _banner_cache_path() -> Optional[Path]:
    try:
        ...
        cache_dir = Path(get_nadia_home()) / "cache"
...
def render_doctor_section(hits: list[AdvisoryHit]) -> tuple[bool, list[str]]:
    """Render the security-advisory section for ``nadia doctor``.
...
def gateway_log_message(hits: list[AdvisoryHit]) -> Optional[str]:
    ...
    return (f"{len(fresh)} security advisories active "
            ...
            f"Run `nadia doctor` on the gateway host for details.")
