...
* We write two files: a shared ``gateway.cmd`` wrapper script (cwd + env + the
  actual ``python -m nadia_cli.main gateway run --replace`` invocation) and
...
def _stable_gateway_working_dir(project_root: Path) -> str:
    ...
    source checkout only if ``NADIA_HOME`` cannot be resolved yet.
    """
    from nadia_cli.config import get_nadia_home
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        from nadia_cli.config import get_nadia_home
        print(f"    type {Path(get_nadia_home()).resolve()}\\logs\\gateway.log")
        print(f"    type {Path(get_nadia_home()).resolve()}\\logs\\gateway-stdio.log")


def _print_next_steps() -> None:
    from nadia_cli.config import get_nadia_home

    nadia_home = Path(get_nadia_home()).resolve()
    print()
    print("Next steps:")
...
def _print_deep_probes() -> None:
    ...
    home = Path(get_nadia_home()).resolve()
...
def stop() -> None:
    ...
    """
    _assert_windows()
    from nadia_cli.gateway import kill_gateway_processes, _get_restart_drain_timeout
    from gateway.status import get_running_pid
