...
Used by ``nadia profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``nadia_cli/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the Nadia Agent kanban board.
...
Rules:
  ...
  - Never write "Nadia Agent profile" or other meta-narration.
    ...
    *,
    ...
    try:
        if canon == "default":
            ...
            profile_dir = Path(get_nadia_home())
        else:
