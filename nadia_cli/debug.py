"""``nadia debug`` debug tools for Nadia Agent.

Currently supports:
    nadia debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.nadia/logs/*.log`` are not leaked into
                          the public paste service. Pass ``--no-redact``
                          to disable.
                          Pass ``--nadia`` to upload instead to Nadia-internal
                          storage (AWS S3) via a signed URL minted by the
                          Nadia account service: the bundle is private
                          (viewable only by Nadia staff / allowlisted mods via
...
_REDACTION_BANNER = (
    "[nadia debug share: log content redacted at upload time. "
...
def _pending_file() -> Path:
    """Path to ``~/.nadia/pastes/pending.json``.
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``nadia debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``nadia debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `nadia debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "nadia-agent/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``nadia debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.nadia/pastes/pending.json``
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``nadia debug share`` also runs an opportunistic sweep as a fallback
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "nadia-agent/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _upload_dpaste_com(content: str, expiry_days: int = 7) -> str:
    ...
    """
    boundary = "----NadiaDebugBoundary9f3c"
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from nadia_cli.logs import LOG_FILES

    filename = LOG_FILES.get(log_name)
    return (get_nadia_home() / "logs" / filename) if filename else None
...
def _capture_dump() -> str:
    """Run ``nadia dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``nadia dump`` is run
        internally.
...
# ---------------------------------------------------------------------------
# Shared bundle collection (used by both the paste.rs and Nadia-S3 paths)
# ---------------------------------------------------------------------------

# Bundle format identifier embedded in the Nadia-S3 JSON envelope. The
# discord-support viewer keys off this string to parse the bundle.
_NOUS_BUNDLE_FORMAT = "nadia-debug-share/1"
...
def collect_share_bundle(
    ...
    Nadia-S3 path (``--nadia``).  Centralising it guarantees the Nadia bundle is
    built from the *same* force-redacted snapshots as the public paste path —
    redaction is the safety boundary, so the Nadia path must never see raw
    logs.
...
def build_nous_bundle(bundle: dict[str, str], redact: bool = True) -> bytes:
    """Gzip-compress a :func:`collect_share_bundle` mapping into the Nadia envelope.

    The JSON shape is what the discord-support viewer (Repo 3) parses::

        {"format": "nadia-debug-share/1",
         "redacted": <bool>,
         "created": <iso8601>,
...
def build_debug_share(
    *,
    ...
    This is the shared core behind ``nadia debug share`` (CLI) and the
    ...
    # shared collector so the paste.rs and Nadia-S3 paths build identical,
    ...
    if redact:
        logger.info(
            "nadia debug share: applied force-mode redaction to log snapshots before upload"
        )
...
def run_debug_share(args):
    ...
    try:
        ...
        print("\nRun `nadia debug share --local` to print the report instead.\n")
        sys.exit(1)
    ...
    print(f"To delete now:  nadia debug delete <url>")

    print(f"\nShare these links with the Nadia team for support.")


_NOUS_PRIVACY_NOTICE = """\
⚠️  --nadia: This uploads your debug bundle to Nadia-INTERNAL storage (AWS S3),
    NOT a public paste service. The following is included:
  • System info (OS, Python/Nadia version, provider, which API keys are
  ...
  • The bundle is viewable only by Nadia staff (and allowlisted Discord mods)
...
def _run_debug_share_nous(args, *, log_lines: int, redact: bool) -> None:
    """Handle ``nadia debug share --nadia``: upload the bundle to Nadia-S3.

    Collects the same force-redacted bundle as the paste path, gzips it into
    the Nadia envelope, requests a signed URL from NAS, uploads, and prints the
    private viewer link. On any failure falls back to a clear error that
    suggests ``--local``.
    """
    from nadia_cli.diagnostics_upload import share_to_nous
    ...
    if redact:
        logger.info(
            "nadia debug share --nadia: applied force-mode redaction before upload"
        )
    blob = build_nous_bundle(bundle, redact=redact)

    print("Uploading to Nadia diagnostics storage...")
    try:
        res = share_to_nous(blob)
    except Exception as exc:
        print(
            f"\nNous upload failed: {exc}\n"
            "\nThe Nadia diagnostics service may be unavailable or not yet "
            "provisioned.\n"
            "Run `nadia debug share --local` to print the report instead, "
            "or `nadia debug share` to upload to a public paste service.\n",
            file=sys.stderr,
        )
        sys.exit(1)

    view_url = res.get("viewUrl") or res.get("view_url")
    print("\nDebug bundle uploaded to Nadia (private):")
    if view_url:
    ...
    print(
        "\nShare this private link with the Nadia team — only Nadia staff "
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: nadia debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'nadia debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``nadia debug`` call.
    ...
    # best-effort — any failure is swallowed so ``nadia debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: nadia debug <command>")
        print()
        print("Commands:")
        ...
        print("  --nadia       Upload to Nadia-internal storage (private, staff-only,")
