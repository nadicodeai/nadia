...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    NADIA_HOME — whichever process grabs the lock first wins the tick.
    """
...
def _spawn_nadia_action(subcommand: List[str], name: str) -> subprocess.Popen:
    ...
    cmd = [sys.executable, "-m", "nadia_cli.main", *subcommand]
...
def _spawn_gateway_restart() -> Tuple[subprocess.Popen, bool]:
    ...
    return _spawn_nadia_action(["gateway", "restart"], "gateway-restart"), False
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    ...
    "slack": {
        "name": "Slack",
        "description": "Use Nadia from Slack via Socket Mode.",
...
async def start_gateway():
    try:
        proc = _spawn_nadia_action(["gateway", "start"], "gateway-start")
    except Exception as exc:
...
async def stop_gateway():
    try:
        proc = _spawn_nadia_action(["gateway", "stop"], "gateway-stop")
    except Exception as exc:
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            raise SystemExit(
                ...
                f"(was the dashboard_auth/nadia plugin removed?).\n"
