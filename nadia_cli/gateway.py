...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    patterns = [
        ...
        "nadia_cli.main --profile",
        "nadia_cli.main -p",
        "nadia_cli/main.py gateway",
        "nadia_cli/main.py --profile",
        "nadia_cli/main.py -p",
        "nadia gateway",
        ...
        # ``nadia.exe --profile`` / ``nadia.exe -p`` would also match
        # ``nadia.exe --profile foo dashboard`` and other CLI subcommands,
...
def install_linux_gateway_from_setup(force: bool = False, enable_on_startup: bool = True) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            print_warning(
                "  System service install requires sudo, so Nadia can't create it from this user session."
            )
            if run_as_user:
                print_info(
                    f"  After setup, run: sudo nadia gateway install --system --run-as-user {run_as_user}"
                )
            else:
                print_info(
                    "  After setup, run: sudo nadia gateway install --system --run-as-user <your-user>"
                )
            print_info("  Then start it with: sudo nadia gateway start --system")
            return scope, False
...
# (the documented `nohup nadia gateway run` workaround). See #23387.
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and Nadia will log in directly",
        ...
        "vars": [
            ...
            },
            {
                ...
                "password": False,
                "help": "Full Matrix user ID, e.g. @nadia:matrix.example.org",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your Nadia agent",
        ...
        "vars": [
            {
                ...
                "password": False,
                "help": "The email address Nadia will use (e.g., nadia@gmail.com).",
            },
            {
                "name": "EMAIL_PASSWORD",
...
def _setup_whatsapp():
    ...
    from nadia_cli.main import cmd_whatsapp
    import argparse
...
def _setup_dingtalk():
    ...
    if method == 0:
        ...
        try:
            from nadia_cli.dingtalk_auth import dingtalk_qr_auth
...
def _setup_wecom():
    ...
    if allowed:
        ...
        if access_idx == 0:
            ...
            print_info("  Approve with: nadia pairing approve <platform> <code>")
        elif access_idx == 2:
            save_env_value("WECOM_DM_POLICY", "disabled")
            print_warning("  Direct messages disabled.")
        else:
            print_info("  Skipped — configure later with 'nadia gateway setup'")
