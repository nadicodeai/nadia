"""`nadia checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.nadia/checkpoints/``.  Actions:

    nadia checkpoints               # same as `status`
    nadia checkpoints status        # total size, project count, breakdown
    nadia checkpoints list          # per-project checkpoint counts + workdir
    nadia checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    nadia checkpoints clear [-f]    # nuke the entire base (asks first)
    nadia checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    nadia checkpoints
    nadia checkpoints prune --retention-days 3 --max-size-mb 200
    nadia checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: nadia checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``nadia checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `nadia checkpoints` → status
