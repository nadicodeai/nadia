"""Profile distributions — shareable, packaged Nadia profiles via git.

A distribution is a Nadia profile published as a git repository (or
...
* ``nadia profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``nadia skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``nadia profile``, not a parallel tree):

    nadia profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "nadia-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            nadia_requires=str(data.get("nadia_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.nadia_requires:
            out["nadia_requires"] = self.nadia_requires
        if self.author:
...
def check_nadia_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Nadia {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``nadia profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Nadia profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Nadia distribution."
        )

    # Version check up-front so we fail fast
    check_nadia_requires(manifest.nadia_requires, nadia_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.nadia).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="nadia_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `nadia profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`nadia profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="nadia_dist_update_") as tmp:
        plan = plan_install(
