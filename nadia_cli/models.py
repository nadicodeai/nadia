...
Add, remove, or reorder entries here — both `nadia setup` and
`nadia` provider-selection will pick up the change automatically.
"""
...
from nadia_cli import __version__ as _NADIA_VERSION

# Identify ourselves so endpoints fronted by Cloudflare's Browser Integrity
# Check (error 1010) don't reject the default ``Python-urllib/*`` signature.
_NADIA_USER_AGENT = f"nadia-cli/{_NADIA_VERSION}"
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `nadia model`
    flow without maintaining a separate static list.
    """
    from nadia_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $NADIA_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_merge_curated_extras(ids: list[str]) -> list[str]:
    """Append Nadia-curated xAI models that are missing from models.dev."""
    out = list(ids)
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $NADIA_HOME/models_dev_cache.json directly (no network) so this
    runs at import time without blocking. Falls back to ``_XAI_STATIC_FALLBACK``
    when the cache is empty or unreadable. Nadia refreshes the cache from
...
def partition_nous_models_by_tier(
    ...
    """Split Nadia models into (selectable, unavailable) based on user tier.
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    older Nadia that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    picker even if the user is running an older Nadia that doesn't ship
...
def _nous_recommended_disk_path() -> "Path":
    ...
    return get_nadia_home() / "cache" / "nous_recommended_cache.json"
...
def _write_nous_recommended_disk(base: str, data: dict[str, Any]) -> None:
    ...
    try:
        ...
        logging.getLogger(__name__).debug(
            "nadia recommended-models disk cache write failed: %s", exc
        )
...
def fetch_nous_recommended_models(
    ...
    *,
    ...
    (``$NADIA_HOME/cache/nous_recommended_cache.json``) as last-known-good.
...
# this list:  nadia model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `nadia model` interactive picker
# ---------------------------------------------------------------------------
...
#
# Some vendors expose several Nadia provider slugs (one per endpoint /
# auth method: global API, China API, OAuth coding plan, ...). Listing every
# slug as a top-level row in the interactive `nadia model` / setup wizard /
...
def group_providers(slugs):
    ...
    DISPLAY ONLY. Used by every interactive picker (``nadia model``, the
...
# path. The *interactive* default (GUI onboarding / ``nadia model``) uses the
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model was ever selected (e.g. ``nadia auth add openai-codex`` without
    ``nadia model``, or a profile that sets ``provider`` with no ``model``).

    For most providers this is the first entry in ``_PROVIDER_MODELS`` — the
    same model the ``nadia model`` picker offers first. For metered aggregators
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    nadia-agent is tool-calling-first — every provider path assumes the model
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from nadia_cli.model_catalog import get_curated_openrouter_models
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    """
    try:
        from nadia_cli.model_catalog import get_curated_nous_models
...
_DEFAULT_NOUS_INFERENCE_BASE = "https://inference-api.nadicode.ai"
...
def get_pricing_for_provider(provider: str, *, force_refresh: bool = False) -> dict[str, dict[str, str]]:
    """Return live pricing for providers that support it (openrouter, nadia, novita)."""
    ...
    if normalized == "nous":
        ...
        if base_url:
            # Nadia base_url typically looks like https://inference-api.nadicode.ai/v1
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``nadia model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from nadia_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def curated_models_for_provider(
    ...
    *,
    ...
    # Try live API first (Codex, Nadia, etc. all support /models)
...
def _resolve_static_model_alias(
    ...
    try:
        from nadia_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def detect_static_provider_for_model(
    ...
    # If someone types `/model nadia` or `/model anthropic`, treat it as a
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Nadia's canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``nadia_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Nadia should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``nadia auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.nadia/.env``.
    ...
    """
    try:
        from nadia_cli.auth import resolve_api_key_provider_credentials
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    Tries live API endpoints for providers that support them (Codex, Nadia),
    falling back to static lists. For providers in ``_MODELS_DEV_PREFERRED``
    (opencode-go/zen, xiaomi, deepseek, smaller inference providers, etc.),
    models.dev entries are merged on top of curated so new models released
    on the platform appear in ``/model`` without a Nadia release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # a Nadia release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "nous":
        ...
        # added Portal models still surface without a Nadia release.
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # agentic catalog so ``/model`` matches what ``nadia model`` shows.
...
#
# Cache strategy:
#   - One JSON file at $NADIA_HOME/provider_models_cache.json
...
def _provider_models_cache_path() -> Path:
    from nadia_constants import get_nadia_home
    return get_nadia_home() / "provider_models_cache.json"
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _NADIA_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Nadia's default Claude IDs elsewhere use
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _NADIA_USER_AGENT}
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from nadia_constants import get_nadia_home
    return get_nadia_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "moa":
        try:
            ...
            return {
                ...
                "message": f"MoA preset `{requested}` was not found. Run `nadia moa list`.",
            }
        except Exception as exc:
            return {
                "accepted": False, "persist": False, "recognized": False,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Nadia will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Nadia cannot verify the model name."
    ...
    if api_models is not None:
        ...
        if requested_for_lookup in set(api_models):
            ...
            # fetch) so we only accept models Nadia actually ships.  (#46850)
