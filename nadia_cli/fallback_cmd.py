"""
nadia fallback — manage the fallback provider chain.
...
Subcommands:
  nadia fallback [list]   Show the current fallback chain (default when no subcommand)
  nadia fallback add      Pick provider + model via the same picker as `nadia model`,
                           then append the selection to the chain
  nadia fallback remove   Pick an entry to delete from the chain
  nadia fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.nadia/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from nadia_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `nadia model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  nadia fallback add")
        print()
        return
    ...
    print("  Docs: https://docs.nadicode.ai/nadia/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `nadia model`, then append the selection to the chain."""
    from nadia_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `nadia model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `nadia fallback list` to view, or `nadia fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``nadia fallback [subcommand]``."""
