...
def resolve_requested_provider(requested: Optional[str] = None) -> str:
    ...
    env_provider = os.getenv("NADIA_INFERENCE_PROVIDER", "").strip().lower()
    if env_provider:
        return env_provider
    ...
    if pool and pool.has_credentials():
        ...
        if provider == "nous" and entry is not None and pool_api_key:
            ...
            if not _agent_key_is_usable(nous_state, min_ttl):
                logger.debug("Nadia pool entry agent_key expired/missing, falling through to runtime resolution")
                pool_api_key = ""
