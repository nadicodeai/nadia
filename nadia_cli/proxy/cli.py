"""CLI handlers for the ``nadia proxy`` subcommand."""
...
from nadia_cli.proxy.adapters import ADAPTERS, get_adapter
from nadia_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "nadia proxy requires aiohttp. Install one of:\n"
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"nadia auth add {adapter.name}")
        print(
    ...
    print(
        f"Starting Nadia proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Nadia proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: nadia proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``nadia proxy <subcommand>``."""
    ...
    print(
        "nadia proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  nadia proxy start [--provider nadia|xai] [--host 127.0.0.1] [--port 8645]\n"
