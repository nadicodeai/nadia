...
# Endpoints inference-api.nadicode.ai actually serves. Anything else
...
class NousPortalAdapter(UpstreamAdapter):
    ...
    def get_retry_credential(
        ...
        *,
        ...
        if status_code != 401:
            return None
        logger.info("proxy: Nadia upstream rejected bearer; force-refreshing invoke JWT")
        return self._get_credential(
            force_refresh=True,
        )
    ...
    # to nadia_cli.auth to avoid expanding that module's public surface.
    # ------------------------------------------------------------------
    ...
    def _save_state(
        ...
        *,
        ...
        try:
            ...
            logger.warning("proxy: failed to persist Nadia quarantine state: %s", exc)
