"""
Configuration management for Nadia Agent.

Config files are stored in ~/.nadia/ for easy access:
- ~/.nadia/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.nadia/.env         - API keys and secrets

This module provides:
- nadia config          - Show current configuration
- nadia config edit     - Open config in editor
- nadia config set      - Set a specific value
- nadia config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``nadia config set`` (which rewrites
    ...
    ``config.yaml`` in place: nadia never silently mutates the user's config,
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.nadia/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``nadia logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  nadia config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Nadia makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Nadia itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``nadia update``, the TUI build.
...
#   on every plugin install / ``nadia update``.
...
# * ``NADIA_HOME`` / ``NADIA_PROFILE`` / ``NADIA_CONFIG`` /
#   ``NADIA_ENV`` — Nadia runtime location flags. Writing these into
...
#
# IMPORTANT: ``NADIA_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (NADIA_LANGFUSE_PUBLIC_KEY,
# NADIA_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Nadia runtime location — never via dashboard env writer.
    # NOT a NADIA_* blanket: integration credentials (NADIA_GEMINI_*,
    # NADIA_LANGFUSE_*, NADIA_SPOTIFY_*, ...) ARE allowed.
    "NADIA_HOME", "NADIA_PROFILE", "NADIA_CONFIG", "NADIA_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Nadia runtime location "
            "(NADIA_HOME, NADIA_PROFILE, ...) cannot be persisted via "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    "NADIA_TOOL_PROGRESS", "NADIA_TOOL_PROGRESS_MODE",
    ...
    # Activation is via plugins.enabled (opt-in through `nadia plugins enable
    # observability/langfuse` or `nadia tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "NADIA_LANGFUSE_ENV",
    "NADIA_LANGFUSE_RELEASE",
    "NADIA_LANGFUSE_SAMPLE_RATE",
    "NADIA_LANGFUSE_MAX_CHARS",
    "NADIA_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
from nadia_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("NADIA_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_nadia_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Nadia is running in package-manager-managed mode.

    Two signals: the NADIA_MANAGED env var (set by the systemd service),
    or a .managed marker file in NADIA_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade nadia-agent"
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``nadia_cli/`` — i.e. the git checkout for source
    installs, ``/opt/nadia`` inside the published image, the venv's
    site-packages root for pip installs. It is a property of the running
    interpreter, NOT of ``$NADIA_HOME``, which is why a code-scoped stamp
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Nadia was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Code-scoped stamp ``<install tree>/.install_method`` (next to the
       running code) — the authoritative marker.
    2. Legacy home-scoped stamp ``$NADIA_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. NADIA_MANAGED env / .managed marker (NixOS, Homebrew)
    4. .git directory presence -> 'git'
    5. Fallback -> 'pip'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.nadia``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$NADIA_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.nadia:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$NADIA_HOME``, a
    ...
    ``docker`` and ``nadia update`` refuses to run ("doesn't apply inside the
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``ghcr.io/nadicodeai/nadia`` image bakes a ``docker``
        stamp into ``/opt/nadia`` at build time.
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $NADIA_HOME.
    try:
    ...
    #    host install whose shared $NADIA_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``nadia update``.
    try:
        method = (
            (get_nadia_home() / ".install_method")
...
def _running_in_container() -> bool:
    """Thin wrapper around ``nadia_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$NADIA_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/nadia`` in the published image, which instead bakes the stamp at
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* Nadia lives in a ``uv tool`` layout.

    ``uv tool install nadia-agent`` places the install at
    ``.../uv/tools/nadia-agent/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``nadia-agent`` happens to be uv-tool-installed on the machine while
    the *active* Nadia is a regular pip/venv install, causing
    ``nadia update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/nadia-agent/" in norm + "/"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade nadia-agent"
    if method == "docker":
        return "docker pull ghcr.io/nadicodeai/nadia:latest"
    if method == "pip":
        if is_uv_tool_install():
            return "uv tool upgrade nadia-agent"
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade nadia-agent"
        return "pip install --upgrade nadia-agent"
    return "nadia update"
...
# Long-form text for ``nadia update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# nadia_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Nadia, it doesn't update
...
✗ ``nadia update`` doesn't apply inside the Docker container.

Nadia Agent runs as a published image (ghcr.io/nadicodeai/nadia), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull ghcr.io/nadicodeai/nadia:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate nadia-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm ghcr.io/nadicodeai/nadia:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://github.com/nadicodeai/nadia/pkgs/container/nadia
  • Your config and session history live under ``$NADIA_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``nadia update`` inside Docker.
...
def format_managed_message(action: str = "modify this Nadia installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("NADIA_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Nadia installation is managed by NixOS "
            f"(NADIA_MANAGED={env_hint}).\n"
            "Edit services.nadia-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Nadia installation is managed by Homebrew "
            f"(NADIA_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade nadia-agent"
        )

    return (
        f"Cannot {action}: this Nadia installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Nadia."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from NADIA_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, nadia_bin
    or None if container mode is not active, we're already inside the
    container, or NADIA_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("NADIA_DEV") == "1":
        return None
    ...
    container_mode_file = get_nadia_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "nadia-agent")
    exec_user = info.get("exec_user", "nadia")
    nadia_bin = info.get("nadia_bin", "/data/current-package/bin/nadia")
...
# Re-export from nadia_constants — canonical definition lives there.
from nadia_constants import get_nadia_home  # noqa: F811,E402
from utils import atomic_replace, fast_safe_load

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_nadia_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_nadia_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_nadia_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the NADIA_UID / NADIA_GID env vars set by Docker deployments.

    Docker containers running Nadia commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level NADIA_HOME once, but
    subdirectories created at runtime by ``ensure_nadia_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("NADIA_UID", "").strip()
    gid_str = os.environ.get("NADIA_GID", "").strip()
    try:
...
def _chown_to_nadia_uid(path) -> None:
    """Chown ``path`` to ``NADIA_UID:NADIA_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_nadia_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_nadia_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the nadia group can
    share state with the gateway service.

    The mode can be overridden via the NADIA_HOME_MODE environment variable
    (e.g. NADIA_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse NADIA_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``NADIA_UID``/``NADIA_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("NADIA_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_nadia_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When Nadia runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("NADIA_CONTAINER") or os.environ.get("NADIA_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set NADIA_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into NADIA_HOME, upgrading legacy empty templates.
...
def ensure_nadia_home():
    """Ensure ~/.nadia directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_nadia_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_nadia_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"NADIA_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Nadia-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Nadia
        ...
        # identity slot (SOUL.md). Empty by default. The NADIA_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Nadia adds a coding operating brief
    ...
    "terminal": {
        ...
        #             NADIA_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force NADIA_HOME/home when it exists (old strict
        ...
        # skipped. When empty, Nadia auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Nadia sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Nadia
        ...
        # bundled Nadia image, which drops to the `nadia` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Nadia sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Nadia should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``nadia chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.nadia/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: nadia sweeps the checkpoint base at startup
    ...
    # AGENTS.md, CLAUDE.md, .nadia.md, or .cursorrules before Nadia applies
    ...
    # single read_file page exceeds these limits, Nadia truncates the
    ...
    # See tools/kanban_tools.py and nadia_cli/kanban_db.py for the actual
    # implementations. Per-platform notification opt-out is handled by the
    # kanban dashboard (see ``nadia dashboard`` -> Notifications).
    "kanban": {
    ...
    "auxiliary": {
        "vision": {
            "provider": "auto",    # auto | openrouter | nadia | codex | custom
        ...
        # Invoked by ``nadia kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``nadia kanban decompose`` and the kanban
        ...
        # ``nadia profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `nadia model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `nadia` (and `nadia chat`) launches by default:
        ...
        # REPL and `--tui` (or NADIA_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `nadia --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `nadia -c` muscle memory.  Default off so existing
        # users aren't surprised.  NADIA_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `nadia --tui` drops a one-time hint
        ...
        # Nadia credits status-bar notices (usage bands, grant-spent, depleted /
        ...
        # e.g. `model · 68% · ~/projects/nadia`. Per-platform overrides go under
    ...
    "dashboard": {
        ...
        # ``NADIA_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # (``NADIA_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # nadia-account-service at deploy time via the
        # ``NADIA_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
        ...
        # Public URL override (env: ``NADIA_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/nadia``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Nadia appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.nadia/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.nadia/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # configured providers (OpenRouter, Nadia, Z.ai, Kimi, etc.) are supported.
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.nadia/cache/delegation/
    ...
    # response. If not, Nadia feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Nadia auto-pauses the goal and
    ...
    "moa": {
        ...
        # <nadia_home>/moa-traces/<session_id>.jsonl. Off by default — turn it
    ...
    # always goes to ~/.nadia/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${NADIA_SKILL_DIR} and ${NADIA_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `nadia curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `nadia curator run --consolidate` overrides
        ...
        # normally restored on every `nadia update`, so pruning them only
        ...
        # skipped), snapshot ~/.nadia/skills/ into
        # ~/.nadia/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `nadia curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for nadia-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Nadia Agent*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (NADIA_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # replace Nadia's built-in platform hint for a single messaging platform
    ...
    # stored approval from ~/.nadia/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or NADIA_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `nadia doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``nadia_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Nadia to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $NADIA_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # Also overridable via NADIA_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `nadia -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `nadia` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `nadia kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     nadia-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Nadia tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.nadia/logs/.
    ...
    "model_catalog": {
        "enabled": True,
        "url": "https://docs.nadicode.ai/nadia/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `nadia model` invocation; network failures
    ...
    "gateway": {
        ...
        # internal NADIA_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # the NADIA_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # `launchctl kickstart ai.nadia.gateway` that defenses 1-2 don't
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.nadia/.env,
        ...
        # behavior: files must live under the Nadia cache, under
        ...
        # user. Bridged to NADIA_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Nadia
        # cache (~/.nadia/cache/{documents,images,audio,video,screenshots})
        ...
        # to NADIA_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # NADIA_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to NADIA_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.nadia/state.db.
    ...
    "sessions": {
        ...
        # default of ``nadia sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.nadia/sessions/session_{sid}.json`` on every turn
    ...
    # ``nadia update`` behaviour.
    "updates": {
        # Run a full ``nadia backup``-style zip of NADIA_HOME before every
        # ``nadia update``.  Backups land in ``<NADIA_HOME>/backups/`` and
        # can be restored with ``nadia import <path>``.  Off by default:
        # zipping a large NADIA_HOME (sessions DB, caches, skills) can add
        # minutes to every update.  The #48200 incident — a ``nadia update
        ...
        "backup_keep": 5,
        # What `nadia update` does with uncommitted local changes to the
    ...
    "lsp": {
        ...
        #              ``<NADIA_HOME>/lsp/bin/`` on first use.
    ...
    # `nadia tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.nadia/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.nadia/.env (or your shell) and never in config.yaml.
            ...
            # ~/.nadia/bin/ on first use.  When False you must install
    ...
    "computer_use": {
        ...
        # by default upstream. Nadia disables it for our users unless they
        # explicitly opt in here. When false (default), Nadia sets
    ...
    # Nadia Desktop (Electron app) launch options. These only affect
    # `nadia desktop`; they do not touch the CLI/gateway.
    "desktop": {
        ...
        # Bridged to the NADIA_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
    },
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials Nadia mints short-lived tokens from. Falls back to "
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'nadia model' for endpoint-specific config)",
    ...
    "FIRECRAWL_GATEWAY_URL": {
        "description": "Exact Firecrawl tool-gateway origin override for Nadia Subscribers only (optional)",
    ...
    "TOOL_GATEWAY_DOMAIN": {
        "description": "Shared tool-gateway domain suffix for Nadia Subscribers only, used to derive vendor hosts, e.g. nadicode.ai -> firecrawl-gateway.nadicode.ai",
    ...
    "TOOL_GATEWAY_SCHEME": {
        "description": "Shared tool-gateway URL scheme for Nadia Subscribers only, used to derive vendor hosts (`https` by default, set `http` for local gateway testing)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nadia Subscriber access token for tool-gateway requests (optional; otherwise read from the Nadia auth store)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Nadia, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @nadia:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. NADIA_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #nadia)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: nadia-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'nadia-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Nadia API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Nadia API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Nadia API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # NADIA_TOOL_PROGRESS and NADIA_TOOL_PROGRESS_MODE are deprecated —
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.nadia.config`` entries, then checks
    ...
    try:
        ...
        # should never break `nadia update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Nadia versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``nadia_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'nadia setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Nadia won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'nadia doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("NADIA_HOME", "~/.nadia")
...
def _persist_migration(config: Dict[str, Any]) -> None:
    ...
    DEFAULT_CONFIG dumps (the "nadia update / nadia -p blows up my config"
    reports).
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("NADIA_TOOL_PROGRESS")
            old_mode = get_env_value("NADIA_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from NADIA_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from NADIA_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("NADIA_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from NADIA_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$NADIA_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_nadia_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `nadia plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `nadia curator status` has no
    ...
    #   3. Creates `~/.nadia/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_nadia_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_nadia_home() / "logs" / "curator"
            ...
            if added_curator:
                ...
                if not quiet:
                    print(
                        ...
                        f"({', '.join(added_curator)}) — edit via `nadia config set`"
                    )
            if added_aux:
                results["config_added"].append(
                ...
                if not quiet:
                    print(
                        ...
                        f"({', '.join(added_aux)}) — edit via `nadia config set`"
                    )
    ...
    # behavior set it to true explicitly via `nadia config set`.
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from nadia_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    try:
        ...
        from nadia_cli.toolset_validation import validate_platform_toolsets
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: nadia config set <key> <value>")
    ...
    # `nadia update`, but only the version bump is persisted.
    ...
    # metadata.nadia.config in their SKILL.md frontmatter.
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``nadia config set``
    ...
    backends) — while display paths (``nadia status``/``dump``) read ``name``
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _nadia_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.nadia/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.nadia/config.yaml.
    ...
    (which change ``NADIA_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/nadia/config.yaml invalidates the
...
def save_config(
    ...
    *,
    ...
    """Save configuration to ~/.nadia/config.yaml.\n
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Nadia writes itself."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.nadia/.env.
    ...
    interactive menu render (`nadia tools`, `nadia setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `nadia tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Nadia env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.nadia/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.nadia/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.nadia/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.nadia/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Nadia — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Nadia vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.nadia/.env or environment."""
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.nadia/.env`` over ``os.environ``.

    Used for Nadia-managed credentials where a deliberate edit to ``.env``
...
def show_config():
    ...
    print(color("│              ⚕ Nadia Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from nadia_cli.auth import get_anthropic_key
    ...
    # Warn on stale NADIA_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("NADIA_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale NADIA_MAX_ITERATIONS={_env_ghost} "
                f"(run 'nadia doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    print(color("  nadia config edit     # Edit config file", Colors.DIM))
    print(color("  nadia config set <key> <value>", Colors.DIM))
    print(color("  nadia setup           # Run setup wizard", Colors.DIM))
    print()
...
def set_config_value(key: str, value: str):
    ...
    # so a fresh `nadia config set model.api_base ...` lands on the canonical
    ...
    # — e.g. `nadia config set model.api_key cfut_...` routes to config.yaml
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: nadia config set <key> <value>")
            print()
            print("Examples:")
            print("  nadia config set model anthropic/claude-sonnet-4")
            print("  nadia config set terminal.backend docker")
            print("  nadia config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'nadia config migrate' to add them")
        ...
        print("  nadia config           Show current configuration")
        print("  nadia config edit      Open config in editor")
        print("  nadia config set <key> <value>   Set a config value")
        print("  nadia config check     Check for missing/outdated config")
        print("  nadia config migrate   Update config with new options")
        print("  nadia config path      Show config file path")
        print("  nadia config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``nadia config`` UI so users
