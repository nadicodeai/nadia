...
# integration credentials follow that prefix (NADIA_GEMINI_CLIENT_ID,
# NADIA_LANGFUSE_PUBLIC_KEY, NADIA_SPOTIFY_CLIENT_ID, ...). The
...
from nadia_cli.default_soul import DEFAULT_SOUL_MD
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    ...
    Resolution order:
    1. Stamped ``~/.nadia/.install_method`` file (written by installers)
    2. NADIA_MANAGED env / .managed marker (NixOS, Homebrew)
    ...
    container detection here:
      ...
      - the published ``ghcr.io/nadicodeai/nadia`` image stamps ``docker``
    ...
    so ``nadia update`` bailed with "doesn't apply inside the Docker
    ...
    """
    stamp = get_nadia_home() / ".install_method"
    try:
...
def stamp_install_method(method: str) -> None:
    """Write the install method to ~/.nadia/.install_method."""
    stamp = get_nadia_home() / ".install_method"
    try:
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into NADIA_HOME if the user doesn't have one yet."""
...
DEFAULT_CONFIG = {
    ...
    "updates": {
        ...
        # on large NADIA_HOME directories the zip can add minutes to every
