...
from nadia_cli.config import get_env_value, load_config
...
# coverage category (nadia_cli.nous_account.TOOL_COVERAGE_CATEGORIES). Lets the
# `nadia tools` picker scope its entitlement gate to the selected backend, so a
...
def _local_stt_backend_available() -> bool:
    ...
    """
    if get_env_value("NADIA_LOCAL_STT_COMMAND"):
        return True
    try:
...
def get_nous_subscription_features(
    ...
    *,
    ...
    # requires `pip install faster-whisper`. For Nadia subscribers we'd
    ...
    # Tool Gateway via `nadia model`, so direct credentials should NOT
    ...
    try:
        ...
        local_stt_available = bool(_HAS_FASTER_WHISPER) or bool(
            get_env_value("NADIA_LOCAL_STT_COMMAND")
        )
    except Exception:
        local_stt_available = bool(get_env_value("NADIA_LOCAL_STT_COMMAND"))
...
def apply_nous_managed_defaults(
    ...
    *,
    ...
    # (requires `pip install faster-whisper`); for Nadia subscribers we
...
def get_gateway_eligible_tools(
    ...
    *,
    ...
    All lists are empty when the user is not a paid Nadia subscriber or
    is not using Nadia as their provider.
    """
...
def prompt_enable_tool_gateway(
    ...
    *,
    ...
    try:
        from nadia_cli.setup import prompt_checklist
    except Exception:
        return set()
...
def _run_nous_portal_login_only(*, capability: str) -> bool:
    ...
    try:
        ...
        # switches the user's inference provider to Nadia.
        ...
        if shared:
            try:
                do_import = input(
                    "  Found existing Nadia OAuth credentials. Import them? [Y/n]: "
                ).strip().lower()
            except (EOFError, KeyboardInterrupt):
                do_import = "y"
