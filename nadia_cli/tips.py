...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or nadia -c.",
    ...
    # /skin is retired; Nadia appearance is not an operator skin choice.
    ...
    "Ctrl+Z suspends Nadia to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "nadia -c resumes your most recent CLI session. nadia -c \"project name\" resumes by title.",
    "nadia -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "nadia -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "nadia chat -t web,terminal enables only specific toolsets for a focused session.",
    "nadia chat -s github-pr-workflow preloads a skill at launch.",
    "nadia chat -q \"query\" runs a single non-interactive query and exits.",
    "nadia chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "nadia chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "nadia --yolo bypasses all dangerous command approval prompts for the entire session.",
    "nadia chat --source telegram tags the session for filtering in nadia sessions list.",
    "nadia -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "nadia doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "nadia dump outputs a compact setup summary — great for bug reports.",
    "nadia config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "nadia config edit opens config.yaml in your default editor.",
    "nadia config check scans for missing or stale configuration options.",
    "nadia sessions browse opens an interactive session picker with search.",
    "nadia sessions stats shows session counts by platform and database size.",
    "nadia sessions prune --older-than 30 cleans up old sessions.",
    "nadia skills search react --source skills-sh searches the skills.sh public directory.",
    "nadia skills check scans installed hub skills for upstream updates.",
    "nadia skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "nadia skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "nadia mcp add github --command npx adds MCP servers from the command line.",
    "nadia mcp serve runs Nadia itself as an MCP server for other agents.",
    "nadia auth add lets you add multiple API keys for credential pool rotation.",
    "nadia completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "nadia logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "nadia backup creates a zip backup of your entire Nadia home directory.",
    "nadia profile create coder creates an isolated profile that becomes its own command.",
    "nadia profile create work --clone copies your current config and keys to a new profile.",
    "nadia update syncs new bundled skills to ALL profiles automatically.",
    "nadia gateway install sets up Nadia as a system service (systemd/launchd).",
    "nadia memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "nadia webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: nadia tools disables unused tools, nadia skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "nadia models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as nadia -w).",
    ...
    "NADIA_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Nadia tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'nadia profile create coder' creates the 'coder' command.",
    "nadia profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Nadia prints a resume command with session ID and stats.",
    "nadia sessions export backup.jsonl exports all sessions for backup or analysis.",
    "nadia -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "nadia skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .nadia/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: nadia cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Nadia runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "nadia gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Nadia via a local macOS server.",
    ...
    "Context files (.nadia.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "Nadia loads project context from .nadia.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "nadia mcp opens an interactive picker of Nadia-approved MCPs you can install in one keystroke.",
    "nadia mcp catalog lists Nadia-approved MCP servers shipped with the repo.",
    "nadia mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — nadia tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.nadia/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "nadia chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "nadia plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "nadia acp runs Nadia as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "NADIA_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "nadia auth add nous or nadia auth add openai-codex sets up OAuth-based providers.",
    ...
    "nadia status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.nadia/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Nadia your own.",
    ...
    "Before a gateway session reset, Nadia auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.nadia/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Profiles scope Nadia state via NADIA_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "NADIA_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in NADIA_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.nadia/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'nadia profile create ls' would be rejected.",
    "nadia profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "nadia claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "NADIA_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via nadia tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.nadia/.env.",
    "Skills from trusted repos (NadicodeAI) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.nadia/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Nadia auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Nadia config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Nadia is working.',
    ...
    '/reload reloads ~/.nadia/.env into the running session — pick up new API keys without restarting.',
    ...
    "NADIA_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.nadia/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.nadia/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'nadia curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "nadia curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'nadia curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',
    ...
    # --- TUI & Dashboard ---
    'NADIA_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "NADIA_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'NADIA_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'NADIA_WRITE_SAFE_ROOT restricts write_file/patch to directory prefixes; multiple paths via os.pathsep (: or ;).',
    'NADIA_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'NADIA_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.nadia/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.nadia/skills/ so drop-in skills appear without restarting the session.',
    ...
    'nadia -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'nadia chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'nadia chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'nadia chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "nadia chat --source tool tags programmatic chats so they don't clutter nadia sessions list.",
    'nadia dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'nadia sessions rename <ID> "new title" renames any past session; nadia sessions delete <ID> removes one.',
    'nadia import restores a session export or profile archive produced by sessions export or profile export.',
    'nadia fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'nadia pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'nadia setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'nadia status --deep runs the full health sweep across every component; plain nadia status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'NADIA_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'NADIA_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.nadia/plugins/ — trust-gated by design.',
    "NADIA_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'NADIA_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'NADIA_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'NADIA_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'NADIA_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'NADIA_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'NADIA_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'NADIA_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'NADIA_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'NADIA_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'NADIA_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Nadia block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from nadia sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.nadia/dashboard-plugins/.',
]
