...
the tool registry or launching Nadia (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``nadia-cli`` to the non-existent ``nadia``. ``resolve_toolset('nadia')``
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``nadia-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``nadia`` instead of ``nadia-cli``),
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `nadia tools` to reconfigure."
        )
    return warnings
