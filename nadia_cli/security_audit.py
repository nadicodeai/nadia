"""On-demand supply-chain audit for Nadia Agent installs.

Scans three surfaces a Nadia user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Nadia venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.nadia/plugins``
...
def _discover_plugins(nadia_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.nadia/plugins``.
    ...
    """
    plugins_dir = nadia_home / "plugins"
...
def run_audit(
    *,
    ...
    home = nadia_home or Path(get_nadia_home())
...
def _count_components(
    *, skip_venv: bool, skip_plugins: bool, skip_mcp: bool, nadia_home: Path
) -> int:
    total = 0
    if not skip_venv:
        total += len(_discover_venv())
    if not skip_plugins:
        total += len(_discover_plugins(nadia_home))
    if not skip_mcp:
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `nadia security audit`."""
    ...
    total = _count_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, nadia_home=home
    )
    if total == 0:
