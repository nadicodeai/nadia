...
Pure logic: walks a Nadia config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `nadia doctor` and a future `nadia migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Nadia config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Nadia config and return retirement issues.
