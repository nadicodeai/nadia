"""Shell completion script generation for nadia CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_nadia_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Nadia Agent bash completion
# Add to ~/.bashrc:
#   eval "$(nadia completion bash)"

_nadia_profiles() {{
    local profiles_dir="$HOME/.nadia/profiles"
...
_nadia_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_nadia_profiles)" -- "$cur"))
        return
    fi
...
complete -F _nadia_completion nadia
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef nadia
# Nadia Agent zsh completion
# Add to ~/.zshrc:
...
_nadia_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.nadia/profiles" ]]; then
        profiles+=($HOME/.nadia/profiles/*(N/:t))
    fi
...
_nadia() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_nadia_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'nadia command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Nadia Agent fish completion",
        "# Add to your config:",
        "#   nadia completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __nadia_profiles",
        "    echo default",
        "    if test -d $HOME/.nadia/profiles",
        "        for d in $HOME/.nadia/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c nadia -f -s p -l profile"
        " -d 'Profile name' -xa '(__nadia_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__nadia_profiles)' -d 'Profile name'"
                )
