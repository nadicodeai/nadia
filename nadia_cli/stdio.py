...
on Japanese locales, etc.).  Nadia's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``nadia_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Nadia is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``NADIA_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("NADIA_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Nadia-managed Git install directories so
    ...
    # already-running shells don't see the change — which means nadia
    ...
    # install without Nadia-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Nadia's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Nadia's
    ``nadia config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Nadia (User env var in Windows Settings, or
    export in a PowerShell profile) and Nadia picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Nadia-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\nadia\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``nadia`` from right after install) retain their old PATH.

    Any subprocess Nadia spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Nadia-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "nadia", "git", "cmd"),
        os.path.join(local_appdata, "nadia", "git", "bin"),
        os.path.join(local_appdata, "nadia", "git", "usr", "bin"),
        # Nadia venv Scripts directory — host of the nadia.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes nadia, but harmless to include.
        os.path.join(local_appdata, "nadia", "nadia-agent", "venv", "Scripts"),
