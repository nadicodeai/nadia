"""``nadia gateway enroll`` — enroll a self-hosted gateway with a relay connector.
...
``docs/connector-gateway-auth-design.md``:
  ...
  4. Persist ``GATEWAY_RELAY_ID`` / ``GATEWAY_RELAY_SECRET`` /
     ...
     ``~/.nadia/.env``. The per-gateway secret authenticates the WS upgrade;
...
def cmd_gateway_enroll(args) -> None:
    ...
    if is_managed():
        print(
            "✗ `nadia gateway enroll` is not available in a managed/hosted install.\n"
    ...
    # 1. Resolve a fresh Nadia access token (the tenant-proving identity).
    try:
    ...
    #    save_env_value writes them to ~/.nadia/.env (0600 dir) and never logs.
    to_write = {
