"""nadia webhook — manage dynamic webhook subscriptions from the CLI.
...
Subscriptions persist to ~/.nadia/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _nadia_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: nadia gateway run
"""
...
def webhook_command(args):
    """Entry point for 'nadia webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: nadia webhook {subscribe|list|remove|test}")
        print("Run 'nadia webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (nadia gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: nadia webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from nadia webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (nadia gateway run)")
