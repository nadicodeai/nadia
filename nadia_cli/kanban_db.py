...
def create_task(
    ...
    *,
    ...
    each name to ``nadia --skills ...`` alongside the built-in
...
def _kanban_worker_skill_available(nadia_home: Optional[str]) -> bool:
    """True if the bundled ``kanban-worker`` skill resolves for the home the
    spawned worker will run under.

    The dispatcher injects ``--skills kanban-worker`` into every worker. When
    the worker activates a profile (``nadia -p <name>``), its ``SKILLS_DIR``
    ...
    # An unset NADIA_HOME means the worker falls back to the default root
    # home (``~/.nadia``), which ships the bundled skill.
    base = _Path(nadia_home) if nadia_home else (_Path.home() / ".nadia")
...
def _default_spawn(
    ...
    *,
    ...
    if _kanban_worker_skill_available(env.get("NADIA_HOME")):
