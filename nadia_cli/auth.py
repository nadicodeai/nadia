...
from nadia_cli.config import get_nadia_home, get_config_path, read_raw_config
from nadia_constants import OPENROUTER_BASE_URL, secure_parent_dir
...
NADIA_BILLING_MANAGE_SCOPE = "billing:manage"
DEFAULT_NOUS_SCOPE = NADIA_INFERENCE_INVOKE_SCOPE
...
try:  # Version tag for the Codex token-endpoint User-Agent; fall back if unavailable.
    from nadia_cli import __version__ as _NADIA_CLI_VERSION
except Exception:  # pragma: no cover - version import should always succeed
    _NADIA_CLI_VERSION = "unknown"
CODEX_OAUTH_USER_AGENT = f"nadia-cli/{_NADIA_CLI_VERSION}"
...
SPOTIFY_DOCS_URL = "https://docs.nadicode.ai/nadia/user-guide/features/spotify"
SPOTIFY_DASHBOARD_URL = "https://developer.spotify.com/dashboard"
SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120

XAI_OAUTH_DOCS_URL = "https://docs.nadicode.ai/nadia/guides/xai-grok-oauth"
OAUTH_OVER_SSH_DOCS_URL = "https://docs.nadicode.ai/nadia/guides/oauth-over-ssh"
...
def get_anthropic_key() -> str:
    ...
    ``~/.nadia/.env`` so a deliberate key rotation isn't shadowed by a stale
    ...
    """
    from nadia_cli.config import get_env_value_prefer_dotenv
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from nadia_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
    ...
    for env_var in pconfig.api_key_env_vars:
        # Prefer ~/.nadia/.env over os.environ so a deliberate key rotation
...
def is_rate_limited_auth_error(error: Exception) -> bool:
    ...
    instead of prompting the operator to run ``nadia auth``.
    """
    return (
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `nadia model` to re-authenticate."
...
def _oauth_trace_enabled() -> bool:
    raw = os.getenv("NADIA_OAUTH_TRACE", "").strip().lower()
...
# =============================================================================
# Auth Store — persistence layer for ~/.nadia/auth.json
# =============================================================================

def _auth_file_path() -> Path:
    path = get_nadia_home() / "auth.json"
    # Seat belt: if pytest is running and NADIA_HOME resolves to the real
    # user's auth store, refuse rather than silently corrupt it. This catches
    # tests that forgot to monkeypatch NADIA_HOME, tests invoked without the
    # hermetic conftest, or sandbox escapes via threads/subprocesses. In
    # production (no PYTEST_CURRENT_TEST) this is a single dict lookup.
    if os.environ.get("PYTEST_CURRENT_TEST"):
        real_home_auth = (Path.home() / ".nadia" / "auth.json").resolve(strict=False)
        try:
        ...
        if resolved == real_home_auth:
            raise RuntimeError(
                ...
                "Set NADIA_HOME to a tmp_path in your test fixture, or run "
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    directory (classic mode, or custom NADIA_HOME that is not a profile).
    ...
    try:
        ...
        global_root = get_default_nadia_root()
    except Exception:
        return None
    profile_home = get_nadia_home()
    try:
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    ``~/.nadia/auth.json`` even when NADIA_HOME is set to a profile
    path. The hermetic conftest does not redirect ``HOME``, so
    ``get_default_nadia_root()`` for a profile-shaped NADIA_HOME can
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if real_home_env:
            real_root = Path(real_home_env) / ".nadia" / "auth.json"
            try:
...
def _file_lock(
    ...
    (e.g. profile auth.json vs shared Nadia store) don't share reentrancy
...
def _auth_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    ...
    (outer) and the shared Nadia lock SECOND (inner). All runtime
...
def _load_provider_state_with_source(
    ...
    global-root fallback state; persisting a rotated Nadia refresh token only to
...
def _load_provider_state(auth_store: Dict[str, Any], provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    global scope. Once the user runs ``nadia auth login <provider>`` inside
...
def mark_provider_active_if_unset(provider_id: str) -> None:
    ...
    Used by ``nadia auth add`` OAuth paths that create credential-pool
...
def read_credential_pool(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    ``nadia auth add <provider>`` inside the profile, profile entries
...
def get_provider_auth_state(provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    OAuth or Nadia device-code session). See issue #18594 follow-up.
    """
    auth_store = _load_auth_store()
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    # not by the user explicitly configuring anthropic in Nadia.
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `nadia logout`.
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    """
    try:
        from nadia_cli.config import validate_config_structure
        issues = validate_config_structure()
        if not issues:
            return ""

        lines = ["Config issue detected — run 'nadia doctor' for full diagnostics:"]
        for ci in issues:
    ...
    *,
    ...
    if normalized != "auto":
        ...
        if _config_hint:
            ...
            msg += " Check 'nadia model' for available providers, or run 'nadia doctor' to diagnose config issues."
    ...
    # Auto-detect an OpenRouter credential added via `nadia auth add openrouter`
    ...
    # `nadia auth list` showing the credential while requests go out with no
    ...
    for pid, pconfig in PROVIDER_REGISTRY.items():
        ...
        for env_var in pconfig.api_key_env_vars:
            if has_usable_secret(os.getenv(env_var, "")):
                ...
                # OAuth but has a stale key in ~/.nadia/.env isn't silently
    ...
    raise AuthError(
        "No inference provider configured. Run 'nadia model' to choose a "
        "provider and model, or set an API key (OPENROUTER_API_KEY, "
        "OPENAI_API_KEY, etc.) in ~/.nadia/.env.",
...
# Allowlist of valid NadicodeAI Portal hosts. A portal_base_url outside this
...
def _migrate_stale_nous_portal_url(providers: Dict[str, Any]) -> None:
    nadia = providers.get("nous")
    if not isinstance(nadia, dict):
        return
    stored = (nadia.get("portal_base_url") or "").strip()
    if stored:
        parsed = urlparse(stored)
        if parsed.hostname in _NOUS_STALE_PORTAL_HOSTS:
            logger.warning(
                "auth: migrating stale NadicodeAI Portal_base_url %s -> %s",
                stored, DEFAULT_NOUS_PORTAL_URL,
            )
            nadia["portal_base_url"] = DEFAULT_NOUS_PORTAL_URL
...
def _validate_nous_inference_url_from_network(url: Optional[str]) -> Optional[str]:
    ...
    if parsed.scheme != "https":
        logger.warning(
            "nadia: refusing non-https inference URL scheme %r from Portal response",
            parsed.scheme,
        )
        return None
    if parsed.hostname not in _ALLOWED_NOUS_INFERENCE_HOSTS:
        logger.warning(
            "nadia: refusing inference URL host %r from Portal response "
...
def _nous_invoke_jwt_status(
    ...
    *,
    ...
    if NADIA_INFERENCE_INVOKE_SCOPE not in scopes:
...
def _log_nous_invoke_jwt_selected(
    *,
    ...
    logger.info("Nadia inference auth: using NAS invoke JWT")
    _oauth_trace(
...
def resolve_qwen_runtime_credentials(
    *,
    ...
    base_url = os.getenv("NADIA_QWEN_BASE_URL", "").strip().rstrip("/") or DEFAULT_QWEN_BASE_URL
    return {
        "provider": "qwen-oauth",
        "base_url": base_url,
...
def get_qwen_auth_status() -> Dict[str, Any]:
    ...
    try:
        ...
        # and `nadia model` walks users into a broken Qwen setup flow.
...
# =============================================================================
# Spotify auth — PKCE tokens stored in ~/.nadia/auth.json
# =============================================================================
...
def _spotify_client_id(
    ...
    candidates = (
        explicit,
        get_env_value("NADIA_SPOTIFY_CLIENT_ID"),
    ...
    for candidate in candidates:
        ...
        if cleaned:
            return cleaned
    raise AuthError(
        "Spotify client_id is required. Set NADIA_SPOTIFY_CLIENT_ID or pass --client-id.",
        provider="spotify",
...
def _spotify_redirect_uri(
    ...
    candidates = (
        explicit,
        get_env_value("NADIA_SPOTIFY_REDIRECT_URI"),
...
def _spotify_api_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("NADIA_SPOTIFY_API_BASE_URL"),
...
def _spotify_accounts_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("NADIA_SPOTIFY_ACCOUNTS_BASE_URL"),
...
def _make_xai_callback_handler(expected_path: str) -> tuple[type[BaseHTTPRequestHandler], dict[str, Any]]:
    ...
    class _XAICallbackHandler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:  # noqa: N802
            ...
            # (#27385 — "callback received but Nadia times out") can produce
            # actionable evidence without a code change.  Logged values are
            # fingerprints / booleans only; no actual code/state strings leak
            # into the log file.  Run with ``NADIA_LOG_LEVEL=INFO`` (or check
            # ``~/.nadia/logs/agent.log`` which captures INFO+ unconditionally).
            try:
                logger.info(
            ...
            if incoming["code"] is None and incoming["error"] is None:
                ...
                body = (
                    ...
                    "<code>nadia auth add xai-oauth</code> to retry.</p>"
                    "</body></html>"
                )
                self.wfile.write(body.encode("utf-8"))
                return
...
def _refresh_spotify_oauth_state(
    ...
    *,
    ...
    if not refresh_token:
        raise AuthError(
            "Spotify refresh token missing. Run `nadia auth spotify` again.",
            provider="spotify",
    ...
    if response.status_code >= 400:
        detail = response.text.strip()
        raise AuthError(
            "Spotify token refresh failed. Run `nadia auth spotify` again."
            + (f" Response: {detail}" if detail else ""),
            provider="spotify",
            code="spotify_refresh_failed",
            relogin_required=True,
...
def resolve_spotify_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "Spotify is not authenticated. Run `nadia auth spotify` first.",
                provider="spotify",
        ...
        if should_refresh:
            try:
                ...
                if exc.relogin_required and state.get("refresh_token"):
                    ...
                    # Mirrors the Nadia / xAI-OAuth / Codex-OAuth / MiniMax pattern.
    ...
    if not access_token:
        raise AuthError(
            "Spotify access token missing. Run `nadia auth spotify` again.",
            provider="spotify",
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    resulting client_id to ~/.nadia/.env, and return it.
    ...
    print("       App name:     anything (e.g. nadia-agent)")
    ...
    # Persist so subsequent `nadia auth spotify` runs skip the wizard.
    save_env_value("NADIA_SPOTIFY_CLIENT_ID", raw)
    ...
    if redirect_uri_hint and redirect_uri_hint != DEFAULT_SPOTIFY_REDIRECT_URI:
        save_env_value("NADIA_SPOTIFY_REDIRECT_URI", redirect_uri_hint)

    print()
    print("Saved NADIA_SPOTIFY_CLIENT_ID to ~/.nadia/.env")
    print()
    return raw
...
def login_spotify_command(args) -> None:
    ...
    # with "NADIA_SPOTIFY_CLIENT_ID is required".
    ...
    print()
    print("Open this URL to authorize Nadia:")
    print(authorize_url)
    print()
...
def _print_loopback_ssh_hint(redirect_uri: str, *, docs_url: str | None = None) -> None:
    ...
    print(divider)
    print(f"Nadia is waiting for the OAuth callback on {redirect_uri}")
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.nadia/auth.json (not ~/.codex/)
#
# Nadia maintains its own Codex OAuth session separate from the Codex CLI
# and VS Code extension. This prevents refresh token rotation conflicts
# where one app's refresh invalidates the other's session.
# =============================================================================

def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from Nadia auth store (~/.nadia/auth.json).
    ...
    if not state:
        raise AuthError(
            "No Codex credentials stored. Run `nadia auth` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "Codex auth state is missing tokens. Run `nadia auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            "Codex auth is missing access_token. Run `nadia auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "Codex auth is missing refresh_token. Run `nadia auth` to re-authenticate.",
            provider="openai-codex",
...
def _sync_codex_pool_entries(
    ...
    * ``device_code`` — the singleton-seeded entry written by the device-code
      OAuth flow when the user logged in via ``nadia setup`` / the model
      picker.  Always synced with the fresh tokens.
    * ``manual:device_code`` — entries created by ``nadia auth add openai-codex``
      ...
      ``nadia auth add openai-codex`` and must not be overwritten by a
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None, label: str = None) -> None:
    """Save Codex OAuth tokens to Nadia auth store (~/.nadia/auth.json)."""
    ...
    with _auth_store_lock():
        ...
        # ``nadia auth add openai-codex`` created (which must not be
...
def _recover_codex_tokens_from_cli(reason: str) -> Optional[Dict[str, str]]:
    """Adopt a valid Codex CLI token pair into Nadia auth, if available."""
...
def refresh_codex_oauth_pure(
    ...
    *,
    ...
    """Refresh Codex OAuth tokens without mutating Nadia auth state."""
    ...
    if response.status_code == 429:
        ...
        # "run nadia auth" prompt (see issue #32790).
    ...
    if response.status_code != 200:
        ...
        if code == "refresh_token_reused":
            message = (
                ...
                "then run `nadia auth` to re-authenticate."
            )
            relogin_required = True
...
def _refresh_codex_auth_tokens(
    ...
    Saves the new tokens to Nadia auth store automatically.
    """
    try:
        ...
        # Self-heal cross-store refresh_token rotation. Nadia keeps its OWN
        ...
        # the Codex CLI (or another Nadia process) rotates the shared token,
...
def resolve_codex_runtime_credentials(
    *,
    ...
    """Resolve runtime credentials from Nadia's own Codex token store.
    ...
    if data is None:
        ...
        if pool_token:
            base_url = (
                os.getenv("NADIA_CODEX_BASE_URL", "").strip().rstrip("/")
    ...
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = env_float("NADIA_CODEX_REFRESH_TIMEOUT_SECONDS", 20)

    should_refresh = bool(force_refresh)
    if (not should_refresh) and refresh_if_expiring:
        should_refresh = _codex_access_token_is_expiring(access_token, refresh_skew_seconds)
    if should_refresh:
        # Re-read under lock to avoid racing with other Nadia processes
...
# =============================================================================
# xAI Grok OAuth — tokens stored in ~/.nadia/auth.json
# =============================================================================
...
def _read_xai_oauth_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    ...
    if not state:
        raise AuthError(
            "No xAI OAuth credentials stored. Select xAI Grok OAuth (SuperGrok / Premium+) in `nadia model`.",
            provider="xai-oauth",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "xAI OAuth state is missing tokens. Re-authenticate with `nadia model`.",
            provider="xai-oauth",
    ...
    if not access_token:
        raise AuthError(
            "xAI OAuth state is missing access_token. Re-authenticate with `nadia model`.",
            provider="xai-oauth",
    ...
    if not refresh_token:
        raise AuthError(
            "xAI OAuth state is missing refresh_token. Re-authenticate with `nadia model`.",
            provider="xai-oauth",
...
def _write_through_xai_oauth_to_global_root(state: Dict[str, Any]) -> None:
    ...
    # ~/.nadia/auth.json even when NADIA_HOME points at a profile path
...
def _xai_validate_oauth_endpoint(url: str, *, field: str) -> str:
    ...
    output is cached in ``~/.nadia/auth.json``. A single MITM during initial
    ...
    if host != "x.ai" and not host.endswith(".x.ai"):
        raise AuthError(
            ...
            f"discovery; re-authenticate with `nadia model` to re-fetch.",
            provider="xai-oauth",
            code="xai_discovery_invalid",
        )
    return url
...
def _xai_validate_inference_base_url(value: str, *, fallback: str) -> str:
    ...
    the user's SuperGrok subscription. ``XAI_BASE_URL`` / ``NADIA_XAI_BASE_URL``
...
def refresh_xai_oauth_pure(
    ...
    *,
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "xAI OAuth is missing refresh_token. Re-authenticate with `nadia model`.",
            provider="xai-oauth",
            code="xai_auth_missing_refresh_token",
            relogin_required=True,
        )
    ...
    # written by an older Nadia (or hand-edited) may carry a non-xAI
    ...
    # with a clear error so the user can re-run `nadia model` to refetch.
    ...
    if response.status_code != 200:
        ...
        # on the allowlist for API access).  Re-running ``nadia model``
...
def resolve_xai_oauth_runtime_credentials(
    *,
    ...
    tokens = dict(data["tokens"])
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = env_float("NADIA_XAI_REFRESH_TIMEOUT_SECONDS", 20)
...
# -----------------------------------------------------------------------------
# Shared Nadia token store — lets OAuth credentials persist across profiles
# so a new `nadia --profile <name> auth add nadia --type oauth` can one-tap
# import instead of running the full device-code flow every time.
#
# File lives at ${NADIA_SHARED_AUTH_DIR}/nous_auth.json, defaulting to
# ``<nadia-root>/shared/nous_auth.json`` where ``<nadia-root>`` is what
# ``get_default_nadia_root()`` returns — ``~/.nadia`` on Linux/macOS,
# ``%LOCALAPPDATA%\nadia`` on native Windows, or the Docker/custom root.
# It is OUTSIDE any named profile's NADIA_HOME so named profiles (which
# typically live under ``<nadia-root>/profiles/<name>/``) all see the
# same file.
#
...
def _nous_shared_auth_dir() -> Path:
    """Resolve the directory that holds the shared Nadia token store.

    Honors ``NADIA_SHARED_AUTH_DIR`` so tests can redirect it to a tmp
    path without touching the real user's home. Defaults to
    ``<nadia-root>/shared/``, where ``<nadia-root>`` is what
    :func:`nadia_constants.get_default_nadia_root` returns — so
    Linux/macOS classic installs land at ``~/.nadia/shared/``, native
    Windows installs at ``%LOCALAPPDATA%\\nadia\\shared\\``, and
    Docker / custom ``NADIA_HOME`` deployments at
    ``<NADIA_HOME>/shared/``. Sits outside any named profile so all
    profiles under the same root share the store.
    """
    override = os.getenv("NADIA_SHARED_AUTH_DIR", "").strip()
    if override:
        return Path(override).expanduser()
    from nadia_constants import get_default_nadia_root
    return get_default_nadia_root() / "shared"


def _nous_shared_store_path() -> Path:
    path = _nous_shared_auth_dir() / NOUS_SHARED_STORE_FILENAME
    # Seat belt: if pytest is running and this resolves to a path under the
    # real user's Nadia root, refuse rather than silently corrupt cross-profile
    # state. Tests must set NADIA_SHARED_AUTH_DIR to a tmp_path (conftest
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        from nadia_constants import get_default_nadia_root
        real_home_shared = (
            get_default_nadia_root() / "shared" / NOUS_SHARED_STORE_FILENAME
        ...
        if resolved == real_home_shared:
            raise RuntimeError(
                f"Refusing to touch real user shared Nadia auth store during test run: "
                f"{path}. Set NADIA_SHARED_AUTH_DIR to a tmp_path in your test fixture."
            )
    return path
...
def _nous_shared_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    """Cross-profile lock for the shared Nadia OAuth store.
    ...
    try:
        ...
        # No NADIA_HOME yet (pre-setup): fall through without locking.
        yield
        return
    ...
    with _file_lock(
        ...
        timeout_seconds,
        "Timed out waiting for shared Nadia auth lock",
    ):
        yield


def _merge_shared_nous_oauth_state(state: Dict[str, Any]) -> bool:
    """Copy fresher shared OAuth tokens into a profile-local Nadia state."""
...
def _write_shared_nous_state(state: Dict[str, Any]) -> None:
    """Persist a minimal copy of the Nadia OAuth state to the shared store.
    ...
    try:
        with _nous_shared_store_lock():
            ...
            # Create with 0o600 atomically via os.open(O_EXCL) — closes the TOCTOU
            # window where write_text() + post-write chmod briefly exposed Nadia
        ...
        logger.debug("Failed to write shared Nadia auth store: %s", exc)


def _read_shared_nous_state() -> Optional[Dict[str, Any]]:
    """Return the shared Nadia OAuth state if present and well-formed.
    ...
    try:
        ...
        logger.debug("Shared Nadia auth store at %s is unreadable: %s", path, exc)
        return None
    if not isinstance(payload, dict):
        return None
...
def _clear_shared_nous_state(reason: str) -> None:
    """Remove the shared Nadia OAuth store after a terminal token failure."""
    try:
        with _nous_shared_store_lock():
            path = _nous_shared_store_path()
            try:
        ...
        logger.debug("Failed to clear shared Nadia auth store: %s", exc)


def _is_terminal_nous_refresh_error(exc: Exception) -> bool:
    """True when retrying the same Nadia refresh token cannot succeed."""
    return (
...
def _quarantine_nous_pool_entries(
    ...
    *,
    ...
    """Remove singleton-seeded Nadia pool entries that contain dead OAuth state."""
    pool = auth_store.get("credential_pool")
    if not isinstance(pool, dict):
        return False
...
def _try_import_shared_nous_state(
    *,
    ...
    """Attempt to rehydrate Nadia OAuth state from the shared store.
    ...
    try:
        ...
        logger.debug("Shared Nadia import failed: %s", exc)
        return None
    except Exception as exc:
        _oauth_trace(
            "nous_shared_import_failed",
...
def _refresh_access_token(
    *,
    ...
    # retires the original RT, Nadia's next refresh uses it, and the whole
    ...
    if code == "refresh_token_reused" or "reuse" in lowered or "reuse detected" in lowered:
        description = (
            ...
            "Nadia refresh tokens are single-use — only Nadia may call the "
            "refresh endpoint. For health checks, use `nadia auth status` "
            "instead.\n"
...
def fetch_nous_models(
    *,
    ...
    """Fetch available model IDs from the Nadia inference API."""
    ...
    for item in data:
        ...
        if isinstance(model_id, str) and model_id.strip():
            mid = model_id.strip()
            # Skip Nadia models — they're not reliable for agentic tool-calling
            if "nadia" in mid.lower():
                continue
    ...
    *,
    ...
    with _auth_store_lock():
        ...
        portal_base_url = (
            ...
            or os.getenv("NADIA_PORTAL_BASE_URL")
...
def refresh_nous_oauth_pure(
    ...
    *,
    ...
    """Refresh Nadia OAuth state without mutating auth.json directly.
...
def refresh_nous_oauth_from_state(
    ...
    *,
    ...
    """Refresh Nadia OAuth from a state dict. Thin wrapper around refresh_nous_oauth_pure."""
    tls = state.get("tls") or {}
    return refresh_nous_oauth_pure(
        state.get("access_token", ""),
        state.get("refresh_token", ""),
        state.get("client_id", "nadia-cli"),
...
def persist_nous_credentials(
    ...
    *,
    ...
    """Persist Nadia OAuth credentials as the singleton provider state
    and ensure the credential pool is in sync.

    Nadia credentials are read at runtime from two independent locations:
    ...
    Historically ``nadia auth add nous`` wrote a ``manual:device_code`` pool
    ...
    ``nadia auth add nous --label <name>``).  It gets embedded in the
    ...
    # these credentials via `nadia auth add nous --type oauth`. Best-
...
def _sync_nous_pool_from_auth_store() -> None:
    ...
    try:
        ...
        logger.debug("Failed to sync Nadia credential pool from auth store: %s", exc)
...
def resolve_nous_runtime_credentials(
    *,
    ...
    """
    Resolve Nadia inference credentials for runtime use.
    ...
    with _auth_store_lock():
        ...
        def _persist_state(reason: str) -> None:
            ...
            # the mtime-keyed Nadia auth-status cache warm during read paths.
            if (
    ...
    if not isinstance(api_key, str) or not api_key:
        raise AuthError("Failed to resolve a Nadia inference API key",
...
# `nadia auth login/logout/add/remove` invalidate naturally on the next call.
...
def invalidate_nous_auth_status_cache() -> None:
    ...
    Call this from any code path that mutates Nadia auth state without going
...
def get_nous_auth_status() -> Dict[str, Any]:
    """Status snapshot for Nadia auth.
...
def get_codex_auth_status() -> Dict[str, Any]:
    ...
    Checks the credential pool first (where `nadia auth` stores credentials),
    then falls back to the legacy provider state.
    """
    # Check credential pool first — this is where `nadia auth` and
    # `nadia model` store device_code tokens.
    try:
        from agent.credential_pool import load_pool
...
def get_external_process_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    command = (
        os.getenv("NADIA_COPILOT_ACP_COMMAND", "").strip()
        or os.getenv("COPILOT_CLI_PATH", "").strip()
        or "copilot"
    )
    raw_args = os.getenv("NADIA_COPILOT_ACP_ARGS", "").strip()
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    checks:
      ...
      * ``auth_mode == "entra_id"`` AND ``azure-identity`` is importable
        (we do NOT mint a token here; ``nadia doctor`` runs the live
    ...
    try:
        from nadia_cli.config import load_config, get_env_value_prefer_dotenv
        cfg = load_config()
    except Exception:
        cfg = {}
        ...
        try:
            ...
            if not installed:
                info["hint"] = (
                    ...
                    "pip install azure-identity  (or rely on Nadia's "
                    "lazy-install at first use)."
                )
            else:
                info["hint"] = (
                    "azure-identity is installed; live credential validation "
                    "is skipped here. Run `nadia doctor` to verify token acquisition."
                )
            return info
        except Exception as exc:
...
def resolve_api_key_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if provider_id in {"kimi-coding", "kimi-coding-cn"}:
        ...
        try:
            from nadia_cli.copilot_auth import (
...
def resolve_external_process_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if not resolved_command and not base_url.startswith("acp+tcp://"):
        raise AuthError(
            ...
            "Install GitHub Copilot CLI or set NADIA_COPILOT_ACP_COMMAND/COPILOT_CLI_PATH.",
            provider=provider_id,
...
def _update_config_for_provider(
    ...
    # model.api_key.
    from nadia_cli.config import clear_model_endpoint_credentials
...
def _logout_default_provider_from_config() -> Optional[str]:
    ...
    `nadia logout` historically keyed off auth.json.active_provider only.
...
def _prompt_model_selection(
    ...
    """
    from nadia_cli.models import _format_price_per_mtok
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from nadia_cli.config import save_config, load_config
...
def login_command(args) -> None:
    """Deprecated: use 'nadia model' or 'nadia setup' instead."""
    print("The 'nadia login' command has been removed.")
    print("Use 'nadia auth' to manage credentials,")
    print("'nadia model' to select a provider, or 'nadia setup' for full setup.")
    raise SystemExit(0)
...
def _login_openai_codex(
    ...
    *,
    ...
    """OpenAI Codex login via device code flow. Tokens stored in ~/.nadia/auth.json."""

    del args, pconfig  # kept for parity with other provider login helpers

    # Check for existing Nadia-owned credentials
    if not force_new_login:
        try:
            ...
            if isinstance(_resolved_key, str) and _resolved_key and not _codex_access_token_is_expiring(_resolved_key, 60):
                print("Existing Codex credentials found in Nadia auth store.")
                try:
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            print("Nadia will create its own session to avoid conflicts with Codex CLI / VS Code.")
            try:
            ...
            if do_import in {"y", "yes"}:
                ...
                base_url = os.getenv("NADIA_CODEX_BASE_URL", "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL
                config_path = _update_config_for_provider("openai-codex", base_url)
                print()
                print("Credentials imported. Note: if Codex CLI refreshes its token,")
                print("Nadia will keep working independently with its own session.")
                print(f"  Config updated: {config_path} (model.provider=openai-codex)")
                return

    # Run a fresh device code flow — Nadia gets its own OAuth session
    print()
    print("Signing in to OpenAI Codex...")
    print("(Nadia creates its own session — won't affect Codex CLI or VS Code)")
    print()

    creds = _codex_device_code_login()

    # Save tokens to Nadia auth store
...
def _login_xai_oauth(
    ...
    *,
    ...
    if not force_new_login:
        try:
            ...
            if isinstance(api_key, str) and api_key and not _xai_access_token_is_expiring(api_key, 60):
                print("Existing xAI OAuth credentials found in Nadia auth store.")
                try:
                    reuse = input("Use existing credentials? [Y/n]: ").strip().lower()
                except (EOFError, KeyboardInterrupt):
                    reuse = "y"
    ...
    print("(Nadia creates its own local OAuth session)")
    print()
...
def _xai_oauth_build_authorize_url(
    *,
    ...
    # `referrer=nadia-agent` lets xAI attribute Nadia-originated logins
    ...
    *,
    ...
    if not code_verifier:
        raise AuthError(
            ...
            "This is a bug in Nadia — please report at "
            "https://github.com/nadicodeai/nadia/issues/26990.",
            provider="xai-oauth",
...
def _xai_oauth_loopback_login(
    *,
    ...
    if manual_paste:
        ...
        print("Open this URL to authorize Nadia with xAI:")
        print(authorize_url)
...
def _minimax_save_auth_state(auth_state: Dict[str, Any]) -> None:
    """Persist MiniMax OAuth state to Nadia auth store (~/.nadia/auth.json)."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
def _minimax_oauth_login(
    *, region: str = "global", open_browser: bool = True,
    ...
    print(f"Starting Nadia login via MiniMax ({region}) OAuth...")
...
def _minimax_oauth_quarantine_on_terminal_refresh(state: Dict[str, Any], exc: AuthError) -> None:
    ...
    provider. Mirrors the Nadia / xAI-OAuth / Codex-OAuth quarantine pattern
    so subsequent calls fail fast without a network retry.
    """
...
def build_minimax_oauth_token_provider() -> Callable[[], str]:
    ...
    def _provide() -> str:
        ...
        if not state or not state.get("access_token"):
            raise AuthError(
                "Not logged into MiniMax OAuth. Run `nadia model` and select "
                "MiniMax (OAuth).",
...
def resolve_minimax_oauth_runtime_credentials(
    *, min_token_ttl_seconds: int = MINIMAX_OAUTH_REFRESH_SKEW_SECONDS,
    ...
    diagnostic call sites like ``nadia status`` that just want to know
...
def _nous_device_code_login(
    *,
    ...
    """Run the Nadia device-code flow and return full OAuth state without persisting."""
    ...
    print(f"Starting Nadia login via {pconfig.name}...")
    print(f"Portal: {portal_base_url}")
    if insecure:
    ...
    try:
        ...
        if exc.code == "subscription_required":
            ...
            print()
            print("After subscribing, run `nadia model` again to finish setup.")
            raise SystemExit(1)
        raise


def nous_token_has_billing_scope() -> bool:
    """Return True if the currently-held Nadia token carries ``billing:manage``.
    ...
    if not isinstance(scope, str):
        return False
    return NADIA_BILLING_MANAGE_SCOPE in scope.split()
...
def step_up_nous_billing_scope(
    *,
    ...
    targets the same deployment (incl. a preview via ``NADIA_PORTAL_BASE_URL`` set
    ...
    requested: list[str] = []
    for tok in (prior_scope.split() or [NADIA_INFERENCE_INVOKE_SCOPE, "tool:invoke"]):
        if tok and tok not in requested:
            requested.append(tok)
    if NADIA_BILLING_MANAGE_SCOPE not in requested:
        requested.append(NADIA_BILLING_MANAGE_SCOPE)
    ...
    return isinstance(granted, str) and NADIA_BILLING_MANAGE_SCOPE in granted.split()
...
def _login_nous(args, pconfig: ProviderConfig) -> None:
    ...
    try:
        ...
        # flow, check the shared store for an existing Nadia credential
        ...
        if shared:
            ...
            print()
            if shared_path:
                print(f"Found existing Nadia OAuth credentials at {shared_path}")
            else:
                print("Found existing shared Nadia OAuth credentials")
            try:
                do_import = input("Import these credentials? [Y/n]: ").strip().lower()
            except (EOFError, KeyboardInterrupt):
                do_import = "y"
            if do_import in {"", "y", "yes"}:
                print("Rehydrating Nadia session from shared credentials...")
        ...
        # leave the config in a half-updated state (provider=nadia but model
        ...
        try:
            ...
            if model_ids:
                ...
                if free_tier:
                    ...
                    # as free so users on older Nadia builds still see
        ...
        # them to Nadia with a mismatched model.  The Nadia OAuth tokens
        ...
        print(f"  Config updated: {config_path} (model.provider=nadia)")
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target) or should_reset_config:
        ...
        if should_reset_config and os.getenv("OPENROUTER_API_KEY"):
            print("Nadia will use OpenRouter for inference.")
        elif should_reset_config:
            print("Run `nadia model` or configure an API key to use Nadia.")
        else:
