...
#
# Tokens live in ~/.nadia/auth/google_oauth.json (managed by agent.google_oauth).
...
def get_gemini_oauth_auth_status() -> Dict[str, Any]:
    """Return a status dict for `nadia auth list` / `nadia status`."""
    try:
...
def resolve_codex_runtime_credentials(
    *,
    ...
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = float(os.getenv("NADIA_CODEX_REFRESH_TIMEOUT_SECONDS", "20"))
...
def resolve_xai_oauth_runtime_credentials(
    *,
    ...
    tokens = dict(data["tokens"])
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = float(os.getenv("NADIA_XAI_REFRESH_TIMEOUT_SECONDS", "20"))
...
# mtime so that `nadia auth login/logout/add/remove` invalidate naturally on the next call.
