...
The Nadia docs site hosts a JSON manifest of curated models for providers
we want to update without shipping a release (currently OpenRouter and
Nadia Agents Portal). This module fetches, validates, and caches that manifest,
...
1. ``get_catalog()`` — returns a parsed manifest dict.
   ...
   - Reads disk cache at ``~/.nadia/cache/model_catalog.json``.
...
DEFAULT_CATALOG_FALLBACK_URLS: tuple[str, ...] = (
    "https://raw.githubusercontent.com/nadicodeai/nadia/main/website/static/api/model-catalog.json",
)
DEFAULT_TTL_HOURS = 1
...
def _cache_path() -> Path:
    ...
    return get_nadia_home() / "cache" / "model_catalog.json"
...
def get_curated_nous_models() -> list[str] | None:
    """Return Nadia Agents Portal's curated list of model ids from the manifest.
...
def seed_cache_from_checkout(project_root: "Path | str") -> bool:
    ...
    ``nadia update`` pulls the latest repo, so the freshly-pulled
    ...
    to ``~/.nadia/cache/model_catalog.json`` via the same atomic writer the
...
def reset_cache() -> None:
    """Clear the in-process cache. Used by tests and ``nadia model --refresh``."""
    global _catalog_cache, _catalog_cache_source_mtime
    _catalog_cache = None
