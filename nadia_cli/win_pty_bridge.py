"""Windows ConPTY bridge for the `nadia dashboard` chat tab.

Drop-in counterpart to ``nadia_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``nadia_cli.web_server`` consumes: ``spawn``, ``read``,
