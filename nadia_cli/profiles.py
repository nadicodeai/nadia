"""
Profile management for multiple isolated Nadia instances.

Each profile is a fully independent NADIA_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.nadia/profiles/<name>/`` by default.

The "default" profile is ``~/.nadia`` itself — backward compatible,
zero migration needed.

Usage::

    nadia profile create coder          # fresh profile + bundled skills
    nadia profile create coder --clone  # also copy config, .env, SOUL.md, skills
    nadia profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    nadia -p coder chat                 # or via flag
    nadia profile use coder             # set as sticky default
    nadia profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See nadia_constants.get_subprocess_home().
    "home",
]
...
# a profile's root, callers of seed_profile_skills() (fresh-create, `nadia
...
# `nadia skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Nadia infrastructure directories that only the default profile
         (``~/.nadia``) ever contains.  Gated on ``source_dir`` actually
...
# Directories/files to exclude when exporting the default (~/.nadia) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "nadia-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "nadia", "default", "test", "tmp", "root", "sudo",
})

# Nadia subcommands that cannot be used as profile names/aliases
_NADIA_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the nadia root, NOT to the current NADIA_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where NADIA_HOME points outside
    ``~/.nadia``, profiles live under ``NADIA_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_nadia_home() / "profiles"


def _get_default_nadia_home() -> Path:
    """Return the default (pre-profile) NADIA_HOME path.

    In standard deployments this is ``~/.nadia``.
    In Docker/custom deployments where NADIA_HOME is outside ``~/.nadia``
    (e.g. ``/opt/data``), returns NADIA_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_nadia_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Nadia installation itself or a common system binary.  "
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its NADIA_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_nadia_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, nadia subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
    ...
    if canon in _NADIA_SUBCOMMANDS:
        return f"'{canon}' conflicts with a nadia subcommand"
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            nadia_exe = shutil.which("nadia") or "nadia"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(nadia_exe)} -p {profile} "$@"\n')
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running Nadia. If we
    ...
    try:
        ...
        # not be migrated. The next `nadia doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``nadia -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``nadia profile alias
...
# Real wrappers are a few hundred bytes of shell; the needle (``nadia -p X``)
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``nadia profile list`` for an unrelated profile.
    """
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``NADIA_HOME``.
    """
    try:
...
# ``config.yaml``. ``config.yaml`` is the user-facing Nadia config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_nadia_home()
...
def profiles_to_serve(multiplex: bool) -> List[Tuple[str, Path]]:
    """Return the ``(profile_name, nadia_home)`` pairs a gateway should serve.
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own NADIA_HOME.
    ...
    The returned ``nadia_home`` is the path to pass to
    ``set_nadia_home_override`` when scoping a turn to that profile.
    """
    active = get_active_profile_name() or "default"
    if not multiplex:
        return [(active, get_profile_dir(active))]

    serve: List[Tuple[str, Path]] = [("default", _get_default_nadia_home())]
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``nadia update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.nadia)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.nadia/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Keys pages, `nadia -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this Nadia profile.\n"
    ...
    # Write the opt-out marker so seed_profile_skills() and `nadia update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`nadia profile create --no-skills`).\n"
    ...
    # Cloned configs can be older than the running Nadia (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `nadia profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `nadia -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches NADIA_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``nadia profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "NADIA_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_nadia_home() / ".env"
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.nadia).\n"
            "To remove everything, use: nadia uninstall"
        )
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``NADIA_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from nadia_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set NADIA_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("NADIA_HOME")
    try:
        os.environ["NADIA_HOME"] = str(profile_dir)
        from nadia_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["NADIA_HOME"] = old_home
        elif "NADIA_HOME" in os.environ:
            del os.environ["NADIA_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.nadia/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: nadia profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from NADIA_HOME.

    Returns ``"default"`` if NADIA_HOME is not set or points to ``~/.nadia``.
    Returns the profile name if NADIA_HOME points into ``~/.nadia/profiles/<name>``.
    Returns ``"custom"`` if NADIA_HOME is set to an unrecognized path.
    """
    from nadia_constants import get_nadia_home
    nadia_home = get_nadia_home()
    resolved = nadia_home.resolve()

    default_resolved = _get_default_nadia_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.nadia itself — its parent is ~/ and its
        # directory name is ".nadia", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: nadia profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.nadia itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.nadia). "
            "Specify a different name: nadia profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="nadia_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"nadia_{old_name}"
    legacy_old_host = f"nadia.{old_name}"
    new_host = f"nadia_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_nadia_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("nadia_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a NADIA_HOME path string.

    Called early in the CLI entry point, before any nadia modules
    are imported, to set the NADIA_HOME environment variable.
    """
