...
# NADIA_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    ...
    # Python dependency trees (plugin / MCP-server venvs under NADIA_HOME) —
...
# File names that ``nadia import`` must never overwrite, matched by basename so
...
#     stuck "starting"/"cooking", disconnecting it from the Nadia Agents Portal
...
# Reserved archive subtree for provider state that lives OUTSIDE NADIA_HOME
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside NADIA_HOME, resolved from config only (no network, no init).
...
def run_backup(args) -> None:
    ...
    # outside NADIA_HOME, so the walk above never sees it. Ask the active
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_nadia_home()}."
        )
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under NADIA_HOME.
            ...
            # Nadia Agents Portal. Matched by basename so both the root profile and
        ...
        if restored_external:
            print(
                ...
                f"their original location(s) outside {display_nadia_home()}."
            )
...
_QUICK_STATE_FILES = (
    ...
    # are at $NADIA_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside NADIA_HOME and the entry is silently
