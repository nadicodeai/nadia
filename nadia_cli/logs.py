"""``nadia logs`` — view and filter Nadia log files.
...
Usage examples::
    ...
    nadia logs                    # last 50 lines of agent.log
    nadia logs -f                 # follow agent.log in real time
    nadia logs errors             # last 50 lines of errors.log
    nadia logs gateway -n 100    # last 100 lines of gateway.log
    nadia logs gui -f            # follow gui.log (dashboard/pty/ws)
    nadia logs desktop -f        # follow desktop.log (Electron app boot/backend)
    nadia logs --level WARNING    # only WARNING+ lines
    nadia logs --session abc123   # filter by session ID substring
    nadia logs --component tools  # only tool-related lines
    nadia logs --since 1h         # lines from the last hour
    nadia logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_nadia_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Nadia runs — try 'nadia chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_nadia_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_nadia_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_nadia_home()}/logs/")
        return

    print(f"Log files in {display_nadia_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'nadia chat' to generate logs)")
