...
Pure display functions with no NadiaCLI state dependency.
"""
...
from nadia_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE
...
# (e.g. nix-built nadia — no local git history to count against).
UPDATE_AVAILABLE_NO_COUNT = -1

_UPSTREAM_REPO_URL = "https://github.com/nadicodeai/nadia.git"
_OFFICIAL_REPO_CANONICAL = "github.com/nadicodeai/nadia"
...
def _check_via_rev(local_rev: str) -> Optional[int]:
    """Compare an embedded git revision to the main branch via ls-remote.
...
def _fetch_pypi_latest(package: str = "nadia-agent") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    """Check whether a Nadia update is available.

    Two paths: if ``NADIA_REVISION`` is set (nix builds embed it), compare
    ...
    cache_file = nadia_home / ".update_check"
    embedded_rev = os.environ.get("NADIA_REVISION") or None

    # Docker images have no working tree to count commits against — the
    # published image excludes `.git` (see .dockerignore) and sets no
    # NADIA_REVISION (that's nix-only). Without this guard the checks below
    ...
    # and `nadia update` correctly refuses to run in-place inside the
    # container anyway. The dashboard's REST `/api/nadia/update/check`
    ...
    if embedded_rev:
        ...
        # $NADIA_HOME/nadia-agent/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = nadia_home / "nadia-agent"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Nadia git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$NADIA_HOME/nadia-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``nadia_cli/build_info.py``) and return it as a frozen
    ``upstream == local`` state with ``ahead=0``.  A built image is by
    definition pinned to one commit, so "ahead" is always zero and the
    banner correctly shows ``· source <sha>`` with no carried-commits
    annotation.
    """
    ...
    if repo_dir is None:
        ...
        try:
            from nadia_cli.build_info import get_build_sha
...
_RELEASE_URL_BASE = "https://github.com/nadicodeai/nadia/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    Nadia checkout. Cached per-process. Release URL always points at the
    canonical nadicodeai/nadia repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"Nadia Agent v{VERSION} ({RELEASE_DATE})"
    ...
    if ahead <= 0 or upstream == local:
        return f"{base} · source {upstream}"

    carried_word = "commit" if ahead == 1 else "commits"
    return f"{base} · source {upstream} · local {local} (+{ahead} carried {carried_word})"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    if (provider or "").strip().lower() == "moa":
        ...
        left_lines.append(f"[{accent}]MoA: {preset_name}[/]{agg_str}{ctx_str} [dim {dim}]·[/] [dim {dim}]NadicodeAI[/]")
    else:
    ...
    left_lines.append(f"[{text}]Nadia Agent by NadicodeAI (nadicode.ai)[/]")

    if os.getenv("NADIA_YOLO_MODE"):
    ...
    try:
        from nadia_cli.codex_runtime_switch import get_current_runtime
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from nadia_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
                ...
                # UPDATE_AVAILABLE_NO_COUNT: nix-built nadia; we know an update
    ...
    # Pip-install warning — `pip install nadia-agent` is not the supported
