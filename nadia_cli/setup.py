"""
Interactive setup wizard for Nadia Agent.
...
_DOCS_BASE = "https://docs.nadicode.ai/nadia"
...
# display_nadia_home imported lazily at call sites (stale-module safety during nadia update)
...
from nadia_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from nadia_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Nadia Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Nadia using environment variables or config commands:")
    print_info("  nadia config set model.provider custom")
    print_info("  nadia config set model.base_url http://localhost:8080/v1")
    print_info("  nadia config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'nadia setup' in an interactive terminal to use the full wizard.")
    print()
...
def is_noninteractive() -> bool:
    ...
    ``NADIA_NONINTERACTIVE=1`` (see ``nadia_cli/web_server.py``). In that
    ...
    """
    return os.environ.get("NADIA_NONINTERACTIVE", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def prompt_yes_no(question: str, default: bool = True) -> bool:
    ...
    Non-interactive callers (``NADIA_NONINTERACTIVE=1`` or a closed/redirected
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from nadia_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'nadia setup')")


def _print_setup_summary(config: dict, nadia_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'nadia setup' to configure"))
    ...
    if subscription_features.browser.managed_by_nous:
        tool_status.append(("Browser Automation (Nadia Browser Use)", True, None))
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        try:
            ...
            from nadia_cli.plugins import _ensure_plugins_discovered
    ...
    # Video generation — opt-in via `nadia tools` → Video Generation.
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from nadia_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'nadia setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'nadia setup tts'"))
    else:
    ...
    # Spotify (OAuth via nadia auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'nadia setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {nadia_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('nadia setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('nadia setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('nadia setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('nadia setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('nadia setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('nadia config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('nadia config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('nadia config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('nadia', Colors.GREEN)}              Start chatting")
    print(f"   {color('nadia gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('nadia doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `nadia tools` and `nadia setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``nadia model``)
    ...
    provider added to ``nadia model`` is automatically available here.
    ...
    # Delegate to the shared nadia model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from nadia_cli.main import select_provider_and_model
    try:
    ...
    # on demand via `nadia auth add`, `nadia setup` vision, and
    # `nadia setup tts`. This keeps both quick and full setup thin.
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``nadia model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
        from nadia_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    selected_via_nous = selected == "nadia-openai"
    if selected == "nadia-openai":
        selected = "openai"
        print_info("OpenAI TTS will use the managed Nadia gateway and bill to your subscription.")
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.nadia/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the Nadia auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via nadia setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'nadia setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Nadia runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `nadia setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `nadia setup terminal`.
        ...
        if use_managed_modal:
            ...
            print_info("Modal execution will use the managed Nadia gateway and bill to your subscription.")
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into NADIA_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("NADIA_MAX_ITERATIONS")
    ...
    print_info("  Run `nadia setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # NADIA_MAX_ITERATIONS from agent.max_turns at startup.
...
def _setup_telegram_auto_result():
    ...
    try:
        from nadia_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None

    profile_name: str | None = None
    try:
        profile_name = _profile_name_from_nadia_home(Path(get_nadia_home()))
    except Exception:
        pass
...
def _profile_name_from_nadia_home(nadia_home) -> str | None:
    """Return the active profile name when NADIA_HOME is a profile dir."""
    if nadia_home.parent.name == "profiles":
        return nadia_home.name
    return None
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Nadia delivers cron job results,")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Nadia to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from nadia_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://docs.nadicode.ai/nadia/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://docs.nadicode.ai/nadia/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  nadia config edit")
    print_info("   Open config in your editor:  nadia config edit")


def setup_gateway(config: dict):
    """Configure messaging platform integrations."""
    from nadia_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Nadia from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'nadia setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     nadia config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_nadia_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                if supports_systemd and os.geteuid() == 0:  # windows-footgun: ok — guarded by supports_systemd (Linux only)
                    print_info("  Or as a boot-time service: nadia gateway install --system")
                print_info("  Or run in foreground:  nadia gateway")
        else:
            from nadia_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   nadia gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   nadia gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `nadia setup tools` and `nadia tools` use the same flow:
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from nadia_cli.config
    ...
    if section_key == "model":
        ...
        from nadia_cli.gateway import _all_platforms, _platform_status
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_nadia.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_nadia migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_nadia", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Nadia.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Nadia to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Nadia at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Nadia at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Nadia at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Nadia at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Nadia equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Nadia config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Nadia.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Nadia's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(nadia_home: Path) -> bool:
    ...
    print_info("Nadia can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: nadia claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=nadia_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: nadia claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Nadia configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Nadia config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Nadia (use nadia claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Nadia session — model selected, provider set, and web/image/tts/browser
    tools routed via their Portal sub — without being told to run
    ``nadia setup`` and hunt for the quick-setup option.
    ...
    (``_run_first_time_quick_setup``) and the same one ``nadia model`` runs
    when you pick Nadia. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``nadia portal`` always offers a model picker,
    and there is a single source of truth for the Nadia onboarding steps.
    """
    ...
    # Nadia model picker), then offers the Tool Gateway opt-in and sets
    # provider=nadia via the login/model save. This is the same routine quick
    # setup calls, so `nadia portal` == quick setup's Nadia step.
    try:
    ...
    print_info("  Run `nadia portal info` to inspect routing.")
    print_info("  Run `nadia` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      nadia setup           — full or quick (auto-detected)
      nadia setup model     — just model/provider
      nadia setup tts       — just text-to-speech
      nadia setup terminal  — just terminal backend
      nadia setup gateway   — just messaging platforms
      nadia setup tools     — just tool configuration
      nadia setup agent     — just agent settings
    """
    from nadia_cli.config import is_managed, managed_error
    if is_managed():
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Nadia Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Nadia Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Nadia Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, nadia_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have Nadia configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'nadia setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(nadia_home)
        if migration_ran:
            config = load_config()
    ...
    print_info(f"Data folder:  {nadia_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'nadia config edit'")
    ...
    # Tune later with `nadia setup agent`.
    if not is_existing:
    ...
    _print_setup_summary(config, nadia_home)
...
def _run_first_time_quick_setup(config: dict, nadia_home, is_existing: bool):
    ...
    or switch providers with ``nadia model``.
    """
    from nadia_cli.config import load_config
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'nadia setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    nadia setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  nadia setup gateway")
    print()
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``nadia tools`` (which
    ...
    try:
        ...
        from nadia_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``nadia setup agent`` /
    ``nadia config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, nadia_home, is_existing: bool):
    ...
    if path == 0:
        ...
        # `nadia update` runs don't re-inject them.
        try:
        ...
        print_info("  Enable tools:        nadia tools")
        print_info("  Seed skills:         nadia skills opt-in --sync")
        print_info("  Add MCP servers:     nadia mcp add")
        print_info("  Enable plugins:      nadia plugins")
        print_info("  Tune agent settings: nadia setup agent")
        print()
        _print_setup_summary(config, nadia_home)
        return

    # ── Walkthrough path — opt in to each capability ──
    _blank_slate_walkthrough(config, nadia_home)


def _blank_slate_walkthrough(config: dict, nadia_home):
    ...
    try:
        ...
        if seed_skills:
            ...
            print_info("`nadia update` runs from re-injecting them. Opt back in any")
            print_info("time with `nadia skills opt-in --sync`.")
    except Exception as exc:
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `nadia tools`.")
    ...
    if prompt_yes_no("Review and enable built-in plugins now?", default=False):
        print_info("Manage plugins with `nadia plugins list` / `nadia plugins install`.")
    else:
        print_info("No plugins enabled. Add later with `nadia plugins`.")

    # ── MCP servers (off unless chosen) ──
    print()
    print_header("MCP Servers")
    if prompt_yes_no("Add an MCP server now?", default=False):
        print_info("Add servers with `nadia mcp add <name> --url ... | --command ...`.")
    else:
        print_info("No MCP servers configured. Add later with `nadia mcp add`.")
    ...
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    print_info("  Enable more tools:   nadia tools")
    print_info("  Seed skills:         nadia skills opt-in --sync")
    print_info("  Add MCP servers:     nadia mcp add")
    print_info("  Tune agent settings: nadia setup agent")
    print()
...
def _run_quick_setup(config: dict, nadia_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'nadia setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Nadia to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'nadia setup gateway'.")
