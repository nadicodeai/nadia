...
def _setup_slack():
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `nadia update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://docs.nadicode.ai/nadia/user-guide/messaging/slack/")
    print()
...
def _write_slack_manifest_and_instruct():
    """Generate the Slack manifest, write it under NADIA_HOME, and print
    ...
    """
    try:
        from nadia_cli.slack_cli import _build_full_manifest
        from nadia_constants import get_nadia_home

        manifest = _build_full_manifest(
            bot_name="Nadia",
            bot_description="Your Nadia agent on Slack",
        )
        target = Path(get_nadia_home()) / "slack-manifest.json"
        ...
        print_info(
            "   Re-run `nadia slack manifest --write` anytime to refresh after "
            "Nadia adds new commands."
        )
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where Nadia delivers cron job results and notifications.")
    ...
    if any_messaging:
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                if supports_systemd:
                    print_info("  Or as a boot-time service: sudo nadia gateway install --system")
