...
- ``nadia_cli.providers``        -- canonical provider identity + overlays
- ``nadia_cli.model_normalize``  -- per-provider name formatting
...
#   nadia-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NOUS_MODEL_FAMILY_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])nadia[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)


def is_nous_model_family_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nadia Hermes 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_NOUS_MODEL_FAMILY_NON_AGENTIC_RE.search(model_name))


def _check_nous_model_family_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nadia Hermes 3/4 chat model."""
    if is_nous_model_family_non_agentic(model_name):
        return _NOUS_MODEL_FAMILY_WARNING
    return ""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``nadia config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from nadia_cli.model_switch import
...
def resolve_display_context_length(
    ...
    about Codex OAuth, Copilot, Nadia, and falls back to models.dev for the
    rest.
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'nadia model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'nadia doctor' — config issues detected:"
        ...
        from nadia_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from nadia_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    # =================================================================
        ...
        try:
            from nadia_cli.config import load_config
            from nadia_cli.moa_config import exact_moa_preset_name, normalize_moa_config
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from nadia_cli.providers import resolve_user_provider as _ruser
    ...
    # Mirrors the same logic in nadia_cli.runtime_provider.resolve_runtime_provider;
    ...
    model_family_warn = _check_nous_model_family_warning(new_model)
    if model_family_warn:
        warnings.append(model_family_warn)
...
def prewarm_picker_cache_async() -> Optional["_threading.Thread"]:
    ...
    def _warm() -> None:
        try:
            from nadia_cli.inventory import load_picker_context
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from nadia_cli/models.py (OPENROUTER_MODELS,
    ...
    ``force_fresh_nous_tier`` bypasses the short Nadia tier cache for explicit
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from nadia_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by nadia provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "nous" pulls from the remote model-catalog manifest published at
    # https://docs.nadicode.ai/nadia/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a Nadia release. Falls back to the in-repo
    # _PROVIDER_MODELS["nous"] snapshot when the manifest is unreachable.
    curated["nous"] = get_curated_nous_model_ids()
    # Ollama Cloud uses dynamic discovery (no static curated list)
    if "ollama-cloud" not in curated:
        from nadia_cli.models import fetch_ollama_cloud_models
    ...
    if "lmstudio" not in curated and (
        ...
        from nadia_cli.models import fetch_lmstudio_models
    ...
    # --- 1. Check Nadia-mapped providers ---
    from nadia_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from nadia_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    for nadia_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        _alias_target = _PROVIDER_ALIAS_TABLE.get(nadia_id)
        if (
            _alias_target
            and _alias_target != nadia_id
        ...
        pconfig = PROVIDER_REGISTRY.get(nadia_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (NADIA_OVERLAYS) with proper auth store checking.
        ...
        if not has_creds:
            try:
                from nadia_cli.auth import _load_auth_store
                store = _load_auth_store()
                if store and store.get("credential_pool", {}).get(nadia_id):
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
        ...
        # /model picker sees the SAME list `nadia model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(nadia_id)
        if not model_ids:
            model_ids = curated.get(nadia_id, [])
            if nadia_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(nadia_id, model_ids)
        total = len(model_ids)
        if nadia_id in _UNCAPPED_PICKER_PROVIDERS:
    ...
    # --- 2. Check Nadia-only providers (nadia, openai-codex, copilot, opencode-go) ---
    from nadia_cli.providers import NADIA_OVERLAYS
    from nadia_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Nadia provider ID.
    # NADIA_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Nadia IDs ("copilot").
    _mdev_to_nadia = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in NADIA_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Nadia slug — e.g. "github-copilot" → "copilot"
        nadia_slug = _mdev_to_nadia.get(pid, pid)
        if nadia_slug.lower() in seen_slugs:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(nadia_slug)
        elif overlay.extra_env_vars:
            has_creds = any(os.environ.get(ev) for ev in overlay.extra_env_vars)
        # Also check api_key_env_vars from PROVIDER_REGISTRY for api_key auth_type
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, nadia_slug):
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or nadia_slug in providers_store):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(nadia_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", nadia_slug, exc)
        ...
        # configured.
        if not has_creds and nadia_slug == "anthropic":
            try:
                ...
                if (nadia_creds and nadia_creds.get("accessToken")) or \
        ...
        if nadia_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(nadia_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = cached_provider_model_ids(nadia_slug)
                model_ids = _ids if _ids else (curated.get(nadia_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(nadia_slug, []) or curated.get(pid, [])
        elif nadia_slug == "nous":
            # Nadia serves a large live /v1/models catalog (vendor-prefixed
            # models from many providers, returned alphabetically). The
            # `nadia model` picker deliberately shows ONLY the curated agentic
            ...
            try:
                ...
                from nadia_cli.auth import get_provider_auth_state as _nous_state
            ...
            if not model_ids:
                model_ids = curated.get(nadia_slug, []) or curated.get(pid, [])
                if nadia_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(nadia_slug, model_ids)
        total = len(model_ids)
        if nadia_slug in _UNCAPPED_PICKER_PROVIDERS:
            top = model_ids  # Aggregator: show full catalog regardless of max_models
        else:
            top = model_ids[:max_models] if max_models is not None else model_ids
        ...
        results.append({
            "slug": nadia_slug,
            "name": get_label(nadia_slug),
            "is_current": nadia_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "nadia",
        ...
        seen_slugs.add(nadia_slug.lower())
        _record_builtin_endpoint(nadia_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or NADIA_OVERLAYS (keeps /model in sync
    # with `nadia model`).
    try:
        from nadia_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Nadia's canonical write key (matches
            ...
            # Nadia writes ``models:`` as a dict keyed by model id
            # (see nadia_cli/main.py::_save_custom_provider); older
            ...
            if should_probe:
                try:
                    from nadia_cli.models import fetch_api_models
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # Nadia's own writer uses; a hyphen variant is accepted
            ...
            # active model. Nadia's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, nadia_cli/config.py) already consume that dict.
...
def _prepend_moa_picker_provider(providers: List[dict], current_provider: str = "") -> List[dict]:
    ...
    """
    try:
        from nadia_cli.inventory import _moa_provider_row
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`nadia_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from nadia_cli.models import fetch_openrouter_models
