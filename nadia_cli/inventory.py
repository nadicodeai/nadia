...
- ``list_authenticated_providers`` already populates each row's
  ...
  curation and pulls in non-agentic models (Nadia /models returns ~400
...
def build_models_payload(
    ...
    *,
    ...
    - ``pricing``: enrich each row with formatted per-model pricing and,
      for Nadia, ``free_tier``/``unavailable_models`` so the GUI picker can
      show $/Mtok columns and gate paid models on free accounts —
      mirroring the ``nadia model`` CLI picker. Adds network calls
      (pricing fetch + Nadia tier check); only set for interactive pickers.
    ...
    - ``force_fresh_nous_tier``: bypass the short Nadia free-tier cache when
      selecting Portal-recommended Nadia models and applying tier gating. Keep
    ...
    try:
        from nadia_cli.providers import is_routing_aggregator as _is_routing_aggregator
    except Exception:
...
def _apply_picker_hints(rows: list[dict]) -> None:
    ...
    for row in rows:
        ...
        row["warning"] = (
            ...
            else f"run `nadia model` to configure ({auth_type})"
        )
...
def _reorder_canonical(rows: list[dict]) -> list[dict]:
    ...
    """
    from nadia_cli.models import CANONICAL_PROVIDERS
...
def _apply_pricing(
    ...
    *,
    ...
    # Resolve Nadia free-tier once (cached in models.py for the TTL window).
...
def _moa_provider_row(current_provider: str = "") -> dict | None:
    ...
    picker path (:func:`nadia_cli.model_switch.list_picker_providers`) so the
