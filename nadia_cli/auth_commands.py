...
def auth_add_command(args) -> None:
    ...
    if provider == "anthropic":
        ...
        creds = anthropic_mod.run_nadia_oauth_login_pure()
        if not creds:
        ...
        entry = PooledCredential(
            ...
            priority=0,
            source=f"{SOURCE_MANUAL}:nadia_pkce",
    ...
    if provider == "nous":
        # Codex-style auto-import: if a shared Nadia credential lives at
        # <nadia-root>/shared/nous_auth.json (written by any previous
        # successful login), offer to import it instead of running the
        # full device-code flow. This makes `nadia --profile <name>
        # auth add nadia --type oauth` a one-tap operation for users who
        ...
        if shared:
            ...
            print()
            if path:
                print(f"Found existing Nadia OAuth credentials at {path}")
            else:
        ...
        # Honor `--label <name>` so nadia matches other providers' UX.  The
    ...
    if provider == "openai-codex":
        ...
        # latest login: a second ``nadia auth add openai-codex`` overwrote
    ...
    raise SystemExit(f"`nadia auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    # Unified removal dispatch.  Every credential source Nadia reads from
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `nadia auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `nadia auth` is called bare."""
