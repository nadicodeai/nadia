...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Nadia Agent. Use 'nadia debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    nadia debug share              Upload debug report (asks for confirmation)
    nadia debug share --yes        Skip confirmation (for scripts/CI)
    nadia debug share --lines 500  Include more log lines
    nadia debug share --expire 30  Keep paste for 30 days
    nadia debug share --local      Print report locally (no upload)
    nadia debug share --no-redact  Disable upload-time secret redaction
    nadia debug share --nadia       Upload to Nadia-internal storage (private)
    nadia debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    share_parser.add_argument(
        ...
        action="store_true",
        help=(
            "Upload the debug bundle to Nadia-internal storage (AWS S3) instead "
            "of a public paste service. The bundle is private — viewable only "
            "by Nadia staff (and allowlisted Discord mods) via a Google-login-"
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'nadia debug share'",
    )
