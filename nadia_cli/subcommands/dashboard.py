"""``nadia dashboard`` / ``nadia serve`` subcommand parsers.
...
``nadia_cli/main.py:main()`` (god-file Phase 2); handler injected to avoid
importing ``main``.
"""
...
def _add_server_runtime_args(parser) -> None:
    ...
    # `nadia dashboard` / `nadia serve` cmdlines and SIGTERM them directly —
    # the same path `nadia update` uses to clean up stale servers.
    parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running Nadia web server processes and exit",
    )
    parser.add_argument(
        "--status",
        action="store_true",
        help="List running Nadia web server processes and exit",
    )
...
def build_dashboard_parser(
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Nadia Agent web dashboard for managing config, API keys, and sessions",
    )
    _add_server_runtime_args(dashboard_parser)
    dashboard_parser.add_argument(
        "--no-open", action="store_true", help="Don't open browser automatically"
    )
    # Backward-compat shim: older Nadia desktop app shells (<= 0.15.x) spawn the
    # backend as `nadia dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "Nadia couldn't start" with no actionable cause. Accept and silently
    ...
    # browser. It exists so the Nadia Desktop app (and headless remote
    ...
    serve_parser = subparsers.add_parser(
        "serve",
        help="Start the Nadia backend server (headless; powers the desktop app and remote backends)",
        description=(
            "Run the Nadia backend server — the JSON-RPC/WebSocket gateway the "
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        ...
        description=(
            ...
            "NADIA_DASHBOARD_OAUTH_CLIENT_ID into ~/.nadia/.env, and prints "
            "how to engage the login gate. Requires being logged in (nadia setup)."
        ),
    )
    ...
    dashboard_register_parser.add_argument(
        ...
        help=(
            ...
            "https://nadia.example.com/auth/callback. Omit for localhost-only use."
        ),
    )
    dashboard_register_parser.add_argument(
        "--portal-url",
