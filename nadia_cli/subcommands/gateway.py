"""``nadia gateway`` and ``nadia proxy`` subcommand parsers.
...
from nadia_cli.subcommands._shared import add_accept_hooks_flag
...
def build_gateway_parser(
    ...
    gateway_run.add_argument(
        ...
        help=(
            ...
            "if NADIA_DASHBOARD is set). Pass --no-supervise to opt out and "
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy nadia.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy Nadia gateway unit files "
            "(e.g. nadia.service) left over from older installs. Profile "
            "units (nadia-gateway-<profile>.service) and unrelated "
    ...
    gateway_enroll = gateway_subparsers.add_parser(
        ...
        description=(
            ...
            "Authenticates as your Nadia Agents Portal account (the connector derives the "
            ...
            "GATEWAY_RELAY_SECRET / GATEWAY_RELAY_DELIVERY_KEY into ~/.nadia/.env. "
            "Requires being logged in (nadia setup). Not available in managed installs."
        ),
    )
    ...
    gateway_enroll.add_argument(
        ...
        help=(
            ...
            "GATEWAY_RELAY_WAKE_URL in ~/.nadia/.env and forwarded at provision. "
    ...
    proxy_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "to an OAuth-authenticated provider (e.g. Nadia Agents Portal). External "
    ...
    proxy_start.add_argument(
        "--provider",
        default="nous",
        help="Upstream provider: nadia or xai (default: nadia). See `nadia proxy providers`.",
    )
    proxy_start.add_argument(
        "--host",
        default=None,
