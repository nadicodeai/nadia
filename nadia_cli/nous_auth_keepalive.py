"""Background keepalive for long-lived Nadia Agents Portal sessions."""
...
NADIA_AUTH_KEEPALIVE_INTERVAL_SECONDS = 6 * 60 * 60
NADIA_AUTH_KEEPALIVE_INITIAL_DELAY_SECONDS = 60
...
def _timeout_seconds(value: Optional[float]) -> float:
    if value is not None:
        return float(value)
    try:
        return float(os.getenv("NADIA_NOUS_TIMEOUT_SECONDS", "15"))
...
def _refresh_selected_pool_entry(
    *,
    ...
    """Refresh the current Nadia credential pool entry when it is stale.

    Returns True when a pool entry exists and is usable/refreshed, False when a
    pool exists but no entry can be used, and None when no Nadia pool exists.
    """
    try:
        ...
        logger.debug("Nadia auth keepalive: credential pool unavailable: %s", exc)
        return None
    ...
    try:
        ...
        logger.debug("Nadia auth keepalive: credential pool selection failed: %s", exc)
        return False
    ...
    if access_expiring or not key_usable:
        ...
        if refreshed is None:
            return False
        logger.debug("Nadia auth keepalive: refreshed credential pool entry")
        return True
...
def refresh_nous_auth_keepalive_once(
    *,
    ...
    """Refresh Nadia auth once if credentials are configured."""
    ...
    try:
        ...
        logger.debug("Nadia auth keepalive: refreshed singleton auth state")
        return True
    except AuthError as exc:
        if exc.relogin_required:
            logger.info("Nadia auth keepalive requires re-login: %s", exc)
        else:
            logger.debug("Nadia auth keepalive failed: %s", exc)
        return False
    except Exception as exc:
        logger.debug("Nadia auth keepalive failed: %s", exc)
        return False
...
def start_nous_auth_keepalive(
    *,
    interval_seconds: int = NADIA_AUTH_KEEPALIVE_INTERVAL_SECONDS,
    initial_delay_seconds: int = NADIA_AUTH_KEEPALIVE_INITIAL_DELAY_SECONDS,
    min_key_ttl_seconds: int = NOUS_INVOKE_JWT_MIN_TTL_SECONDS,
    timeout_seconds: Optional[float] = None,
) -> Optional[threading.Thread]:
    """Start the process-wide Nadia auth keepalive thread."""
    ...
    with _keepalive_lock:
        ...
        _keepalive_thread = threading.Thread(
            ...
            daemon=True,
            name="nadia-auth-keepalive",
        )
        _keepalive_thread.start()
        logger.debug("Nadia auth keepalive started")
        return _keepalive_thread
