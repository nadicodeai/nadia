...
await build({
  ...
  // Skip the prebuilt @nadia/ink bundle — esbuild's __esm helper doesn't
  // await nested async init, which breaks lazy-initialized exports like
  // `render`. Bundling from source sidesteps that.
  alias: { '@nadia/ink': resolve(root, 'packages/nadia-ink/src/entry-exports.ts') },
...
// the `node` token, leaving a broken interpreter). The nadia_cli launcher
