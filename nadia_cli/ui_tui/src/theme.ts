import {
  ...
  NADIA_BANNER_LOGO,
  NADIA_BRAND,
  NADIA_DARK_THEME_COLORS,
  NADIA_LIGHT_THEME_COLORS
} from './nadiaTheme.generated.js'
...
// Mirrors @nadia/ink's colorize.ts. Keep local: app code compiles from
// ui-tui/src, while @nadia/ink is bundled separately from packages/.
...
const BRAND: ThemeBrand = NADIA_BRAND
...
export const LIGHT_THEME: Theme = {
  color: NADIA_LIGHT_THEME_COLORS,
...
// Terminal. Explicit NADIA_TUI_THEME / COLORFGBG signals above still win,
...
// `NADIA_TUI_BACKGROUND` is intentionally generic so a future OSC11
...
//
//   1. `NADIA_TUI_LIGHT` boolean — `1`/`true`/`yes`/`on` → light;
//      `0`/`false`/`no`/`off` → dark.  Either explicit value wins
//      regardless of any later signal.
//   2. `NADIA_TUI_THEME` named override — `light` / `dark` win over
//      every signal below.
//   3. `NADIA_TUI_BACKGROUND` hex hint (3- or 6-digit) — luminance
...
//
// Anything we can't decide stays dark — the default Nadia palette
// is the dark one.
export function detectLightMode(
  ...
  const lightFlag = (env.NADIA_TUI_LIGHT ?? '').trim().toLowerCase()
  ...
  const themeFlag = (env.NADIA_TUI_THEME ?? '').trim().toLowerCase()
  ...
  const bgHint = backgroundLuminance(env.NADIA_TUI_BACKGROUND ?? '')
...
export function fromSkin(
  ...
  const completionMetaCurrentBg = c('completion_menu_meta_current_bg') ?? (hasSkinColors ? completionCurrentBg : d.color.completionMetaCurrentBg)
