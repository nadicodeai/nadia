...
const logDir = join(process.env.NADIA_HOME?.trim() || join(homedir(), '.nadia'), 'logs')
...
// a real ~/.nadia (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('nadia-tui: parent lifecycle log unavailable\n')
    }
  }
}
