import type { RunExternalProcess } from '@nadia/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`nadia ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching nadia: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`nadia ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
