import { AlternateScreen, Box, NoSelect, ScrollBox, Text } from '@nadia/ink'
...
import { getPlaceholder } from '../content/placeholders.js'
...
const fillCount = (template: string, count: number) => template.replace('{count}', String(count))
    ...
    >
      ...
      {ui.bgTasks.size > 0 && (
        <Text color={ui.theme.color.muted}>
          {fillCount(
            tr(ui.bgTasks.size === 1 ? 'appLayout.backgroundTaskRunningSingular' : 'appLayout.backgroundTaskRunningPlural'),
            ui.bgTasks.size
          )}
        </Text>
      )}
            ...
            >
              ...
              <Box flexGrow={0} flexShrink={0} height={inputHeight} width={inputColumns}>
                ...
                <TextInput
                  ...
                  placeholder={composer.empty ? getPlaceholder() : ui.busy ? tr('appLayout.interruptPlaceholder') : ''}
      ...
      {!composer.empty && !ui.sid && <Text color={ui.theme.color.muted}>{ui.theme.brand.prompt} {ui.status}</Text>}
