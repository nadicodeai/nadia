import { Box, Text, useInput, useStdout } from '@nadia/ink'
...
export function ModelPicker({ allowPersistGlobal = true, gw, onCancel, onSelect, sessionId, t }: ModelPickerProps) {
  ...
  useInput((ch, key) => {
    ...
    if (key.return) {
      if (stage === 'provider') {
        ...
        if (provider.authenticated === false) {
          ...
          // Other auth types: no-op (warning shown tells them to run nadia model)
          return
        }
    ...
    // filter. With Ctrl held, @nadia/ink reports `ch` as the key name ('g'),
