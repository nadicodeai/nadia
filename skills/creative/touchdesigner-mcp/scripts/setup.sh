...
NADIA_HOME_DIR="${NADIA_HOME:-$HOME/.nadia}"
NADIA_CFG="${NADIA_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Nadia config has twozero_td MCP entry ──
if [[ ! -f "$NADIA_CFG" ]]; then
    echo -e " ${FAIL} Nadia config not found at ${NADIA_CFG}"
    manual_steps+=("Create ${NADIA_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$NADIA_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Nadia config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Nadia config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${NADIA_CFG} manually"); }
    manual_steps+=("Restart Nadia session to pick up config change")
fi
