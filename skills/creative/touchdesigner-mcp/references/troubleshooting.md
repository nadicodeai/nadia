...
## 5. Nadia Configuration

### Config location

`$NADIA_HOME/config.yaml` (defaults to `~/.nadia/config.yaml` when `NADIA_HOME` is unset)
...
Restart the Nadia session for changes to take effect. The MCP connection is
