#!/usr/bin/env python3
"""Google Workspace API CLI for Nadia Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Nadia-facing JSON contract and falls back to the Python client
...
from _nadia_home import get_nadia_home
...
def _gws_binary() -> str | None:
    override = os.getenv("NADIA_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Nadia Agent")
