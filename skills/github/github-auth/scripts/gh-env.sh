#!/usr/bin/env bash
# GitHub environment detection helper for Nadia Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _nadia_env="${NADIA_HOME:-$HOME/.nadia}/.env"; [ -f "$_nadia_env" ] && grep -q "^GITHUB_TOKEN=" "$_nadia_env" 2>/dev/null; then
