...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with nadia-agent)
2. User plugins: ``$NADIA_HOME/plugins/model-providers/<name>/``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$NADIA_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$NADIA_HOME/plugins/model-providers/`` if it exists."""
    try:
        from nadia_constants import get_nadia_home

        d = get_nadia_home() / "plugins" / "model-providers"
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple NADIA_HOME profiles don't alias each other.
    safe_name = plugin_dir.name.replace("-", "_")
    if source == "bundled":
        module_name = f"plugins.model_providers.{safe_name}"
    else:
        module_name = f"_nadia_user_provider_{safe_name}"
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$NADIA_HOME/plugins/model-providers/<name>/``
    ...
    # 1. Bundled plugins — shipped with nadia-agent.
    ...
    # 2. User plugins — under $NADIA_HOME/plugins/model-providers/<name>/.
