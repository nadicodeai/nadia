...
it is automatically downloaded from GitHub releases to $NADIA_HOME/bin/tirith.
...
def _get_nadia_home() -> str:
    """Return the Nadia home directory, respecting NADIA_HOME env var."""
    return str(get_nadia_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_nadia_home(), ".tirith-install-failed")
...
def _nadia_bin_dir() -> str:
    """Return $NADIA_HOME/bin, creating it if needed."""
    d = os.path.join(_get_nadia_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $NADIA_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_nadia_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $NADIA_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $NADIA_HOME/bin/tirith
    ...
    nadia_bin = os.path.join(_nadia_bin_dir(), "tirith")
    if os.path.isfile(nadia_bin) and os.access(nadia_bin, os.X_OK):
        _resolved_path = nadia_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return nadia_bin
