...
`nadia skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.nadia.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.nadia.blueprint`` is a mapping containing
    ...
    nadia = meta.get("nadia") if isinstance(meta, dict) else None
    blueprint = nadia.get("blueprint") if isinstance(nadia, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.nadia.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.nadia.blueprint`` block) they can hand
    to ``nadia skills publish`` to share. ``body`` is the plain-language
