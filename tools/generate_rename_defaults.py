#!/usr/bin/env python3
"""tools/generate_rename_defaults.py — bake nadia-rename.yaml into Python constants.

Reads ``nadia-rename.yaml`` at the repo root and writes a generated module to
...
``nadia doctor --static`` loads ``nadia-rename.yaml`` at runtime to drive the
leakage scan. Inside the published Docker image (``ghcr.io/nadicodeai/nadia``)
the yaml is intentionally absent: ``nadia-rename.yaml`` is build-time config
only and the Dockerfile does not copy it into the runtime stage (spec FR-7).

To keep ``nadia doctor --static`` self-contained in the runtime image we bake
the mappings + exceptions + skip_contexts as a tiny Python constants module
that ships with ``nadia_cli`` (the renamed ``hermes_cli`` overlay package).

``nadia-rename.yaml`` REMAINS the single source of truth — this generator runs
...
- ``GENERATED_FROM`` — the basename of the yaml file the constants were
  generated from (``"nadia-rename.yaml"`` — string literal kept out of the
...
_DEFAULT_YAML = _REPO_ROOT / "nadia-rename.yaml"
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        ...
        description=(
            "Generate overlay/hermes_cli/_rename_defaults.py from nadia-rename.yaml "
