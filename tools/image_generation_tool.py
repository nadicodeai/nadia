...
selectable via ``nadia tools`` → Image Generation; the active model is
...
FAL_MODELS: Dict[str, Dict[str, Any]] = {
    ...
    "fal-ai/gpt-image-2": {
        ...
        "defaults": {
            # Same quality pinning as gpt-image-1.5: medium keeps Nadia
...
def _agent_cache_base_for_env(env: Any) -> str | None:
    if env is not None:
        ...
        if remote_home:
            return f"{str(remote_home).rstrip('/')}/.nadia"
    ...
    # Nadia cache roots can be translated without side effects. SSH can still
    ...
    try:
        ...
        if source_images and not edit_endpoint:
            raise ValueError(
                ...
                f"via `nadia tools` → Image Generation."
            )
...
def _build_no_backend_setup_message() -> str:
    ...
    Used by the in-tree FAL path. Mentions:
      - FAL_KEY signup link
      - managed-gateway status (if Nadia tools are enabled)
    ...
    if managed_nous_tools_enabled():
        lines.append(
            "  2. Sign in to a Nadia account that has the managed FAL "
            "gateway enabled (`nadia setup`)"
        )
    lines.append(
        "  3. Configure a different image_gen provider via `nadia tools` "
        "→ Image Generation (run `nadia plugins list` to see installed "
        "backends)"
    )
    return "\n".join(lines)
...
def check_image_generation_requirements() -> bool:
    ...
    the historical behavior: shipping nadia with a FAL key configured
    ...
    if provider is None:
        return json.dumps({
            ...
            "error": (
                ...
                f"registered that name. Run `nadia plugins list` to see "
        ...
        if "image_url" in kwargs or "reference_image_urls" in kwargs:
            ...
            return json.dumps({
                ...
                "error": (
                    ...
                    f"supports editing via `nadia tools` → Image Generation."
                ),
...
# model/provider via `nadia tools` or `/skills`.
