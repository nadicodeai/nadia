...
`mcp_tool.py`, and `nadia_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# NadiaMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_nadia_provider_class() -> Optional[type]:
    ...
    class NadiaMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(
            ...
            **kwargs: Any,
            ...
            self._nadia_server_name = server_name
            ...
            self._nadia_preregistered = preregistered
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`NadiaTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, NadiaTokenStorage)
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._nadia_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import NadiaTokenStorage
                        if isinstance(storage, NadiaTokenStorage):
                            storage.save_oauth_metadata(asm)
                        logger.debug(
                            "MCP OAuth '%s': pre-flight ASM discovered "
                            "token_endpoint=%s",
                            self._nadia_server_name, asm.token_endpoint,
                        )
                        break
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import NadiaTokenStorage
            if not isinstance(storage, NadiaTokenStorage):
                return
        ...
        async def _maybe_flag_poisoned_client(self, response: Any) -> None:
            ...
            and is handled by ``nadia mcp reauth``.
            ...
            back to ``nadia mcp reauth``.
            """
            try:
                if self._nadia_preregistered:
                    return
    ...
    return NadiaMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_NADIA_PROVIDER_CLS: Optional[type] = _make_nadia_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`NadiaMCPOAuthProvider` directly using the helpers
        ...
        Returns None if the MCP SDK's OAuth support is unavailable.
        """
        if _NADIA_PROVIDER_CLS is None:
            logger.warning(
        ...
        storage = NadiaTokenStorage(server_name)

        if not _is_interactive() and not storage.has_cached_tokens():
            raise OAuthNonInteractiveError(
                "MCP OAuth for "
                f"'{server_name}': non-interactive environment and no "
                "cached tokens found. Run `nadia mcp login "
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``nadia mcp remove <name>`` and (indirectly) by
        ``nadia mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
