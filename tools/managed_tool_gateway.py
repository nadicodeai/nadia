"""Generic managed-tool gateway helpers for Nadia-hosted vendor passthroughs."""
...
_DEFAULT_TOOL_GATEWAY_DOMAIN = "nadicode.ai"
...
def auth_json_path():
    """Return the Nadia auth store path, respecting NADIA_HOME overrides."""
    return get_nadia_home() / "auth.json"
...
def peek_nous_access_token() -> Optional[str]:
    """Cheap probe for a Nadia gateway token without triggering refresh.

    Availability scans (`nadia tools`, banner/status paint, provider
...
def read_nous_access_token() -> Optional[str]:
    """Read a Nadia Subscriber OAuth access token from auth store or env override."""
    explicit = os.getenv("TOOL_GATEWAY_USER_TOKEN")
    if isinstance(explicit, str) and explicit.strip():
        return explicit.strip()
    nous_provider = _read_nous_provider_state() or {}
    ...
    try:
        ...
        logger.debug("Nadia access token refresh failed: %s", exc)
...
def is_managed_tool_gateway_ready(
    ...
    """Return True when gateway URL and a likely-usable Nadia token are present.
