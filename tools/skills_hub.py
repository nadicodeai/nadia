#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Nadia Skills Hub.
...
Used by nadia_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
def _nadia_home() -> Path:
    return get_nadia_home()


def _skills_dir() -> Path:
    forced = _override("SKILLS_DIR")
    return Path(forced) if forced is not None else _nadia_home() / "skills"
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            nadia_meta = metadata.get("nadia", {})
            if isinstance(nadia_meta, dict):
                tags = nadia_meta.get("tags", [])
        if not tags:
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by NadicodeAI) but not activated
    ...
    OFFICIAL_REPO = "nadicodeai/nadia"
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                nadia_meta = meta_block.get("nadia", {})
                if isinstance(nadia_meta, dict):
                    tags = nadia_meta.get("tags", [])
...
# ---------------------------------------------------------------------------
# Nadia centralized index source
# ---------------------------------------------------------------------------

NADIA_INDEX_URL = "https://docs.nadicode.ai/nadia/api/skills-index.json"
NADIA_INDEX_TTL = 6 * 3600  # 6 hours


def _nadia_index_cache_file() -> Path:
    return _index_cache_dir() / "nadia-index.json"


def _load_nadia_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for NADIA_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    nadia_index_cache_file = _nadia_index_cache_file()
    if nadia_index_cache_file.exists():
        try:
            age = time.time() - nadia_index_cache_file.stat().st_mtime
            if age < NADIA_INDEX_TTL:
                return json.loads(nadia_index_cache_file.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    # Fetch from docs site
    try:
        resp = httpx.get(NADIA_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
            logger.debug("Nadia index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("Nadia index fetch failed: %s", e)
        return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        nadia_index_cache_file.parent.mkdir(parents=True, exist_ok=True)
        nadia_index_cache_file.write_text(json.dumps(data))
    except OSError:
        pass
...
class NadiaIndexSource(SkillSource):
    """Skill source backed by the centralized Nadia Skills Index.
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "nadia-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "nadia-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        NadiaIndexSource(auth=auth), # Centralized index (search + resolved install paths)
