...
This module provides the glue:
    - ``NadiaTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Nadia Agent"
"""
...
from nadia_constants import secure_parent_dir
...
_USER_SKIPPED_SENTINEL = "__nadia_user_skipped__"
...
def _get_token_dir() -> Path:
    ...
    Uses NADIA_HOME so each profile gets its own OAuth tokens.
    Layout: ``NADIA_HOME/mcp-tokens/``
    """
    try:
        from nadia_constants import get_nadia_home
        base = Path(get_nadia_home())
    except ImportError:
        base = Path(os.environ.get("NADIA_HOME", str(Path.home() / ".nadia")))
...
# ---------------------------------------------------------------------------
# NadiaTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class NadiaTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        NADIA_HOME/mcp-tokens/<server_name>.json         -- tokens
        NADIA_HOME/mcp-tokens/<server_name>.client.json   -- client info
        NADIA_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Nadia records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Nadia.</p></body></html>"
            ) if code else (
...
async def _redirect_handler(authorization_url: str) -> None:
    ...
    if _oauth_port and (os.getenv("SSH_CLIENT") or os.getenv("SSH_TTY")):
        print(
            ...
            f"\n"
            f"  See: https://docs.nadicode.ai/nadia/guides/oauth-over-ssh\n",
            file=sys.stderr,
        )
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `nadia mcp login <server>` later to "
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Nadia Agent")
    scope = cfg.get("scope")
...
def build_oauth_auth(
    ...
    if not _is_interactive() and not storage.has_cached_tokens():
        raise OAuthNonInteractiveError(
            ...
            f"`nadia mcp login {server_name}` interactively first to complete "
