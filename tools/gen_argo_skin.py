#!/usr/bin/env python3
"""Generate Argo terminal skin artifacts from NadicodeAI DTCG tokens."""
...
DEFAULT_PYTHON_OUTPUT = REPO_ROOT / "overlay" / "hermes_cli" / "_argo_skin.py"
DEFAULT_TYPESCRIPT_OUTPUT = REPO_ROOT / "overlay" / "ui-tui" / "src" / "argoTheme.generated.ts"
...
ARGO_BANNER_LOGO = ""
ARGO_BANNER_HERO_TEMPLATE = """[{link_deep}]⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⣿⡈⢳⣄⠀⠀⠀⣹⣏⠀⠀⠀⣠⡞⢁⣿⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀[/]
[{link_deep}]⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢀⣈⡻⣦⣿⠀⠀⠀⣿⣿⠀⠀⠀⣿⣴⢟⣁⡀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀[/]
[{link_deep}]⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠘⠛⠃⢸⡟⢷⣄⠀⣽⣯⠀⣠⡾⢻⡇⠘⠛⠃⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀[/]
[{link_deep}]⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠈⠳⣄⣻⠆⣿⣿⠰⣟⣠⠞⠁⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀[/]
[{link_deep}]⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⣀⣤⣾⣿⣾⣿⣿⣷⣿⣷⣤⣀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀[/]
...
def build_skin_data(ds_tokens: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "branding": BRANDING,
        "tool_prefix": "|",
        "banner_logo": ARGO_BANNER_LOGO,
        "banner_hero": _format_template(ARGO_BANNER_HERO_TEMPLATE, values),
    }
...
def _typescript_artifact(ds_tokens: Path) -> str:
    ...
    lines = [
        ...
        "",
        _ts_object("ARGO_DARK_THEME_COLORS", data["dark_colors"], "ThemeColors"),
        "",
        _ts_object("ARGO_LIGHT_THEME_COLORS", data["light_colors"], "ThemeColors"),
        "",
    ...
    lines.extend(
        [
            "}",
            "",
            f"export const ARGO_BANNER_LOGO = {_ts_string(str(data['banner_logo']))}",
            f"export const ARGO_BANNER_HERO = {_ts_string(str(data['banner_hero']))}",
            "",
        ]
    )
    return "\n".join(lines)
...
def preview(ds_tokens: Path) -> str:
    ...
    return "\n\n".join(
        [
            "CP-A Argo banner_logo candidate:",
            str(skin["banner_logo"]),
            "CP-A Argo banner_hero candidate:",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"gen_argo_skin: {exc}", file=sys.stderr)
        return 1
    return 0
