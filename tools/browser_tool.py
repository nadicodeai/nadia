...
def _find_agent_browser() -> str:
    ...
    try:
        ...
        if ensure_dependency("browser"):
            ...
            if not recheck:
                nadia_nm = str(get_nadia_home() / "node_modules" / ".bin")
                recheck = shutil.which("agent-browser", path=nadia_nm)
            if not recheck:
                nadia_node_bin = str(get_nadia_home() / "node" / "bin")
                recheck = shutil.which("agent-browser", path=nadia_node_bin)
            if not recheck:
                nadia_node_root = str(get_nadia_home() / "node")
                recheck = shutil.which("agent-browser", path=nadia_node_root)
            if recheck:
