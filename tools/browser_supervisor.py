...
One ``CDPSupervisor`` runs per Nadia ``task_id`` that has a reachable CDP
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach Nadia logs or tracebacks. Falls back to a
...
DIALOG_BRIDGE_HOST = "nadia-dialog-bridge.invalid"
...
(() => {
  if (window.__nadiaDialogBridgeInstalled) return;
  window.__nadiaDialogBridgeInstalled = true;
  const ENDPOINT = "http://nadia-dialog-bridge.invalid/";
