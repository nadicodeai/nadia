...
Backend is selected during ``nadia tools`` setup (web.backend in config.yaml).
When available, Nadia can route Firecrawl calls through a Nadia-hosted tool-gateway
for Nadia Subscribers only.
...
- Firecrawl: https://docs.firecrawl.dev/introduction (search, extract; direct or derived firecrawl-gateway.<domain> for Nadia Subscribers)
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via Nadia config-aware env, falling back to process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set
    through Nadia's config/.env layer (``nadia config set``, ``nadia tools``)
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.nadia/config.yaml."""
    try:
...
def _get_backend(capability: str = "search") -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``nadia tools``).
    ...
    # pre-empted by a Nadia OAuth token whose subscription tier may not
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `nadia tools` repaint.
        try:
...
def _is_nous_auxiliary_client(client: Any) -> bool:
    ...
    return host == "nadicode.ai" or host.endswith(".nadicode.ai")
...
def _register_bundled_web_providers_directly() -> None:
    ...
    (:func:`nadia_cli.plugins._ensure_plugins_discovered`), which auto-loads
