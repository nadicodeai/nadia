...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # ~/.nadia/ form so the Nadia config/env patterns catch it. In Docker and
    # gateway deployments the agent often references the resolved absolute path
    # directly (e.g. `sed -i ... /home/nadia/.nadia/config.yaml`) rather than
    # ~, $HOME, or $NADIA_HOME. Done at detection time (not via an import-time
    # pattern snapshot) so it tracks the live NADIA_HOME even when that is set
...
def _rewrite_resolved_nadia_home(command: str) -> str:
    ...
    for path in candidates:
        if not path or path in seen:
            continue
        seen.add(path)
        # Guard against a degenerate NADIA_HOME (e.g. "/" or "") rewriting
        ...
        # /home/nadia/.nadia or a per-test tempdir, never a bare root.
        normalized = path.rstrip("/")
        if not normalized.startswith("/") or normalized.count("/") < 2:
            continue
        command = command.replace(normalized + "/", "~/.nadia/")
    return command
