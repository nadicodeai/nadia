#!/usr/bin/env python3
"""Close a read-only agent terminal tab in the Nadia desktop GUI.
...
``read_terminal`` it is gated on ``NADIA_DESKTOP`` so it never appears outside
the GUI.
"""
...
def check_close_terminal_requirements() -> bool:
    """Desktop GUI only — NADIA_DESKTOP is set on the gateway the app spawns."""
    return (os.getenv("NADIA_DESKTOP") or "").strip().lower() in ("1", "true", "yes")
...
CLOSE_TERMINAL_SCHEMA = {
    ...
    "description": (
        ...
        "the Nadia desktop GUI (the tabs mirroring terminal(background=true) runs). "
