...
React + Ink terminal UI for Nadia. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `NADIA_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `nadia setup`, suspends Ink while it runs, opens a new session on success
...
- `commands/billing.ts` — `/billing`: manage Nadia terminal billing — buy credits, auto-reload, limits
...
- Input history is stored in `~/.nadia/.nadia_history` or under `NADIA_HOME`.
...
### Billing (`billing.ts`)
`/billing` — manage Nadia terminal billing — buy credits, auto-reload, limits
...
### Credits (`credits.ts`)
`/credits` — Nadia credit balance and browser top-up

### Setup (`setup.ts`)
`/setup` — launches external `nadia setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/nadia-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external nadia setup, suspends Ink while it runs
      ...
      slash/
        ...
        commands/
          billing.ts                /billing — manage Nadia terminal billing
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.nadia/tui-parent.log
    ...
    types/
      nadia-ink.d.ts            type declarations for @nadia/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent NadiaCLI subprocess for slash commands
```