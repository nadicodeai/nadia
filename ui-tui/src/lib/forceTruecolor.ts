...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.NADIA_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Nadia on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via NADIA_TUI_TRUECOLOR=1.
