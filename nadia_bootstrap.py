"""Windows UTF-8 bootstrap for Nadia entry points.
...
should be imported at the very top of every Nadia entry point
(``nadia``, ``nadia-agent``, ``nadia-acp``, ``python -m gateway.run``,
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Nadia modules.

    Nadia ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real Nadia source root
    ...
    """
    root = src_root or os.environ.get("NADIA_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``NADIA_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("NADIA_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import nadia_bootstrap``
# (or ``from nadia_bootstrap import apply_windows_utf8_bootstrap``) at
