# Phase 1 Implementation Plan (REVISED 2026-06-07) — Nadia default skin from the NadicodeAI Design System (private git-dependency model)

> Spec: `/home/vadim/Code/nadia/specs/nadia-default-skin-design-system.md` (decisions locked 2026-06-07). Source of truth.
...
> Scope: terminal CLI skin + `ui-tui` (Ink), both colored from the DS DTCG tokens; Nadia ASCII art replacing the caduceus; neutralized minimal spinner; `nadia` made the default skin (image AND native). **Web dashboard is Phase 2 — out of scope.**
...
**F1a (CORRECTION — residue surface is LARGER than the baseline found).** The ui-tui residues `fromSkin`/the gateway cannot reach, all re-verified by exact line:
  ...
  - **`upstream/ui-tui/src/components/branding.tsx:163` — `const heroLines = caduceus(t.color, t.bannerHero || undefined)`** — the hero FALLS BACK to the caduceus unless `bannerHero` is non-empty. The baseline OMITTED this. Setting `DARK_THEME`/`LIGHT_THEME` `bannerHero` to the Nadia art defeats it.
  - **`upstream/ui-tui/src/content/fortunes.ts:26` — `return \`${rare ? '🌟' : '🔮'} ${bag[n % bag.length]}\`\`** — emits kawaii glyphs. The baseline OMITTED this.

**F2 — the rich CLI banner retires the caduceus purely via skin data (no `banner.py`/`cli.py` patch).** `upstream/cli.py:2884` hardcodes `"⚕ NOUS HERMES"` ONLY when `skin_name == "default"`; the `else` branch (`cli.py:2888-2890`) uses `agent_name`. A non-`default` skin (`nadia`) bypasses it. `banner.py` picks `_bskin.banner_hero`/`banner_logo` when populated. A fully-populated `nadia` skin + a non-`default` default switch needs no banner patch. (Re-verified `cli.py:2884-2890`.)
...
**F4 — build order (drives generator placement).** `tools/build.py` `build()`: `_apply_patches(562) → _strip_quilt_state → _regenerate_rename_defaults(564) → _copy_overlay(565) → _copy_fde_scripts → _strip_excluded_paths → _apply_content_edits(568) → _run_rebrand(569) → _run_assertions`. Overlay copied (565) BEFORE content_edits (568) BEFORE rebrand (569). The rename is one-directional `hermes→nadia`, so literal `nadia` + lowercase hex + `@nadicodeai/design-system` are rename-invariant. `content_edits` fail loud on a stale anchor (`build.py:387`). (Re-verified `tools/build.py:562-569`.)
...
**F6 — skin color-key surface.** The `default` skin dict defines **16** color keys (`grep`-verified). The full superset consumed via `get_color("<key>")` across `upstream/` is **30** keys (verified union: `banner_×5, ui_×5, status_bar_×8 (bg/text/dim/good/warn/bad/critical/strong), prompt, input_rule, response_border, session_label, session_border, selection_bg, voice_status_bg, completion_menu_×4`). `_build_skin_config` (`skin_engine.py:692-704`) merges a skin's `colors` ON TOP of `default`, so any key the `nadia` skin omits inherits the Hermes value. The generator MUST populate the full 30-key superset (+ `banner_logo`/`banner_hero`) so nothing inherits.
...
`_BUILTIN_SKINS` opens at `skin_engine.py:164` and the dict closes `}` at **line 645** (verified `awk 'NR>=164 && /^}/'` → `645`; this corrects a draft that said 647). `load_skin` checks user `~/.<app>/skins/<name>.yaml` FIRST, so a built-in is the only always-available, non-shadowable config default. `patches/0016-nadia-skin.patch` inserts before the close at line 645:
```python
    "nadia": _NADIA_SKIN_DATA,
```
preceded by a guarded import in the dict region:
```python
try:
    from hermes_cli._nadia_skin import NADIA_SKIN_DATA as _NADIA_SKIN_DATA
except Exception:  # pragma: no cover - keeps upstream/ importable standalone
    _NADIA_SKIN_DATA = {"name": "nadia", "description": "NadicodeAI ink"}
```
The full 30-key color map + neutralized spinner + branding + `banner_logo`/`banner_hero` live in the **generated** `overlay/hermes_cli/_nadia_skin.py` (renamed to `nadia_cli/_nadia_skin.py` by rebrand, mirroring `overlay/hermes_cli/_rename_defaults.py`). Patch carries STRUCTURE only (no hexes) → a DS bump regenerates only the overlay; the patch never refreshes on a sync. The patch also extends `upstream/tests/hermes_cli/test_skin_engine.py` so the new builtin passes the existing completeness test — that test edit ships inside patch 0016 (it lives under `upstream/`).
...
Upstream default `"skin": "default"` is at `cli.py:458` (verified). Flip to `"nadia"` via a `content_edits` rule (`_apply_content_edits`, `build.py:356`; runs before rebrand on hermes-named content; anchor rename-invariant; **fails loud on a stale anchor** at `build.py:387`). `cli.py` is top-level `upstream/cli.py` → ships to `dist/nadia/cli.py` (verify with `grep '"skin": "nadia"' dist/nadia/cli.py`; the post-rename module dir is `nadia_cli/` for the skin engine, but the top-level entry stays `cli.py`). Per locked decision #4 this IS the documented distribution-default-switch standard; the user approved touching `packaging-strip.yaml` provided the mechanism is the documented standard (T-DOCS documents it). The rule shape mirrors existing `content_edits` (a `name`/`file`/`why`/`find`/`replace` block).
_Rejected:_ `nadia-rename.yaml` (`"default"` is not a hermes token, can't be renamed); a dedicated quilt patch (keeps the switch in the series → refresh churn on a config-shape sync; `content_edits` keeps it out and is loud-on-drift).
...
`tools/gen_nadia_skin.py` reads the installed DS (`tools/skin-gen/node_modules/@nadicodeai/design-system/dist/tokens/nadicode.dtcg.json`, overridable via `--ds-tokens`) and emits two deterministic committed artifacts: `overlay/hermes_cli/_nadia_skin.py` and `overlay/ui-tui/src/nadiaTheme.generated.ts`. It is invoked by **`make gen-skin`** (= `npm --prefix tools/skin-gen ci` then `python tools/gen_nadia_skin.py`), NOT inside `make build` (Fact #1: no node at make-build time). `make build` ships the committed overlay verbatim, so native installs, `make test`, `make dist-test`, and the Docker builder need NO DS/npm/git access. Freshness is proven by a CI job that runs `make gen-skin && git diff --exit-code overlay/` (the git-dep analogue of the baseline's stale-output gate).
...
DS SVGs are glyph-based (`logo-mark.svg`: seam + green `#007a5e` / red `#cd212a` nodes; `logo-wordmark.svg`: text + seam), so auto-tracing yields garbage. `banner_logo` (block-letter "NADIA" in the `ares`/`poseidon` style) and `banner_hero` (compact seam + node mark) are **hand-authored Rich-markup templates** with COLOR placeholders the generator fills from DS tokens. Same templates feed ui-tui via `parseRichMarkup`. Approved DIRECTION per locked decision #5 (block-letter NADIA + compact seam/node hero, ares/poseidon style). The **green-red node** motif (green `#007a5e` + red `#cd212a`, both present on current DS master — F5) is on-token; FINAL glyphs need USER sign-off at CP-A. Art is a static template; only colors track tokens.
...
`ThemeColors` is **28 keys** (`theme.ts:1-35`, verified: primary, accent, border, text, muted, 4×completion, label, ok, error, warn, prompt, sessionLabel, sessionBorder, 6×status, selectionBg, 4×diff, shellDollar). The generator emits dark+light literals from the SAME `TOKEN_MAP` as the Python skin. `patches/0017-ui-tui-nadia.patch`:
1. import generated literals → replace inline `DARK_THEME.color` / `LIGHT_THEME.color` bodies;
2. patch `BRAND` (`theme.ts:239-247`) to Nadia (`name:'Nadia'`, drop `⚕` icon, neutral `goodbye`/`welcome`/`helpHeader`) — kills the `⚕` `fromSkin` can't reach (F1);
3. set `DARK_THEME`/`LIGHT_THEME` `bannerLogo`/`bannerHero` (currently `''`) to the generated Nadia art — also defeats `branding.tsx:163`'s caduceus default (F1a);
...
5. refresh `upstream/ui-tui/src/__tests__/theme.test.ts` — `:46` `'Hermes Agent'`→`'Nadia'`, `:54` `DEFAULT_THEME.color.primary` `'#FFD700'`→the DS hex, `:55` `error '#ef5350'`→the DS error hex (these three break when the theme is patched; verified) + add a DS-hex golden.
Leave `normalizeThemeForAnsiLightTerminal` + `ANSI_NORMALIZED_FOREGROUNDS` UNTOUCHED. **CI gap (verified):** `make test` = `pytest -m 'not integration'`; ui-tui vitest runs in NO Nadia CI gate; the image runs only `npm run build` (=`tsc`) at `Dockerfile:405`. Mitigation: refresh the assertions in-patch + add a Python-side golden in `make test` reading the generated `.ts`; `make image-full` exercises `tsc`.

### AD7 — Native (non-Docker) installs default to `nadia` too (Open Q8, locked #5)
The `content_edits` switch (AD2) flips `display.skin` across the whole `dist/nadia/` tree = the native-install artifact; no install-mode branch exists to make native differ. So native defaults to `nadia`, matching locked decision #5. `default` (Hermes) stays opt-in via `/skin default`.

### AD8 — Spinner + branding fully populated to prevent Hermes inheritance (Assumption 7)
`_build_skin_config` (`skin_engine.py:692-704`) merges `colors`/`spinner`/`branding` ON TOP of `default`, so omitted keys inherit Hermes. The generator emits ALL spinner keys (plain verbs, minimal faces, no `wings`) and ALL branding keys (`agent_name:"Nadia"`, neutral rest, no `⚕`). A unit test asserts no kawaii/`⚕`/`⚔` in the resolved `nadia` skin.

### AD9 — Standards-first + documentation as a first-class deliverable (locked #3, SC-6)
Establish and document the four mechanisms BEFORE/WITH the implementation, in **AGENTS.md** (concise, progressive-disclosure — extends `## Common tasks` and `## Where to read more`, which already follow this convention) AND **`docs/nadia-design-system.md`** (full written process) AND a small extension to **`.shepherd/standards.md`** (its `## Dependencies` + a new `## Distribution Defaults` subsection — the repo's existing standards layer):
...
T1 (tools/skin-gen git DS dep + committed lockfile)  ── UPHILL: closes Q4 wiring
   │
   ▼
T2 (generator gen_nadia_skin.py + TOKEN_MAP + golden tests; reads node_modules)  ── UPHILL: closes Q3/Q6/Q7 contract
   │
   ▼
T3 (ASCII art authoring + USER sign-off @ CP-A)  ── UPHILL: aesthetic unknown
   │
   ├──────► T4 (overlay/_nadia_skin.py + patch 0016 register + skin tests)   (Q1)
   │             │
   │             ▼
   │        T5 (content_edits default-skin switch + cli integration test)   (Q2/Q8)
   │
   └──────► T6 (overlay/nadiaTheme.generated.ts + patch 0017 ui-tui: DARK/LIGHT/BRAND + 5 residues + theme.test)   (Q6)
                  │
...
- **Description:** Author the documented standards BEFORE/WITH the implementation (locked #3, AD9). New `docs/nadia-design-system.md` (full written process for the four standards). Extend AGENTS.md `## Common tasks` with concise actionable entries ("Regenerate the Nadia skin from the DS — `make gen-skin`", "Change a distribution default — add a `content_edits` rule", "Bump the design system — `npm --prefix tools/skin-gen update` + `make gen-skin`") and add the doc to `## Where to read more` (progressive disclosure; AGENTS.md additions kept tight). Extend `.shepherd/standards.md` (`## Dependencies` + a new `## Distribution Defaults` subsection). Draft now; finalize to match the as-built code at T-DOCS2.
- **Acceptance:** `docs/nadia-design-system.md` exists covering all four standards; AGENTS.md links to it and has the new task entries; `.shepherd/standards.md` documents the content_edits default-switch + the DS-dep standard; no contradiction with existing "Hard rules"/standards (e.g. "no `.npmrc`").
- **Verification:** `test -f docs/nadia-design-system.md && grep -q 'nadia-design-system' AGENTS.md && grep -qE 'git dependency|package-lock|content_edits|gen-skin' docs/nadia-design-system.md`
- **Dependencies:** none (DS git-dep is proven; T1 formalizes it).
- **Files:** `docs/nadia-design-system.md` (new), `AGENTS.md` (extend `## Common tasks` + `## Where to read more`), `.shepherd/standards.md` (extend).
- **Scope:** M.
...
- **Verification:** `python tools/gen_nadia_skin.py --ds-tokens tests/fixtures/nadicode.dtcg.sample.json && pytest tests/test_gen_nadia_skin.py -q && python tools/gen_nadia_skin.py --ds-tokens tests/fixtures/nadicode.dtcg.sample.json && git diff --exit-code overlay/hermes_cli/_nadia_skin.py overlay/ui-tui/src/nadiaTheme.generated.ts`
- **Dependencies:** T1.
- **Files:** `tools/gen_nadia_skin.py` (new), `tests/test_gen_nadia_skin.py` (new), `tests/fixtures/nadicode.dtcg.sample.json` (new, copied from the git-installed DS package).
- **Scope:** M.
...
- **Verification:** `python tools/gen_nadia_skin.py --preview` (prints rendered art); visual review; glyph denylist (T7) passes.
- **Dependencies:** T2.
- **Files:** art templates in `tools/gen_nadia_skin.py` (or `tools/nadia_art.py`).
- **Scope:** M.
...
### T4 — Commit `overlay/hermes_cli/_nadia_skin.py`; patch 0016 register `nadia` in `_BUILTIN_SKINS` [M]
- **Description:** Run `make gen-skin` to produce the committed overlay. Create `patches/0016-nadia-skin.patch` via the quilt workdir over `upstream/` (memory: quilt-patch-editing-workflow, `QUILT_PATCHES`): guarded import + `"nadia": _NADIA_SKIN_DATA` before the dict close at `skin_engine.py:645` (AD1), plus extend `upstream/tests/hermes_cli/test_skin_engine.py` (`test_nadia_skin_loads`, `test_nadia_has_complete_colors`, `test_set_active_skin_nadia`, `test_nadia_branding_no_hermes_glyph`). Add `patches/asserts/0016-nadia-skin.txt` POSITIVE contract (`path:nadia_cli/_nadia_skin.py "nadia"`, `path:nadia_cli/skin_engine.py _NADIA_SKIN_DATA`, `path:nadia_cli/_nadia_skin.py "agent_name": "Nadia"`). Add to `patches/series` + `patches/asserts/manifest.txt`.
- **Acceptance:** `make build` applies 0016; `dist/nadia/nadia_cli/_nadia_skin.py` present; `load_skin("nadia")` resolves all 30 color keys; `list_skins()` includes nadia; `git diff upstream/` empty after build.
- **Verification:** `make build && make check-upstream-pristine && cd dist/nadia && PYTHONPATH=. python -c "from nadia_cli.skin_engine import load_skin; s=load_skin('nadia'); assert s.name=='nadia' and s.colors['banner_title']=='#50e3c2'" && cd - && make leakage-static`
- **Dependencies:** T2, T3.
- **Files:** `overlay/hermes_cli/_nadia_skin.py` (generated, committed), `patches/0016-nadia-skin.patch`, `patches/asserts/0016-nadia-skin.txt`, `patches/series`, `patches/asserts/manifest.txt`. (Test edit carried inside patch 0016.)
- **Scope:** M.

### T5 — Default-skin switch via `packaging-strip.yaml` content_edit + CLI integration test [S]
- **Description:** Add a `content_edits` rule (AD2) flipping `"skin": "default"` → `"skin": "nadia"` at `cli.py:458` (rename-invariant anchor; fails loud on drift at `build.py:387`), mirroring the existing rule shape (`name`/`file`/`why`/`find`/`replace`). Extend `upstream/tests/cli/test_cli_skin_integration.py`: under the dist config `get_active_skin().name == "nadia"`, branding resolves to "Nadia", the `cli.py:2884` branch yields no "NOUS HERMES". (Native inherits nadia — AD7.)
- **Acceptance:** Fresh `dist/nadia` resolves active skin = `nadia`; banner takes the `agent_name` branch; `grep '"skin": "nadia"' dist/nadia/cli.py` hits (**top-level `dist/nadia/cli.py`**).
- **Verification:** `make build && grep -q '"skin": "nadia"' dist/nadia/cli.py && make dist-test DIST_TEST_ARGS="--paths tests/cli/test_cli_skin_integration.py:tests/hermes_cli/test_skin_engine.py"`
- **Dependencies:** T4.
...
### T6 — Commit `overlay/ui-tui/src/nadiaTheme.generated.ts`; patch 0017 ui-tui (DARK/LIGHT/BRAND + 5 residues + theme.test) [M]
- **Description:** Commit the generated `.ts`. Create `patches/0017-ui-tui-nadia.patch` per AD6: (1) import generated literals → `DARK_THEME.color`/`LIGHT_THEME.color`; (2) patch `BRAND` (`theme.ts:239-247`) to Nadia, drop `⚕`; (3) set `DARK_THEME`/`LIGHT_THEME` `bannerLogo`/`bannerHero` (defeats `branding.tsx:163` caduceus default); (4) patch the **5 residues** `components/appLayout.tsx:328`, `components/appChrome.tsx:30,54` (keep `EMOJI_FRAME_WIDTH` at `:81` valid), `app/uiStore.ts:28`, `content/fortunes.ts:26`; (5) refresh `upstream/ui-tui/src/__tests__/theme.test.ts` (`:46` name, `:54` primary, `:55` error) + add a DS-hex golden. Leave `normalizeThemeForAnsiLightTerminal`/`ANSI_NORMALIZED_FOREGROUNDS` untouched. Add `patches/asserts/0017-ui-tui-nadia.txt` POSITIVE asserts (`path:ui-tui/src/nadiaTheme.generated.ts #50e3c2`, `path:ui-tui/src/theme.ts name: 'Nadia'`). Add a Python-side golden in `make test` reading the generated `.ts` (vitest is in no CI gate).
- **Acceptance:** `DARK_THEME.color.primary` is DS hex (not `#FFD700`); `BRAND.name === 'Nadia'`, no `⚕`; all 5 residues neutralized; `npm run type-check && npm run test` green in `dist/nadia/ui-tui`; `git diff upstream/` empty after build.
- **Verification:** `make build && make check-upstream-pristine && cd dist/nadia/ui-tui && npm install --prefer-offline --no-audit && npm run type-check && npm run test`
- **Dependencies:** T2, T3.
- **Files:** `overlay/ui-tui/src/nadiaTheme.generated.ts` (generated, committed), `patches/0017-ui-tui-nadia.patch`, `patches/asserts/0017-ui-tui-nadia.txt`, `patches/series`, `patches/asserts/manifest.txt`. (Test edit carried inside patch 0017.)
- **Scope:** M.

### T7 — SC-5 glyph denylist scanner + assertion contracts + throwaway mutation proof [S]
- **Description:** Add `tools/check_no_legacy_glyphs.py` — a denylist scan over `dist/nadia/` for `caduceus`, `⚕`, `⚔`, the kawaii faces (`🌀🤔✨🍵🔮🌟`), and `#FFD700`/`#ffd700` on the **Nadia-owned surfaces** (the `nadia` skin data + ui-tui theme/banner). Wire into `make leakage-static` (`Makefile:81`) — it is the REAL SC-5 enforcement (`verify_no_leakage.py` is glyph-blind, F3; the assertion runner is positive-only). Add `tests/test_check_no_legacy_glyphs.py`. Prove the gate trips on a planted `⚕` in the T0 harness (never live-fire). **Scope the denylist to Nadia surfaces so it does NOT flag the still-present `default` skin's legitimate `⚕`** (which MUST remain).
- **Acceptance:** `make leakage-static` fails on a planted banned glyph in an Nadia surface, passes on the shipped art + the untouched `default` skin; mutation test green.
...
- **Scope:** S.
- **Decision recorded:** SC-5 enforced by a dedicated denylist scanner + unit tests, scoped to Nadia surfaces (preserves `default`'s `⚕`).

### T8 — `make gen-skin` target + CI regen-drift gate; pre-commit hook [S]
- **Description:** Add a `make gen-skin` target: `npm --prefix tools/skin-gen ci --no-audit --no-fund && python tools/gen_nadia_skin.py`. Add a CI job (or extend an existing one) with `setup-node` + git auth to the private DS (cross-repo token/deploy-key — Decisions #2) + `make gen-skin` then `git diff --exit-code overlay/hermes_cli/_nadia_skin.py overlay/ui-tui/src/nadiaTheme.generated.ts` (the git-dep analogue of the baseline stale-output gate — proves the committed overlay matches a fresh regen from the git-installed DS). `make build` is UNCHANGED (consumes committed overlay; runs no npm/git). Confirm `.githooks/pre-commit` (build+leakage on patch/overlay/tools commits) passes (`make install-hooks` once). Document `make gen-skin` in T-DOCS.
...
- **Verification:** `make install-hooks && make gen-skin && git diff --exit-code overlay/hermes_cli/_nadia_skin.py overlay/ui-tui/src/nadiaTheme.generated.ts && pytest -m integration tests/test_dist_determinism.py`
- **Dependencies:** T4, T6.
- **Files:** `Makefile` (`gen-skin` target), `.github/workflows/ci.yml` (regen-drift job: `setup-node` + `npm --prefix tools/skin-gen ci`), `docs/nadia-design-system.md` (update).
- **Scope:** S.
...
- **Description:** Reconcile the T-DOCS drafts with the as-built reality: final generator flags, the exact `content_edits` rule name, exact overlay paths, the `make gen-skin` command, the node-aware CI job. Ensure AGENTS.md is concise and links to `docs/nadia-design-system.md`; ensure docs and `.shepherd/standards.md` match the code. This is the SC-6 gate.
- **Acceptance:** AGENTS.md + `docs/nadia-design-system.md` describe the actual `make gen-skin` command, the actual `content_edits` rule, the git-dependency model, the overlay paths, and the patch conventions — verified against the code paths.
- **Verification:** Read-review against T1-T8 artifacts; `grep -qE 'gen-skin|content_edits|@nadicodeai/design-system' docs/nadia-design-system.md`; confirm every command in the doc runs (`python tools/gen_nadia_skin.py --help`, `npm view @nadicodeai/design-system version`, `make gen-skin`).
- **Dependencies:** T1-T8.
- **Files:** `AGENTS.md`, `docs/nadia-design-system.md`, `.shepherd/standards.md`.
- **Scope:** S.
- **Decision recorded (#3, SC-6):** docs match the implementation; this is a release gate.

### T9 — Full gate run + PR [M]
- **Description:** Run the complete Phase-1 gate set and fix fallout (capture output to files — memory: no-fabricated-results). `make dist-test` is REQUIRED before merging a dist-affecting change (memory: nadia-dist-test-gate; non-blocking in CI). `make image` = runtime-slim (no ui-tui); **`make image-full`** = runtime-full and exercises the ui-tui `npm run build`/`tsc` (`Dockerfile:405`) — run it. `check-packaging-contract` is unaffected by the npm dep (F7) but run it to confirm. Open a PR so CI `dist-nadia-tests` + image build (no push) + the T8 regen-drift job run.
...
- **CP-0 — after T-DOCS + T0 + T1 (foundation):** the DS git dep resolves via org git auth (`npm --prefix tools/skin-gen install`); standards/docs drafted in AGENTS.md + `docs/nadia-design-system.md` + `.shepherd/standards.md`; throwaway harness ready. System still builds (committed overlay path untouched).
...
- **CP-D — after T-DOCS2 + T9:** Full gate set green; AGENTS.md + docs match the implementation (SC-6); PR opened so CI dist-nadia-tests + image build + regen-drift run; SC-5 glyph gate enforced. Merge-readiness.
...
| SC-5 silently unenforced (`verify_no_leakage.py` hermes-only; assertion runner positive-only) | High (confirmed F3) | High | T7 `tools/check_no_legacy_glyphs.py` wired into `make leakage-static` + unit tests; proven in T0 harness; scoped to Nadia surfaces (preserves `default`'s `⚕`). Never rely on leakage-static or `.txt` negative greps. |
...
| ui-tui vitest in NO Nadia CI gate → a `DEFAULT_THEME`/`BRAND` patch silently breaks `theme.test.ts:46/54/55` | High (confirmed) | Med | T6 refreshes those assertions in-patch + Python-side golden in `make test`; `make image-full` exercises `tsc`. |
...
| Generated overlay hex/art mangled by `tools/rebrand.py` | Low | High | Rename one-directional hermes→nadia; hex + literal `nadia` + `@nadicodeai/design-system` rename-invariant; overlay copied before rebrand (build.py:565→569); T4/T6 asserts grep POST-rename `dist/nadia`; check-upstream-pristine. |
...
| `make dist-test` red on a dist change merged straight to main (non-blocking CI) | Med | High | T9 runs `make dist-test` locally OR via PR before merge (memory: nadia-dist-test-gate, sync-pr-squash). |
...
| Quilt patch to skin_engine.py / theme.ts drifts on next sync | Med | Med | Patches tiny + anchored; volatile data in regenerated overlay; sync conflicts per memory: nadia-sync-conflict-protocol. |
...
|---|---|---|---|
| 1 | Skin registration mechanism | Built-in via quilt patch 0016 into `_BUILTIN_SKINS` (close at `skin_engine.py:645`) importing generated `overlay/hermes_cli/_nadia_skin.py` (AD1) | Architect |
| 2 | Default-skin switch location | `content_edits` in `packaging-strip.yaml`, `file: cli.py`, flipping `cli.py:458` → `dist/nadia/cli.py` (AD2); documented as THE distribution-default standard (T-DOCS) | Architect (user pre-approved per locked #4) |
...
| 8 | Native (non-Docker) installs | Default to `nadia` too (the dist-tree `content_edits` IS the native artifact) — confirmed by locked #5 (AD7) | Resolved (locked) |
| 9 | Standards/docs deliverable (NEW) | First-class uphill task T-DOCS: `docs/nadia-design-system.md` + AGENTS.md section + `.shepherd/standards.md` extension; SC-6 gates it (AD9) | Architect |
...
3. **Final Nadia ASCII art (logo + hero) — aesthetic sign-off @ CP-A (locked #5, Boundaries).**
   ...
   - *Recommendation:* approve a block-letter "NADIA" wordmark + a compact seam/node hero in the ares/poseidon style, node motif in the DS logo colors green `#007a5e` + red `#cd212a` (both present on current DS master — the green-red direction is on-token), plus cyan/link accents. Re-verify against the pinned DS commit at CP-A.
