...
#
# Wired into the image as /etc/cont-init.d/01-nadia-setup by the
...
NADIA_HOME="${NADIA_HOME:-/opt/data}"
INSTALL_DIR="/opt/nadia"

# Drop to nadia via s6-setuidgid, but skip it when already non-root.
as_nadia() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid nadia "$@"; }
...
# starts non-root. The baked install tree under /opt/nadia is intentionally
# root-owned and non-writable; mutable runtime state must live under
# $NADIA_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
# default) and pass NADIA_UID/NADIA_GID — or the PUID/PGID aliases — which the
...
# `id -u nadia` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# nadia UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u nadia)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-nadia UID).
...
and the baked /opt/nadia install tree is intentionally root-owned and
...
Start the container as root (the default) and pass your host UID/GID instead:
    ...
    docker run -e NADIA_UID=\$(id -u) -e NADIA_GID=\$(id -g) ...

NAS users (Synology / unRAID / UGOS) can use the PUID/PGID aliases:

    docker run -e PUID=\$(id -u) -e PGID=\$(id -g) ...

The image remaps the nadia user to that UID/GID at boot and chowns the data
...
# --- Bootstrap NADIA_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid nadia mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom NADIA_HOME paths whose parents only
# root can create (e.g. `NADIA_HOME=/home/nadia/.nadia` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for NADIA_UID/NADIA_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  NADIA_UID/
# NADIA_GID still win when both are set.  See #15290, salvages #25872.
NADIA_UID="${NADIA_UID:-${PUID:-}}"
NADIA_GID="${NADIA_GID:-${PGID:-}}"

if [ -n "${NADIA_UID:-}" ] && validate_uid_gid "$NADIA_UID" && [ "$NADIA_UID" != "$(id -u nadia)" ]; then
    echo "[stage2] Changing nadia UID to $NADIA_UID"
    usermod -u "$NADIA_UID" nadia
fi
if [ -n "${NADIA_GID:-}" ] && validate_uid_gid "$NADIA_GID" && [ "$NADIA_GID" != "$(id -g nadia)" ]; then
    echo "[stage2] Changing nadia GID to $NADIA_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$NADIA_GID" nadia 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised nadia user (UID 10000)
...
# nadia process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes nadia. Idempotent across container
...
#   - nadia is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G nadia 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] nadia already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" nadia 2>/dev/null; then
        echo "[stage2] Added nadia to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group nadia failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When NADIA_UID is remapped or the top-level $NADIA_HOME isn't owned by
# the runtime nadia UID, restore ownership to nadia — but ONLY for the
# directories nadia actually writes to. The full $NADIA_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of nadia-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_nadia_uid=$(id -u nadia)
...
chown_nadia_tree() {
    ...
    chown -R nadia:nadia "$target" 2>/dev/null || \
        echo "[stage2] Warning: chown $target failed (rootless container?) — continuing"
}

needs_chown=false
if [ "$(stat -c %u "$NADIA_HOME" 2>/dev/null)" != "$actual_nadia_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $NADIA_HOME (targeted) to nadia ($actual_nadia_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $NADIA_HOME: chown the directory itself (not its contents)
    # so nadia can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    if refuse_symlinked_path "chown" "$NADIA_HOME"; then
        :
    else
        chown nadia:nadia "$NADIA_HOME" 2>/dev/null || \
            echo "[stage2] Warning: chown $NADIA_HOME failed (rootless container?) — continuing"
    fi
    # Nadia-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by nadia (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing lazy-packages; do
        if [ -e "$NADIA_HOME/$sub" ]; then
            chown_nadia_tree "$NADIA_HOME/$sub"
        fi
    done
fi
...
# nadia user. Hosted/container instances keep mutable state under
# $NADIA_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# NADIA_DISABLE_LAZY_INSTALLS=1. Keeping /opt/nadia root-owned and
...
# $NADIA_HOME/lazy-packages dir on the data volume (Dockerfile sets
# NADIA_LAZY_INSTALL_TARGET). That dir is appended to the END of sys.path,
...
# even though installs are re-enabled. The dir is seeded + chowned to nadia
...
# Always reset ownership of $NADIA_HOME/profiles to nadia on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> nadia …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as nadia and walks
# the profiles dir. Idempotent; skipped on rootless containers where
# chown would fail.
if [ -d "$NADIA_HOME/profiles" ]; then
    chown_nadia_tree "$NADIA_HOME/profiles"
fi

# Always reset ownership of $NADIA_HOME/cron on every boot for the same
# docker-exec/root-write reason as profiles/. The cron scheduler state
# (jobs.json) must stay readable by the unprivileged nadia runtime even
# after root-context maintenance commands or scheduler writes.
if [ -d "$NADIA_HOME/cron" ]; then
    chown_nadia_tree "$NADIA_HOME/cron"
fi

# Reset ownership of nadia-owned top-level state files on every boot.
# The targeted data-volume chown above only covers nadia-owned
# *subdirectories*; loose state files living directly under $NADIA_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> nadia …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged nadia runtime then hits
...
# sweep so host-owned files in a bind-mounted $NADIA_HOME are never
...
# entries of nadia_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$NADIA_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$NADIA_HOME/$f"; then
            :
        else
            chown nadia:nadia "$NADIA_HOME/$f" 2>/dev/null || true
        fi
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the nadia runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$NADIA_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$NADIA_HOME/config.yaml"; then
        :
    else
        chown nadia:nadia "$NADIA_HOME/config.yaml" 2>/dev/null || true
        chmod 640 "$NADIA_HOME/config.yaml" 2>/dev/null || true
    fi
fi

# --- Seed directory structure as nadia user ---
# Run as nadia via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $NADIA_HOME values
...
as_nadia mkdir -p \
    ...
    "$NADIA_HOME/sessions" \
    "$NADIA_HOME/logs" \
    "$NADIA_HOME/logs/gateways" \
    "$NADIA_HOME/hooks" \
    "$NADIA_HOME/memories" \
    "$NADIA_HOME/skills" \
    "$NADIA_HOME/skins" \
    "$NADIA_HOME/plans" \
    "$NADIA_HOME/workspace" \
    "$NADIA_HOME/home" \
    "$NADIA_HOME/pairing" \
    "$NADIA_HOME/platforms/pairing" \
    "$NADIA_HOME/lazy-packages"

# --- Install-method stamp ---
# The 'docker' stamp is baked into the immutable install tree at
# /opt/nadia/.install_method (see Dockerfile), NOT written here into
# $NADIA_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $NADIA_HOME: it is a shared DATA volume, commonly
# bind-mounted from the host (~/.nadia:/opt/data) and sometimes shared with a
...
# 'nadia update'. To heal homes already poisoned by older images, remove a
# stale 'docker' stamp from $NADIA_HOME if one is present (the host install's
# own installer re-creates its code-scoped stamp; a genuine container relies on
# the baked /opt/nadia stamp, so deleting the data-dir copy is safe).
if [ -f "$NADIA_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$NADIA_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$NADIA_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$NADIA_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        if refuse_symlinked_path "seed" "$NADIA_HOME/$dest"; then
            :
        else
            as_nadia cp "$INSTALL_DIR/$src" "$NADIA_HOME/$dest"
        fi
    fi
}
...
if [ -f "$NADIA_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$NADIA_HOME/.env"; then
        :
    else
        chown nadia:nadia "$NADIA_HOME/.env" 2>/dev/null || true
        chmod 600 "$NADIA_HOME/.env" 2>/dev/null || true
    fi
fi

# --- Migrate persisted config schema ---
# Docker image upgrades replace the code under $INSTALL_DIR but preserve
# $NADIA_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `nadia update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set NADIA_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$NADIA_HOME/config.yaml" ]; then
    s6-setuidgid nadia "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$NADIA_HOME/auth.json" ] && [ -n "${NADIA_AUTH_JSON_BOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "seed" "$NADIA_HOME/auth.json"; then
        :
    else
        printf '%s' "$NADIA_AUTH_JSON_BOOTSTRAP" > "$NADIA_HOME/auth.json"
        chown nadia:nadia "$NADIA_HOME/auth.json" 2>/dev/null || true
        chmod 600 "$NADIA_HOME/auth.json"
    fi
fi
...
# can set NADIA_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# NADIA_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$NADIA_HOME/gateway_state.json" ] && \
        [ "${NADIA_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    if refuse_symlinked_path "seed" "$NADIA_HOME/gateway_state.json"; then
        :
    else
        printf '{"gateway_state":"running"}\n' > "$NADIA_HOME/gateway_state.json"
        chown nadia:nadia "$NADIA_HOME/gateway_state.json" 2>/dev/null || true
        chmod 644 "$NADIA_HOME/gateway_state.json"
    fi
fi
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_nadia "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
        || echo "[stage2] Warning: skills_sync.py failed; continuing"
fi

# --- Discover agent-browser's Chromium binary ---
# The image's Dockerfile runs `npx playwright install chromium`, which
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/nadia/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI Nadia spawns for the
...
# main-wrapper.sh propagates it into the supervised ``nadia`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-nadia, dashboard, etc.).
