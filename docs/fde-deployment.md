...
- Argo installed from the supported `release` installer or from the repo-built
...
- `argo-customer-init` or the Windows `argo-customer-init.ps1` equivalent.
...
```bash
curl -fsSL https://raw.githubusercontent.com/nadicodeai/argo/release/scripts/argo-fde-provision.sh -o /tmp/argo-fde-provision.sh
curl -fsSL https://raw.githubusercontent.com/nadicodeai/argo/release/scripts/argo-customer-init -o /tmp/argo-customer-init
chmod +x /tmp/argo-fde-provision.sh /tmp/argo-customer-init
/tmp/argo-fde-provision.sh
```

Windows uses:

```powershell
irm https://raw.githubusercontent.com/nadicodeai/argo/release/scripts/argo-fde-provision.ps1 -OutFile $env:TEMP\argo-fde-provision.ps1
irm https://raw.githubusercontent.com/nadicodeai/argo/release/scripts/argo-customer-init.ps1 -OutFile $env:TEMP\argo-customer-init.ps1
powershell -ExecutionPolicy Bypass -File $env:TEMP\argo-fde-provision.ps1
```
...
```powershell
& "$env:LOCALAPPDATA\argo\bin\argo-customer-init.ps1" `
  -Profile acme-prod `
...
```text
ghcr.io/nadicodeai/argo:fde-dev
```
...
```text
dist/images/argo-fde-ubuntu-22.04.qcow2
dist/images/argo-fde-ubuntu-22.04.qcow2.sha256
dist/images/argo-fde-ubuntu-22.04.qcow2.info
dist/images/argo-fde-ubuntu-22.04.qcow2.build.log
```
...
installer behavior on Ubuntu 22.04, while Argo's installer provisions the Python
...
export FDE_HONCHO_WORKSPACE=argo-fde-live
...
bash -n scripts/argo-fde-provision.sh scripts/golden-vm-bake.sh scripts/argo-customer-init scripts/fde-vm-image.sh tests/fde_container/run.sh tests/fde_live/run.sh
