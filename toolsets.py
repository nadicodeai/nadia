...
_NADIA_CORE_TOOLS = [
    ...
    # read-only terminal tab (both gated on NADIA_DESKTOP via check_fn —
    ...
    # spawned as a kanban worker (NADIA_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `nadia tools` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (NADIA_KANBAN_TASK env "
    ...
    # Coding posture (base Nadia — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in nadia_cli/tools_config.py).
        "posture": True,
    },
    
    # ==========================================================================
    # Full Nadia toolsets (CLI + messaging platforms)
    #
    ...
    # the `nadia send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "nadia-cli": {
        ...
        "tools": _NADIA_CORE_TOOLS,
        "includes": []
    },

    "nadia-cron": {
        # Mirrors nadia-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `nadia tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as nadia-cli; gated by `nadia tools`",
        "tools": _NADIA_CORE_TOOLS,
        "includes": []
    },
    ...
    "nadia-email": {
        "description": "Email bot toolset - interact with Nadia via email (IMAP/SMTP)",
        "tools": _NADIA_CORE_TOOLS,
        "includes": []
    },
    ...
    "nadia-sms": {
        "description": "SMS bot toolset - interact with Nadia via SMS (Twilio)",
        "tools": _NADIA_CORE_TOOLS,
        "includes": []
    },
    ...
    "nadia-webhook": {
        ...
        "tools": _NADIA_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "nadia-gateway": {
        ...
        "tools": [],
        "includes": ["nadia-telegram", "nadia-discord", "nadia-whatsapp", "nadia-slack", "nadia-signal", "nadia-bluebubbles", "nadia-homeassistant", "nadia-email", "nadia-sms", "nadia-mattermost", "nadia-matrix", "nadia-dingtalk", "nadia-feishu", "nadia-wecom", "nadia-wecom-callback", "nadia-weixin", "nadia-qqbot", "nadia-webhook", "nadia-yuanbao"]
    }
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``nadia-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_NADIA_CORE_TOOLS + [platform extras]``.
    ...
    Bundle nesting is one level deep in practice (only ``nadia-gateway``
...
def resolve_toolset(name: str, visited: Set[str] = None) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (nadia-<name>).
        # Gives them _NADIA_CORE_TOOLS plus any tools the plugin registered
        # into a toolset matching the platform name.
        if name.startswith("nadia-"):
            platform_name = name[len("nadia-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_NADIA_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
