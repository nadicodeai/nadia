#!/usr/bin/env python3
"""Canvas LMS API CLI for Nadia Agent.
...
def _check_config():
    ...
    if missing:
        nadia_env = os.path.join(
            os.environ.get("NADIA_HOME", os.path.expanduser("~/.nadia")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {nadia_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Nadia Agent"
    )
