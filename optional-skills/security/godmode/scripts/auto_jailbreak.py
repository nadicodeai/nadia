...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("NADIA_HOME", os.path.expanduser("~/.nadia")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("NADIA_HOME", Path.home() / ".nadia")) / "skills" / "red-teaming" / "godmode"
...
NADIA_HOME = Path(os.getenv("NADIA_HOME", Path.home() / ".nadia"))
CONFIG_PATH = NADIA_HOME / "config.yaml"
PREFILL_PATH = NADIA_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nadia models — already uncensored, just needs clean prompt
    NADIA_MODEL_FAMILY: {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "nadia" in model_lower or "nous" in model_lower:
        return NADIA_MODEL_FAMILY
...
def _get_current_model() -> tuple:
    """Read current model and provider from Nadia config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.nadia/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Nadia for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Nadia for changes to take effect.")
