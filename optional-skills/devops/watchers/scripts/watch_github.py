...
Usage (via cron with --no-agent):
    ...
    nadia cron create nadia-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$NADIA_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name nadia-issues --repo nadicodeai/nadia --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the Nadia .env file
(``${NADIA_HOME:-~/.nadia}/.env``) to avoid the 60 req/hr
