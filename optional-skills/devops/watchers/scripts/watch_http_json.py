...
Usage (via cron with --no-agent):
    ...
    nadia cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$NADIA_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Nadia-Watcher/1.0"})
