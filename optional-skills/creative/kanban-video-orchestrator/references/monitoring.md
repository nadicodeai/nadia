...
nadia kanban watch --tenant <project-slug>

# Snapshot of the board
nadia kanban list --tenant <project-slug>
nadia kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
nadia kanban stats --tenant <project-slug>

# Visual dashboard (browser)
nadia dashboard

# Inspect a specific task (includes comments + events)
nadia kanban show <task-id>

# Follow a single task's event stream
nadia kanban tail <task-id>
```

Verify available subcommands with `nadia kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `nadia kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `nadia kanban block <id>` then `nadia kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `nadia kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `nadia kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `nadia profile list` (profile exists?), `nadia status` (gateway/dispatcher up?), and verify tenant. |
...
nadia kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
nadia kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(nadia kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
nadia kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
nadia kanban block <task-id>
nadia kanban archive <task-id>

# Diagnose what happened
nadia kanban show <task-id>      # task body, comments, recent events
nadia kanban tail <task-id>      # follow the live event stream
nadia kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `nadia kanban create` for the director
...
while true; do
    clear
    nadia kanban list --tenant <slug>
    echo "---"
    nadia kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `nadia kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `nadia profile list` and start
