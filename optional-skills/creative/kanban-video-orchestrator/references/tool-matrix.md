...
Maps each role archetype to the Nadia skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public nadia-agent
...
## Nadia skills relevant to video production

### Visual / rendering skills (`nadia-agent/skills/creative/`)
...
### Design / pre-production skills (`nadia-agent/skills/creative/`)
...
### Audio / media skills (`nadia-agent/skills/creative/` + `skills/media/`)
...
### Kanban infrastructure (`nadia-agent/skills/devops/`)
...
These are **not** Nadia skills but external CLIs / APIs that profiles invoke.
...
## Built-in Nadia tools for media review

These are native Nadia tools — not invoked via terminal but through their own
...
key is present in `${NADIA_HOME:-~/.nadia}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Nadia profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${NADIA_HOME:-~/.nadia}/.env` → environment variable.
...
When a new Nadia-public video skill ships:
