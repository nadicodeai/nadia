...
the actual `setup.sh` that creates the project workspace, configures Nadia
...
> [NadicodeAI/kanban-video-pipeline](https://github.com/NadicodeAI/kanban-video-pipeline).
...
2. **Create profiles** — `nadia profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.nadia/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `nadia kanban create` assigned to the director
...
```bash
nadia profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.nadia/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.nadia/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Nadia
...
Each profile gets a `SOUL.md` at `~/.nadia/profiles/<name>/SOUL.md` that
...
```bash
nadia kanban create "Direct production of <video title>" \
    --assignee director \
...
the Nadia `.env` (`${NADIA_HOME:-$HOME/.nadia}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _nadia_env="${NADIA_HOME:-$HOME/.nadia}/.env"
    if grep -q "^${var}=" "$_nadia_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_nadia_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_nadia_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY nadia ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY nadia OPENROUTER_API_KEY || exit 1
# ...
```
