#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `nadia kanban list` and
...
def nadia_available() -> bool:
    return shutil.which("nadia") is not None


def kanban_list(tenant: str) -> list[dict]:
    """Returns parsed task rows. Falls back to plain stdout parsing if JSON
    output isn't supported by the installed nadia CLI."""
    try:
        out = subprocess.run(
            ["nadia", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `nadia kanban list`
    out = subprocess.run(
        ["nadia", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, check=False,
    )
    rows = []
...
def kanban_show(task_id: str) -> dict | None:
    out = subprocess.run(
        ["nadia", "kanban", "show", task_id, "--json"],
        capture_output=True, text=True, check=False,
    )
    if out.returncode != 0:
        return None
...
def main():
    ...
    if not nadia_available():
        print("ERROR: 'nadia' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)
