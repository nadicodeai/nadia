...
//!
//! Driven when the installer is launched as `Nadia-Setup.exe --update` (see
//! `AppMode` in lib.rs). The desktop app hands off to us — it exits, then we:
//!
//!   1. wait for the old Nadia desktop process to fully exit (so both the
//!      venv shim and packaged app.asar are free; otherwise `nadia update`
//!      or repair bootstrap can race locked files),
//!   2. run `nadia update --yes --gateway` (Python/repo update; this does NOT
//!      rebuild apps/desktop by design — see cmd_update in nadia_cli/main.py),
//!   3. run `nadia desktop --build-only` (the rebuild step update skips),
...
//!
//! Cross-platform note: `nadia update` already handles macOS/Linux (git/pip).
//! The only OS-specific bits here are the venv shim path (resolve_nadia) and
...
/// `nadia update` exit code meaning "another nadia process is holding the
/// venv shim open / dirty precondition" — see _cmd_update_impl in
/// nadia_cli/main.py (sys.exit(2)). We surface a targeted message for this.
const UPDATE_EXIT_CONCURRENT: i32 = 2;

/// How long to wait for the old desktop process to release files under the
/// install tree before giving up and letting `nadia update`'s own guard decide.
...
pub async fn start_update(app: AppHandle) -> Result<(), String> {
        ...
        .is_err()
    {
        ...
        let mut stages = vec![
            stage_info("update", "Updating Nadia"),
...
async fn run_update(app: AppHandle) -> Result<()> {
    let nadia_home = crate::paths::nadia_home();
    let install_root = nadia_home.join("nadia-agent");
    ...
    // it, that backend re-locks the venv shim, our `force_kill_other_nadia`
    ...
    let nadia = resolve_nadia(&install_root).ok_or_else(|| {
        let msg = format!(
            "Could not find the nadia CLI under {}. Is Nadia installed? \
    ...
    // async on Windows. If it still holds the venv shim, `nadia update`
    ...
    // ---- stage 1: nadia update -----------------------------------------
    // Pass --branch so `nadia update` targets the branch this installer was
    ...
    // without --branch, `nadia update` switches the checkout to `main` (a
    ...
    // --force skips `nadia update`'s Windows running-exe guard (which would
    ...
    // time `nadia update` runs there is no legitimate nadia.exe to protect,
    // and the guard would only produce a false "Nadia is still running" stop.
    ...
    // Retry-once for the update-boundary crash. `nadia update` lazily imports
    ...
    // second `nadia update` runs clean because the now-current module is loaded
    ...
    // for the concurrent-instance guard (exit 2) — that's a "close Nadia" state
    // a retry can't fix.
    ...
    match update.exit_code {
        ...
        Some(code) if code == UPDATE_EXIT_CONCURRENT => {
            let msg = "Nadia is still running. Close all Nadia windows and try \
                       the update again."
                .to_string();
            emit_stage(
                &app,
        ...
        other => {
            let msg = format!(
                "nadia update failed (exit {:?}). See {} for details.",
                other,
                crate::paths::nadia_home()
                    .join("logs")
                    .join("update.log")
                    .display()
            );
    ...
    // ---- stage 2: nadia desktop --build-only ----------------------------
    // `nadia update` deliberately does NOT build apps/desktop (it installs
    ...
    // retry-once `nadia update` already does above, and `nadia update`'s own
    ...
    if rebuild.exit_code != Some(0) {
        let msg = format!(
            "Rebuilding the desktop app failed (exit {:?}). The update was \
             applied but the app could not be rebuilt; run `nadia desktop` \
    ...
    if let Some(target_app) = launch_target {
        if let Err(err) = launch_macos_app_and_exit(&app, &target_app).await {
            emit_log(
                ...
                None,
                LogStream::Stderr,
                &format!("[update] could not auto-launch desktop: {err}. Launch Nadia manually."),
            );
        }
    } else if let Err(err) =
        crate::bootstrap::launch_nadia_desktop(app.clone(), install_root.to_string_lossy().into_owned()).await
    {
...
pub(crate) async fn wait_for_install_locks_free(install_root: &Path, app: &AppHandle, stage: &str) {
    ...
    emit_log(app, Some(stage), LogStream::Stdout, "[handoff] waiting for Nadia to exit…");
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            // Last resort: a backend nadia.exe (or the desktop Nadia Agent.exe
            ...
            // force-kill every Nadia Agent.exe except ourselves, then give the OS a
            ...
            emit_log(
                ...
                LogStream::Stdout,
                &format!(
                    "[handoff] Nadia still holding install files ({}); force-killing stragglers…",
                    format_locked_paths(&locked)
                ),
            );
            force_kill_other_nadia();
...
fn install_lock_probe_paths(install_root: &Path) -> Vec<PathBuf> {
    let mut paths = vec![venv_nadia(install_root)];
...
fn desktop_app_payload_paths(install_root: &Path) -> Vec<PathBuf> {
    ...
    if cfg!(target_os = "windows") {
        ...
        vec![
            release.join("mac").join("Nadia Agent.app").join("Contents").join("Resources").join("app.asar"),
            release.join("mac-arm64").join("Nadia Agent.app").join("Contents").join("Resources").join("app.asar"),
        ]
    } else {
...
/// Force-kill any `nadia.exe` other than this process. Windows-only; a no-op
...
fn force_kill_other_nadia() {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        for image in ["nadia.exe", "Nadia Agent.exe"] {
...
/// Spawn `nadia <args>` from `cwd`, stream stdout/stderr as Log events on the
...
/// Path to the venv nadia shim under an install root, regardless of existence.
fn venv_nadia(install_root: &Path) -> PathBuf {
    if cfg!(target_os = "windows") {
        install_root.join("venv").join("Scripts").join("nadia.exe")
    } else {
        install_root.join("venv").join("bin").join("nadia")
    }
}

/// Resolve the nadia CLI to drive. Prefer the venv shim in the install we
/// just updated; fall back to `nadia` on PATH.
fn resolve_nadia(install_root: &Path) -> Option<PathBuf> {
    let shim = venv_nadia(install_root);
    if shim.exists() {
        return Some(shim);
    }
    // PATH fallback. which-style probe via env, kept dependency-free.
    let exe = if cfg!(target_os = "windows") { "nadia.exe" } else { "nadia" };
...
fn update_child_env(install_root: &Path) -> Vec<(String, OsString)> {
    let nadia_home = crate::paths::nadia_home();
    let mut envs = vec![(
        "NADIA_HOME".to_string(),
        nadia_home.as_os_str().to_os_string(),
    )];
    if let Some(path) = path_with_prepended_entries(&[
        nadia_home.join("node").join("bin"),
...
async fn install_macos_app_update(
    ...
    let rebuilt_app = crate::bootstrap::resolve_nadia_desktop_app(install_root).ok_or_else(|| {
        anyhow!(
            "desktop rebuild succeeded but no Nadia Agent.app was found under {}",
    ...
    let tmp = PathBuf::from(format!("{}.nadia-update-new", target_app.display()));
    let old = PathBuf::from(format!("{}.nadia-update-old", target_app.display()));
    ...
    let ditto = Command::new("/usr/bin/ditto")
        .arg(&rebuilt_app)
        .arg(&tmp)
        .current_dir(crate::paths::nadia_home())
        .status()
        .await
...
mod tests {
    ...
    #[test]
    fn venv_nadia_is_under_install_root() {
        let root = Path::new("/x/nadia-agent");
        let shim = venv_nadia(root);
    ...
    fn lock_probe_paths_include_desktop_app_payload() {
        ...
        assert!(
            probes.iter().any(|p| p == &venv_nadia(root)),
    ...
    fn locked_paths_ignores_missing_payloads() {
        let root = Path::new("/nonexistent/nadia-agent");
        let probes = install_lock_probe_paths(root);
    ...
    fn update_marker_guard_writes_then_removes_on_drop() {
        ...
        let marker = dir.join(".nadia-update-in-progress");
    ...
    fn parses_only_app_targets() {
        assert_eq!(
            target_app_from_args(["--update", "--target-app", "/Applications/Nadia Agent.app"]),
            Some(PathBuf::from("/Applications/Nadia Agent.app"))
        );
    ...
    async fn swap_installs_new_bundle_and_cleans_up() {
        ...
        let target = base.join("Nadia Agent.app");
        let tmp = base.join("Nadia Agent.app.nadia-update-new");
        let old = base.join("Nadia Agent.app.nadia-update-old");
    ...
    async fn swap_failure_never_leaves_target_missing() {
        ...
        let target = base.join("Nadia Agent.app");
        let tmp = base.join("Nadia Agent.app.nadia-update-new"); // intentionally absent
        let old = base.join("Nadia Agent.app.nadia-update-old");
        write_marker(&target, "OLD");
    ...
    async fn swap_rolls_back_when_install_step_fails() {
        ...
        let target = base.join("Nadia Agent.app");
        let tmp = base.join("Nadia Agent.app.nadia-update-new"); // absent
        let old = base.join("Nadia Agent.app.nadia-update-old");
        write_marker(&target, "OLD");
