...
export const $nadiaHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  if (fake) {
    unlisten = () => {}
    $logPath.set('~/.nadia/logs/bootstrap-installer.log')
    $nadiaHome.set('~/.nadia')
  ...
  try {
    const [logPath, nadiaHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_nadia_home'),
      invoke<AppMode>('get_mode')
    ])
    $logPath.set(logPath)
    $nadiaHome.set(nadiaHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Nadia" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  ...
  // Update is driven by the desktop handing off (Nadia-Setup.exe --update);
...
export async function launchNadiaDesktop(): Promise<void> {
  ...
  await invoke('launch_nadia_desktop', { installRoot })
}
...
const FAKE_INSTALL_STAGES: FakeStage[] = [
  ...
  { name: 'repo', title: 'Nadia repository' },
