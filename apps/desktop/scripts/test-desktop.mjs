...
// no longer bundles the Nadia Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Nadia Agent.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Nadia Agent'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Nadia Agent.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'nadia'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default NADIA_HOME for non-sandboxed runs -- matches main.cjs's
// resolveNadiaHome(). On Windows it's %LOCALAPPDATA%\nadia; elsewhere
// it's ~/.nadia. The fresh-install sandbox launchFresh() sets its own
// NADIA_HOME and never touches this.
const DEFAULT_NADIA_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'nadia')
  }
  return path.join(os.homedir(), '.nadia')
})()
const VENV_ROOT = path.join(DEFAULT_NADIA_HOME, 'nadia-agent', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'nadia-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.NADIA_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Nadia-Agent-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Nadia-Agent-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Nadia-Agent-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Nadia-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.NADIA_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.NADIA_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const nadiaHome = path.join(sandbox, 'nadia-home')
  const cwd = path.join(sandbox, 'workspace')

  fs.mkdirSync(userDataDir, { recursive: true })
  fs.mkdirSync(nadiaHome, { recursive: true })
  ...
  env.NADIA_DESKTOP_CWD = cwd
  env.NADIA_DESKTOP_IGNORE_EXISTING = '1'
  env.NADIA_DESKTOP_TEST_MODE = 'fresh-install'
  env.NADIA_DESKTOP_USER_DATA_DIR = userDataDir
  env.NADIA_HOME = nadiaHome
  delete env.NADIA_DESKTOP_NADIA
  delete env.NADIA_DESKTOP_NADIA_ROOT
  ...
  console.log(`  NADIA_HOME: ${nadiaHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(nadiaHome, 'nadia-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The Nadia Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of nadia_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'nadia-agent', 'nadia_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Nadia
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + NADIA_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  NADIA_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
