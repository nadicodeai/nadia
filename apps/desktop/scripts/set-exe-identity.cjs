#!/usr/bin/env node
// set-exe-identity.cjs — stamp the Nadia icon + version metadata onto the
// built Nadia Agent.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Nadia Agent.exe keeps the stock Electron icon and
...
// so EVERY packed build — first install, `nadia desktop`, the installer's
...
// path (which rebuilds via `nadia desktop --build-only`, never install.ps1)
// shipped a stock "Electron" exe. Keeping it in afterPack closes that gap.
//
// Also runnable standalone for ad-hoc re-stamping:
//   node scripts/set-exe-identity.cjs <path-to-Nadia Agent.exe>
//
...
// Stamp the Nadia icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = path.resolve(__dirname, '..')) {
  ...
  // Icon lives at apps/desktop/assets/nadia-icon.ico
  const icon = path.join(desktopRoot, 'assets', 'nadia-icon.ico')
  ...
  console.log('[set-exe-identity] done — Nadia icon + identity stamped')
}
