 ...
 * product name (`Nadia`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Nadia'
 *
 * This is a hard failure with no obvious cause for the user — `nadia desktop`
 ...
 * (the mac-unpacked Nadia Agent.app bundle) and Windows (win-unpacked), so we
