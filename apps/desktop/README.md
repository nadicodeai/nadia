...
The packaged app ships only the Electron shell. On first launch it installs the Nadia Agent runtime into `NADIA_HOME` (`~/.nadia`, or `%LOCALAPPDATA%\nadia` on Windows) — the **same layout a CLI install uses**, so the two are interchangeable. The renderer (React, in `src/`) talks to a `nadia dashboard` backend over the standard gateway APIs and reuses the embedded TUI rather than reimplementing chat. The install, backend-resolution, and self-update logic all live in `electron/main.cjs`.
