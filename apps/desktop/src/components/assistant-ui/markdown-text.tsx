...
async function mediaSrc(path: string): Promise<string> {
  ...
  if (window.nadiaDesktop && ['audio', 'video'].includes(mediaKind(path))) {
  ...
  if (window.nadiaDesktop && isRemoteGateway()) {
    return gatewayMediaDataUrl(path)
  }

  if (!window.nadiaDesktop?.readFileDataUrl) {
    return mediaExternalUrl(path)
  }

  return window.nadiaDesktop.readFileDataUrl(filePathFromMediaPath(path))
}
...
function OpenMediaButton({ kind, path }: { kind: 'audio' | 'video'; path: string }) {
  return (
    <button
      ...
      onClick={() => void window.nadiaDesktop?.openExternal(mediaExternalUrl(path))}
      type="button"
    >
      Open {kind} file
    </button>
...
function MarkdownLink({ children, className, href, ...props }: ComponentProps<'a'>) {
  ...
  if (window.nadiaDesktop && text && normalizeExternalUrl(text) === target) {
