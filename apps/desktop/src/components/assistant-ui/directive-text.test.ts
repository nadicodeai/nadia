...
import { formatRefValue, nadiaDirectiveFormatter } from './directive-text'
...
describe('nadiaDirectiveFormatter.parse', () => {
  it('keeps quoted file paths whole when parsing', () => {
    const segments = nadiaDirectiveFormatter.parse('see @image:`apple-touch-icon (1).png` for the icon')
  ...
  it('still parses unquoted paths', () => {
    const segments = nadiaDirectiveFormatter.parse('@file:src/main.tsx the entry point')
