...
const NADIA_REF_TYPES = ['file', 'folder', 'url', 'image', 'tool', 'line', 'terminal', 'session'] as const
type NadiaRefType = (typeof NADIA_REF_TYPES)[number]
...
const ICON_PATHS: Record<NadiaRefType, string[]> = {
  file: [
...
const iconPathsFor = (type: string) => ICON_PATHS[type as NadiaRefType] ?? ICON_FALLBACK
...
const NADIA_DIRECTIVE_RE = new RegExp(
...
export const nadiaDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(NADIA_DIRECTIVE_RE)).map(match => {
      ...
      return {
        ...
        type: match[1] || 'file',
        label: shortLabel(match[1] as NadiaRefType, id),
        id
      }
    })
  ]
...
function shortLabel(type: NadiaRefType, id: string): string {
  if (type === 'terminal') {
    return id || 'terminal'
  }
...
/**
 * Renders text containing Nadia directives (`@file:...`, `@image:...`) as
...
export function DirectiveContent({ text }: { text: string }) {
  ...
  const segments = useMemo(() => nadiaDirectiveFormatter.parse(cleanedText), [cleanedText])
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.nadiaDesktop && isRemoteGateway()
        ? gatewayMediaDataUrl(id)
        : window.nadiaDesktop?.readFileDataUrl(id)
