...
export function RemoteDisplayBanner() {
  ...
  useEffect(() => {
    void window.nadiaDesktop?.getRemoteDisplayReason?.().then(result => setReason(result))
  }, [])
  ...
  return (
    <div className="pointer-events-none fixed left-1/2 top-[calc(var(--titlebar-height,34px)+0.75rem)] z-[200] w-[min(32rem,calc(100%-2rem))] -translate-x-1/2">
      <Alert className="pointer-events-auto grid-cols-[auto_minmax(0,1fr)_auto] border-(--stroke-nadia) bg-popover/95 pr-2.5 shadow-nadia backdrop-blur-md">
