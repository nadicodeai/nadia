...
describe('LanguageSwitcher', () => {
  ...
  it('persists language changes through display.language config', async () => {
    ...
    const latestConfig: NadiaConfigRecord = { display: { language: 'en', skin: 'slate' } }
    ...
    fireEvent.click(screen.getByRole('option', { name: /Italiano/i }))

    await waitFor(() => expect(saveConfig).toHaveBeenCalledTimes(1))
    expect(saveConfig).toHaveBeenCalledWith({ display: { language: 'it', skin: 'slate' } })
  })
})
