...
export function FeaturedProviderRow({
  ...
  return (
    <button
      ...
      type="button"
    >
      <span aria-hidden className="arc-border arc-reverse arc-nadia" />
      <div className="min-w-0">
        <div className="flex items-center gap-2">
          <img alt="" className="size-5 shrink-0 rounded" src={assetPath('nadia-apple-touch-icon.png')} />
          ...
          <span
            className={cn(
              ...
              copied
                ...
                : 'border-(--stroke-nadia) text-foreground group-hover:border-(--ui-stroke-secondary)'
            )}
            key={i}
          >
            {ch}
...
function CodeBlock({ copied, onCopy, text }: { copied: boolean; onCopy: () => void; text: string }) {
  ...
  return (
    <div className="flex items-center justify-between gap-3 rounded-md border border-(--stroke-nadia) px-3 py-2">
...
function HackeryButton({
  ...
  return (
    <button
      className={cn(
        'group inline-flex items-center gap-2 rounded-md border border-(--stroke-nadia) px-6 py-2.5',
