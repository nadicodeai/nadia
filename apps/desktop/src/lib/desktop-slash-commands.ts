...
/** Why a known Nadia command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/version', description: 'Show Nadia Agent version', surface: exec() },
...
function isKnownNadiaSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * Nadia's built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownNadiaSlashCommand(normalized)
}
