...
import {
  getElevenLabsVoices,
  getNadiaConfigDefaults,
  getNadiaConfigRecord,
  getNadiaConfigSchema,
  saveNadiaConfig
} from '@/nadia'
import { useI18n } from '@/i18n'
import { cn } from '@/lib/utils'
import { notify, notifyError } from '@/store/notifications'
import type { ConfigFieldSchema, NadiaConfigRecord } from '@/types/nadia'
...
export function voiceFieldVisible(key: string, config: NadiaConfigRecord): boolean {
...
export function ConfigSettings({
  ...
  const [_defaults, setDefaults] = useState<NadiaConfigRecord | null>(null)
  ...
  useEffect(() => {
    let cancelled = false
    Promise.all([getNadiaConfigRecord(), getNadiaConfigDefaults(), getNadiaConfigSchema()])
      .then(([c, d, s]) => {
        if (cancelled) {
          return
        }
  ...
  const updateConfig = (next: NadiaConfigRecord) => {
