...
const getNadiaConfigRecord = vi.fn()
const saveNadiaConfig = vi.fn()
...
vi.mock('@/nadia', () => ({
  ...
  getNadiaConfigRecord: () => getNadiaConfigRecord(),
  saveNadiaConfig: (config: unknown) => saveNadiaConfig(config)
}))
...
beforeEach(() => {
  ...
  getNadiaConfigRecord.mockResolvedValue({ agent: { reasoning_effort: 'medium', service_tier: 'normal' } })
  saveNadiaConfig.mockResolvedValue({ ok: true })
})
...
describe('ModelSettings', () => {
  it('loads the current main model and lists the full provider universe', async () => {
    ...
    // "Nadia" shows in both the trigger and the open list; the unconfigured
    // provider + its setup hint are the unique signal of the full universe.
    expect((await screen.findAllByText('Nadia')).length).toBeGreaterThan(0)
  ...
  it('writes the profile default speed (service_tier) when the fast switch is toggled', async () => {
    await renderModelSettings()
    await waitFor(() => expect(getNadiaConfigRecord).toHaveBeenCalled())

    const fastSwitch = await screen.findByRole('switch')
    fireEvent.click(fastSwitch)

    await waitFor(() =>
      expect(saveNadiaConfig).toHaveBeenCalledWith(
