...
import type { NadiaConfigRecord } from '@/types/nadia'

import { CONTROL_TEXT } from './constants'
import { getNested, setNested } from './helpers'
import { ListRow, LoadingState, Pill, SectionHeading } from './primitives'

// Nadia's reasoning levels (VALID_REASONING_EFFORTS); `none` = thinking off.
// Empty config = Nadia default (medium), shown as Medium.
...
// backend now surfaces the full `nadia model` universe (every canonical
...
// Mirrors `_AUX_TASK_SLOTS` in nadia_cli/web_server.py. Friendly labels and
...
export function ModelSettings({ onMainModelChanged }: ModelSettingsProps) {
  ...
  const [config, setConfig] = useState<NadiaConfigRecord | null>(null)
  ...
  const activateApiKeyProvider = useCallback(async () => {
    ...
    try {
      ...
      // `nadia model` curation). Best-effort — fall through to the refreshed
        ...
        {needsSetup && !setupIsApiKey && (
          <p className="mt-2 text-xs text-muted-foreground">
            {selectedProviderRow?.auth_type === 'api_key'
              ...
              : `${selectedProviderRow?.name} signs in through your browser — Nadia runs the flow for you.`}
          </p>
        )}
