...
import type { EnvVarInfo, OAuthProvider } from '@/types/nadia'
...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.nadiaDesktop?.terminal)
...
//      in nadia_cli/provider_catalog.py) — the SAME provider identity
//      `nadia model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // Nadia can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider Nadia can't clear via the API, but ships a
...
export function ProvidersSettings({ onClose, onViewChange, view }: ProvidersSettingsProps) {
  ...
  // Nadia never deletes creds another tool owns behind a silent API call.
