...
import type { ActionStatusResponse, ToolEnvVar, ToolProvider, ToolsetConfig } from '@/types/nadia'
 ...
 * log inline — the GUI equivalent of the install step `nadia tools` runs
...
export function ToolsetConfigPanel({ toolset, onConfiguredChange }: ToolsetConfigPanelProps) {
  ...
  // panel highlighted the first keyless provider (e.g. Nadia Agents Portal) even when
            ...
            {isActive && (
              <div className="grid gap-2 bg-muted/20 p-3">
                ...
                {provider.requires_nous_auth && (
                  <p className="text-[0.72rem] text-muted-foreground">{copy.nadiaIncluded}</p>
                )}
