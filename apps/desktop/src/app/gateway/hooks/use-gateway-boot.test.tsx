...
// bug that drives the REAL useGatewayBoot hook + REAL NadiaGateway through a
...
function Harness() {
  useGatewayBoot({
    ...
    refreshNadiaConfig: async () => undefined,
...
beforeEach(() => {
  ...
  ;(window as { nadiaDesktop?: unknown }).nadiaDesktop = fakeDesktop()
...
afterEach(() => {
  ...
  delete (window as { nadiaDesktop?: unknown }).nadiaDesktop
})
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForNadia) → app sits in the connecting combo, then fails', async () => {
    // The report's actual path: a fresh launch pointed at an unreachable VPS.
    // startNadia()'s remote branch awaits waitForNadia() for 45s before it
    ...
    ;(window as { nadiaDesktop?: unknown }).nadiaDesktop = desktop
    ...
    // After ~45s waitForNadia gives up and getConnection rejects → boot()
    // catch → failDesktopBoot → the BootFailureOverlay recovery surface.
    await act(async () => {
      rejectConn(new Error('Nadia backend did not become ready: timeout'))
      await vi.advanceTimersByTimeAsync(0)
    })
