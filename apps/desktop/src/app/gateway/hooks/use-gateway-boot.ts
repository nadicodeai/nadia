import { isGatewayReauthRequired, resolveGatewayWsUrl } from '@nadia/shared'
...
import type { RpcEvent } from '@/types/nadia'
...
interface GatewayBootOptions {
  ...
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.nadiaDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: NadiaGateway | null) => void
  refreshNadiaConfig: () => Promise<void>
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    let cancelled = false
    const desktop = window.nadiaDesktop

    const publish = (next: NadiaConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting Nadia...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting Nadia…". The probe is a no-op for a healthy or local backend.
        ...
        // as an opaque "Could not connect to Nadia gateway". resolveGatewayWsUrl
        ...
        await callbacksRef.current.refreshNadiaConfig().catch(() => undefined)
    ...
    const gateway = new NadiaGateway()
    ...
    async function boot() {
      try {
        ...
        await callbacksRef.current.refreshNadiaConfig()
