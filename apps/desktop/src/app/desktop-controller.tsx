...
// /skin theme-cycling command retired: one NadicodeAI skin,
// not an operator choice.

import { formatRefValue } from '../components/assistant-ui/directive-text'
import { getSessionMessages, triggerCronJob } from '../nadia'
...
import { useNadiaConfig } from './session/hooks/use-nadia-config'
...
export function DesktopController() {
  ...
  useEffect(() => {
    window.nadiaDesktop?.setPreviewShortcutActive?.(Boolean(chatOpen && (filePreviewTarget || previewTarget)))
  }, [chatOpen, filePreviewTarget, previewTarget])

  useEffect(() => {
    startUpdatePoller()
    const unsubscribe = window.nadiaDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow())
  ...
  useEffect(() => {
    const unsubscribe = window.nadiaDesktop?.onFocusSession?.(sessionId => {
      if (sessionId) {
  ...
  useEffect(() => {
    const unsubscribe = window.nadiaDesktop?.onNotificationAction?.(({ actionId, sessionId }) => {
  ...
  // nadia:// deep links (e.g. a docs "Send to App" button for an automation blueprint).
  ...
  useEffect(() => {
    const unsubscribe = window.nadiaDesktop?.onDeepLink?.(payload => {
    ...
    void window.nadiaDesktop?.signalDeepLinkReady?.()
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.nadiaDesktop?.onClosePreviewRequested?.(closeActiveRightRailTab)
  ...
  const { refreshNadiaConfig, sttEnabled, voiceMaxRecordingSeconds } = useNadiaConfig({
    activeSessionIdRef,
  ...
  useEffect(() => {
    if (gatewayState === 'open' && !activeSessionId && freshDraftReady) {
      void refreshCurrentModel()
      void refreshNadiaConfig()
    }
  }, [activeSessionId, freshDraftReady, gatewayState, refreshCurrentModel, refreshNadiaConfig])
