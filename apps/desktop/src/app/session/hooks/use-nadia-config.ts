...
import { getNadiaConfig, getNadiaConfigDefaults } from '@/nadia'
...
interface NadiaConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
  refreshProjectBranch: (cwd: string) => Promise<void>
}

export function useNadiaConfig({ activeSessionIdRef, refreshProjectBranch }: NadiaConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)

  const refreshNadiaConfig = useCallback(async () => {
    try {
      const [config, defaults] = await Promise.all([getNadiaConfig(), getNadiaConfigDefaults().catch(() => ({}))])
  ...
  return { refreshNadiaConfig, sttEnabled, voiceMaxRecordingSeconds }
}
