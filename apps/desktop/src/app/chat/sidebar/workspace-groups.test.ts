...
import type { NadiaWorktreeInfo } from '@/global'
import type { SessionInfo } from '@/types/nadia'
...
describe('workspaceGroupsFor', () => {
  it('groups by full cwd, not by basename — same-named folders are separate groups', () => {
    const groups = workspaceGroupsFor(
      [makeSession('/a/nadia-agent/apps/desktop'), makeSession('/a/nadia-agent-wt-rtl/apps/desktop')],
      'No workspace'
    )
  ...
  it('disambiguates colliding basenames by walking up the path', () => {
    expect(
      labels([makeSession('/a/nadia-agent/apps/desktop'), makeSession('/a/nadia-agent-wt-rtl/apps/desktop')])
    ).toEqual(['nadia-agent/apps/desktop', 'nadia-agent-wt-rtl/apps/desktop'])
  })

  it('leaves a unique basename as its short label', () => {
    expect(labels([makeSession('/a/nadia-agent/apps/desktop'), makeSession('/b/heval-py')])).toEqual([
      'desktop',
      'heval-py'
    ])
  })
...
const info = (over: Partial<NadiaWorktreeInfo> & Pick<NadiaWorktreeInfo, 'repoRoot' | 'worktreeRoot'>): NadiaWorktreeInfo => ({
  branch: null,
...
describe('workspaceTreeFor', () => {
  it('heuristic nests `<repo>-wt-<branch>` under its sibling repo', () => {
    const tree = workspaceTreeFor(
      [makeSession('/www/nadia-agent'), makeSession('/www/nadia-agent-wt-rtl')],
      'No workspace'
    )

    expect(tree).toHaveLength(1)
    expect(tree[0].label).toBe('nadia-agent')
    expect(tree[0].groups.map(g => g.label).sort()).toEqual(['nadia-agent', 'rtl'])
  })
  ...
  it('git metadata is authoritative — worktrees group by repoRoot regardless of directory naming', () => {
    const resolver: WorktreeResolver = cwd => {
      if (cwd === '/www/nadia-agent') {
        return info({ repoRoot: '/www/nadia-agent', worktreeRoot: '/www/nadia-agent', isMainWorktree: true, branch: 'main' })
      }

      if (cwd === '/elsewhere/ha-rtl') {
        return info({ repoRoot: '/www/nadia-agent', worktreeRoot: '/elsewhere/ha-rtl', branch: 'rtl' })
      }
    ...
    const tree = workspaceTreeFor(
      [makeSession('/www/nadia-agent'), makeSession('/elsewhere/ha-rtl')],
      'No workspace',
      resolver
    )
    ...
    expect(tree[0].groups.map(g => g.label)).toEqual(['nadia-agent', 'rtl'])
  })
