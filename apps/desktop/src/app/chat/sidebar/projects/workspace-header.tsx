...
const branchActionLabel = (branch: NadiaGitBranch, copy: BranchActionCopy) => {
...
// inside it. Naming is explicit — no auto-generated `nadia/work-<ts>` trees.
export function StartWorkButton({ repoPath, onStarted }: { repoPath: string; onStarted: (path: string) => void }) {
  ...
  const [branches, setBranches] = useState<NadiaGitBranch[]>([])
  ...
  const convert = async (branch: NadiaGitBranch) => {
    if (pending || !repoPath || !branch) {
      return
    }
