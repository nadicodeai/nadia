        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.nadiaDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask Nadia to restart the server" looked like it did nothing —
  ...
  useEffect(() => {
    if (
      ...
      !window.nadiaDesktop?.watchPreviewFile ||
      !window.nadiaDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.nadiaDesktop.onPreviewFileChanged(payload => {
    ...
    void window.nadiaDesktop
      ...
      .then(watch => {
        if (!active) {
          void window.nadiaDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.nadiaDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:nadia-preview')
