...
import type { NadiaReadDirEntry, NadiaReadDirResult } from '@/global'

export type ProjectTreeEntry = NadiaReadDirEntry
...
function ignoredBy(rules: GitignoreRule[], entry: NadiaReadDirEntry) {
...
async function filterIgnored(entries: NadiaReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<NadiaReadDirResult> {
  if (!window.nadiaDesktop) {
