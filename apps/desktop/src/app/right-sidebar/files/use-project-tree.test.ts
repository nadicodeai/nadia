...
import type { NadiaReadDirResult } from '@/global'
import { $connection } from '@/store/session'

import { clearProjectDirCache, readProjectDir } from './ipc'
import { resetProjectTreeState, useProjectTree } from './use-project-tree'

const readDir = vi.fn<(path: string) => Promise<NadiaReadDirResult>>()

beforeEach(() => {
  $connection.set(null)
  resetProjectTreeState()
  readDir.mockReset()
  ;(window as unknown as { nadiaDesktop: { readDir: typeof readDir } }).nadiaDesktop = { readDir }
})
...
afterEach(() => {
  ...
  $connection.set(null)
  resetProjectTreeState()
  delete (window as unknown as { nadiaDesktop?: unknown }).nadiaDesktop
})

function ok(entries: { name: string; path: string; isDirectory: boolean }[]): NadiaReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { nadiaDesktop: unknown }).nadiaDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: NadiaReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<NadiaReadDirResult>(resolve => {
  ...
  it('falls back to the sanitized workspace dir when the session cwd is gone', async () => {
    ...
    ;(window as unknown as { nadiaDesktop: unknown }).nadiaDesktop = { readDir, sanitizeWorkspaceCwd }
  ...
  it('returns no-bridge gracefully when window.nadiaDesktop is missing', async () => {
    delete (window as unknown as { nadiaDesktop?: unknown }).nadiaDesktop
