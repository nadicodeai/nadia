...
const SKIN_KEY = 'nadia-desktop-theme-v2'
...
const PROFILE_SKINS_KEY = 'nadia-desktop-profile-themes-v1'
...
const RETIRED_SKINS = new Set(['nadia-light', 'default', 'gold'])
...
export function getBaseColors(skinName: string, mode: 'light' | 'dark'): DesktopThemeColors {
  const seed = resolveTheme(skinName) ?? nadiaTheme
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  try {
    window.localStorage.setItem('nadia-boot-background', c.background)
...
/** Sync the desktop skin with the active Nadia backend theme on connect. */
