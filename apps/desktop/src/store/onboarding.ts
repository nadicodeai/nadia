...
import type { ModelOptionProvider, OAuthProvider, OAuthStartResponse } from '@/types/nadia'
...
const CONFIGURED_CACHE_KEY = 'nadia-desktop-onboarded-v1'
const SKIP_CACHE_KEY = 'nadia-onboarding-skipped-v1'
const POLL_MS = 2000
...
// When switching to Nadia auto-routes unconfigured tools through the Tool
...
async function fetchProviderDefaultModel(
  ...
  // `nadia model` does (for Nadia it honors the user's free/paid tier, so a
...
function providerResolutionFailure(reason: null | string) {
  ...
  return detail
    ? `Connected, but Nadia still cannot resolve a usable provider. ${detail}`
    : 'Connected, but Nadia still cannot resolve a usable provider.'
}
...
export async function refreshOnboarding(ctx: OnboardingContext) {
  ...
  if (shouldPreserveConfiguredOnFallback(runtime, state)) {
    ...
    notify({
      ...
      message:
        'Nadia Desktop could not verify the running backend on startup. Some features may be unavailable until the gateway is reachable.'
    })
...
async function openSignInUrl(url: string) {
  if (window.nadiaDesktop?.openExternal) {
    try {
      await window.nadiaDesktop.openExternal(url)
...
export async function recheckExternalSignin(ctx: OnboardingContext) {
  ...
  await completeWithModelConfirm(ctx, provider.name, [provider.id], reason =>
    setFlow({
      ...
      message:
        reason?.trim() ||
        `Nadia still cannot reach ${provider.name}. Run \`${provider.cli_command}\` in a terminal first.`
    })
  )
}
...
export async function saveOnboardingLocalEndpoint(baseUrl: string, apiKey: string, ctx: OnboardingContext) {
  ...
  try {
    ...
    if (!runtime.ready) {
      ...
      return { ok: false, message: detail || `Saved, but Nadia still cannot reach ${url}.` }
    }
