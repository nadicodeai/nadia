...
import type { NadiaConnection } from '@/global'
import type { ChatMessage } from '@/lib/chat-messages'
import { persistBoolean, persistString, storedBoolean, storedString } from '@/lib/storage'
import type { SessionInfo, UsageStats } from '@/types/nadia'

type Updater<T> = T | ((current: T) => T)

const WORKSPACE_CWD_KEY = 'nadia.desktop.workspace-cwd'
...
const COMPOSER_MODEL_KEY = 'nadia.desktop.composer.model'
const COMPOSER_PROVIDER_KEY = 'nadia.desktop.composer.provider'
const COMPOSER_EFFORT_KEY = 'nadia.desktop.composer.reasoning-effort'
const COMPOSER_FAST_KEY = 'nadia.desktop.composer.fast'
...
const LAST_SESSION_KEY = 'nadia.desktop.lastSessionId'
...
function workspaceCwdKey(connection: NadiaConnection | null = $connection.get()): string {
...
export async function syncConfiguredDefaultProjectDir(): Promise<string> {
  const settings = window.nadiaDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(): Promise<void> {
  const sanitize = window.nadiaDesktop?.sanitizeWorkspaceCwd
...
export const $connection = atom<NadiaConnection | null>(null)
...
export const setConnection = (next: Updater<NadiaConnection | null>) => updateAtom($connection, next)
