 ...
 * app's bounds and stays visible while Nadia is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'nadia.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'nadia.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.nadiaDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.nadiaDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.nadiaDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.nadiaDesktop?.petOverlay?.close()
}
