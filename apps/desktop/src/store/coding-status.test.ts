...
import type { NadiaRepoStatus } from '@/global'

import { $repoStatus, refreshRepoStatus } from './coding-status'
import { $currentCwd } from './session'

const sampleStatus: NadiaRepoStatus = {
...
function stubProbe(impl: (cwd: string) => Promise<NadiaRepoStatus | null>) {
  ;(window as unknown as { nadiaDesktop?: unknown }).nadiaDesktop = { git: { repoStatus: impl } }
}
