...
import type { NadiaReviewFile, NadiaReviewShipInfo } from '@/global'
...
//
// Scope is always "uncommitted" — Nadia's flow is agent edits you review BEFORE
...
const OPEN_KEY = 'nadia.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'nadia.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'nadia.desktop.reviewTreeMode'
const SELECTED_KEY = 'nadia.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<NadiaReviewFile[]>([])
...
export const $reviewShipInfo = atom<NadiaReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['nadiaDesktop']>['git']>['review']>
...
export async function selectReviewFile(file: NadiaReviewFile): Promise<void> {
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.nadiaDesktop?.openExternal?.(existing.url)
  ...
  await runShip(async () => {
    ...
    if (url) {
      void window.nadiaDesktop?.openExternal?.(url)
    }
