...
import type { NadiaGitBranch } from '@/global'
...
import type { ProjectInfo, ProjectsPayload } from '@/types/nadia'
...
const PROJECT_SCOPE_KEY = 'nadia.desktop.projectScope'
...
async function gatewayRequest<T>(method: string, params: Record<string, unknown> = {}): Promise<T> {
  ...
  if (!gateway) {
    throw new Error('Nadia gateway is not connected')
  }
...
// repo, returning where Nadia should start working. Git is the source of
...
export async function listRepoBranches(repoPath: string): Promise<NadiaGitBranch[]> {
  const git = desktopGit()
...
export async function revealPath(path: null | string): Promise<void> {
  if (path) {
    await window.nadiaDesktop?.revealPath?.(path)
  }
}
...
export async function copyPath(path: null | string): Promise<void> {
  if (path) {
    await window.nadiaDesktop?.writeClipboard?.(path)
  }
}
