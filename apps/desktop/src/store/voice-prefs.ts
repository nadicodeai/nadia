...
import { getNadiaConfigRecord, saveNadiaConfig } from '@/nadia'
...
export async function setAutoSpeakReplies(enabled: boolean): Promise<void> {
  ...
  try {
    const record = await getNadiaConfigRecord()
    const voice = record.voice && typeof record.voice === 'object' ? (record.voice as Record<string, unknown>) : {}

    await saveNadiaConfig({ ...record, voice: { ...voice, auto_tts: enabled } })
  } catch (error) {
