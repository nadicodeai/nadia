...
import { getProfiles, setApiRequestProfile } from '@/nadia'
...
import type { ProfileInfo } from '@/types/nadia'
...
// /api/profiles/active `current`). "default" is the root ~/.nadia. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'nadia.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'nadia.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  try {
    const res = await window.nadiaDesktop.api<ActiveProfileResponse>({ path: '/api/profiles/active' })
...
// Persist the choice and relaunch the backend under the new NADIA_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.nadiaDesktop.profile.set(name)
}
...
async function syncConnectionToActiveProfile(profile: string): Promise<void> {
  const getConnection = window.nadiaDesktop?.getConnection
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'nadia.desktop.showAllProfiles'
...
// Switch to the default (root ~/.nadia) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.nadiaDesktop?.touchBackend?.(target).catch(() => undefined)
}
