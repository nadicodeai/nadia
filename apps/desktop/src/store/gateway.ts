...
const isOpen = (gateway: NadiaGateway | null): boolean => gateway?.connectionState === 'open'
...
export const $gateway = atom<NadiaGateway | null>(null)
...
let primaryGateway: NadiaGateway | null = null
let primaryProfile = 'default'

export function setPrimaryGateway(gateway: NadiaGateway | null, profile = 'default'): void {
...
export function activeGateway(): NadiaGateway | null {
...
export async function ensureActiveGatewayOpen(): Promise<NadiaGateway | null> {
  if (activeKey === primaryProfile) {
    return primaryGateway
  }
