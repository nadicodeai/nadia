...
const STORAGE_KEY = 'nadia:native-notifications'
...
// "Backgrounded" = the user isn't on Nadia. `document.hidden` only flips when
...
export function dispatchNativeNotification(input: NativeNotificationInput): void {
  ...
  void window.nadiaDesktop?.notify({
...
export async function sendTestNativeNotification(title: string, body: string): Promise<boolean> {
  const bridge = window.nadiaDesktop
