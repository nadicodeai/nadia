...
export interface I18nConfigClient {
  getConfig: () => Promise<NadiaConfigRecord>
  saveConfig: (config: NadiaConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.nadiaDesktop?.api) {
      return Promise.resolve({})
    }

    return getNadiaConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.nadiaDesktop?.api) {
