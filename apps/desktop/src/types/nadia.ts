...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Nadia doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in nadia_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `nadia model` uses — instead of
...
export type NadiaConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `nadia model` CLI / onboarding OAuth flow. */
  auth_type?: string
  ...
  /** Nadia only: whether the current account is on the free tier. */
  free_tier?: boolean
  /** Nadia only: paid models a free-tier user cannot select (shown disabled). */
...
// folders. Mirrors nadia_cli/projects_db.Project.to_dict().
...
export interface ToolProvider {
   ...
   *  CLI `nadia tools` active-provider detection). */
  is_active: boolean
}
 ...
 *  not Nadia); elsewhere, driver health from `cua-driver doctor`. `null`
...
/** Shape of `GET /api/nadia/update/check` — the backend's own update state.
