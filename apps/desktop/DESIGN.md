...
2. **Borderless + shadow for elevation.** Overlays float on `shadow-nadia` + a
   `--stroke-nadia` hairline, not hard borders.
3. **One primitive per concern.** One `Button`, one set of control variants,
   ...
   fork.
4. **Tokens, not literals.** Reference CSS vars (`--ui-*`, `--shadow-nadia`,
...
```
shadow-nadia           /* downward-weighted, layered contact→ambient falloff */
border-(--stroke-nadia) /* currentColor hairline, theme-adaptive */
```
...
| `--stroke-nadia` | the overlay hairline (pairs with `shadow-nadia`) |
...
- [ ] Tokens (`--ui-*`, `shadow-nadia`, `--stroke-nadia`) — zero raw colors /
      one-off shadows?
- [ ] No `className` overriding a primitive's padding / size / radius / chrome?
- [ ] Overlay uses `shadow-nadia` + `border-(--stroke-nadia)`, no hard border?
