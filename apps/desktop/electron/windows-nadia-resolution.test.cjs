...
// Regression guards for Windows `nadia` resolution in main.cjs.
//
...
//      Git-Bash `nadia` shim shadowed the real nadia.cmd/nadia.exe; the
...
test('findOnPath tries PATHEXT extensions before the bare (empty) name on Windows', () => {
  ...
  assert.doesNotMatch(
    source,
    /\['', \.\.\.\(process\.env\.PATHEXT/,
    'empty-extension-first order regressed: an extensionless shim can shadow nadia.cmd/.exe'
  )
})
...
test('Windows bootstrap recovery chooses --update when any real-install signal is present', () => {
  ...
  // The old too-narrow check (only venv\Scripts\nadia.exe) must not return.
  assert.doesNotMatch(
    source,
    /updaterArgs = fileExists\(venvNadia\) \?/,
    'recovery regressed to gating only on the nadia.exe shim, which forces destructive --repair'
  )
})
