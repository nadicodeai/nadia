 ...
 * Background
 * ----------
 * After `nadia update` + `nadia desktop --build-only`, the freshly-rebuilt
...
// backend/profile/root it talks to). Anything NADIA_DESKTOP_* is preserved
// plus NADIA_HOME. We snapshot the values, not the live env, so the new
...
const PRESERVED_ENV_KEYS = ['NADIA_HOME', 'ELECTRON_DISABLE_SANDBOX']
const PRESERVED_ENV_PREFIXES = ['NADIA_DESKTOP_']
