...
// Backend subcommand routing for the desktop-managed Nadia process.
//
// The desktop app launches its own headless backend via `nadia serve` — it
...
// app hasn't updated yet, or an older `nadia` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional Nadia profile to pin via `--profile`.
 */
 ...
 * `-m nadia_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `nadia_cli/subcommands/dashboard.py` source registers
