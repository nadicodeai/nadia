 ...
 *
 * Drives apps/desktop's first-launch install of Nadia Agent by spawning
...
const DESKTOP_INSTALL_BRANCH = 'main'
...
function bootstrapCacheDir(nadiaHome) {
  return path.join(nadiaHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.nadia/nadia-agent. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(nadiaHome) {
  if (!nadiaHome) return null
  const candidate = path.join(nadiaHome, 'nadia-agent', 'scripts', installScriptName())
  try {
    fs.accessSync(candidate, fs.constants.R_OK)
    return candidate
  } catch {
...
function cachedScriptPath(nadiaHome, commit) {
  return path.join(bootstrapCacheDir(nadiaHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
async function resolveInstallScript({
  ...
  const cached = cachedScriptPath(nadiaHome, installStamp.commit)
  try {
  ...
  try {
    ...
    const installed = installedAgentInstallScript(nadiaHome)
    if (installed) {
      emit({
        type: 'log',
        line:
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, nadiaHome } = {}) {
  return new Promise((resolve, reject) => {
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, nadiaHome } = {}) {
  return new Promise((resolve, reject) => {
...
// so desktop first-launch always installs the runnable customer tree for this release.
function buildPinArgs(_installStamp) {
  return ['-Branch', DESKTOP_INSTALL_BRANCH]
}

function buildPosixPinArgs({ activeRoot, nadiaHome }) {
  return [
    '--dir',
    activeRoot,
    '--nadia-home',
...
async function fetchManifest({ scriptPath, installerKind, emit, nadiaHome, activeRoot, installStamp }) {
  const isPosix = installerKind === 'posix'
  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, nadiaHome })]
...
async function runStage({ scriptPath, installerKind, stage, emit, nadiaHome, activeRoot, abortSignal, installStamp }) {
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, nadiaHome })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(nadiaHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, nadiaHome, emit })
