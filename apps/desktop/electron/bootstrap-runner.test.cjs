...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'nadia-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/nadia-runner-test',
    sourceRepoRoot: null,
    nadiaHome: '/tmp/nadia-runner-test',
    logRoot: '/tmp/nadia-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'nadia-agent', 'scripts')
