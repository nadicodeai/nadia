...
test('uninstallArgsForMode maps each mode to the module-runner argv', () => {
  assert.deepEqual(uninstallArgsForMode('gui'), ['-m', 'nadia_cli.uninstall', '--mode', 'gui'])
  assert.deepEqual(uninstallArgsForMode('lite'), ['-m', 'nadia_cli.uninstall', '--mode', 'lite'])
  assert.deepEqual(uninstallArgsForMode('full'), ['-m', 'nadia_cli.uninstall', '--mode', 'full'])
})
...
test('resolveRemovableAppPath finds the .app bundle on macOS', () => {
  assert.equal(
    resolveRemovableAppPath('/Applications/Nadia Agent.app/Contents/MacOS/Nadia Agent', 'darwin'),
    '/Applications/Nadia Agent.app'
  )
  assert.equal(
    resolveRemovableAppPath('/Users/x/Applications/Nadia Agent.app/Contents/MacOS/Nadia Agent', 'darwin'),
    '/Users/x/Applications/Nadia Agent.app'
  )
})
...
test('resolveRemovableAppPath finds the install dir on Windows', () => {
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\Programs\\Nadia Agent\\Nadia Agent.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\Programs\\Nadia Agent'
  )
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\nadia-desktop\\Nadia Agent.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\nadia-desktop'
  )
})

test('resolveRemovableAppPath returns null for an unrecognized Windows dir', () => {
  assert.equal(resolveRemovableAppPath('C:\\Temp\\foo\\Nadia Agent.exe', 'win32'), null)
})

test('resolveRemovableAppPath uses APPIMAGE on Linux when set', () => {
  assert.equal(
    resolveRemovableAppPath('/tmp/.mount_NadiaXXXX/nadia', 'linux', { APPIMAGE: '/home/x/Apps/Nadia.AppImage' }),
    '/home/x/Apps/Nadia.AppImage'
  )
})
...
test('shouldRemoveAppBundle requires packaged AND a resolved path', () => {
  assert.equal(shouldRemoveAppBundle(true, '/Applications/Nadia Agent.app'), true)
  assert.equal(shouldRemoveAppBundle(false, '/Applications/Nadia Agent.app'), false)
...
test('buildPosixCleanupScript waits for the PID, runs the uninstall module, removes bundle', () => {
  const script = buildPosixCleanupScript({
    desktopPid: 4321,
    pythonExe: '/home/x/.nadia/nadia-agent/venv/bin/python',
    pythonPath: null,
    agentRoot: '/home/x/.nadia/nadia-agent',
    uninstallArgs: ['-m', 'nadia_cli.uninstall', '--mode', 'gui'],
    appPath: '/opt/nadia/linux-unpacked',
    nadiaHome: '/home/x/.nadia'
  })
  ...
  assert.match(script, /'-m' 'nadia_cli\.uninstall' '--mode' 'gui'/)
  assert.match(script, /rm -rf '\/opt\/nadia\/linux-unpacked'/)
  assert.match(script, /export NADIA_HOME='\/home\/x\/\.nadia'/)
})
...
test('buildPosixCleanupScript exports PYTHONPATH when pythonPath is set (lite/full)', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: '/home/x/.nadia/nadia-agent',
    agentRoot: '/home/x/.nadia/nadia-agent',
    uninstallArgs: ['-m', 'nadia_cli.uninstall', '--mode', 'full'],
    appPath: null,
    nadiaHome: '/home/x/.nadia'
  })
  // System python + source on PYTHONPATH so import nadia_cli works while the
  // venv is torn down.
  assert.match(script, /export PYTHONPATH='\/home\/x\/\.nadia\/nadia-agent'/)
  assert.match(script, /'\/usr\/bin\/python3' '-m' 'nadia_cli\.uninstall' '--mode' 'full'/)
})
...
test('buildPosixCleanupScript omits the bundle rm when appPath is null', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: null,
    agentRoot: '/a',
    uninstallArgs: ['-m', 'nadia_cli.uninstall', '--mode', 'lite'],
    appPath: null,
    nadiaHome: '/h'
  })
  assert.doesNotMatch(script, /rm -rf '\//)
  // Still runs the uninstall.
  assert.match(script, /'-m' 'nadia_cli\.uninstall' '--mode' 'lite'/)
})
...
test('buildWindowsCleanupScript waits (bounded) for PID, runs uninstall, rmdir bundle', () => {
  const script = buildWindowsCleanupScript({
    ...
    agentRoot: 'C:\\nadia',
    uninstallArgs: ['-m', 'nadia_cli.uninstall', '--mode', 'full'],
    appPath: 'C:\\Users\\x\\AppData\\Local\\Programs\\Nadia Agent',
    nadiaHome: 'C:\\Users\\x\\AppData\\Local\\nadia'
  })
  assert.match(script, /@echo off/)
  assert.match(script, /set "PID=9988"/)
  // PYTHONPATH set so a system python can import nadia_cli from source.
  assert.match(script, /set "PYTHONPATH=C:\\nadia;%PYTHONPATH%"/)
  assert.match(script, /"C:\\Python313\\python.exe" "-m" "nadia_cli\.uninstall" "--mode" "full"/)
  ...
  assert.match(script, /rmdir \/s \/q "C:\\Users\\x\\AppData\\Local\\Programs\\Nadia Agent" >nul 2>&1/)
