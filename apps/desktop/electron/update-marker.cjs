...
function markerPath(nadiaHome) {
  return path.join(nadiaHome, '.nadia-update-in-progress')
}
 ...
 * clock for tests.
 */
function readLiveUpdateMarker(nadiaHome, { kill, now = Date.now, maxAgeMs = UPDATE_MARKER_MAX_AGE_MS } = {}) {
  const file = markerPath(nadiaHome)
  let raw
  try {
