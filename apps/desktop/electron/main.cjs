...
const USER_DATA_OVERRIDE = process.env.NADIA_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.NADIA_DESKTOP_DEV_SERVER
...
// switches only apply pre-launch. Override with NADIA_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[nadia] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
...
if (IS_WSL && !REMOTE_DISPLAY_REASON && fs.existsSync('/dev/dxg')) {
  ...
  console.log('[nadia] WSL GPU passthrough (/dev/dxg) detected; enabling GPU acceleration')
}

ipcMain.handle('nadia:get-remote-display-reason', () => REMOTE_DISPLAY_REASON)
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[nadia] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
          continue
        }
...
if (INSTALL_STAMP) {
  console.log(
    `[nadia] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[nadia] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// NADIA_HOME — the user-facing root for everything Nadia-related. Mirrors
// scripts/install.ps1's $NadiaHome and scripts/install.sh's $NADIA_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\nadia (matches install.ps1)
//   macOS / Linux: ~/.nadia (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.nadia directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\nadia yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// NADIA_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// NADIA_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.nadia / %LOCALAPPDATA%\nadia.
function resolveNadiaHome() {
  if (process.env.NADIA_HOME) return normalizeNadiaHomeRoot(process.env.NADIA_HOME)
  if (USER_DATA_OVERRIDE) return path.join(path.resolve(USER_DATA_OVERRIDE), 'nadia-home')
  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so a NADIA_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\nadia and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('NADIA_HOME')
    if (fromRegistry) return normalizeNadiaHomeRoot(fromRegistry)
  }
  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'nadia')
    const legacy = path.join(app.getPath('home'), '.nadia')
    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.nadia setup (no LOCALAPPDATA install yet) so users don't lose state.
    if (!directoryExists(localappdata) && directoryExists(legacy)) return legacy
    return localappdata
  }
  return path.join(app.getPath('home'), '.nadia')
}

const NADIA_HOME = resolveNadiaHome()

function nadiaManagedNodePathEntries() {
  // NOTE: keep this ordering in sync with iter_nadia_node_dirs() in
  // nadia_constants.py — this Node main process cannot import the Python
  // module, so the platform-ordering rule is mirrored here.
  const root = path.join(NADIA_HOME, 'node')
  const bin = path.join(root, 'bin')
  const entries = IS_WINDOWS ? [root, bin] : [bin, root]
  return entries.filter(directoryExists)
}

function pathWithNadiaManagedNode(...entries) {
  return [...nadiaManagedNodePathEntries(), ...entries, process.env.PATH].filter(Boolean).join(path.delimiter)
}

// ACTIVE_NADIA_ROOT — the canonical mutable Nadia install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_NADIA_ROOT = path.join(NADIA_HOME, 'nadia-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_NADIA_ROOT, 'venv')
...
// boots, going straight to `resolveNadiaBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_NADIA_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_NADIA_ROOT, '.nadia-bootstrap-complete')
const BOOTSTRAP_MARKER_SCHEMA_VERSION = 1

const DESKTOP_CONNECTION_CONFIG_PATH = path.join(app.getPath('userData'), 'connection.json')
const DESKTOP_UPDATE_CONFIG_PATH = path.join(app.getPath('userData'), 'updates.json')
const DESKTOP_WINDOW_STATE_PATH = path.join(app.getPath('userData'), 'window-state.json')
// active-profile.json records which Nadia profile the desktop launches its
// local backend as. When set, startNadia() passes `nadia --profile <name>
// dashboard …`, which deterministically pins NADIA_HOME (see
// _apply_profile_override in nadia_cli/main.py) and bypasses the sticky
// ~/.nadia/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors nadia_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/
// Branch we track for self-update. Public Nadia installs track main;
// users can override at runtime via nadiaDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under NADIA_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by nadia_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(NADIA_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (nadia_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.NADIA_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.NADIA_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
const APP_ICON_PATHS = [
  path.join(APP_ROOT, 'public', 'nadia-apple-touch-icon.png'),
  path.join(APP_ROOT, 'dist', 'nadia-apple-touch-icon.png'),
  path.join(unpackedPathFor(APP_ROOT), 'dist', 'nadia-apple-touch-icon.png')
]
...
// 'nadia:native-theme' ('dark' | 'light' | 'system'); we pin
...
// an in-place `nadia update` mid-session is reflected without an app restart;
...
const MEDIA_PROTOCOL = 'nadia-media'
// Only audio/video may be streamed. Without this the handler would read any
// non-blocklisted local file (no size cap) for any `fetch(nadia-media://…)`.
...
let mainWindow = null
let nadiaProcess = null
let connectionPromise = null
// Additional per-profile backends, keyed by profile name. The PRIMARY backend
// (the desktop's launch profile) stays managed by nadiaProcess +
// connectionPromise + startNadia(); this pool only holds EXTRA profile
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.NADIA_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.NADIA_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
// onto the error so subsequent startNadia() calls (e.g. the renderer's
// ensureGatewayOpen retrying after the WS won't open) return the same error
// instead of re-running install.ps1 in a hot loop. Cleared explicitly by
// the renderer's "Reload and retry" path or by quitting the app.
let bootstrapFailure = null
// Latched non-bootstrap backend spawn failure — stops getConnection() from
// respawning nadia serve backend children in a tight loop while boot is broken.
...
let bootProgressState = {
  ...
  message: 'Waiting to start Nadia backend',
  phase: 'idle',
  progress: 0,
  running: false,
  timestamp: Date.now()
...
function rememberLog(chunk) {
  ...
  nadiaLog.push(...lines)
  if (nadiaLog.length > 300) {
    nadiaLog.splice(0, nadiaLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-nadia-wsl-windows-fonts.conf')
    let existing = ''
    try {
...
function broadcastBootProgress() {
  ...
  webContents.send('nadia:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the nadia:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('nadia:bootstrap:event', ev)
}
...
// The Tauri updater writes NADIA_HOME/.nadia-update-in-progress for the whole
...
// (`force_kill_other_nadia`, taskkill /IM nadia.exe) kills it, the launch
...
async function waitForUpdateToFinish() {
  let marker = readLiveUpdateMarker(NADIA_HOME)
  ...
  while (marker && Date.now() < deadline) {
    await advanceBootProgress(
      'backend.update-wait',
      'An update is finishing — Nadia will start automatically when it completes…',
      12
    )
    await new Promise(r => setTimeout(r, UPDATE_WAIT_POLL_MS))
    marker = readLiveUpdateMarker(NADIA_HOME)
  }
  if (marker) {
...
function findOnPath(command) {
  ...
  // shell-script shim named `nadia` — must not shadow `nadia.cmd`/`nadia.exe`.
...
function unwrapWindowsVenvNadiaCommand(command, backendArgs) {
  if (!IS_WINDOWS || !command || isCommandScript(command)) return null

  const resolved = path.resolve(String(command))
  if (!/^nadia(?:\.exe)?$/i.test(path.basename(resolved))) return null
  ...
  return {
    label: `existing Nadia Python at ${python}`,
    command: python,
    args: ['-m', 'nadia_cli.main', ...backendArgs],
    bootstrap: false,
    env: buildDesktopBackendEnv({
      nadiaHome: NADIA_HOME,
...
// spawns `nadia serve`; runtimes older than serve only have `dashboard`. We
...
// (covers a bare `nadia` resolved from PATH with no known source root). Result
  ...
  if (backend.root) {
    try {
      const src = fs.readFileSync(
        path.join(backend.root, 'nadia_cli', 'subcommands', 'dashboard.py'),
        'utf8'
      )
  ...
  if (supported === null) {
    try {
      ...
      execFileSync(backend.command, [...prefix, 'serve', '--help'], {
        ...
        env: { ...process.env, NADIA_HOME, ...(backend.env || {}) },
        timeout: 15000,
        stdio: 'ignore',
        windowsHide: true
      })
...
function isNadiaSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'nadia_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.NADIA_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several Nadia deps (notably pywinpty's Rust-built
...
// findGitBash — locate bash.exe on Windows. Nadia's terminal tool requires
...
function findGitBash() {
  ...
  const candidates = []
  if (localAppData) {
    candidates.push(path.join(localAppData, 'nadia', 'git', 'bin', 'bash.exe'))
    candidates.push(path.join(localAppData, 'nadia', 'git', 'usr', 'bin', 'bash.exe'))
  }
...
// normal NADIA_DASHBOARD_READY stdout line and no ready-file side channel is
// needed.
...
function resolveGitBinary() {
  ...
  const candidates = []
  if (localAppData) {
    candidates.push(path.join(localAppData, 'nadia', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'nadia', 'git', 'bin', 'git.exe'))
  }
...
function recentNadiaLog() {
  return nadiaLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's nadia root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_NADIA_ROOT.
// NADIA_DESKTOP_NADIA_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.NADIA_DESKTOP_NADIA_ROOT && path.resolve(process.env.NADIA_DESKTOP_NADIA_ROOT),
    !IS_PACKAGED && isNadiaSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isNadiaSourceRoot(ACTIVE_NADIA_ROOT) ? ACTIVE_NADIA_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_NADIA_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('nadia:updates:progress', merged)
  }
}
...
// NADIA_HOME/nadia-setup.exe on a successful install (see
// apps/bootstrap-installer paths::copy_self_to_nadia_home). That binary owns
// ALL repo mutation — running `nadia update` + rebuilding the desktop — so
...
function resolveUpdaterBinary() {
  const name = IS_WINDOWS ? 'nadia-setup.exe' : 'nadia-setup'
  const candidate = path.join(NADIA_HOME, name)
...
// Path to the venv shim whose lock decides whether `nadia update` can write
// fresh entry points. On Windows this is the file the running backend
// `nadia.exe` holds open; on POSIX it's never mandatory-locked.
function venvNadiaShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'nadia.exe')
    : path.join(updateRoot, 'venv', 'bin', 'nadia')
}
...
// only signals the direct child, so on Windows a backend `nadia.exe` that
// spawned its own grandchildren (a `nadia` REPL, a pty terminal session, the
...
// `nadiaProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `nadia.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a nadia
// REPL, a pty terminal, the gateway) keep `nadia.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const pids = []
  if (nadiaProcess && Number.isInteger(nadiaProcess.pid)) pids.push(nadiaProcess.pid)
  ...
  const shim = venvNadiaShimPath(updateRoot)
...
// `nadia update`). Instead we spawn the staged Nadia-Setup binary with
// --update and quit, so it can run `nadia update` (which refuses while we
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      // macOS/Linux drag-install: no staged Tauri nadia-setup. Unlike Windows
      ...
      // whole update itself: `nadia update` (backend) + `nadia desktop
    ...
    if (!updater) {
      ...
      // `nadia desktop`, never the Tauri installer that self-copies
      // nadia-setup.exe into NADIA_HOME). They DO have a working `nadia`
      ...
      // checkout they're on — bare `nadia update` defaults to main and would
      ...
      const updateRoot = resolveUpdateRoot()
      let command = 'nadia update'
      try {
        const head = await runGit(['rev-parse', '--abbrev-ref', 'HEAD'], { cwd: updateRoot })
        const current = (head.stdout || '').trim()
        if (head.code === 0 && current && current !== 'HEAD') {
          const branch = await resolveHealedBranch(updateRoot, current)
          if (branch !== 'main') command = `nadia update --branch ${branch}`
        }
      } catch {
        // Best-effort: fall back to bare `nadia update` if branch detection fails.
      }
      rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
      emitUpdateProgress({ stage: 'manual', message: command, percent: null })
      return { ok: true, manual: true, command, nadiaRoot: updateRoot }
    }

    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating Nadia — this window will close and the updater will open. Don’t reopen Nadia yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    // nadia.exe (held by the backend child / its grandchildren) and the update
    // bricks. See releaseBackendLockForUpdate for the full failure analysis.
    await releaseBackendLockForUpdate(updateRoot)

    // Detached so the updater outlives this process — it needs us GONE before
    // `nadia update` will run (the venv shim is locked while we live).
    const child = spawn(updater, updaterArgs, {
      cwd: NADIA_HOME,
      env: {
        ...process.env,
        NADIA_HOME,
        PATH: pathWithNadiaManagedNode(venvBin)
      },
      detached: true,
      stdio: 'ignore',
      windowsHide: false
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvNadia = path.join(venvBin, IS_WINDOWS ? 'nadia.exe' : 'nadia')
  ...
  // not just the `nadia.exe` console-script shim. That shim is generated at the
  ...
  const haveRealInstall =
    ...
    fileExists(venvNadia) ||
    fileExists(path.join(updateRoot, '.nadia-bootstrap-complete'))
...
// Resolve the nadia CLI to drive an in-app update: prefer the venv shim in
// the install we're updating, fall back to `nadia` on PATH.
function resolveNadiaCliBinary(updateRoot) {
  const venvNadia = path.join(updateRoot, 'venv', 'bin', 'nadia')
  if (fileExists(venvNadia)) return venvNadia
  return findOnPath('nadia') || null
}
...
// macOS/Linux in-app update: backend (`nadia update`) + OS-aware GUI rebuild
// (`nadia desktop --build-only`), then atomically swap the running .app bundle
// with the freshly built one and relaunch. Degrades to "backend updated,
// restart to load the new GUI" if the swap can't be performed.
async function applyUpdatesPosixInApp() {
  const updateRoot = resolveUpdateRoot()
  const nadia = resolveNadiaCliBinary(updateRoot)
  if (!nadia) {
    emitUpdateProgress({ stage: 'manual', message: 'nadia update', percent: null })
    return { ok: true, manual: true, command: 'nadia update', nadiaRoot: updateRoot }
  }

  // Put the Nadia-managed Node and the venv on PATH so `nadia desktop`'s
  // npm build can find them on a machine with no system Node. Windows portable
  // Node lives directly under %LOCALAPPDATA%\nadia\node, not node\bin.
  const env = {
    NADIA_HOME,
    PATH: pathWithNadiaManagedNode(path.join(updateRoot, 'venv', 'bin'))
  }

  // `nadia update` reaps stale `nadia serve` backends (a code update
  ...
  // _kill_stale_dashboard_processes reads NADIA_DESKTOP_CHILD_PID and excludes
  ...
  if (nadiaProcess && Number.isInteger(nadiaProcess.pid)) {
    desktopChildPids.push(nadiaProcess.pid)
  }
  for (const entry of backendPool.values()) {
  ...
  if (desktopChildPids.length) {
    env.NADIA_DESKTOP_CHILD_PID = desktopChildPids.join(',')
  }
  ...
  emitUpdateProgress({ stage: 'update', message: 'Updating Nadia (git + dependencies)…', percent: 10 })
  const updated = await runStreamedUpdate(nadia, ['update', '--yes', ...branchArgs], {
    cwd: updateRoot,
    env,
    stage: 'update'
  })
  if (updated.code !== 0) {
    emitUpdateProgress({ stage: 'error', message: 'nadia update failed.', error: updated.error || 'update-failed' })
    return { ok: false, error: 'nadia update failed' }
  }
  ...
  const rebuilt = await runRebuildWithRetry(attempt => {
    ...
    return runStreamedUpdate(nadia, ['desktop', '--build-only'], { cwd: updateRoot, env, stage: 'rebuild' })
  })
  if (rebuilt.code !== 0) {
    emitUpdateProgress({
      stage: 'error',
      message: 'Backend updated, but the desktop rebuild failed. Restart Nadia to retry.',
      error: rebuilt.error || 'rebuild-failed'
    })
    return { ok: false, backendUpdated: true, error: 'desktop rebuild failed' }
  }

  // Linux in-app update terminal state (#45205). `nadia desktop --build-only`
  ...
  if (!IS_MAC) {
    ...
    if (outcome === 'relaunch') {
      emitUpdateProgress({ stage: 'restart', message: 'Restarting Nadia…', percent: 100 })
      ...
      const scriptPath = path.join(app.getPath('temp'), `nadia-desktop-update-${Date.now()}.sh`)
      try {
        ...
        return {
          ...
          guiUpdated: false,
          manualRestart: true,
          message: 'Backend updated. Quit and reopen Nadia to load the new version.'
        }
      }
    }
    ...
    if (outcome === 'guiSkew') {
      emitUpdateProgress({
        ...
        message:
          ...
          'Update or reinstall the Nadia desktop app to match.',
        percent: 100
      })
      rememberLog(
    ...
    return {
      ...
      message:
        ...
        '(sandbox helper needs root). Quit and reopen Nadia to finish.'
    }
  }

  const rebuiltApp = [
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac-arm64', 'Nadia Agent.app'),
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac', 'Nadia Agent.app')
  ...
  if (!rebuiltApp || !targetApp) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend updated. Restart Nadia to load the new version.',
      percent: 100
    })
...
if [ "$SRC" != "$DST" ]; then
  if /usr/bin/ditto "$SRC" "$DST.nadia-update-new"; then
    rm -rf "$DST.nadia-update-old" 2>/dev/null || true
    mv "$DST" "$DST.nadia-update-old" 2>/dev/null || rm -rf "$DST"
    mv "$DST.nadia-update-new" "$DST"
    rm -rf "$DST.nadia-update-old" 2>/dev/null || true
  fi
fi
...
`
  ...
  try {
    ...
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend + app updated. Restart Nadia to load the new version.',
      percent: 100
    })
...
// Marker-independent: is the canonical install at ACTIVE_NADIA_ROOT actually
...
function isActiveRuntimeUsable() {
  ...
  return (
    isNadiaSourceRoot(ACTIVE_NADIA_ROOT) &&
    fileExists(venvPython) &&
    canImportNadiaCli(venvPython, {
      env: {
        PYTHONPATH: [ACTIVE_NADIA_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
      }
    })
  )
}
...
function isBootstrapComplete() {
  ...
  // pinned commit -- users update via the in-app update path or `nadia
...
function resolveWebDist() {
  const override = process.env.NADIA_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set NADIA_DESKTOP_WEB_DIST.`
    )
  }
  return fallback
}
...
function resolveRendererIndex() {
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: nadia desktop --force-build`
  )
  return candidates[0]
}
...
function resolveNadiaCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/Nadia Agent.app/Contents/...`
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveNadiaCwd(), sanitized: Boolean(trimmed) }
  }
...
// createActiveBackend — build a backend pointing at ACTIVE_NADIA_ROOT, the
...
function createActiveBackend(backendArgs) {
  ...
  return {
    kind: 'python',
    label: `Nadia at ${ACTIVE_NADIA_ROOT}`,
    command,
    args: ['-m', 'nadia_cli.main', ...backendArgs],
    env: buildDesktopBackendEnv({
      nadiaHome: NADIA_HOME,
      pythonPathEntries: [ACTIVE_NADIA_ROOT, ...getVenvSitePackagesEntries(VENV_ROOT)],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_NADIA_ROOT,
    bootstrap: true,
    shell: false
  }
}

function resolveNadiaBackend(backendArgs) {
  // 1. Explicit override -- NADIA_DESKTOP_NADIA_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.NADIA_DESKTOP_NADIA_ROOT && path.resolve(process.env.NADIA_DESKTOP_NADIA_ROOT)
  if (overrideRoot && isNadiaSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Nadia source at ${overrideRoot}`, backendArgs)
  ...
  //    installed `nadia` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isNadiaSourceRoot.)
  if (!IS_PACKAGED && isNadiaSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Nadia source at ${SOURCE_REPO_ROOT}`, backendArgs)
    if (backend) return backend
  }

  // 3. Bootstrap-complete ACTIVE_NADIA_ROOT -- the canonical install at
  //    %LOCALAPPDATA%\nadia\nadia-agent (Windows) or ~/.nadia/nadia-agent.
  ...
  //    to spawning nadia. Updates flow through the in-app update path
  //    (applyUpdates -> git pull) or `nadia update` from the CLI.
  if (isBootstrapComplete()) {
    return createActiveBackend(backendArgs)
  }

  // 4. Existing `nadia` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    NADIA_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.NADIA_DESKTOP_IGNORE_EXISTING !== '1') {
    let nadiaCommand = null
    const nadiaOverride = process.env.NADIA_DESKTOP_NADIA

    if (nadiaOverride) {
      const resolvedOverride = findOnPath(nadiaOverride)
      if (resolvedOverride) {
        nadiaCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(nadiaOverride, { isWsl: IS_WSL })) {
        nadiaCommand = nadiaOverride
      } else {
        rememberLog(`Ignoring Windows Nadia override under WSL: ${nadiaOverride}`)
      }
    } else {
      nadiaCommand = findOnPath('nadia')
    }

    if (nadiaCommand) {
      if (looksLikeDesktopAppBinary(nadiaCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Nadia CLI: ${nadiaCommand}`)
        nadiaCommand = null
      }
    }

    if (nadiaCommand) {
      const unwrapped = unwrapWindowsVenvNadiaCommand(nadiaCommand, backendArgs)
      if (unwrapped) {
        return unwrapped
      }

      // Smoke-test the candidate before trusting it. A `nadia` shim
      ...
      const shellForProbe = isCommandScript(nadiaCommand)
      if (verifyNadiaCli(nadiaCommand, { shell: shellForProbe })) {
        return (
          unwrapWindowsVenvNadiaCommand(nadiaCommand, backendArgs) || {
            label: `existing Nadia CLI at ${nadiaCommand}`,
            command: nadiaCommand,
            args: backendArgs,
            bootstrap: false,
            env: {},
            kind: 'command',
      ...
      rememberLog(
        `Ignoring existing Nadia CLI at ${nadiaCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed nadia_cli module via system Python.
  ...
  if (python) {
    ...
    // having nadia_cli installed -- common on dev boxes that have
    ...
    if (canImportNadiaCli(python)) {
      return {
        kind: 'python',
        label: `installed nadia_cli module via ${python}`,
        command: python,
        args: ['-m', 'nadia_cli.main', ...backendArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: nadia_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveNadiaBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'Nadia Agent not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveNadiaBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Nadia install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError = new Error(
        'Nadia recovery was handed off to Nadia Setup. The desktop will restart when recovery completes.'
      )
    ...
    const bootstrapResult = await runBootstrap({
      ...
      nadiaHome: NADIA_HOME,
      logRoot: path.join(NADIA_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Nadia install was cancelled.')
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Nadia bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(NADIA_HOME, 'logs', 'desktop.log')} for the full transcript.`
      )
      bootstrapError.isBootstrapFailure = true
      bootstrapError.failedStage = bootstrapResult.failedStage || null
      // Latch the failure so subsequent startNadia() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // nadia:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolveNadiaBackend(backend.args))
  }
  ...
  if (!isNadiaSourceRoot(ACTIVE_NADIA_ROOT)) {
    throw new Error(
      `Nadia install at ${ACTIVE_NADIA_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. Nadia's terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\nadia\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `nadia` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for Nadia on Windows (provides Git Bash, ' +
  ...
  if (!fileExists(venvPython)) {
    ...
    // isNadiaSourceRoot() and the bootstrap writes the marker only after
    // install.ps1 succeeds. If we hit this, the user (or a deleted venv)
    // broke the invariant; tell them to re-run the install.
    throw new Error(
      `Nadia venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = getVenvPython(VENV_ROOT)
  backend.label = `Nadia at ${ACTIVE_NADIA_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Nadia runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function fetchJson(url, token, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      reject(new Error(`Unsupported Nadia backend URL protocol: ${parsed.protocol}`))
      return
    }
            ...
            reject(
              new Error(
                `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                  'The endpoint is likely missing on the Nadia backend.'
              )
            )
            return
          }
    ...
    req.setTimeout(timeoutMs, () => {
      req.destroy(new Error(`Timed out connecting to Nadia backend after ${timeoutMs}ms`))
    })
...
function fetchPublicJson(url, options = {}) {
  ...
  // NO ``X-Nadia-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('nadia:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveNadiaCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('nadia:preview-file-changed', payload)
}
...
async function waitForNadia(baseUrl, token) {
  ...
  throw new Error(`Nadia backend did not become ready: ${lastError?.message || 'timeout'}`)
}
...
function sendBackendExit(payload) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('nadia:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('nadia:close-preview-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting Nadia...".
function sendPowerResume() {
  if (!mainWindow || mainWindow.isDestroyed()) return
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('nadia:power-resume')
}
...
function sendOpenUpdatesRequested() {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('nadia:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen) {
  ...
  webContents.send('nadia:window-state-changed', state)
}
...
const ZOOM_STORAGE_KEY = 'nadia:desktop:zoomLevel'
...
//
// Hosted Nadia gateways gate the dashboard behind an OAuth provider (e.g.
// NadicodeAI) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``nadia_session_at``),
...
const OAUTH_SESSION_PARTITION = 'persist:nadia-remote-oauth'
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: 'Sign in to Nadia gateway',
...
function fetchJsonViaOauthSession(url, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Nadia backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// a valid stored value (pins the root NADIA_HOME explicitly); null means "no
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.NADIA_DESKTOP_REMOTE_URL)

  const remoteToken = decryptDesktopSecret(block.token)
  const authMode = normAuthMode(block.authMode)
  const remoteUrl = envOverride ? String(process.env.NADIA_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  ...
  return {
    ...
    // scope is never overridden by NADIA_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
async function buildRemoteConnection(rawUrl, authMode, token, source) {
  ...
  if (authMode === 'oauth') {
    ...
    // Portal issues a 24h rotating refresh token (nadia #37247), and the
    ...
    if (!(await hasLiveOauthSession(baseUrl))) {
      const err = new Error(
        'Remote Nadia gateway uses OAuth, but you are not signed in. ' +
  ...
  if (!token) {
    throw new Error(
      'Remote Nadia gateway is selected, but no session token is saved. ' +
...
async function resolveRemoteBackend(profile) {
  ...
  if (rawEnvUrl) {
    if (!rawEnvToken) {
      throw new Error(
        'NADIA_DESKTOP_REMOTE_URL is set but NADIA_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Nadia backend.'
      )
    }
...
function globalRemoteActive() {
  if (process.env.NADIA_DESKTOP_REMOTE_URL) {
    return true
  }
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Nadia
  ...
  if (authRequired) {
    ...
    // button can read "Sign in with NadicodeAI" instead of a generic
...
// startNadia() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait() {
  // Capture the reference before resetNadiaConnection() nulls nadiaProcess.
  const dying = nadiaProcess && !nadiaProcess.killed ? nadiaProcess : null
  resetNadiaConnection()
...
// profile to startNadia() (the window backend: boot UI, bootstrap, remote
...
// local-spawn portion of startNadia() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry) {
  ...
  if (remote) {
    await waitForNadia(remote.baseUrl, remote.token)
    return {
      ...remote,
      profile,
      logs: nadiaLog.slice(-80),
      ...getWindowState()
    }
  }

  const token = crypto.randomBytes(32).toString('base64url')
  // --profile wins over the inherited NADIA_HOME env (see _apply_profile_override
  // step 3 in nadia_cli/main.py), so the child re-homes to this profile.
  // --port 0: the OS assigns an ephemeral port; the child announces it on stdout.
  const backendArgs = ['--profile', profile, 'serve', '--host', '127.0.0.1', '--port', '0']
  const backend = await ensureRuntime(resolveNadiaBackend(backendArgs))
  // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
  backend.args = getBackendArgsForRuntime(backend)
  const nadiaCwd = resolveNadiaCwd()
  const webDist = resolveWebDist()
  const readyFile = backend.readyFile ? makeDashboardReadyFile() : null

  rememberLog(`Starting Nadia backend for profile "${profile}" via ${backend.label}`)
  ...
  const child = spawn(
    ...
    hiddenWindowsChildOptions({
      ...
      env: {
        ...
        TERMINAL_CWD: nadiaCwd,
        NADIA_DASHBOARD_SESSION_TOKEN: token,
        // Marks this dashboard backend as desktop-spawned so it runs the cron
        // scheduler tick loop (the gateway isn't running under the app).
        NADIA_DESKTOP: '1',
        NADIA_WEB_DIST: webDist,
        ...(readyFile ? { NADIA_DESKTOP_READY_FILE: readyFile } : {})
      },
      shell: backend.shell,
      stdio: ['ignore', 'pipe', 'pipe']
    })
  ...
  child.once('error', error => {
    rememberLog(`Nadia backend for profile "${profile}" failed to start: ${error.message}`)
    backendPool.delete(profile)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`Nadia backend for profile "${profile}" exited (${signal || code})`)
    backendPool.delete(profile)
    if (!ready) {
      rejectStart?.(
        new Error(`Nadia backend for profile "${profile}" exited before it became ready (${signal || code}).`)
      )
    }
  })
  ...
  await Promise.race([waitForNadia(baseUrl, token), startFailed])
  ready = true
  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive: () => child.exitCode === null && !child.killed,
    label: `Nadia backend for profile "${profile}"`,
    rememberLog
  })
...
async function startNadia() {
  ...
  // process, every subsequent startNadia() call re-throws the same error
  ...
  connectionPromise = (async () => {
    await advanceBootProgress('backend.resolve', 'Resolving Nadia backend', 8)
    // Resolve for the desktop's primary profile so a per-profile remote
    // override on the active profile is honored (falls back to env / global).
    const remote = await resolveRemoteBackend(primaryProfileKey())
    if (remote) {
      await advanceBootProgress('backend.remote', `Connecting to remote Nadia backend at ${remote.baseUrl}`, 24)
      await waitForNadia(remote.baseUrl, remote.token)
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Nadia backend is ready',
        progress: 94,
        running: true,
        error: null
      })
    ...
    // deterministic (it wins over the sticky ~/.nadia/active_profile file) and
    // resolves NADIA_HOME the same way `nadia -p <name>` does on the CLI. An
    ...
    await advanceBootProgress('backend.runtime', 'Resolving Nadia runtime', 28)
    const backend = await ensureRuntime(resolveNadiaBackend(backendArgs))
    // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
    backend.args = getBackendArgsForRuntime(backend)
    const nadiaCwd = resolveNadiaCwd()
    ...
    await advanceBootProgress('backend.spawn', `Starting Nadia backend via ${backend.label}`, 84)
    rememberLog(`Starting Nadia backend via ${backend.label}`)
    ...
    nadiaProcess = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Explicitly pin NADIA_HOME for the child so Python's get_nadia_home()
          // resolves to the SAME location our resolveNadiaHome() picked. Without
          // this pin, Python falls back to ~/.nadia on every platform — fine on
          // mac/linux (where our default matches), but on Windows our default is
          // %LOCALAPPDATA%\nadia, which differs from C:\Users\<u>\.nadia.
          // Mismatch would split config / sessions / .env / logs across two
          // directories. install.ps1 sets NADIA_HOME via setx; the desktop
    ...
    nadiaProcess.stdout.on('data', rememberLog)
    nadiaProcess.stderr.on('data', rememberLog)
    let backendReady = false
    let rejectBackendStart = null
    const backendStartFailed = new Promise((_resolve, reject) => {
      rejectBackendStart = reject
    })
    nadiaProcess.once('error', error => {
      rememberLog(`Nadia backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Nadia backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    nadiaProcess.once('exit', (code, signal) => {
      rememberLog(`Nadia backend exited (${signal || code})`)
      nadiaProcess = null
      connectionPromise = null
      sendBackendExit({ code, signal })
      if (!backendReady) {
        const message = `Nadia backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Nadia backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentNadiaLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for Nadia backend to launch', 86)
    // Discover the ephemeral port the child bound to
    const port = await Promise.race([
      waitForDashboardPortAnnouncement(nadiaProcess, { readyFile }),
      backendStartFailed
    ])
    if (readyFile) {
      fs.unlink(readyFile, () => {})
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Nadia backend to become ready', 90)
    await Promise.race([waitForNadia(baseUrl, token), backendStartFailed])
    backendReady = true
    backendStartFailure = null
    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      // The exit/error handlers null nadiaProcess when the child dies.
      childAlive: () => nadiaProcess !== null && nadiaProcess.exitCode === null && !nadiaProcess.killed,
      rememberLog
    })
    updateBootProgress({
      phase: 'backend.ready',
      message: 'Nadia backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
function createWindow() {
  ...
  const showMainWindow = reason => {
    ...
    rememberLog(`[window] showing main window after ${reason}`)
    mainWindow.show()
  }
  ...
  mainWindow.webContents.once('did-finish-load', () => {
    // Some packaged Electron/macOS launches reach a complete renderer paint
    // without emitting ready-to-show, leaving the app alive but windowless.
    // At this point the themed renderer has loaded, so showing is preferable
    // to a hidden desktop shell that only DevTools can see.
    showMainWindow('did-finish-load')
    restorePersistedZoomLevel(mainWindow)
    broadcastBootProgress()
    sendWindowStateChanged()
    startNadia().catch(error => rememberLog(error.stack || error.message))
  })
}

ipcMain.handle('nadia:connection', async (_event, profile) => ensureBackend(profile))
...
// "Starting Nadia…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('nadia:connection:revalidate', async () => {
  ...
  try {
    ...
    // tick rebuilds a fresh, reachable descriptor. resetNadiaConnection only
    // nulls connectionPromise for a remote (no child to SIGTERM).
    rememberLog('Cached remote Nadia backend failed liveness probe; dropping stale connection.')
    resetNadiaConnection()
    return { ok: true, rebuilt: true }
  }
})
ipcMain.handle('nadia:backend:touch', async (_event, profile) => {
  touchPoolBackend(profile)
  return { ok: true }
})
ipcMain.handle('nadia:gateway:ws-url', async (_event, profile) => freshGatewayWsUrl(profile))
ipcMain.handle('nadia:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('nadia:window:openNewSession', async () => {
...
ipcMain.handle('nadia:bootstrap:repair', async () => {
  // Forceful repair: drop the bootstrap-complete marker so the next
  // startNadia() re-runs the full installer (refreshing a broken/partial
...
ipcMain.handle('nadia:bootstrap:cancel', async () => {
...
ipcMain.handle('nadia:boot-progress:get', async () => bootProgressState)
ipcMain.handle('nadia:bootstrap:get', async () => getBootstrapState())
...
ipcMain.handle('nadia:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
ipcMain.handle('nadia:profile:set', async (_event, name) => {
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new NADIA_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('nadia:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('nadia:requestMicrophoneAccess', async () => {
...
ipcMain.handle('nadia:api', async (_event, request) => {
...
ipcMain.handle('nadia:notify', (_event, payload) => {
  ...
  notification.on('click', () => {
    ...
    if (payload?.sessionId) {
      mainWindow.webContents.send('nadia:focus-session', payload.sessionId)
    }
  })
  notification.on('action', (_actionEvent, index) => {
    ...
    if (action?.id) {
      mainWindow.webContents.send('nadia:notification-action', { sessionId: payload?.sessionId, actionId: action.id })
    }
  })
  notification.show()
  return true
...
ipcMain.handle('nadia:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('nadia:readFileText', async (_event, filePath) => {
...
ipcMain.handle('nadia:selectPaths', async (_event, options = {}) => {
...
ipcMain.handle('nadia:writeClipboard', (_event, text) => {
  clipboard.writeText(String(text || ''))
  return true
})

ipcMain.handle('nadia:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))

ipcMain.handle('nadia:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('nadia:saveClipboardImage', async () => {
...
ipcMain.handle('nadia:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('nadia:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('nadia:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))

ipcMain.on('nadia:titlebar-theme', (_event, payload) => {
...
ipcMain.on('nadia:native-theme', (_event, mode) => {
...
ipcMain.on('nadia:translucency', (_event, payload) => {
...
ipcMain.handle('nadia:openExternal', (_event, url) => {
  if (!openExternalUrl(url)) {
    throw new Error('Invalid external URL')
  }
})

ipcMain.handle('nadia:openPreviewInBrowser', async (_event, url) => {
...
// it via writeDefaultProjectDir so resolveNadiaCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('nadia:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveNadiaCwd()
}))

ipcMain.handle('nadia:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('nadia:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('nadia:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('nadia:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('nadia:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('nadia:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: nadiaLog.slice(-200) }))
...
function terminalShellCommand() {
  // NADIA_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
  ...
  const override = (process.env.NADIA_DESKTOP_SHELL || (IS_WINDOWS ? '' : process.env.SHELL) || '').trim()
...
function terminalShellEnv() {
  ...
  // /TERM=dumb; some terminals set COLORFGBG which would flip Nadia's TUI into
  ...
  env.TERM_PROGRAM = 'Nadia'
  env.TERM_PROGRAM_VERSION = app.getVersion()

  // Let a nadia/--tui launched in this pane know it's embedded in the desktop
  // GUI (build_environment_hints surfaces this). Distinct from NADIA_DESKTOP,
  // which marks the agent *backend* and gates cron/gateway behavior.
  env.NADIA_DESKTOP_TERMINAL = '1'

  return env
}

function terminalChannel(id, suffix) {
  return `nadia:terminal:${id}:${suffix}`
}
...
ipcMain.handle('nadia:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

ipcMain.handle('nadia:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

// Reveal a path in the OS file manager (Finder / Explorer / Files).
ipcMain.handle('nadia:fs:reveal', async (_event, targetPath) => {
...
ipcMain.handle('nadia:fs:rename', async (_event, targetPath, newName) => {
...
ipcMain.handle('nadia:fs:writeText', async (_event, filePath, content) => {
...
ipcMain.handle('nadia:fs:trash', async (_event, targetPath) => {
  const target = String(targetPath || '').trim()
...
ipcMain.handle('nadia:git:worktreeList', async (_event, repoPath) => listWorktrees(repoPath, resolveGitBinary()))

ipcMain.handle('nadia:git:worktreeAdd', async (_event, repoPath, options) =>
  addWorktree(repoPath, options || {}, resolveGitBinary())
)

ipcMain.handle('nadia:git:worktreeRemove', async (_event, repoPath, worktreePath, options) =>
  removeWorktree(repoPath, worktreePath, options || {}, resolveGitBinary())
)

ipcMain.handle('nadia:git:branchSwitch', async (_event, repoPath, branch) =>
  switchBranch(repoPath, branch, resolveGitBinary())
)

ipcMain.handle('nadia:git:branchList', async (_event, repoPath) => listBranches(repoPath, resolveGitBinary()))
...
ipcMain.handle('nadia:git:repoStatus', async (_event, repoPath) => repoStatus(repoPath, resolveGitBinary()))
...
ipcMain.handle('nadia:git:review:list', async (_event, repoPath, scope, baseRef) =>
  reviewList(repoPath, scope, baseRef, resolveGitBinary())
)
ipcMain.handle('nadia:git:review:diff', async (_event, repoPath, filePath, scope, baseRef, staged) =>
...
ipcMain.handle('nadia:git:fileDiff', async (_event, repoPath, filePath) =>
  fileDiffVsHead(repoPath, filePath, resolveGitBinary())
)
ipcMain.handle('nadia:git:review:stage', async (_event, repoPath, filePath) =>
  reviewStage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('nadia:git:review:unstage', async (_event, repoPath, filePath) =>
  reviewUnstage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('nadia:git:review:revert', async (_event, repoPath, filePath) =>
  reviewRevert(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('nadia:git:review:revParse', async (_event, repoPath, ref) =>
  reviewRevParse(repoPath, ref, resolveGitBinary())
)
ipcMain.handle('nadia:git:review:commit', async (_event, repoPath, message, push) =>
  reviewCommit(repoPath, message, Boolean(push), resolveGitBinary())
)
ipcMain.handle('nadia:git:review:commitContext', async (_event, repoPath) =>
  reviewCommitContext(repoPath, resolveGitBinary())
)
ipcMain.handle('nadia:git:review:push', async (_event, repoPath) => reviewPush(repoPath, resolveGitBinary()))
ipcMain.handle('nadia:git:review:shipInfo', async (_event, repoPath) => reviewShipInfo(repoPath, resolveGhBinary()))
ipcMain.handle('nadia:git:review:createPr', async (_event, repoPath) =>
...
ipcMain.handle('nadia:git:scanRepos', async (_event, roots, options) => {
  try {
...
ipcMain.handle('nadia:terminal:start', async (event, payload = {}) => {
  if (!nodePty) {
    throw new Error('PTY support is unavailable. Reinstall desktop dependencies and restart Nadia.')
  }
...
ipcMain.handle('nadia:terminal:write', (_event, id, data) => {
...
ipcMain.handle('nadia:terminal:resize', (_event, id, size = {}) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('nadia:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))

ipcMain.handle('nadia:updates:check', async () =>
...
ipcMain.handle('nadia:updates:apply', async (_event, payload) =>
...
ipcMain.handle('nadia:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('nadia:updates:branch:set', async (_event, name) => {
  const branch = typeof name === 'string' && name.trim() ? name.trim() : DEFAULT_UPDATE_BRANCH
  writeDesktopUpdateConfig({ branch })
  return { branch }
})

// Resolve the canonical Nadia version (the one `release.py` bumps in
// nadia_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Nadia version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveNadiaVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'nadia_cli', '__init__.py')
...
// Re-resolve the live Nadia version and push it into the native About panel
// just before showing it, so an in-place `nadia update` is reflected without
...
// the actual removal via `nadia uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (nadia_cli/uninstall.py + nadia_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_NADIA_ROOT
  ...
  const fallback = () => ({
    nadia_home: NADIA_HOME,
    agent_installed: isNadiaSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'nadia_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
          env: { ...process.env, NADIA_HOME, NO_COLOR: '1' },
          stdio: ['ignore', 'pipe', 'ignore']
        })
      )
      child.stdout.on('data', chunk => {
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no Nadia agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import nadia_cli` resolves from
  ...
  try {
    await releaseBackendLock(ACTIVE_NADIA_ROOT, 'uninstall')
  } catch (error) {
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `nadia-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `nadia-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('nadia:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('nadia:uninstall:run', async (_event, payload) => {
...
// ---------------------------------------------------------------------------
const NADIA_PROTOCOL = 'nadia'
let _pendingDeepLink = null
let _rendererReadyForDeepLink = false

function _extractDeepLink(argv) {
  if (!Array.isArray(argv)) return null
  return argv.find(a => typeof a === 'string' && a.startsWith(`${NADIA_PROTOCOL}://`)) || null
}
...
function handleDeepLink(url) {
  ...
  // nadia://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('nadia:deep-link', payload)
...
ipcMain.handle('nadia:deep-link-ready', () => {
  _rendererReadyForDeepLink = true
  if (_pendingDeepLink) {
    const queued = _pendingDeepLink
    _pendingDeepLink = null
    handleDeepLink(
      `${NADIA_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(NADIA_PROTOCOL, process.execPath, [path.resolve(process.argv[1])])
    } else {
      app.setAsDefaultProtocolClient(NADIA_PROTOCOL)
    }
  } catch (err) {
...
// second-instance argv. Without the lock a second `nadia://` launch spawns a
...
app.whenReady().then(() => {
  ...
  // Win/Linux cold start: the launching nadia:// URL is in our own argv.
