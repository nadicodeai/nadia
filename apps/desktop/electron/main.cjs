...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    emitUpdateProgress({ stage: 'restart', message: 'Handing off to the Nadia updater…', percent: 100 })
    ...
    const child = spawn(updater, updaterArgs, {
      ...
      env: {
        ...process.env,
        NADIA_HOME,
        PATH: [path.join(NADIA_HOME, 'node', 'bin'), venvBin, process.env.PATH].filter(Boolean).join(path.delimiter)
      },
      detached: true,
      stdio: 'ignore',
      windowsHide: false
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const updaterArgs = fileExists(venvNadia) ? ['--update', '--branch', branch] : ['--repair', '--branch', branch]
...
async function applyUpdatesPosixInApp() {
  ...
  const extraPath = [path.join(NADIA_HOME, 'node', 'bin'), path.join(updateRoot, 'venv', 'bin')]
    .filter(Boolean)
    .join(path.delimiter)
  const env = {
    NADIA_HOME,
  ...
  // `nadia update` reaps stale `nadia dashboard` backends (a code update
  ...
  const rebuilt = await runStreamedUpdate(nadia, ['desktop', '--build-only'], {
    cwd: updateRoot,
    env,
    stage: 'rebuild'
  })
...
function isBootstrapComplete() {
  ...
  return isNadiaSourceRoot(ACTIVE_NADIA_ROOT) && fileExists(getVenvPython(VENV_ROOT))
}
...
function createPythonBackend(root, label, dashboardArgs, options = {}) {
  ...
  return {
    ...
    label,
    command: python,
    args: ['-m', 'nadia_cli.main', ...dashboardArgs],
...
function createActiveBackend(dashboardArgs) {
  ...
  return {
    ...
    env: buildDesktopBackendEnv({
      nadiaHome: NADIA_HOME,
      pythonPathEntries: [ACTIVE_NADIA_ROOT],
      venvRoot: VENV_ROOT
    }),
...
function resolveNadiaBackend(dashboardArgs) {
  ...
  if (overrideRoot && isNadiaSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Nadia source at ${overrideRoot}`, dashboardArgs)
  ...
  if (!IS_PACKAGED && isNadiaSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Nadia source at ${SOURCE_REPO_ROOT}`, dashboardArgs)
    if (backend) return backend
  }
...
async function ensureRuntime(backend) {
  ...
  if (backend.kind === 'bootstrap-needed') {
    ...
    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError = new Error('Nadia recovery was handed off to Nadia Setup. The desktop will restart when recovery completes.')
...
async function spawnPoolBackend(profile, entry) {
  ...
  const backend = await ensureRuntime(resolveNadiaBackend(dashboardArgs))
...
async function startNadia() {
  ...
  connectionPromise = (async () => {
    ...
    // Discover the ephemeral port the child bound to
    const port = await Promise.race([waitForDashboardPort(nadiaProcess), backendStartFailed])
...
ipcMain.handle('nadia:fs:worktrees', async (_event, cwds) => worktreesForIpc(cwds))
