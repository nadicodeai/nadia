...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      nadia-agent = self'.packages.default;
      nadiaVenv = nadia-agent.nadiaVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "nadia-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${nadiaVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "nadia-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "nadia-build-package" { } ''
          echo "PASS: package built at ${nadia-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "nadia-build-devshell" { } ''
        ...
        package-contents = pkgs.runCommand "nadia-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${nadia-agent}/bin/nadia || (echo "FAIL: nadia binary missing"; exit 1)
          test -x ${nadia-agent}/bin/nadia-agent || (echo "FAIL: nadia-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${nadia-agent}/bin/nadia version 2>&1 | grep -qi "nadia" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "nadia-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in nadia nadia-agent nadia-acp; do
            test -x ${nadia-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "nadia-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking nadia --help ==="
          ${nadia-agent}/bin/nadia --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${nadia-agent}/bin/nadia --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "nadia-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${nadia-agent}/share/nadia-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${nadia-agent}/share/nadia-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "NADIA_BUNDLED_SKILLS" ${nadia-agent}/bin/nadia || \
            (echo "FAIL: NADIA_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: NADIA_BUNDLED_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "nadia-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${nadia-agent}/share/nadia-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${nadia-agent}/share/nadia-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "NADIA_BUNDLED_PLUGINS" ${nadia-agent}/bin/nadia || \
            (echo "FAIL: NADIA_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: NADIA_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "nadia-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${nadia-agent}/share/nadia-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          LOC_COUNT=$(find ${nadia-agent}/share/nadia-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${nadia-agent}/share/nadia-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "NADIA_BUNDLED_LOCALES" ${nadia-agent}/bin/nadia || \
            (echo "FAIL: NADIA_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: NADIA_BUNDLED_LOCALES set in wrapper"

          echo "=== Rendering via the wrapper override (NADIA_BUNDLED_LOCALES) ==="
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && NADIA_BUNDLED_LOCALES=${nadia-agent}/share/nadia-agent/locales \
            ${nadiaVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with NADIA_BUNDLED_LOCALES set"; exit 1)
          ...
          BARE_DIR=$(cd "$HOME" && ${nadiaVenv}/bin/python3 -c "from agent import i18n; print(i18n._locales_dir())")
          BARE=$(cd "$HOME" && ${nadiaVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "resolved dir (no env var): $BARE_DIR"
          echo "rendered: $BARE"
          test "$BARE" != "gateway.reset.header_default" || \
            (echo "FAIL: sealed venv could not resolve locales without NADIA_BUNDLED_LOCALES — data-files materialization regressed"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "nadia-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${nadia-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${nadia-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "NADIA_TUI_DIR" ${nadia-agent}/bin/nadia || \
            (echo "FAIL: NADIA_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: NADIA_TUI_DIR set in wrapper"
        ...
        # Verify NADIA_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        nadia-node = pkgs.runCommand "nadia-node-version" { } ''
          set -e
          echo "=== Checking NADIA_NODE in wrapper ==="
          grep -q "NADIA_NODE" ${nadia-agent}/bin/nadia || \
            (echo "FAIL: NADIA_NODE not set in wrapper"; exit 1)
          echo "PASS: NADIA_NODE present in wrapper"

          NADIA_NODE=$(sed -n "s/^export NADIA_NODE='\(.*\)'/\1/p" ${nadia-agent}/bin/nadia)
          test -x "$NADIA_NODE" || (echo "FAIL: NADIA_NODE=$NADIA_NODE not executable"; exit 1)
          echo "PASS: NADIA_NODE executable at $NADIA_NODE"

          NODE_MAJOR=$("$NADIA_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All NADIA_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify NADIA_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "nadia-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(NADIA_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking NADIA_MANAGED guards ==="
          check_blocked "config set" ${nadia-agent}/bin/nadia config set model foo
          check_blocked "config edit" ${nadia-agent}/bin/nadia config edit
        ...
        extra-python-packages = let
          ...
          nadiaWithExtra = nadia-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "nadia-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${nadiaWithExtra}/bin/nadia || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${nadiaWithExtra}/bin/nadia || \
          ...
          if grep -q "PYTHONPATH" ${nadia-agent}/bin/nadia; then
        ...
        extra-dependency-groups = let
          nadiaWithGroups = nadia-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "nadia-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${nadiaWithGroups}"
          echo "venv: ${nadiaWithGroups.nadiaVenv}"
        ...
        messaging-variant = pkgs.runCommand "nadia-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.nadiaVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "nadia-config-roundtrip" {
          ...
          merge_and_load() {
            local nadia_home="$1"
            export NADIA_HOME="$nadia_home"
            ${configMergeScript} ${nixSettings} "$nadia_home/config.yaml"
            ${nadiaVenv}/bin/python3 -c '
import json, sys
