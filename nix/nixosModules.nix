# nix/nixosModules.nix — NixOS module for nadia-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: nadia runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $NADIA_HOME/.env
# and read by nadia at startup — no container recreation needed for env changes.
#
# Tool resolution: the nadia wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.nadia-agent;
    effectivePackage =
    ...
    nadia-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-nadia-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "nadia-config-attrs";
      description = "Nadia YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "nadia-config.yaml" configJson;
    configFile = if cfg.configFile != null then cfg.configFile else generatedConfigFile;

    configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

    # config.yaml mode: group-writable (0660) when interactive users share this
    # NADIA_HOME via addToSystemPackages, so they can save settings through the
    ...
    documentDerivation = pkgs.runCommand "nadia-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'NADIA_DOC_EOF'\n${value}\nNADIA_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "nadia-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/nadia";
    ...
    # nadia user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "nadia-container-entrypoint" ''
      set -eu

      NADIA_UID="''${NADIA_UID:?NADIA_UID must be set}"
      NADIA_GID="''${NADIA_GID:?NADIA_GID must be set}"

      # ── Group: ensure a group with GID=$NADIA_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$NADIA_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="nadia"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$NADIA_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$NADIA_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$NADIA_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$NADIA_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="nadia"
        TARGET_HOME="/home/nadia"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$NADIA_UID" -g "$NADIA_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$NADIA_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$NADIA_UID:$NADIA_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure NADIA_HOME is owned by the target user.
      ...
      if [ -n "''${NADIA_HOME:-}" ] && [ -d "$NADIA_HOME" ]; then
        find "$NADIA_HOME" \! -user "$NADIA_UID" -exec chown "$NADIA_UID:$NADIA_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/nadia-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/nadia-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/nadia ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/nadia
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$NADIA_UID" --regid="$NADIA_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $NADIA_HOME/.env.
    ...
    # Default: /var/lib/nadia/workspace → /data/workspace.
  ...
  in {
    options.services.nadia-agent = with lib; {
      enable = mkEnableOption "Nadia Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = nadia-agent;
        description = "The nadia-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/nadia";
        description = "State directory. Contains .nadia/ subdir (NADIA_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Nadia config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $NADIA_HOME/.env at activation time.
          Nadia reads this file on every startup via load_nadia_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $NADIA_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $NADIA_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `nadia gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the nadia user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the nadia plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Nadia discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          nadia_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as nadia (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in nadia-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the nadia CLI to environment.systemPackages and export
          NADIA_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.nadia symlink to the service
            stateDir. These users are automatically added to the nadia group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.nadia-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the nadia CLI to system PATH and export NADIA_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.nadia/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.NADIA_HOME = "${cfg.stateDir}/.nadia";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.nadia-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the nadia user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.nadia-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed nadia
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure nadia is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.nadia        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.nadia/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.nadia/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.nadia/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.nadia/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.nadia/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."nadia-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.nadia
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.nadia ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.nadia ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.nadia -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.nadia/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.nadia/$_subdir"
            chmod 2770 "${cfg.stateDir}/.nadia/$_subdir"
            find "${cfg.stateDir}/.nadia/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          # nadia-group users can save settings via the CLI/TUI, else 0640).
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m ${configYamlMode} -D ${configFile} ${cfg.stateDir}/.nadia/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.nadia/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.nadia/config.yaml
            chmod ${configYamlMode} ${cfg.stateDir}/.nadia/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.nadia/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.nadia/.managed
          chmod 0644 ${cfg.stateDir}/.nadia/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.nadia/.container-mode <<'NADIA_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    nadia_bin=${containerDataDir}/current-package/bin/nadia
    NADIA_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.nadia/.container-mode
            chmod 0644 ${cfg.stateDir}/.nadia/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.nadia/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.nadia";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.nadia" ]; then
                  rm -f "${symlinkPath}"
                  echo "nadia-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.nadia -> stateDir/.nadia for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.nadia";
                target = "${cfg.stateDir}/.nadia";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "nadia-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.nadia/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.nadia/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.nadia/auth.json
              fi
            ''}
          ''}
          ...
          # Nadia reads $NADIA_HOME/.env at startup via load_nadia_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.nadia/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'NADIA_NIX_ENV_EOF'
    ${envFileContent}
    NADIA_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.nadia/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.nadia/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.nadia/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.nadia-agent = {
          description = "Nadia Agent Gateway";
          ...
          serviceConfig = {
            ...
            # $NADIA_HOME/.env by the activation script. load_nadia_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/nadia"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the nadia group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.nadia-agent = {
          description = "Nadia Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              NADIA_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              NADIA_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env NADIA_UID="$NADIA_UID" \
                --env NADIA_GID="$NADIA_GID" \
                --env NADIA_HOME=${containerDataDir}/.nadia \
                --env NADIA_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/nadia gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
