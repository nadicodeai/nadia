# nix/desktop.nix — Nadia Desktop (Electron) app build + wrapper
#
# `nadiaAgent` is the fully-built `.#default` package — it ships the
# `nadia` binary with the venv, runtime PATH, bundled skills/plugins, etc.
# already wired up.  We point the desktop at it via the existing
# `NADIA_DESKTOP_NADIA` override env var, so the desktop's resolver
# uses our fully wrapped binary at step 4 ("existing Nadia CLI").
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/nadia-desktop $out/bin
    cp -r ${renderer}/* $out/share/nadia-desktop/
    ...
    substituteInPlace $out/share/nadia-desktop/electron/main.cjs \
      --replace-fail "process.resourcesPath" "'$out/share/nadia-desktop'"
    ...
    substituteInPlace $out/share/nadia-desktop/electron/git-review-ops.cjs \
      --replace-fail "process.resourcesPath" "'$out/share/nadia-desktop'"

    # Wrap the nixpkgs electron binary to launch our app.  Set
    # NADIA_DESKTOP_NADIA to the absolute path of the nix-built `nadia`
    # binary so the desktop's resolver step 4 ("existing Nadia CLI on
    # PATH") uses our fully wrapped binary — venv with all deps,
    # bundled skills/plugins, runtime PATH (ripgrep/git/ffmpeg/etc).
    # No reimplementation of the agent resolver in the wrapper.
    makeWrapper ${lib.getExe electron} $out/bin/nadia-desktop \
      --add-flags "$out/share/nadia-desktop" \
      --set NADIA_DESKTOP_NADIA "${lib.getExe nadiaAgent}" \
  ...
  meta = with lib; {
    description = "Native Electron desktop shell for Nadia Agent";
    homepage = "https://github.com/nadicodeai/nadia";
    license = licenses.mit;
    platforms = platforms.unix;
    mainProgram = "nadia-desktop";
  };
}
