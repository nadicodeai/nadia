...
``nadia gateway restart`` (or ``launchctl kickstart ai.nadia.gateway``
or ``systemctl restart nadia-gateway``).  When the cron fires, the
...
``nadia cron create`` CLI subcommand AND the agent's ``cronjob`` model
...
command identifier (``nadia gateway``, ``launchctl ... nadia-gateway``,
``systemctl ... nadia-gateway``, ``pkill`` against the gateway) so it
...
blocks these commands at *execution* time when ``_NADIA_GATEWAY=1``, and
``nadia gateway stop|restart`` refuse to self-target from inside the
...
_GATEWAY_LIFECYCLE_PATTERN = re.compile(
    r"(?i)"
    # Branch A: `nadia gateway restart|stop` — the canonical foot-gun.
    # `start` is intentionally excluded: starting a gateway from inside a
    # gateway is benign (a no-op or "already running" error), and a
    # legitimate cron job might start a sibling profile's gateway.
    r"(?:nadia\s+gateway\s+(?:restart|stop))"
    # Branch B: launchctl ops on a nadia-gateway label. macOS launchd
    # labels look like `ai.nadia.gateway` / `nadia-gateway`. Requiring the
    # gateway identifier prevents blocking unrelated nadia services (e.g.
    # `launchctl unload ai.nadia.update-checker.plist`).
    r"|(?:launchctl\s+(?:kickstart|unload|load|stop|restart)\b[^\n]*\bnadia[.\-]?gateway)"
    # Branch C: systemctl ops on a nadia-gateway unit.
    r"|(?:systemctl\s+(?:-\S+\s+)*(?:restart|stop|start)\b[^\n]*\bnadia[.\-]?gateway)"
    # Branch D: pkill / kill targeting the nadia gateway process. Both
...
def _resolve_script_path(script_path: str) -> Path:
    ...
    under ``<NADIA_HOME>/scripts/`` and only accepts absolute paths as-is.
    ...
    (``~/.nadia/scripts/restart.sh``) but is passed as the bare name
    ...
    if raw.is_absolute():
        return raw
    return get_nadia_home() / "scripts" / raw
...
def check_gateway_lifecycle(
    ...
    if contains_gateway_lifecycle_command(combined):
        raise GatewayLifecycleBlocked(
            ...
            "(#30719). Run `nadia gateway restart` from a shell outside "
