...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``nadia://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``nadia://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"nadia://blueprint/{quote(blueprint.key)}{qs}"
