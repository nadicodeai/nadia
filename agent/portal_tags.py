...
The version is sourced live from ``nadia_cli.__version__`` so it auto-aligns
...
``nadia_cli.__version__`` is the canonical source of truth.
"""
...
def _nadia_version() -> str:
    """Return the current Nadia release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``nadia_cli`` cannot be imported (should
...
def nous_portal_tags() -> List[str]:
    ...
    """
    return ["product=nadia", nadia_client_tag()]
