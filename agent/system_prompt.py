...
upstream prefix cache warm.  See ``nadia-agent-dev``'s
...
* ``stable``   — identity (SOUL.md or DEFAULT_AGENT_IDENTITY), tool
  guidance, computer-use guidance, nadia subscription block, tool-use
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Nadia never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want NADIA_HOME persona while keeping
    ...
    # Pointer to the nadia-agent skill + docs for user questions about Nadia itself.
    stable_parts.append(NADIA_AGENT_HELP_GUIDANCE)
    ...
    # NADIA_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Coding posture (base Nadia, any interactive coding surface in a code
    ...
    # Active-profile hint — names the Nadia profile the agent is running
    # under so it doesn't conflate ~/.nadia/skills/ (default profile) with
    # ~/.nadia/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active Nadia profile: default. Other profiles (if any) live "
            "under ~/.nadia/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            f"Active Nadia profile: {active_profile}. This session reads "
            f"and writes ~/.nadia/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.nadia/skills/, ~/.nadia/plugins/, "
            f"~/.nadia/cron/, ~/.nadia/memories/ — those belong to a "
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Nadia never rebuilds or reinjects parts of it
