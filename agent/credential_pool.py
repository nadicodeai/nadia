...
from nadia_cli.config import load_env
from agent.secret_scope import get_secret as _get_secret
from agent.credential_persistence import (
    is_borrowed_credential_source,
    sanitize_borrowed_credential_payload,
)
import nadia_cli.auth as auth_mod
...
class PooledCredential:
    ...
    def runtime_api_key(self) -> str:
        if self.provider == "nous":
            # Nadia stores the runtime inference credential in agent_key for
...
def _write_through_provider_state_to_global_root(
    ...
    (#48415 / #43589): nadia, openai-codex, and xai-oauth rotate the
    ...
    ``nadia_cli.auth._write_through_xai_oauth_to_global_root`` (which covers
...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``nadia model`` / ``nadia auth`` which
        ...
        Mirrors the Nadia/Anthropic resync paths above.  Only applies to
    ...
    def _sync_xai_oauth_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        xAI OAuth refresh tokens are single-use.  When another Nadia process
    ...
    def _sync_nous_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        """Sync a Nadia pool entry from auth.json if tokens differ.

        Nadia OAuth refresh tokens are single-use.  When another process
        ...
        try:
            ...
            if should_sync:
                logger.debug(
                    "Pool entry %s: syncing Nadia state from auth.json",
                    entry.id,
                )
                field_updates: Dict[str, Any] = {
                    "last_status": None,
            ...
            logger.debug("Failed to sync Nadia entry from auth.json: %s", exc)
        return entry
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        (currently Nadia, OpenAI Codex, and xAI Grok OAuth).
        ...
        here would mean every Nadia/Codex/xAI refresh in a multi-provider
        setup silently flips the ``active_provider`` flag — the next
        ``nadia`` invocation that defaults to the active provider
        (e.g. setup wizard, ``nadia auth status``) would land on
        ...
        try:
            with _auth_store_lock():
                ...
                # nadia_cli.auth._save_xai_oauth_tokens (#43589); the pool
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        # sequence below must run atomically across Nadia processes: otherwise
        ...
        if self.provider == "openai-codex":
            refresh_timeout_seconds = auth_mod.env_float(
                "NADIA_CODEX_REFRESH_TIMEOUT_SECONDS", 20
            )
            lock_timeout = max(
        ...
        try:
            if self.provider == "anthropic":
                ...
                refreshed = refresh_anthropic_oauth_pure(
                    entry.refresh_token,
                    use_json=entry.source.endswith("nadia_pkce"),
                )
                updated = replace(
                    entry,
                ...
                # refresh_token — single-use tokens consumed by another Nadia
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            ...
                            use_json=synced.source.endswith("nadia_pkce"),
                        )
                        updated = replace(
                            synced,
                            access_token=refreshed["access_token"],
                    ...
                    return synced
            # For xai-oauth: same race as nadia — another process may have
            ...
            if self.provider == "xai-oauth":
                ...
                # in-memory pool.  Mirrors the Nadia quarantine path above.
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    return None
            # For openai-codex: same race as xAI/nadia — another Nadia process
            ...
            if self.provider == "openai-codex":
                ...
                # in-memory pool.  Mirrors the xAI and Nadia quarantine paths.
                if auth_mod._is_terminal_codex_oauth_refresh_error(exc):
                    ...
                    self._persist(removed_ids=removed_ids)
                    return None
            # For nadia: another process may have consumed the refresh token
            # between our proactive sync and the HTTP call.  Re-sync from
            # auth.json and adopt the fresh tokens if available.
            if self.provider == "nous":
                synced = self._sync_nous_entry_from_auth_store(entry)
                if synced.refresh_token != entry.refresh_token:
                    logger.debug("Nadia refresh failed but auth.json has newer tokens — adopting")
                    updated = replace(
                        synced,
                        last_status=STATUS_OK,
                        last_status_at=None,
                ...
                if auth_mod._is_terminal_nous_refresh_error(exc):
                    logger.debug("Nadia refresh token is terminally invalid; clearing local token state")
                    try:
                        with _auth_store_lock():
                            auth_store = _load_auth_store()
                        ...
                        logger.debug("Failed to clear terminal Nadia OAuth state: %s", clear_exc)
    ...
    def _entry_needs_refresh(self, entry: PooledCredential) -> bool:
        ...
        if self.provider == "nous":
            # Nadia refresh can require network access and should happen when
    ...
    def _available_entries(self, *, clear_expired: bool = False, refresh: bool = False) -> List[PooledCredential]:
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other Nadia profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
                ...
                if synced is not entry:
                    entry = synced
                    cleared_any = True
            # For nadia entries, sync from auth.json before status checks.
            ...
            # re-authed via `nadia model` / `nadia auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `nadia model` ->
            ...
            if entry.last_status == STATUS_DEAD:
                ...
                # user can always re-add via ``nadia auth add``.  Singleton-
                ...
                if _is_manual_source(entry.source):
                    ...
                    if dead_at and now - dead_at > DEAD_MANUAL_PRUNE_TTL_SECONDS:
                        ...
                        logger.warning(
                            ...
                            "(reason=%s, age=%.1fh) — re-add via `nadia auth add %s`",
                            _label,
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `nadia auth remove <provider> <N>` is stable across all source types.
    try:
        from nadia_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
        def _is_suppressed(_p, _s):  # type: ignore[misc]
            return False

    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, Nadia PKCE)
        ...
        # API-key vs OAuth is a user-visible choice at `nadia setup` ("Claude
        ...
        # Claude Code CLI, nadia_pkce creds from a previous OAuth login)
        ...
        # that masquerade.  Prefer ~/.nadia/.env over os.environ for the
        # same reason `_seed_from_env` does — that's the authoritative file
        # that `nadia setup` writes.
        _env_file = load_env()
        ...
        if api_key_path_explicit:
            ...
            retained = [
                ...
                if entry.source not in {"nadia_pkce", "claude_code"}
            ]
        ...
        from agent.anthropic_adapter import read_claude_code_credentials, read_nadia_oauth_credentials

        for source_name, creds in (
            ("nadia_pkce", read_nadia_oauth_credentials()),
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # `nadia auth add nous --label <name>`).  Fall back to the
        ...
        # Nadia auth store or env vars, so resolve them here.
        # Use refresh_if_expiring=False to avoid network calls during
        # pool loading / provider discovery.
        try:
            from nadia_cli.auth import resolve_qwen_runtime_credentials
        ...
        # MiniMax OAuth tokens live in ~/.nadia/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `nadia auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `nadia auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the Nadia auth store.  Without this gate the removal is instantly
        ...
        # Nadia owns its own Codex auth state — we do NOT auto-import from
        ...
        # When the user logs in via ``nadia model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``nadia auth list`` reflects the logged-in state and so the pool
        ...
        if isinstance(tokens, dict) and tokens.get("access_token"):
            ...
            from nadia_cli.auth import DEFAULT_XAI_OAUTH_BASE_URL
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Prefer ~/.nadia/.env over os.environ — the user's config file is the
    # authoritative source for Nadia credentials. Stale env vars from parent
    ...
    # Honour user suppression — `nadia auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.nadia/.env.
    ...
    try:
        from nadia_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
    ...
    def _secret_source_for_env(env_var: str) -> Optional[str]:
        try:
            from nadia_cli.env_loader import get_secret_source
    ...
    if provider == "openrouter":
        # Prefer ~/.nadia/.env over os.environ
...
def _prune_stale_seeded_entries(
    ...
    *,
    ...
    def _is_prunable(entry: PooledCredential) -> bool:
        ...
        # (e.g. an `nadia auth` command that confirmed the source is gone).
        if entry.source.startswith("env:"):
            return prune_env_sources
        # File-backed singletons (device-code OAuth, claude_code) and Nadia
