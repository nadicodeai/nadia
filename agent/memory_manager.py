...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        _core_tool_names = set(_NADIA_CORE_TOOLS)
    ...
    def _strip_skill_scaffolding(text: str) -> Optional[str]:
        ...
        When a user invokes a /skill or /bundle, Nadia expands the turn into
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``nadia_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_nadia_home()`` themselves.
        """
        if "nadia_home" not in kwargs:
            from nadia_constants import get_nadia_home
            kwargs["nadia_home"] = str(get_nadia_home())
        for provider in self._providers:
            try:
