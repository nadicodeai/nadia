...
(3 SDK retries x 3 Nadia retries), and every one of those calls counts
...
_STATE_FILENAME = "nadia.json"


def _state_path() -> str:
    """Return the path to the Nadia rate limit state file."""
    try:
        from nadia_constants import get_nadia_home
        base = get_nadia_home()
    except ImportError:
        base = os.path.join(os.path.expanduser("~"), ".nadia")
...
def record_nous_rate_limit(
    *,
    ...
    try:
        ...
        logger.info(
            "Nadia rate limit recorded: resets in %.0fs (at %.0f)",
            reset_at - now, reset_at,
        )
    except Exception as exc:
        logger.debug("Failed to write Nadia rate limit state: %s", exc)
...
def clear_nous_rate_limit() -> None:
    """Clear the rate limit state (e.g., after a successful Nadia request)."""
    try:
        os.unlink(_state_path())
    except FileNotFoundError:
        pass
    except OSError as exc:
        logger.debug("Failed to clear Nadia rate limit state: %s", exc)
    ...
    *,
    ...
    MiMo, Nadia, ...) behind one endpoint.  A 429 can mean either:
      ...
      (b) The upstream provider is out of capacity for a specific model
          ...
          the caller's quota on Nadia.

    Tripping the cross-session breaker on (b) blocks ALL Nadia requests
    (and all models, since Nadia is one provider key) for minutes even
    ...
    We tell the two apart by looking at:
      ...
      1. The 429 response's own ``x-ratelimit-*`` headers.  Nadia emits
