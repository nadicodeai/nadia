...
from nadia_constants import PARTIAL_STREAM_STUB_ID
from nadia_logging import set_session_context
...
def _ollama_context_limit_error(agent: Any, request_tokens: int) -> Optional[str]:
    ...
    logger.warning(
        "Ollama runtime context too small for Nadia tool use: "
    ...
    return (
        ...
        f"context, but Nadia needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens "
        "for reliable tool use.\n\n"
        "Increase the Ollama context for this model and restart/reload the "
        "model before trying again. A known-good starting point is 65,536 "
        "tokens. In Nadia config, set `model.ollama_num_ctx: 65536` "
...
def _is_nous_inference_route(provider: str, base_url: str) -> bool:
    ...
    return (
        base_url_host_matches(base, "inference-api.nadicode.ai")
        or base_url_host_matches(base, "inference.nadicode.ai")
    )
...
def _try_refresh_nous_paid_entitlement_credentials(agent) -> bool:
    """Refresh Nadia runtime credentials after a fresh paid-entitlement check."""
    try:
...
_CONTENT_POLICY_RECOVERY_HINT = (
    ...
    "adding a fallback provider with `nadia fallback add`."
)
...
def run_conversation(
    ...
    if moa_config is None:
        try:
            from nadia_cli.moa_config import decode_moa_turn
    ...
    # all run inside Codex). Default Nadia path is bypassed entirely.
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        # cache prefix.  The system prompt is reserved for Nadia internals.
        #
        # Nadia invariant: the system prompt is built ONCE per session
        ...
        if _runtime_context_error:
            ...
            agent._emit_status("❌ Ollama runtime context is too small for Nadia tool use")
            api_call_count -= 1
            agent._api_call_count = api_call_count
            try:
        ...
        while retry_count < max_retries:
            ...
            try:
                ...
                try:
                    from nadia_cli.middleware import apply_llm_request_middleware
                ...
                if env_var_enabled("NADIA_DUMP_REQUESTS"):
                ...
                from nadia_cli.middleware import run_llm_execution_middleware
                ...
                if finish_reason == "content_filter":
                    ...
                    _refusal_response = (
                        ...
                        "(safety refusal — not a Nadia/gateway failure).\n\n"
                        f"{_refusal_detail}\n\n"
                ...
                # Clear Nadia rate limit state on successful request —
                # proves the limit has reset and other sessions can
                # resume hitting Nadia.
                if agent.provider == "nous":
                    try:
                ...
                if (
                    ...
                    if _try_refresh_nous_paid_entitlement_credentials(agent):
                        agent._vprint(
                            f"{agent.log_prefix}🔐 Nadia paid access verified — "
                ...
                if (
                    ...
                    if agent._try_refresh_nous_client_credentials(force=True):
                        print(f"{agent.log_prefix}🔐 Nadia agent key refreshed after 401. Retrying request...")
                        continue
                    ...
                    try:
                        ...
                        pass
                    print(f"{agent.log_prefix}🔐 Nadia 401 — Portal authentication failed.")
                    if _body_text:
                        print(f"{agent.log_prefix}   Response: {_body_text}")
                    if not _print_nous_entitlement_guidance(agent, "Nadia model access"):
                ...
                if (
                    ...
                    if is_token_provider(key):
                        ...
                        print(f"{agent.log_prefix}   Run `nadia doctor` for credential-chain diagnostics, or")
                    ...
                    from nadia_constants import display_nadia_home as _dhh_fn
                    _dhh = _dhh_fn()
                    print(f"{agent.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for Nadia-managed OAuth/setup tokens")
                    ...
                    print(f"{agent.log_prefix}     • Legacy cleanup: nadia config set ANTHROPIC_TOKEN \"\"")
                    print(f"{agent.log_prefix}     • Clear stale keys: nadia config set ANTHROPIC_API_KEY \"\"")
                ...
                # block every Nadia model for minutes.  We use
                ...
                if (
                    ...
                    try:
                        ...
                        if _genuine_nous_rate_limit:
                            ...
                            logger.info(
                                "Nadia 429 looks like upstream capacity "
                    ...
                    if _genuine_nous_rate_limit:
                        ...
                        # top-of-loop Nadia guard handles fallback or
                ...
                # which Nadia's system prompt + tool schemas alone exceed.
                ...
                if (
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      request at ~8K tokens. Nadia's system prompt + tool schemas baseline",
                        force=True,
                    )
                    agent._vprint(
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      Use the `copilot` provider with a Copilot subscription token (`nadia",
                        force=True,
                    )
                    agent._vprint(
                ...
                if is_client_error:
                    ...
                    if classified.is_auth or classified.reason == FailoverReason.billing:
                        if classified.reason == FailoverReason.billing and _print_billing_or_entitlement_guidance(
                            ...
                            if _provider == "openai-codex":
                                ...
                                agent._vprint(f"{agent.log_prefix}      2. Then run `nadia auth` to re-authenticate.", force=True)
                            ...
                            agent._vprint(f"{agent.log_prefix}      • Is the key valid? Run: nadia setup", force=True)
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        ...
                        agent._vprint(
                            f"{agent.log_prefix}        nadia fallback add   (interactive picker — same as `nadia model`)",
                            force=True,
                        )
                ...
                if retry_count >= max_retries:
                    ...
                    if _is_thinking_timeout:
                        ...
                        agent._vprint(
                            ...
                            f"in `~/.nadia/config.yaml` to extend the per-call "
                            f"timeout. (Nadia's built-in floor is 600s for "
        ...
        try:
            ...
            if assistant_message.tool_calls:
                ...
                try:
                    ...
                    # terminates Nadia mid-turn, resume logic still sees the
                ...
                try:
                    ...
                    from nadia_cli.plugins import get_pre_verify_continue_message, has_hook
