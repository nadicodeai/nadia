...
Data resolution order (like TypeScript OpenCode):
  ...
  2. Disk cache (~/.nadia/models_dev_cache.json)
...
# ---------------------------------------------------------------------------
# Provider ID mapping: Nadia ↔ models.dev
# ---------------------------------------------------------------------------

# Nadia provider names → models.dev provider IDs
...
# Reverse mapping: models.dev → Nadia (built lazily)
...
def _get_cache_path() -> Path:
    ...
    return get_nadia_home() / "models_dev_cache.json"
...
def fetch_models_dev(force_refresh: bool = False) -> Dict[str, Any]:
    ...
    When ``force_refresh=True`` (used by ``nadia config refresh``, the
...
def _get_provider_models(provider: str) -> Optional[Dict[str, Any]]:
    """Resolve a Nadia provider ID to its models dict from models.dev.
...
def list_provider_models(provider: str) -> List[str]:
    ...
    """
    from nadia_cli.models import normalize_provider
...
# Gemma models whose TPM quotas are too small for normal Nadia agent traffic.
...
def get_provider_info(provider_id: str) -> Optional[ProviderInfo]:
    ...
    Accepts either a Nadia provider ID (e.g. "kilocode") or a models.dev
    ID (e.g. "kilo").  Returns None if the provider is not in the catalog.
    """
    # Resolve Nadia ID → models.dev ID
...
def get_model_info(
    ...
    Accepts Nadia or models.dev provider ID.  Tries exact match then
