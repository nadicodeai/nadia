...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Nadia starts and finds ``~/.openclaw/``.

    Points users at ``nadia claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``nadia claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Nadia, run "
        "`nadia claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `nadia claw cleanup` (renames it to ~/.openclaw.pre-migration — "
