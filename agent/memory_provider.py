...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `nadia backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - nadia_home (str): The active NADIA_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.nadia``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "nadia").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'nadia memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], nadia_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'nadia memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``nadia_home`` is the active NADIA_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE NADIA_HOME.

        ``nadia backup`` only walks NADIA_HOME, so any provider state kept
        ...
        of the archive, and ``nadia import`` restores them to their original
