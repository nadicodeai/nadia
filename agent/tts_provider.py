...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Nadia with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.nadia/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Nadia
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``nadia tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``nadia setup`` for
