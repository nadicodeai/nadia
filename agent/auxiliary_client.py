...
from nadia_constants import OPENROUTER_BASE_URL
...
def _create_openai_client(*, api_key: str, base_url: str, **kwargs: Any) -> Any:
    ...
    # Nadia owns auxiliary retry + provider/model fallback policy (the
    ...
    # Nadia sees a single failure (issue #54465). Disable SDK-internal retries
    # by default and let Nadia control the budget; explicit callers can still
    # override via kwargs.
...
def _compression_threshold_for_model(
    ...
    *,
    ...
    consumed before Nadia triggers summarization.  Higher values delay
...
_OR_HEADERS_BASE = {
    "HTTP-Referer": "https://docs.nadicode.ai/nadia",
    "X-Title": "Nadia Agent",
...
def _apply_user_default_headers(headers: dict | None) -> dict | None:
    ...
    """
    try:
        from nadia_cli.config import cfg_get, load_config
...
def build_or_headers(or_config: dict | None = None) -> dict:
    ...
    Environment variables:
        ``NADIA_OPENROUTER_CACHE`` — truthy (``1``/``true``/``yes``/``on``)
            enables caching; ``0``/``false``/``no``/``off`` disables.
            Overrides ``openrouter.response_cache`` in config.yaml.
        ``NADIA_OPENROUTER_CACHE_TTL`` — integer seconds (1-86400).
    ...
    *or_config* is the ``openrouter`` section from config.yaml.  When *None*,
    ...
    env_cache = os.environ.get("NADIA_OPENROUTER_CACHE", "").strip().lower()
    if env_cache:
    ...
    env_ttl = os.environ.get("NADIA_OPENROUTER_CACHE_TTL", "").strip()
    if env_ttl:
        if env_ttl.isdigit():
            ttl = int(env_ttl)
            if 1 <= ttl <= 86400:
...
_NVIDIA_NIM_CLOUD_HEADERS = {
    "X-BILLING-INVOKE-ORIGIN": "NadiaAgent",
}
...
_NOUS_DEFAULT_BASE_URL = "https://inference-api.nadicode.ai/v1"
_ANTHROPIC_DEFAULT_BASE_URL = "https://api.anthropic.com"
_AUTH_JSON_PATH = get_nadia_home() / "auth.json"
...
def _codex_cloudflare_headers(access_token: str) -> Dict[str, str]:
    ...
    """
    headers = {
        "User-Agent": "codex_cli_rs/0.0.0 (Nadia Agent)",
...
def _pool_runtime_api_key(entry: Any) -> str:
    ...
    # provider-specific fallback (e.g. agent_key for nadia).
...
def _pool_runtime_base_url(entry: Any, fallback: str = "") -> str:
    ...
    if getattr(entry, "provider", None) == "nous":
        ...
        # shares one normalization path with the rest of the NADIA resolution.
        from nadia_cli.auth import _nous_inference_env_override

        env_url = _nous_inference_env_override()
        if env_url:
            return env_url
    # runtime_base_url handles provider-specific logic (e.g. nadia prefers inference_base_url).
...
def _nous_min_key_ttl_seconds() -> int:
    try:
        return max(60, int(os.getenv("NADIA_NOUS_MIN_KEY_TTL_SECONDS", "1800")))
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``nadia_cli.runtime_provider._detect_api_mode_for_url`` so the
...
def _read_nous_auth() -> Optional[dict]:
    """Read and validate ~/.nadia/auth.json for an active Nadia provider.

    Returns the provider state dict if Nadia is active with tokens,
    otherwise None.
    """
    ...
    try:
        ...
        logger.debug("Could not read Nadia auth: %s", exc)
        return None


def _nous_api_key(provider: dict) -> str:
    """Extract a usable Nadia inference JWT from stored auth state."""
    from nadia_cli.auth import _nous_invoke_jwt_is_usable
...
def _nous_base_url() -> str:
    """Resolve the Nadia inference base URL from env or default."""
    return os.getenv("NOUS_INFERENCE_BASE_URL", _NOUS_DEFAULT_BASE_URL)


def _resolve_nous_pool_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    """Resolve Nadia auxiliary credentials from the selected pool entry."""
    try:
        from nadia_cli.auth import _agent_key_is_usable

        pool = load_pool("nous")
    except Exception as exc:
        logger.debug("Auxiliary Nadia pool credential resolution failed: %s", exc)
        return None
    ...
    try:
        ...
        logger.debug("Auxiliary Nadia pool selection failed: %s", exc)
        return None
    ...
    if force_refresh or not _agent_key_is_usable(state, _nous_min_key_ttl_seconds()):
        try:
            ...
            logger.debug("Auxiliary Nadia pool refresh failed: %s", exc)
            refreshed = None
...
def _resolve_nous_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    """Return fresh Nadia runtime credentials when available.
    ...
    try:
        from nadia_cli.auth import resolve_nous_runtime_credentials

        creds = resolve_nous_runtime_credentials(
            timeout_seconds=env_float("NADIA_NOUS_TIMEOUT_SECONDS", 15),
            force_refresh=force_refresh,
        )
    except Exception as exc:
        logger.debug("Auxiliary Nadia runtime credential resolution failed: %s", exc)
        return None
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    compression report "no provider configured" even though ``nadia auth
    status`` shows xAI OAuth as logged in.

    Falls back to ``nadia_cli.auth``'s singleton runtime resolver for older
    ...
    try:
        from nadia_cli.auth import resolve_xai_oauth_runtime_credentials
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from Nadia auth store.
    ...
    try:
        from nadia_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from nadia_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if provider_id == "anthropic":
            ...
            try:
                from nadia_cli.auth import is_provider_explicitly_configured
        ...
        if pool_present:
            ...
            if base_url_host_matches(base_url, "api.kimi.com"):
                ...
                from nadia_cli.models import copilot_default_headers
...
def _try_nous(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    # Check cross-session rate limit guard before attempting Nadia —
    # if another session already recorded a 429, skip Nadia entirely
    ...
    nadia = _read_nous_auth()
    runtime = _resolve_nous_runtime_api(force_refresh=False)
    if runtime is None and not nadia:
        logger.warning(
            "Auxiliary Nadia client unavailable: no Nadia authentication found "
            "(run: nadia auth)."
        )
        _mark_provider_unhealthy("nous", ttl=60)
        return None, None
    if runtime is None and nadia:
        logger.debug(
            "Auxiliary Nadia: runtime JWT refresh failed; checking stored "
            "auth.json token."
        )
    ...
    model = _NOUS_MODEL
    try:
        from nadia_cli.models import get_nous_recommended_aux_model
    ...
    if runtime is not None:
        ...
        api_key = _nous_api_key(nadia or {})
        if not api_key:
            logger.warning(
                "Auxiliary Nadia client unavailable: no usable inference JWT found "
                "(run: nadia auth add nous)."
            )
            _mark_provider_unhealthy("nous", ttl=60)
            return None, None
        base_url = str((nadia or {}).get("inference_base_url") or _nous_base_url()).rstrip("/")
    return (
...
def _refresh_nous_recommended_model(
    *, vision: bool, stale_model: Optional[str]
    ...
    the Nadia → OpenRouter catalog, every auxiliary call 404s with
    ...
    try:
        ...
        logger.debug(
            "Nadia recommended-model refresh failed (%s); using default %s",
            exc, _NOUS_MODEL,
        )
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `nadia setup` or `nadia model` and enter a valid http(s) base URL."
        ) from exc
...
def _try_azure_foundry(
    *,
    ...
    :func:`nadia_cli.runtime_provider._resolve_azure_foundry_runtime` —
    ...
    * Non-default ``model.base_url`` overrides are honored.
    ...
    """
    try:
        from nadia_cli.runtime_provider import _resolve_azure_foundry_runtime
...
# manual intervention.
#
# Failure isolation: the cache is in-process only. A second nadia
...
def _reset_aux_unhealthy_cache() -> None:
    ...
    user trigger (e.g. ``nadia config aux reset``)."""
...
def _nous_portal_account_has_fresh_paid_access() -> bool:
    """Return True only when the fresh Nadia account API says paid access is allowed."""
    try:
        from nadia_cli.nous_account import get_nous_portal_account_info

        account_info = get_nous_portal_account_info(force_fresh=True)
        return account_info.paid_service_access is True
    except Exception as exc:
        logger.debug("Auxiliary Nadia paid-entitlement refresh check failed: %s", exc)
        return False
...
def _is_model_not_found_error(exc: Exception) -> bool:
    ...
    that has since been dropped from the Nadia → OpenRouter catalog. The Nadia
...
def _recoverable_pool_provider(
    ...
    if base_url_host_matches(base, "openrouter.ai"):
        return "openrouter"
    if base_url_host_matches(base, "inference-api.nadicode.ai"):
        return "nous"
...
def _refresh_provider_credentials(provider: str) -> bool:
    ...
    try:
        if normalized == "openai-codex":
            from nadia_cli.auth import resolve_codex_runtime_credentials
...
def _try_main_fallback_chain(
    ...
    main fallback policy before dropping into Nadia's built-in discovery
...
def _resolve_auto(
    ...
    Priority:
      1. User's main provider + main model, regardless of provider type.
         ...
         chat.  Users on OpenRouter/Nadia get their chosen chat model; users
      ...
      2. OpenRouter → Nadia → custom → Codex → API-key providers (fallback
    ...
    #    scenario where a user switches providers via `nadia model` but the
    #    old OPENAI_BASE_URL lingers in ~/.nadia/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "Run: nadia model to reconfigure, or remove "
                "OPENAI_BASE_URL from ~/.nadia/.env",
    ...
    # on aggregators (OpenRouter, Nadia) who previously got routed to a
    ...
    if main_provider == "moa":
        try:
            from nadia_cli.config import load_config
            from nadia_cli.moa_config import resolve_moa_preset
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    if base_url_host_matches(sync_base_url, "openrouter.ai"):
        ...
        from nadia_cli.copilot_auth import copilot_request_headers
    ...
    # See _create_openai_client: disable SDK-internal retries so Nadia owns
...
def resolve_provider_client(
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: nadia model)")
                return None, None
    ...
    # OpenRouter / Nadia bills for side tasks they thought were running on
    ...
    if provider == "xai-oauth":
        ...
        if client is None:
            logger.warning(
                ...
                "OAuth token found (run: nadia model -> xAI Grok OAuth — SuperGrok / Premium+)"
            )
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
    ...
    try:
        from nadia_cli.runtime_provider import _get_named_custom_provider
    ...
    if provider == "azure-foundry":
        ...
        if client is None:
            logger.warning(
                ...
                "runtime resolution failed (run: nadia doctor for "
                "diagnostics)"
            )
            return None, None
    ...
    try:
        ...
        logger.debug("nadia_cli.auth not available for provider %s", provider)
        return None, None
    ...
    if pconfig.auth_type == "api_key":
        ...
        if provider == "copilot" and final_model and not raw_codex:
            try:
                from nadia_cli.models import _should_use_copilot_responses_api
...
def get_available_vision_backends() -> List[str]:
    ...
    Order: active provider → OpenRouter → Nadia → stop.  This is the single
    ...
    if main_provider and main_provider not in {"auto", ""}:
        if main_provider in _VISION_AUTO_PROVIDER_ORDER:
            ...
            if client is not None:
                available.append(main_provider)
    # 2. OpenRouter, 3. Nadia — skip if already covered by main provider.
...
def resolve_vision_provider_client(
    ...
    *,
    ...
    if requested == "auto":
        ...
        #      zai → glm-5v-turbo). Nadia is the exception: it has a dedicated
...
def _refresh_nous_auxiliary_client(
    *,
    ...
    """Refresh Nadia runtime creds, rebuild the client, and replace the cache entry."""
...
def _resolve_task_provider_model(
    ...
    def _preserve_provider_with_base_url(prov: Optional[str]) -> bool:
        ...
        if normalized in {"", "auto", "custom"} or normalized.startswith("custom:"):
            return False
        try:
            from nadia_cli.providers import get_provider
...
def _get_auxiliary_task_config(task: str) -> Dict[str, Any]:
    ...
    :meth:`nadia_cli.plugins.PluginContext.register_auxiliary_task`) the
...
def call_llm(
    ...
    *,
            ...
            if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
                ...
                if fb_client is not None:
                    ...
                    raise RuntimeError(
                        ...
                        f"variable, or switch to a different provider with `nadia model`."
                    )
    ...
    try:
        ...
        # known-good default). Only applies to Nadia-routed calls.
        _heal_is_nous = (
            resolved_provider == "nous"
            or base_url_host_matches(_base_info, "inference-api.nadicode.ai")
        )
        if _is_model_not_found_error(first_err) and _heal_is_nous:
            ...
            if healed_model and healed_model != kwargs.get("model"):
                logger.warning(
                    "Auxiliary %s: model %r no longer in Nadia catalog; "
        ...
        # ── Nadia auth refresh parity with main agent ──────────────────
        client_is_nous = (
            resolved_provider == "nous"
            or base_url_host_matches(_base_info, "inference-api.nadicode.ai")
        )
        if (
            ...
            if refreshed_client is not None:
                logger.info(
                    "Auxiliary %s: refreshed Nadia runtime credentials after paid account check, retrying",
                    task or "call",
                )
        ...
        if _is_auth_error(first_err) and client_is_nous:
            ...
            if refreshed_client is not None:
                logger.info("Auxiliary %s: refreshed Nadia runtime credentials after 401, retrying",
                            task or "call")
                if refreshed_model and refreshed_model != kwargs.get("model"):
                    kwargs["model"] = refreshed_model
                return _validate_llm_response(
        ...
        # When the resolved provider returns 401 and neither the Nadia
...
async def async_call_llm(
    ...
    *,
    ...
    try:
        ...
        _heal_is_nous = (
            resolved_provider == "nous"
            or base_url_host_matches(_client_base, "inference-api.nadicode.ai")
        )
        if _is_model_not_found_error(first_err) and _heal_is_nous:
            healed_model = _refresh_nous_recommended_model(
                vision=(task == "vision"), stale_model=kwargs.get("model"))
            if healed_model and healed_model != kwargs.get("model"):
                logger.warning(
                    "Auxiliary %s (async): model %r no longer in Nadia catalog; "
                    "retrying with refreshed recommendation %r",
                    task or "call", kwargs.get("model"), healed_model,
                )
                kwargs["model"] = healed_model
        ...
        if (
            ...
            if refreshed_client is not None:
                logger.info(
                    "Auxiliary %s (async): refreshed Nadia runtime credentials after paid account check, retrying",
                    task or "call",
                )
                if refreshed_model and refreshed_model != kwargs.get("model"):
                    kwargs["model"] = refreshed_model
        ...
        if _is_auth_error(first_err) and client_is_nous:
            ...
            if refreshed_client is not None:
                logger.info("Auxiliary %s (async): refreshed Nadia runtime credentials after 401, retrying",
                            task or "call")
                if refreshed_model and refreshed_model != kwargs.get("model"):
                    kwargs["model"] = refreshed_model
                return _validate_llm_response(
