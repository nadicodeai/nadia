...
def _resolve_requests_verify() -> bool | str:
    ...
    by default. Nadia also honours NADIA_CA_BUNDLE (its own convention)
    ...
    """
    for env_var in ("NADIA_CA_BUNDLE", "REQUESTS_CA_BUNDLE", "SSL_CERT_FILE"):
...
def _get_model_metadata_cache_path() -> Path:
    ...
    return get_nadia_home() / "cache" / "openrouter_model_metadata.json"
...
# Minimum context length required to run Nadia Agent.  Models with fewer
...
DEFAULT_CONTEXT_LENGTHS = {
    ...
    # (native MiniMax-M3, OpenRouter/Nadia minimax/minimax-m3).
    ...
    # so these hardcoded fallbacks prevent Nadia from probing-down to
...
_URL_TO_PROVIDER: Dict[str, str] = {
    ...
    "inference-api.nadicode.ai": "nous",
    ...
    # (often 8K) makes it unusable for Nadia's system prompt, but mapping
...
def _get_context_cache_path() -> Path:
    ...
    from nadia_constants import get_nadia_home
    return get_nadia_home() / "context_length_cache.yaml"
...
def parse_available_output_tokens_from_error(error_msg: str) -> Optional[int]:
    ...
    is_output_cap_error = (
        ...
        # OpenRouter/Nadia phrasing of the same condition.
    ...
    # OpenRouter/Nadia format: "maximum context length is N … (A of text input,
...
def is_output_cap_error(error_msg: str) -> bool:
    ...
    output_cap_signal = (
        ...
        or ("in the output" in error_lower                  # OpenRouter / Nadia
            and "maximum context length" in error_lower)
...
def _model_name_suggests_minimax_m3(model: str) -> bool:
    ...
    across surfaces (native MiniMax-M3, OpenRouter/Nadia minimax/minimax-m3).
        ...
        with httpx.Client(timeout=3.0, headers=headers) as client:
            ...
            if server_type == "ollama":
                ...
                if resp.status_code == 200:
                    ...
                    # Nadia grow conversations past the runtime limit and Ollama
...
def _normalize_model_version(model: str) -> str:
    ...
    Nadia uses dashes: claude-opus-4-6, claude-sonnet-4-5
...
def _resolve_nous_context_length(
    ...
    Nadia model IDs are bare after prefix-stripping (e.g. 'qwen3.6-plus',
    ...
    Returns ``(context_length, source)`` where ``source`` is one of:
      ...
      - ``""``           — could not resolve
    """
    # Portal first — the Nadia /models endpoint is authoritative for what our
    ...
    def _safe_ctx(or_id: str, entry: dict) -> Optional[int]:
        ...
        if ctx <= 32768 and _model_name_suggests_kimi(or_id):
            logger.info(
                ...
                "(Kimi-family underreport, Nadia path); falling through to hardcoded defaults",
                ctx, or_id,
            )
            return None
        return ctx
...
def get_model_context_length(
    ...
    1. Persistent cache (previously discovered via probing).  Nadia URLs
    ...
    5. Provider-aware lookups (before generic OpenRouter cache):
       ...
       b. Nadia: live /v1/models probe first (authoritative), then OR
    ...
    if effective_provider in {"copilot", "copilot-acp", "github-copilot"}:
        try:
            from nadia_cli.models import get_copilot_model_context
    ...
    if effective_provider == "nous":
        ...
        if ctx:
            ...
            # want it leaking into the persistent cache for Nadia URLs.
    ...
    # the dedicated Nadia/Copilot/GMI branches above.
...
def estimate_request_tokens_rough(
    ...
    *,
    ...
    Includes the major payload buckets Nadia sends to providers:
