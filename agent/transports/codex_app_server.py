...
projection into Nadia's display, approval bridging, transcript projection into
...
"codex_app_server"`. Nadia's default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # `os.environ.copy()` also handed it every Tier-1 Nadia secret — gateway
        ...
        spawn_env = nadia_subprocess_env(inherit_credentials=True)
        if env:
        ...
        if spawn_env.get("NADIA_KANBAN_TASK"):
            kanban_db = spawn_env.get("NADIA_KANBAN_DB")
            kanban_root = (
                ...
                if kanban_db
                    ...
                    "NADIA_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("NADIA_HOME", os.path.expanduser("~/.nadia")),
                        "kanban",
                    ),
                )
            )
    ...
    def initialize(
        self,
        client_name: str = "nadia",
        client_title: str = "Nadia Agent",
