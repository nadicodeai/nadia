...
Owns one Codex thread per Nadia session. Drives `turn/start`, consumes
...
# Nadia's tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_NADIA_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Nadia/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Nadia session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _NADIA_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("NADIA_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="nadia",
            client_title="Nadia Agent",
            client_version=_get_nadia_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Nadia's text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Nadia's approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own nadia-tools callback we
            # auto-accept — the user already approved Nadia's tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Nadia approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Nadia returns 'once', 'session', 'always', or 'deny'.
...
def _get_nadia_version() -> str:
    """Best-effort Nadia version string for codex's userAgent line."""
    try:
