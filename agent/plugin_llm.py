...
Plugins built on Nadia Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Nadia
...
:class:`~nadia_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Nadia's main config
...
handles every provider, fallback chain, and per-task override Nadia
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`nadia_cli.plugins.PluginContext`
