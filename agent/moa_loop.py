...
MoA-enabled; the normal Nadia agent loop still owns tool calling and turn
...
def _slot_runtime(slot: dict[str, str]) -> dict[str, Any]:
    ...
    try:
        ...
        # bedrock SigV4 signing, nadia Portal tags — still fire. Those branches
...
def _reference_messages(messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    conversation flow, but flatten it into clean user/assistant *text* turns:
      ...
      - system prompt: dropped (8K of Nadia boilerplate, not advisory signal).
...
def aggregate_moa_context(
    *,
    ...
    synth_prompt = (
        ...
        "Nadia agent. Focus on next steps, tool-use strategy, risks, and any "
    ...
    return (
        ...
        "normal Nadia agent loop. You may call tools, continue reasoning, or "
        "finish normally.]\n"
