...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the NADIA_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    # Nadia runtime / deployment
    "NADIA_HOME", "NADIA_PROFILE", "NADIA_GATEWAY_LOCK_DIR",
    "NADIA_MAX_ITERATIONS", "NADIA_MAX_TOKENS", "NADIA_API_TIMEOUT",
    "NADIA_REDACT_SECRETS", "NADIA_NOUS_TIMEOUT_SECONDS",
    "_NADIA_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "NADIA_KANBAN_DB", "NADIA_KANBAN_WORKSPACES_ROOT", "NADIA_KANBAN_BOARD",
})
_GLOBAL_ENV_PREFIXES = (
    "NADIA_KANBAN_",
    "NADIA_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def build_profile_secret_scope(nadia_home: Path) -> Dict[str, str]:
    ...
    """
    return load_env_file(Path(nadia_home) / ".env")
