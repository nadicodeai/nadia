"""Lightweight internationalization (i18n) for Nadia static user-facing messages.

Scope (thin slice, by design): only the highest-impact static strings shown
to the user by Nadia itself -- approval prompts, a handful of gateway slash
...
def _locales_dir() -> Path:
    ...
    1. ``NADIA_BUNDLED_LOCALES`` env var -- set by the Nix wrapper (or any
    ...
    """
    override = os.getenv("NADIA_BUNDLED_LOCALES", "").strip()
    if override:
        candidate = Path(override)
        if candidate.is_dir():
            return candidate
        logger.warning(
            "NADIA_BUNDLED_LOCALES points to a non-directory path (%s); "
...
def _load_catalog(lang: str) -> dict[str, str]:
    ...
    try:
        import yaml  # PyYAML is already a nadia dependency
...
def reset_language_cache() -> None:
    ...
    Call after :func:`nadia_cli.config.save_config` if a running process
