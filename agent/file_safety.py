...
def _nadia_home_path() -> Path:
    """Resolve the active NADIA_HOME (profile-aware) without circular imports."""
    try:
        from nadia_constants import get_nadia_home  # local import to avoid cycles
        return get_nadia_home()
    except Exception:
        return Path(os.path.expanduser("~/.nadia"))


def _nadia_root_path() -> Path:
    """Resolve the Nadia root dir (always the parent of any profile, never per-profile)."""
    try:
        from nadia_constants import get_default_nadia_root  # local import to avoid cycles
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    nadia_home = _nadia_home_path()
    nadia_root = _nadia_root_path()
    return {
        os.path.realpath(p)
        for p in [
            ...
            str(nadia_home / ".anthropic_oauth.json"),
            # Top-level Anthropic PKCE credential store remains sensitive even
            # when a profile is active; default/non-profile sessions still read it.
            str(nadia_root / ".anthropic_oauth.json"),
...
def get_safe_write_roots() -> set[str]:
    """Return resolved NADIA_WRITE_SAFE_ROOT paths. Supports multiple directories
    separated by ``os.pathsep`` (``:`` on Unix, ``;`` on Windows).
    E.g., ``/opt/data:/var/www/html`` on Unix, ``C:\\data;D:\\www`` on Windows."""
    env = os.getenv("NADIA_WRITE_SAFE_ROOT", "")
    if not env:
        return set()
    roots: set[str] = set()
...
def is_write_denied(path: str) -> bool:
    ...
    nadia_dirs = []
    for base in (_nadia_home_path(), _nadia_root_path()):
        try:
            real = os.path.realpath(base)
            if real not in nadia_dirs:
                nadia_dirs.append(real)
        except Exception:
            continue

    for base_real in nadia_dirs:
        try:
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied Nadia path.

    Three categories are blocked:

      * Internal Nadia cache files under ``NADIA_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under NADIA_HOME and the global Nadia
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.nadia/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Resolve BOTH the active NADIA_HOME (profile-aware) AND the global
    # Nadia root so credential stores at <root>/auth.json etc. are also
    # blocked when running under a profile (NADIA_HOME points at
    # <root>/profiles/<name> in profile mode). Same shape as the write
    # deny widening (#15981, #14157).
    nadia_dirs: list[Path] = []
    for base in (_nadia_home_path(), _nadia_root_path()):
        try:
            real = base.resolve()
            if real not in nadia_dirs:
    ...
    for hd in nadia_dirs:
        ...
        for blocked in blocked_dirs:
            try:
                ...
                continue
            return (
                f"Access denied: {path} is an internal Nadia cache file "
    ...
    for hd in nadia_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a Nadia credential store "
    ...
    for hd in nadia_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the Nadia MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a Nadia MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
...
#
# Nadia profiles are separate NADIA_HOME dirs under
...
#
# Reference: May 2026 incident where a nadia-security profile session
# edited skills under both ``~/.nadia/profiles/nadia-security/skills/``
# AND ``~/.nadia/skills/`` (the default profile's skills) without realizing
# the second path belonged to a different profile.
# ---------------------------------------------------------------------------

# Profile-scoped directories under NADIA_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from NADIA_HOME.
    ...
    """
    try:
        home_real = _nadia_home_path().resolve()
        root_real = _nadia_root_path().resolve()
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside Nadia scope, or is inside
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    Nadia entirely. Caller is expected to surface the warning to the
    ...
    return (
        ...
        f"belongs to Nadia profile {info['target_profile']!r}, but the "
...
#
#   <NADIA_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.nadia/...
#
...
# ``…/sandboxes/<backend>/<task>/home/.nadia/…`` segment and warns
# regardless of which Nadia profile is active. It does NOT cover the
# inner-container case where the bind mount strips the ``sandboxes/`` prefix
# (the agent's view inside the container is plain ``/root/.nadia/...``);
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    """Return the index of the inner ``.nadia`` part in a sandbox-mirror path.

    Matches ``…/sandboxes/<backend>/<task>/home/.nadia/…`` and returns the
    index where the inner Nadia-state portion starts. Returns ``None`` for
    paths that do not contain the sandbox-mirror shape.
    """
    for i, part in enumerate(parts):
        if part != "sandboxes":
            continue
        # Need at least: sandboxes / <backend> / <task> / home / .nadia / <thing>
        if i + 5 >= len(parts):
            continue
        if parts[i + 3] == "home" and parts[i + 4] == ".nadia":
            return i + 4
    return None


def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative Nadia state.

    Returns ``None`` when the path does not match the sandbox-mirror shape.
    Otherwise returns a dict with:

      * ``target_path``: the resolved path string
      * ``mirror_root``: the ``…/sandboxes/<backend>/<task>/home/.nadia``
        prefix (so callers can show users which sandbox owns the mirror)
      * ``inner_path``: the portion under the mirror's ``.nadia`` (what the
        agent likely meant to address on the host)

    Detection is path-shape-only — does not require any Nadia resolver to
    succeed, so it works correctly even when called from contexts where
    NADIA_HOME resolution would be ambiguous.
    """
    try:
        target = Path(os.path.expanduser(str(path))).resolve()
    except (OSError, RuntimeError):
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"Writes here land on a copy that the host Nadia process never "
        f"reads — the authoritative file is likely {info['inner_path']!r} "
        f"under the real NADIA_HOME. Use the host-side tool for "
...
# full ``…/sandboxes/<backend>/<task>/home/.nadia/…`` prefix on the host.
# But when file tools execute *inside* the container the bind-mount strips
# that prefix: the agent sees plain ``/root/.nadia/…``.  The root:root
...
def classify_container_mirror_target(
    ...
    under the mirror prefix. Otherwise returns:
      ...
      * ``inner_path``: portion under the mirror root (what the agent
        likely meant to address in the host NADIA_HOME)
    """
...
def get_container_mirror_warning(
    ...
    sandbox mirror of authoritative Nadia state.
    ...
    return (
        ...
        f"bind-mounted home — a per-task mirror that the host Nadia "
        f"process never reads. The authoritative file is "
        f"{info['inner_path']!r} under the real NADIA_HOME. Use the "
