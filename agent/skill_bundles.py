...
Storage
-------
Bundles live in ``~/.nadia/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``NADIA_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``nadia bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under NADIA_HOME.

    Honors ``NADIA_BUNDLES_DIR`` for tests; falls back to
    ``<NADIA_HOME>/skill-bundles``.
    """
    override = os.environ.get("NADIA_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_nadia_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `nadia bundles` CLI subcommand.
# ---------------------------------------------------------------------------
