...
row taxonomy is inferred from the atlas shape so Nadia can render both legacy
...
# rows (1536x1872), while older Nadia/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Nadia's activity state names. They are not always identical to the
...
# Legacy Nadia/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Nadia activity names -> accepted row-name aliases in descending
