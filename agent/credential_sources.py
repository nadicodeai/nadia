"""Unified removal contract for every credential source Nadia reads from.

Nadia seeds its credential pool from many places:

    env:<VAR>     — os.environ / ~/.nadia/.env
    claude_code   — ~/.claude/.credentials.json
    nadia_pkce   — ~/.nadia/.anthropic_oauth.json
    device_code   — auth.json providers.<provider> (nadia, openai-codex, ...)
    qwen-cli      — ~/.qwen/oauth_creds.json
    gh_cli        — gh auth token
    config:<name> — custom_providers config entry
    model_config  — model.api_key when model.provider == "custom"
    manual        — user ran `nadia auth add`
...
unify here is **removal**:
    ...
    ``nadia auth remove <provider> <N>`` must make the pool entry stay gone.

Before this module, every source had an ad-hoc removal branch in
``auth_remove_command``, and several sources had no branch at all — so
``auth remove`` silently reverted on the next ``load_pool()`` call for
qwen-cli, nadia device_code (partial), nadia_pkce, copilot gh_cli, and
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.nadia/.env  → clear it
    ...
    """
    from nadia_cli.config import get_env_path, remove_env_value
    ...
    if shell_exported:
        result.hints.extend([
            ...
            "launchd plist, etc.) or it will keep being visible to Nadia.",
            f"  The pool entry is now suppressed — Nadia will ignore "
            f"{env_var} until you run `nadia auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    work.  We just suppress it so Nadia stops reading it.
    """
    return RemovalResult(hints=[
        "Suppressed claude_code credential — it will not be re-seeded.",
        "Note: Claude Code credentials still live in ~/.claude/.credentials.json",
        "Run `nadia auth add anthropic` to re-enable if needed.",
    ])


def _remove_nadia_pkce(provider: str, removed) -> RemovalResult:
    """~/.nadia/.anthropic_oauth.json is ours — delete it outright."""
    from nadia_constants import get_nadia_home

    result = RemovalResult()
    oauth_file = get_nadia_home() / ".anthropic_oauth.json"
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared Nadia Anthropic OAuth credentials")
...
def _remove_nous_device_code(provider: str, removed) -> RemovalResult:
    """Nadia OAuth lives in auth.json providers.nous — clear it and suppress.

    We suppress in addition to clearing because nothing else stops a future
    `nadia auth add nous` (or any other path that writes providers.nous)
    from re-seeding before the user has decided to.  Suppression forces
    them to go through `nadia auth add nous` to re-engage, which is the
...
def _remove_minimax_oauth(provider: str, removed) -> RemovalResult:
    ...
    Same pattern as Nadia: single-source OAuth state with refresh tokens.
...
def _remove_xai_oauth_loopback_pkce(provider: str, removed) -> RemovalResult:
    ...
    Without this step, ``nadia auth remove xai-oauth <N>`` silently undoes
    ...
    Belt-and-braces against the manual entry path: ``nadia auth add
    ...
    if _clear_auth_store_provider(provider):
        result.cleaned.append(f"Cleared {provider} OAuth tokens from auth store")
    result.hints.append(
        "Run `nadia model` → xAI Grok OAuth (SuperGrok / Premium+) to re-authenticate if needed."
    )
    return result
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    the Nadia auth store is not enough — _seed_from_singletons() would
    ...
    via ``nadia auth add openai-codex``), but in both cases the re-seed
    ...
    """
    from nadia_cli.auth import suppress_credential_source
    ...
    result.hints.extend([
        ...
        "Run `nadia auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `nadia auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    Nadia stops picking the token up.
    """
    ...
    return RemovalResult(hints=[
        ...
        "Run `nadia auth add copilot` to re-enable if needed.",
    ])
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        provider="anthropic", source_id="nadia_pkce",
        remove_fn=_remove_nadia_pkce,
        description="~/.nadia/.anthropic_oauth.json",
    ))
    register(RemovalStep(
