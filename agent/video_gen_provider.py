...
as ``kind: backend``) or ``~/.nadia/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``nadia tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$NADIA_HOME/cache/videos/``, creating parents as needed."""
    from nadia_constants import get_nadia_home

    path = get_nadia_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$NADIA_HOME/cache/videos/``.
