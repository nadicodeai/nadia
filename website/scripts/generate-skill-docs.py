...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/nadicodeai/nadia/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    nadia_meta = (fm.get("metadata") or {}).get("nadia") or {}
    tags = nadia_meta.get("tags") or []
    related = nadia_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `nadia skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Nadia Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Nadia ships with a large built-in skill library copied into `~/.nadia/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Nadia also syncs bundled skills on `nadia update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.nadia/skills/` tree, it is still shipped with Nadia; restore it with `nadia skills reset <name> --restore`.",
        "",
