...
title: "Git Worktrees"
description: "Run multiple Nadia agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Nadia Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Nadia so each session has a clean, isolated working directory.

## Why Use Worktrees with Nadia?

Nadia treats the **current working directory** as the project root:

- CLI: the directory where you run `nadia` or `nadia chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.nadia/config.yaml`
...
git worktree add ../repo-feature feature/nadia-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/nadia-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Nadia there:

```bash
cd ../repo-feature

# Start Nadia in the worktree
nadia
```
...
git worktree add ../repo-experiment-a feature/nadia-a
git worktree add ../repo-experiment-b feature/nadia-b
```
...
- Works on its own branch (`feature/nadia-a` vs `feature/nadia-b`).
...
- Nadia checkpoint data under `~/.nadia/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Nadia experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/nadia-checkpoints-docs`, `feature/nadia-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Nadia from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `nadia -w` (Automatic Worktree Mode)

Nadia has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `nadia/nadia-<hash>`).
...
```bash
nadia -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `nadia -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Nadia session its own clean checkout.
