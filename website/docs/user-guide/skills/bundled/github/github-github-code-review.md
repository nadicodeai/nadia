...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Nadia Agent\",
    \"comments\": [
...
```bash
source "${NADIA_HOME:-$HOME/.nadia}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Nadia Agent. Code looks clean — good test coverage, no security concerns."
