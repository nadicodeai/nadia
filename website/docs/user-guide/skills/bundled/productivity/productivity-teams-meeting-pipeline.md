...
description: "Operate the Teams meeting summary pipeline via Nadia CLI — summarize meetings, inspect pipeline status, replay jobs, manage Microsoft Graph subscriptions"
---
...
Operate the Teams meeting summary pipeline via Nadia CLI — summarize meetings, inspect pipeline status, replay jobs, manage Microsoft Graph subscriptions.
...
Everything operator-facing is a `nadia teams-pipeline` subcommand run via the terminal tool. There are no new model tools for this pipeline — the CLI is the surface.
...
Before using the pipeline, verify these are set in `${NADIA_HOME:-~/.nadia}/.env`:
...
```bash
nadia teams-pipeline validate              # config snapshot — run first after any change
nadia teams-pipeline token-health          # Graph token status
nadia teams-pipeline token-health --force-refresh   # force a fresh token acquisition
nadia teams-pipeline list                  # recent meeting jobs
nadia teams-pipeline list --status failed  # only failed jobs
nadia teams-pipeline show <job-id>         # full detail of one job
nadia teams-pipeline subscriptions         # current Graph webhook subscriptions
```

### Re-running / debugging

```bash
nadia teams-pipeline run <job-id>          # replay a stored job (re-summarize, re-deliver)
nadia teams-pipeline fetch --meeting-id <id>   # dry-run: resolve meeting + transcript without persisting
nadia teams-pipeline fetch --join-web-url "<url>"   # dry-run by join URL
```

### Subscription management

```bash
nadia teams-pipeline subscribe \
  --resource communications/onlineMeetings/getAllTranscripts \
  --notification-url https://<your-public-host>/msgraph/webhook \
  --client-state "$MSGRAPH_WEBHOOK_CLIENT_STATE"

nadia teams-pipeline renew-subscription <sub-id> --expiration <iso-8601>
nadia teams-pipeline delete-subscription <sub-id>
nadia teams-pipeline maintain-subscriptions            # renew near-expiry ones
nadia teams-pipeline maintain-subscriptions --dry-run  # show what would be renewed
```
