...
4. Store the token in `~/.nadia/.env` (or via `nadia setup`):
   ```
...
- **`AIRTABLE_API_KEY` flows from `~/.nadia/.env` into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.
