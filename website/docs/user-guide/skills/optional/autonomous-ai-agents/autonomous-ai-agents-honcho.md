...
description: "Configure and use Honcho memory with Nadia -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with Nadia -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `nadia skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Nadia

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Nadia profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/hermes#running-honcho-locally-with-nadia

### Verify

```bash
nadia honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Nadia profile's AI peer
...
Honcho models conversations as interactions between **peers**. Nadia creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Nadia instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Nadia run |
| `global` | Single session across all directories |

Manual override: `nadia honcho map my-project-name`
...
Each Nadia profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block nadia.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `nadia.coder` host block in `honcho.json`
...
```bash
nadia honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Nadia when Honcho memory is active.
...
Config file: `$NADIA_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `nadia honcho setup`. Ensure `memory.provider: honcho` is in `~/.nadia/config.yaml`.

### Memory not persisting across sessions
Check `nadia honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `nadia profile create <name> --clone`. For existing profiles: `nadia honcho sync`.
...
| Command | Description |
|---------|-------------|
| `nadia honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `nadia honcho status` | Show resolved config, connection test, peer info for active profile |
| `nadia honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `nadia honcho disable` | Disable Honcho for the active profile |
| `nadia honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `nadia honcho peers` | Show peer identities across all profiles |
| `nadia honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `nadia honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `nadia honcho sessions` | List known directory-to-session-name mappings |
| `nadia honcho map <name>` | Map current working directory to a Honcho session name |
| `nadia honcho identity` | Seed AI peer identity or show both peer representations |
| `nadia honcho sync` | Create host blocks for all Nadia profiles that don't have one yet |
| `nadia honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Nadia + Honcho |
| `nadia memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `nadia memory status` | Show active memory provider and config |
| `nadia memory off` | Disable external memory provider |
