...
| | |
|---|---|
| Source | Optional — install with `nadia skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research/darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), Nadia Agent |
| License | MIT |
...
import upstream classes into Nadia itself.
...
```bash
mkdir -p ~/.nadia/cache/darwinian-evolver && cd ~/.nadia/cache/darwinian-evolver
...
```bash
cd ~/.nadia/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.nadia/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.nadia/skills/research/darwinian-evolver
DE_DIR=~/.nadia/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Nadia core.
   Custom driver scripts under `~/.nadia/skills/...` are user-side and fine.
