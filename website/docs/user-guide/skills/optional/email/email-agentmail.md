...
Give the agent its own dedicated email inbox via AgentMail. Send, receive, and manage email autonomously using agent-owned email addresses (e.g. nadia-agent@agentmail.to).

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `nadia skills install official/email/agentmail` |
...
Add to `~/.nadia/config.yaml` (paste your actual key — MCP env vars are not expanded from .env):
```yaml
mcp_servers:
  agentmail:
    command: "npx"
...
1. Create a dedicated inbox:
   - Use `create_inbox` with a username (e.g. `nadia-agent`)
   - The agent gets address: `nadia-agent@agentmail.to`
2. Send an email:
...
```
1. create_inbox (username: "nadia-outreach")
...
```
nadia --toolsets mcp -q "Create an AgentMail inbox called test-agent and tell me its email address"
```
