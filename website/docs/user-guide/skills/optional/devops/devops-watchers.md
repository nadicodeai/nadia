...
| | |
|---|---|
| Source | Optional — install with `nadia skills install official/devops/watchers` |
...
All three live in `$NADIA_HOME/skills/devops/watchers/scripts/` once the skill is installed. Each reads `WATCHER_STATE_DIR` (defaults to `$NADIA_HOME/watcher-state/`) for its state file, keyed by the `--name` argument.
...
```bash
python $NADIA_HOME/skills/devops/watchers/scripts/watch_rss.py \
...
python $NADIA_HOME/skills/devops/watchers/scripts/watch_github.py \
  --name nadia-issues --repo nadicodeai/nadia --scope issues
```

Poll an arbitrary JSON API:

```bash
python $NADIA_HOME/skills/devops/watchers/scripts/watch_http_json.py \
...
Every watcher writes `$NADIA_HOME/watcher-state/<name>.json`. Inspect:

```bash
cat $NADIA_HOME/watcher-state/hn.json
```

Force a replay (next run treated as first poll):

```bash
rm $NADIA_HOME/watcher-state/hn.json
```
...
4. **Putting the state dir where the agent's sandbox can't write.** `$NADIA_HOME/watcher-state/` is always writable. Docker/Modal backends may not see arbitrary host paths.
