...
| | |
|---|---|
| Source | Optional — install with `nadia skills install official/productivity/shopify` |
...
4. Save to `${NADIA_HOME:-~/.nadia}/.env`:
   ```
...
}' '{"input":{"title":"Test Hoodie","status":"DRAFT","vendor":"Nadia","productType":"Apparel","tags":["test"]}}'
```
