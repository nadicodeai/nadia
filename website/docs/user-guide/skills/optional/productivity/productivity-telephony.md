---
title: "Telephony — Give Nadia phone capabilities without core tool changes"
sidebar_label: "Telephony"
description: "Give Nadia phone capabilities without core tool changes"
---
...
Give Nadia phone capabilities without core tool changes. Provision and persist a Twilio number, send and receive SMS/MMS, make direct calls, and place AI-driven outbound calls through Bland.ai or Vapi.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `nadia skills install official/productivity/telephony` |
...
This optional skill gives Nadia practical phone capabilities while keeping telephony out of the core tool list.

It ships with a helper script, `scripts/telephony.py`, that can:
- save provider credentials into `${NADIA_HOME:-~/.nadia}/.env`
...
It does **not** turn Nadia into a real-time inbound phone gateway. Inbound SMS is handled by polling the Twilio REST API. That is enough for many workflows, including notifications and some one-time-code retrieval, without adding core webhook infrastructure.
...
4. Treat third-party phone numbers as sensitive operational data:
   - do not save them to Nadia memory
...
### 1) "I want Nadia to own a real phone number"
Use **Twilio**.
...
- pairs well with Nadia `text_to_speech` plus a public file host or tunnel

## Files and persistent state

The skill persists telephony state in two places:

### `${NADIA_HOME:-~/.nadia}/.env`
...
```bash
SCRIPT="$(find ~/.nadia/skills -path '*/telephony/scripts/telephony.py' -print -quit)"
```
...
```bash
nadia skills search telephony
nadia skills install official/productivity/telephony
```
...
Then save credentials into Nadia:
...
3. Buy it and save it into `${NADIA_HOME:-~/.nadia}/.env` + state:
```bash
python3 "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
```bash
python3 "$SCRIPT" twilio-call "+15551230000" --message "Hello! This is Nadia calling with your status update." --voice Polly.Joanna
```

### E. Call with a prerecorded / custom voice message

This is the main path for reusing Nadia's existing `text_to_speech` support.

Use this when:
- you want the call to use Nadia's configured TTS voice rather than Twilio `<Say>`
...
Recommended Nadia TTS -> Twilio Play workflow:

1. Generate the audio with Nadia `text_to_speech`.
2. Make the resulting MP3 publicly reachable.
3. Place the Twilio call with `--audio-url`.

Example agent flow:
- Ask Nadia to create the message audio with `text_to_speech`
...
Important note:
- Nadia TTS is great for prerecorded outbound messages
...
- Nadia STT/TTS alone is not being used here as a full duplex phone conversation engine; that would require a much heavier streaming/webhook integration than this skill is trying to introduce
...
7. Summarize the outcome without persisting third-party numbers to Nadia memory.
...
- Do not store arbitrary third-party phone numbers in Nadia memory.
...
3. persist that number to `${NADIA_HOME:-~/.nadia}/.env`
