...
| | |
|---|---|
| Source | Optional — install with `nadia skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${NADIA_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${NADIA_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${NADIA_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client nadia
```
...
```bash
PUBLISH="${NADIA_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client nadia
```
...
```bash
DRIVE="${NADIA_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `nadia`)    |
