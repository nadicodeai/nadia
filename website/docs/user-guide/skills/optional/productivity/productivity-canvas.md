...
| | |
|---|---|
| Source | Optional — install with `nadia skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Nadia Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `${NADIA_HOME:-~/.nadia}/.env`:
...
```bash
CANVAS="python $NADIA_HOME/skills/productivity/canvas/scripts/canvas_api.py"
