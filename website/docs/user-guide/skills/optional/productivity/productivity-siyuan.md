...
| | |
|---|---|
| Source | Optional — install with `nadia skills install official/productivity/siyuan` |
...
3. Store it in `${NADIA_HOME:-~/.nadia}/.env`:
   ```
...
```yaml
# In ~/.nadia/config.yaml under mcp_servers:
mcp_servers:
  siyuan:
    command: npx
