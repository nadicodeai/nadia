...
| | |
|---|---|
| Source | Optional — install with `nadia skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Nadia or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Nadia. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.nadia/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.nadia/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Nadia integration, either:

- configure the server in `~/.nadia/config.yaml` using the `native-mcp` skill, or
...
### Nadia cannot see the deployed server

The server-building part may be correct while the Nadia config is not. Load the `native-mcp` skill and configure the server in `~/.nadia/config.yaml`, then restart Nadia.
