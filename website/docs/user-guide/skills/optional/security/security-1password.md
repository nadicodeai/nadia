...
| | |
|---|---|
| Source | Optional — install with `nadia skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Nadia Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Nadia terminal calls (desktop app flow only)
...
### Service Account (recommended for Nadia)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${NADIA_HOME:-~/.nadia}/.env` (the skill will prompt for this on first load).
...
## Nadia Execution Pattern (desktop app flow)

Nadia terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/nadia-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/nadia-op.sock"
