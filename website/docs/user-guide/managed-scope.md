...
`~/.nadia/config.yaml`, `~/.nadia/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/nadia`:

```text
/etc/nadia/
├── config.yaml     # managed config layer (wins over ~/.nadia/config.yaml)
└── .env            # managed env layer (wins over ~/.nadia/.env + shell)
```
...
The location can be relocated with the `NADIA_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `NADIA_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by Nadia.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export NADIA_MANAGED_DIR=/opt/org/nadia-policy
```

:::warning
A user who can set `NADIA_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `nadia doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/nadia/config.yaml` (managed) | `/etc/nadia/.env` (managed) |
| 2 | `~/.nadia/config.yaml` (user) | `~/.nadia/.env` (user) |
...
```bash
nadia config        # shows a header naming the managed source + the pinned keys
nadia doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, Nadia refuses and names the source:

```bash
$ nadia config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/nadia/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `nadia config set` / setup will not write
a user value for an env key pinned by the managed `.env`.

## Setting up a managed scope (administrators)

```bash
sudo mkdir -p /etc/nadia

# Pin some config values for every user on this machine
sudo tee /etc/nadia/config.yaml >/dev/null <<'YAML'
model:
  provider: nous
security:
  redact_secrets: true
...
sudo tee /etc/nadia/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/nadia
sudo chmod 0644 /etc/nadia/config.yaml /etc/nadia/.env
```

Changes take effect on the next Nadia start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`nadia doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs Nadia as `root`), managed scope is advisory.
