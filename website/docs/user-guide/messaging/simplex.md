...
`simplex:group:12` as a cron `deliver=` target or in a `nadia send` call.
...
Or target a specific contact via the cron job's `deliver:` field, or from a shell script with the [`nadia send` CLI](/guides/pipe-script-output):

```bash
nadia send simplex:<contact-id> "Done!"
```
