...
Connect Nadia to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Nadia — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Nadia runs it in a
...
nadia photon setup --phone +15551234567
```
...
2. **Finds or creates** the `Nadia Agent` project on your account.
...
Runtime credentials are written to `~/.nadia/.env`
...
(device token, dashboard project id) lives in `~/.nadia/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other Nadia
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Nadia replies with a pairing code. Approve it with:

```bash
nadia pairing approve photon <CODE>
```

Use `nadia pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.nadia/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.nadia/.env`):
...
By default Nadia responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Nadia` and
`@Nadia agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and Nadia will reply.
...
assigned iMessage line Nadia uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run nadia photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `nadia photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `nadia photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `nadia photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** Nadia sends images, voice
...
| `PHOTON_MENTION_PATTERNS` | Nadia wake words  | JSON list / comma / newline regex patterns for group mentions |
