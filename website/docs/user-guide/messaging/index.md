...
description: "Chat with Nadia from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, Microsoft Teams, LINE, Raft, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with Nadia from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, BlueBubbles (iMessage), Microsoft Teams, LINE, ntfy, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/user-guide/features/voice-mode) and [Use Voice Mode with Nadia](/guides/use-voice-mode-with-nadia).

:::tip
Bots need both a model provider and tool providers (TTS, web). A [Nadia Agents Portal](/integrations/nadia-portal) subscription bundles all of them.
:::
...
```mermaid
flowchart TB
    subgraph Gateway["Nadia Gateway"]
...
For group chats, hooks, and automation flows, Nadia supports explicit silence tokens. If the agent's final response is exactly one supported token, the gateway suppresses outbound delivery and sends nothing to the chat.
...
Silence is a delivery decision only. Nadia keeps the assistant silence turn in the session transcript, so the conversation still alternates normally:
...
Failed turns still surface as errors; Nadia does not hide failures just because the text resembles a silence token.
...
```bash
nadia gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
nadia gateway              # Run in foreground
nadia gateway setup        # Configure messaging platforms interactively
nadia gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo nadia gateway install --system   # Linux only: install a boot-time system service
nadia gateway start        # Start the default service
nadia gateway stop         # Stop the default service
nadia gateway status       # Check default service status
nadia gateway status --system         # Linux only: inspect the system service explicitly
```
...
| `/update` | Update Nadia Agent to the latest version |
...
Configure per-platform overrides in `~/.nadia/gateway.json`:
...
nadia pairing approve telegram XKGH5N7P

# Other pairing commands:
nadia pairing list          # View pending + approved users
nadia pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, Nadia appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.nadia/config.yaml`:
...
Off by default. When enabled, Nadia prepends a human-readable timestamp
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.nadia/config.yaml`:
...
```bash
NADIA_BACKGROUND_NOTIFICATIONS=result
```
...
```bash
nadia gateway install               # Install as user service
nadia gateway start                 # Start the service
nadia gateway stop                  # Stop the service
nadia gateway status                # Check status
journalctl --user -u nadia-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo nadia gateway install --system
sudo nadia gateway start --system
sudo nadia gateway status --system
journalctl -u nadia-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

:::danger Don't add a custom `ExecStopPost` kill drop-in
The unit Nadia installs already shuts the gateway down cleanly with `KillMode=mixed` + `KillSignal=SIGTERM`, and uses `Restart=always` with `RestartForceExitStatus` so updates and `/restart` respawn correctly. Do **not** add a systemd drop-in such as `ExecStopPost=/bin/kill -9 $MAINPID` — `ExecStopPost` fires on *every* stop, including clean restarts, so it `SIGKILL`s the freshly spawned instance before it stabilizes and `Restart=always` immediately respawns it. The result is an infinite restart loop (and, on Telegram, a flood of restart messages). If you've added such a drop-in, remove it: `systemctl --user edit nadia-gateway` (or `sudo systemctl edit nadia-gateway` for a system service) and delete the `ExecStopPost` line, then `systemctl --user daemon-reload`.
:::

:::tip Headless VMs: user service + linger avoids root prompts
A system service needs root for every restart — including the automatic gateway restart at the end of `nadia update`. When `nadia update` runs as a non-root user, it tries passwordless `sudo systemctl`; if that's unavailable, it skips the restart and prints the manual `sudo systemctl restart nadia-gateway` command (it never blocks on an interactive password prompt).
...
```bash
nadia gateway install          # user service
sudo loginctl enable-linger $USER   # one-time: start at boot, survive logout
```

After that, `nadia update` can restart the gateway without any privileges. If you prefer to keep the system service, either run updates with `sudo nadia update`, or grant the service account passwordless sudo for systemctl, e.g. in `sudo visudo -f /etc/sudoers.d/nadia-gateway`:

```
nadia ALL=(root) NOPASSWD: /usr/bin/systemctl --no-ask-password reset-failed nadia-gateway*, /usr/bin/systemctl --no-ask-password start nadia-gateway*, /usr/bin/systemctl --no-ask-password restart nadia-gateway*
```
:::

Avoid keeping both the user and system gateway units installed at once unless you really mean to. Nadia will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple Nadia installations on the same machine (with different `NADIA_HOME` directories), each gets its own systemd service name. The default `~/.nadia` uses `nadia-gateway`; other installations use `nadia-gateway-<hash>`. The `nadia gateway` commands automatically target the correct service for your current `NADIA_HOME`.
:::

### macOS (launchd)

```bash
nadia gateway install               # Install as launchd agent
nadia gateway start                 # Start the service
nadia gateway stop                  # Stop the service
nadia gateway status                # Check status
tail -f ~/.nadia/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.nadia.gateway.plist`. It includes three environment variables:
...
- **NADIA_HOME** — scopes the gateway to your Nadia installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `nadia gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `NADIA_HOME` directory gets its own launchd label. The default `~/.nadia` uses `ai.nadia.gateway`; other installations use `ai.nadia.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `nadia-cli` | Full access |
| Telegram | `nadia-telegram` | Full tools including terminal |
| Discord | `nadia-discord` | Full tools including terminal |
| WhatsApp | `nadia-whatsapp` | Full tools including terminal |
| WhatsApp Cloud API | `nadia-whatsapp` | Full tools including terminal (shares toolset with the Baileys bridge) |
| Slack | `nadia-slack` | Full tools including terminal |
| Google Chat | `nadia-google_chat` | Full tools including terminal |
| Signal | `nadia-signal` | Full tools including terminal |
| SMS | `nadia-sms` | Full tools including terminal |
| Email | `nadia-email` | Full tools including terminal |
| Home Assistant | `nadia-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `nadia-mattermost` | Full tools including terminal |
| Matrix | `nadia-matrix` | Full tools including terminal |
| BlueBubbles | `nadia-bluebubbles` | Full tools including terminal |
| Microsoft Teams | `nadia-teams` | Full tools including terminal |
| API Server | `nadia-api-server` | Full tools (drops `clarify`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `nadia-webhook` | Full tools including terminal |
| Raft | `nadia-raft` | Wake-only channel; agent uses Raft CLI for message I/O |
...
1. **Gateway log** (`~/.nadia/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
