...
sidebar_position: 12
title: "Google Chat"
description: "Set up Nadia Agent as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect Nadia Agent to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Nadia
...
> Run `nadia gateway setup` and pick **Google Chat** for a guided walk-through.
...
download the file. Save it somewhere only Nadia can read (e.g.,
`~/.nadia/google-chat-sa.json`, `chmod 600`).
...
- Topic ID: `nadia-chat-events`
- Leave the defaults for everything else.

After creation, the topic's detail page has a **Subscriptions** tab. Create one:

- Subscription ID: `nadia-chat-events-sub`
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a nadia restart)
...
- Principal: `nadia-chat-bot@<your-project>.iam.gserviceaccount.com`
- Role: `Pub/Sub Subscriber`

Also grant `Pub/Sub Viewer` on the same subscription — Nadia calls
...
- **App name**: whatever you want users to see ("Nadia" is reasonable).
...
- **Connection settings**: select **Cloud Pub/Sub**, enter the topic name
  `projects/<your-project>/topics/nadia-chat-events`.
...
`ADDED_TO_SPACE` event that Nadia uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Nadia

Add the Google Chat section to `~/.nadia/.env`:

```bash
# Required
GOOGLE_CHAT_PROJECT_ID=my-chat-bot-123
GOOGLE_CHAT_SUBSCRIPTION_NAME=projects/my-chat-bot-123/subscriptions/nadia-chat-events-sub
GOOGLE_CHAT_SERVICE_ACCOUNT_JSON=/home/you/.nadia/google-chat-sa.json
...
Install the dependencies the Google Chat adapter needs (no Nadia extra is currently published — install them directly):
...
Send "hola" in the test DM. The bot posts a "Nadia is thinking…" marker, then
...
Thread support: when a user replies inside a thread, Nadia detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate Nadia session.
...
3. Download the JSON. Move it onto the host that runs Nadia.
4. Register the client with Nadia (run under the profile you want it scoped to):
...
nadia -p <profile> python -m plugins.platforms.google_chat.oauth \
    --client-secret /path/to/client_secret.json
```

That writes the client secret into the active profile's Nadia home (e.g.
`~/.nadia/google_chat_user_client_secret.json` for the default profile). The
...
The token lands at `~/.nadia/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.nadia/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Nadia isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `nadia gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: Nadia will only attach the SA bearer
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.nadia/google_chat_user_tokens/<sanitized_email>.json` (filesystem
