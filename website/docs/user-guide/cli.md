...
:::tip
Nadia also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `nadia --tui` — see the [TUI](tui.md) guide.
:::
...
nadia -s nadia-agent-dev,github-auth
nadia chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
nadia --continue             # Resume the most recent CLI session (-c)
nadia --resume <session_id>  # Resume a specific session by ID (-r)

# Verbose mode (debug output)
nadia chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
nadia -w                         # Interactive mode in worktree
nadia -w -z "Fix issue #123"     # Single query in worktree
```
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `NADIA_YOLO_MODE` is on (either `nadia --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend Nadia to background (Unix only). Run `fg` in the shell to resume. |
...
Type `/` to see the autocomplete dropdown. Nadia supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for Nadia replies |
...
```bash
nadia -s nadia-agent-dev,github-auth
nadia chat -s github-pr-workflow -s github-auth
```

Nadia loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.nadia/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.nadia/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain Nadia..."
  # Add your own!
```
...
Most terminals send the same byte sequence for `Enter` and `Shift+Enter` by default, so applications cannot distinguish them. Nadia recognises `Shift+Enter` only when the terminal sends a distinct sequence via the [Kitty keyboard protocol](https://sw.kovidgoyal.net/kitty/keyboard-protocol/) or xterm's `modifyOtherKeys` mode.
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work everywhere. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches Nadia — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The very first time you press Enter while Nadia is working, Nadia prints a one-line reminder explaining the `/busy` knob (`"(tip) Your message interrupted the current run…"`). It only fires once per install — a flag in `config.yaml` under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend Nadia to the background — just like any terminal process. The shell prints a confirmation:

```
Nadia Agent has been suspended. Run `fg` to bring Nadia Agent back.
```
...
Resume this session with:
  nadia --resume 20260225_143052_a1b2c3
...
```bash
nadia --continue                          # Resume the most recent CLI session
nadia -c                                  # Short form
nadia -c "my project"                     # Resume a named session (latest in lineage)
nadia --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
nadia --resume "refactoring auth"         # Resume by title
nadia -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `nadia sessions rename <id> <title>` from the command line. Use `nadia sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in Nadia's SQLite state database under `~/.nadia/state.db`. The database keeps:
...
Nadia immediately confirms the task and gives you back the prompt:
