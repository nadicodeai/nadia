...
If you only run one Nadia agent, you don't need this page — see
[Profiles](./profiles.md) for the basics.

## When to use this

You want this setup when you have two or more Nadia agents that should all
...
(`ai.nadia.gateway-<name>.plist`) or systemd user service
(`nadia-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
nadia profile create coder
nadia profile create personal-bot
nadia profile create research
...
```bash
nadia config set gateway.multiplex_profiles true
nadia gateway restart
```

Equivalently, in the default profile's `~/.nadia/config.yaml`:
...
You do **not** run `nadia gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `nadia gateway start` / `run` is a
...
`msgraph_webhook`, `bluebubbles`, `sms`. Configure
...
home). `nadia status` reports the multiplexer and the profiles it serves;
`nadia status -p <name>` slices to one profile. Each profile still writes its
...
`~/.local/bin/nadia-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: nadia-gateways {start|stop|restart|status|list}"
}
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    nadia gateway "$action"
  else
    nadia -p "$profile" gateway "$action"
  fi
}
...
```bash
nadia-gateways start      # start every configured profile
nadia-gateways stop       # stop every configured profile
nadia-gateways restart    # restart all
nadia-gateways status     # status across all
nadia-gateways list       # delegates to `nadia gateway list`
```

:::tip
The `default` profile is targeted with `nadia gateway <action>` (no `-p`),
not `nadia -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `nadia -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.nadia.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/nadia-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.nadia.gateway.plist` /
`nadia-gateway.service`.
...
```bash
# Default profile
tail -f ~/.nadia/logs/gateway.log
tail -f ~/.nadia/logs/gateway.error.log

# Named profile
tail -f ~/.nadia/profiles/<name>/logs/gateway.log
tail -f ~/.nadia/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.nadia/logs/gateway.log ~/.nadia/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
nadia logs -f                  # follow default profile
nadia -p coder logs -f         # follow one profile
nadia logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
nadia profile list             # profiles + model + gateway state
nadia-gateways status          # full status across every profile
launchctl list | grep nadia    # macOS — PIDs and labels
systemctl --user list-units 'nadia-gateway-*'   # Linux — units
```
...
The default profile uses `~/.nadia/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
nadia config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.nadia/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=nadia --why="gateways running" \
  sleep infinity &
...
`nadia-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.nadia/.env ~/.nadia/profiles/*/.env
```

## Updating the code

`nadia update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `nadia gateway start` after a previous `nadia gateway stop`. The
...
```bash
ps -ef | grep "nadia_cli.*-p <profile>"
cat ~/.nadia/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.nadia.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.nadia.gateway-<profile>.plist

# Linux
systemctl --user restart nadia-gateway-<profile>.service
```

### Health check

```bash
nadia doctor                  # default profile
nadia -p <profile> doctor     # one profile
```
