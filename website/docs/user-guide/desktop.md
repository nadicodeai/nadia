...
The packaged app ships the Electron shell and a native React chat surface. On first launch it can install the Nadia Agent runtime into `NADIA_HOME` (`~/.nadia`, or `%LOCALAPPDATA%\nadia` on Windows) — **the same layout a CLI install uses**, which is why the two are interchangeable. Backend resolution first honours `NADIA_DESKTOP_NADIA_ROOT`, then a completed managed install, then a probed `nadia` on `PATH` (unless `--ignore-existing` / `NADIA_DESKTOP_IGNORE_EXISTING=1` is set), and finally an explicit `NADIA_DESKTOP_NADIA` command override for packagers such as Nix. The React renderer talks to a headless backend the app launches for you — a `nadia serve` process that serves the `tui_gateway` JSON-RPC/WebSocket API — and reuses the agent runtime rather than embedding `nadia --tui`. The desktop app is **self-contained**: it runs its own `nadia serve` backend and never opens or requires the [web dashboard](./features/web-dashboard.md). (Runtimes older than the `serve` command fall back to a headless `dashboard --no-open` automatically, so an app update never outruns its backend.) Install, backend-resolution, and self-update logic live in the Electron main process.
...
- [TUI](./tui.md) — the modern terminal UI used by `nadia --tui` and the dashboard chat tab
