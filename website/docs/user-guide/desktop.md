...
sidebar_position: 3
title: "Desktop App"
description: "The native Nadia desktop app — a polished experience for chatting with Nadia, with streaming tool output, side-by-side previews, a file browser, voice, cron, profiles, skills, and settings. macOS, Windows, and Linux."
---

# Desktop App

The Nadia desktop app is a native app built around the **same** agent you get from the CLI and the gateway — same config, same API keys, same sessions, same skills, same memory. It is not a separate product or a lightweight clone; it uses the same Nadia Agent core and settings, and drives it through a modern & thoughtfully designed UI. If you have used `nadia` in a terminal, everything you set up there is already here, and anything you do here shows up there.

It runs on **macOS, Windows, and Linux**.

:::tip Which interface is which?
Nadia has several front ends that all talk to the same agent:

- **Desktop App** (this page) — a native application with a purpose-built UI for chat, configuration, and management.
- **CLI** (`nadia`) and **[TUI](./tui.md)** (`nadia --tui`) — terminal interfaces.
- **[Web Dashboard](./features/web-dashboard.md)** (`nadia dashboard`) — a browser admin panel; its optional **Chat** tab embeds the TUI through a pseudo-terminal.
...
Follow the [installation instructions for Nadia Desktop](../getting-started/installation.md).

If you already have Nadia installed, simply run
...
- **The same conversation history** as every other Nadia surface — sessions started here resume in the CLI/TUI and vice versa.
...
Explore and preview the working directory without leaving the app — useful for following along as the agent reads, writes, and edits files. Set the initial project directory with `nadia desktop --cwd <path>` (or the `NADIA_DESKTOP_CWD` environment variable).

### Voice

Talk to Nadia and hear it back, the same [voice mode](./features/voice-mode.md) available elsewhere. On macOS the OS will prompt once for microphone access.
...
The app also surfaces the broader Nadia management surface so you don't have to drop to a terminal:

- **Skills** — browse, install, and manage [skills](./features/skills.md).
- **Cron** — view and manage [scheduled jobs](../reference/cli-commands.md#nadia-cron).
- **Profiles** — switch between [Nadia profiles](./profiles.md) (isolated config/skills/sessions).
...
The [manual update process](https://docs.nadicode.ai/nadia/getting-started/updating) also works with the GUI.

## Uninstalling

Open **Settings → About → Danger zone** and pick how much to remove:

- **Uninstall Chat GUI only** — removes the desktop app and its data; the Nadia agent, your config, and your chats stay. (Same as `nadia uninstall --gui`.)
- **Uninstall GUI + agent, keep my data** — removes the app and the agent but keeps config, chats, and secrets for a future reinstall. (Same as `nadia uninstall`.)
- **Uninstall everything** — removes the app, the agent, and all user data. (Same as `nadia uninstall --full`.)
...
You can do the same from the terminal — `nadia uninstall --gui` for the GUI alone, or `nadia uninstall` / `nadia uninstall --full` for the agent too.

:::note
Running `nadia uninstall --gui` from a **source checkout** (a `nadia desktop` dev build) also removes the workspace `node_modules` and `apps/desktop/{dist,release}` build output, since those are GUI build artifacts. They're recoverable with `nadia desktop` (or `npm install` + a rebuild) — but if you're actively hacking on the desktop app, expect to reinstall dependencies afterward.
:::

## CLI reference: `nadia desktop`

To launch via the CLI, simply run `nadia desktop`. By default it installs workspace Node dependencies, builds the current OS's unpacked Electron app, then launches that packaged artifact.
...
| `--build-only`       | Build the desktop app but do not launch it (used by `nadia update`)                      |
| `--source`           | Launch via `electron .` against `apps/desktop/dist` instead of the packaged app           |
| `--cwd PATH`         | Initial project directory for desktop chat sessions (sets `NADIA_DESKTOP_CWD`)           |
| `--nadia-root PATH` | Override the Nadia source root the app uses (sets `NADIA_DESKTOP_NADIA_ROOT`)          |
| `--ignore-existing`  | Force the app to ignore any `nadia` CLI already on `PATH` during backend resolution      |
| `--fake-boot`        | Enable deterministic boot delays for validating the startup UI                            |

## How it works

The packaged app ships only the Electron shell. On first launch it installs the Nadia Agent runtime into `NADIA_HOME` (`~/.nadia`, or `%LOCALAPPDATA%\nadia` on Windows) — **the same layout a CLI install uses**, which is why the two are interchangeable. The React renderer talks to a `nadia dashboard` backend over the standard gateway APIs and reuses the agent rather than reimplementing it. Install, backend-resolution, and self-update logic live in the Electron main process.
...
:::info The remote backend is a running `nadia dashboard` process
"Remote backend" means a **`nadia dashboard`** server running on the remote machine — that is the process the desktop app connects to. Nothing in this section works unless that dashboard is actually up and reachable. The desktop app does not start it for you; you (or a `systemd` service) keep `nadia dashboard` running on the remote host, and the app attaches to it. If you also use messaging channels (Telegram, Discord, etc.), the **gateway** is a *separate* long-running process you start independently — see the note after the setup steps.
:::
...
- **OAuth (Nadia Agents Portal) — preferred for anything reachable beyond your own machine.** Logins are verified against your Nadia account, so this is the option suitable for a VPS, a public host, or any remote backend. Register the dashboard with `nadia dashboard register` (or the Portal [`/local-dashboards`](https://portal.nadicode.ai/local-dashboards) page) to provision its OAuth client, then sign in from the app with **Sign in with NadicodeAI**. A self-hosted OIDC provider works the same way if you run your own identity provider.
...
The rest of this section shows the username/password path because it's the quickest to stand up on a trusted network; for the OAuth path see [Web Dashboard → Default provider: NadicodeAI](./features/web-dashboard.md#default-provider-nous-research).

### On the backend (the remote machine)

Set a username and password, then start the dashboard bound to a reachable address. The credentials live in `~/.nadia/.env` (the secrets file, mode 0600):
...
Keep that `nadia dashboard` process running for as long as you want the desktop app to be able to connect — if it stops, the app can no longer reach the backend. Run it under `systemd`, `tmux`, or your process manager of choice so it survives logout and reboots.
...
Running the dashboard as a systemd service? Give the unit `EnvironmentFile=%h/.nadia/.env` so the credentials are in the environment at boot.
...
Boot logs land in `NADIA_HOME/logs/desktop.log` (it includes backend output and recent Python tracebacks) — check it first if the app reports a boot failure. You can also tail it from the CLI:
...
tccutil reset Microphone com.nadicode.nadia
```
...
ELECTRON_MIRROR=https://npmmirror.com/mirrors/electron/ \
  bash -c 'cd "$HOME/.nadia/nadia-agent/apps/desktop" && CSC_IDENTITY_AUTO_DISCOVERY=false npm run pack'
```
