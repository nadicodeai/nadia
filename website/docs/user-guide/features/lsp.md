...
Nadia runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Nadia
...
1. Nadia captures a baseline of current diagnostics for the file.
...
…). Nadia auto-detects the binary on PATH or in
`<NADIA_HOME>/lsp/bin/`.
...
2. Download the latest release zip from
   ...
   and extract it.
3. Point Nadia at the extracted bundle — the directory that contains
   `PowerShellEditorServices/Start-EditorServices.ps1`. Either:
   - set `lsp.servers.powershell.command: ["/path/to/bundle"]` in
     `config.yaml`, or
   - extract it to `<NADIA_HOME>/lsp/PowerShellEditorServices`, or
   - export `PSES_BUNDLE_PATH=/path/to/bundle`.

`nadia lsp status` reports `installed` once `pwsh` is found; if the
...
`node_modules` tree — Nadia installs both packages together when you
run `nadia lsp install typescript` or auto-install fires on first
use.

## CLI

```
nadia lsp status          # service state + per-server install status
nadia lsp list            # registry, optionally --installed-only
nadia lsp install <id>    # eagerly install one server
nadia lsp install-all     # try every server with a known recipe
nadia lsp restart         # tear down running clients
nadia lsp which <id>      # print resolved binary path
```

`nadia lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <NADIA_HOME>/lsp/bin
...
When `install_strategy: auto`, Nadia installs binaries into
`<NADIA_HOME>/lsp/bin/`. NPM packages land in
`<NADIA_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Nadia-owned and is
...
Servers are kept alive for the life of the Nadia process. There's
...
**`nadia lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<NADIA_HOME>/lsp/bin/`. Run
`nadia lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `nadia lsp status`**
...
When `nadia lsp status` shows a `Backend warnings` section, install
...
`~/.nadia/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.nadia/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `nadia lsp restart` to clear the set;
