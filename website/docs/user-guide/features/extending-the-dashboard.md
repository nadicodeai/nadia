...
description: "Build themes and plugins for the Nadia web dashboard — palettes, typography, layouts, custom tabs, shell slots, page-scoped slots, and backend API routes"
---

# Extending the Dashboard

The Nadia web dashboard (`nadia dashboard`) is built to be reskinned and extended without forking the codebase. Three layers are exposed:

1. **Themes** — YAML files that repaint the dashboard's palette, typography, layout, and per-component chrome. Drop a file in `~/.nadia/dashboard-themes/`; it appears in the theme switcher.
...
Themes are YAML files stored in `~/.nadia/dashboard-themes/`. The file name doesn't matter (the theme's `name:` field is what the system uses), but convention is `<name>.yaml`. Every field is optional — missing keys fall back to the built-in `default` theme, so a theme can be as small as one color.

### Quick start — your first theme

```bash
mkdir -p ~/.nadia/dashboard-themes
```

```yaml
# ~/.nadia/dashboard-themes/neon.yaml
name: neon
label: Neon
...
The CSS is injected as a single scoped `<style data-nadia-theme-css>` tag on theme apply and cleaned up on theme switch. **Capped at 32 KiB per theme.**
...
|-------|---------|------------|--------|
| **Nadia Teal** (`default`) | Dark teal + cream | System stack, 15px | 0.5rem radius, comfortable |
| **Nadia Teal (Large)** (`default-large`) | Same as default | System stack, 18px, line-height 1.65 | 0.5rem radius, spacious |
...
Themes that reference Google Fonts (all except Nadia Teal) load the stylesheet on demand — the first time you switch to them a `<link>` tag is injected into `<head>`.
...
```yaml
# ~/.nadia/dashboard-themes/ocean.yaml
name: ocean
label: Ocean Deep
...
A reference plugin (`example-dashboard` in [`nadia-example-plugins`](https://github.com/NadicodeAI/nadia-example-plugins/tree/main/example-dashboard)) ships a live demo that injects a banner into `sessions:top` — install it to see the pattern end-to-end.
...
The [`strike-freedom-cockpit`](https://github.com/NadicodeAI/nadia-example-plugins/tree/main/strike-freedom-cockpit) plugin (companion repo `nadia-example-plugins`) is a complete reskin demo. It pairs a theme YAML with a slot-only plugin to produce a cockpit-style HUD without forking the dashboard.
...
```bash
git clone https://github.com/NadicodeAI/nadia-example-plugins.git

# Theme
cp nadia-example-plugins/strike-freedom-cockpit/theme/strike-freedom.yaml \
   ~/.nadia/dashboard-themes/

# Plugin
cp -r nadia-example-plugins/strike-freedom-cockpit ~/.nadia/plugins/
```

Open the dashboard, pick **Strike Freedom** from the theme switcher. The cockpit sidebar appears, the crest shows in the header, the tagline replaces the footer. Switch back to **Nadia Teal** and the plugin remains installed but invisible (the `sidebar` slot only renders under the `cockpit` layout variant).
...
Check that the file is in `~/.nadia/dashboard-themes/` and ends in `.yaml` or `.yml`. Refresh the page. Run `curl http://127.0.0.1:9119/api/dashboard/themes` — your theme should be in the response. If the YAML has a parse error, the dashboard logs to `errors.log` under `~/.nadia/logs/`.
