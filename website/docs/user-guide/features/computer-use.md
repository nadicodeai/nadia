...
Nadia Agent can drive your Mac's desktop — clicking, typing, scrolling,
...
Use `nadia computer-use status` to verify the install.

**Option 2: enable the toolset interactively.**

1. Run `nadia tools`, pick `🖱️ Computer Use (macOS)` → `cua-driver (background)`.
...
window-focus bug for UTM workflows). Nadia refreshes the binary in two
places so you don't get stuck on a stale release:

- **`nadia update`** — when you update Nadia itself, if `cua-driver` is
...
- **`nadia computer-use install --upgrade`** — manual force-refresh.
...
`nadia computer-use status` shows the installed version next to the
binary path.
...
Pair with `approvals.mode: manual` in `~/.nadia/config.yaml` if you want every action confirmed.
...
- **Private SPI risk.** Apple can change SkyLight's symbol surface in any
  OS update. Pin the driver version with the `NADIA_CUA_DRIVER_VERSION`
...
```
NADIA_CUA_DRIVER_CMD=/opt/homebrew/bin/cua-driver
NADIA_CUA_DRIVER_VERSION=0.5.0    # optional pin
```
...
`nadia computer-use install` to fetch the cua-driver binary, or run
`nadia tools` and enable the Computer Use toolset.
...
- [Universal skill: `macos-computer-use`](https://github.com/nadicodeai/nadia/blob/main/skills/apple/macos-computer-use/SKILL.md)
