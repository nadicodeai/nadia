...
Nadia Agent can drive your desktop — clicking, typing, scrolling,
...
```
nadia computer-use install
```

This fetches and runs the upstream cua-driver installer — `install.sh`
on macOS/Linux, `install.ps1` on Windows. Use `nadia computer-use
status` to verify the install.

**Option 2: enable the toolset interactively.**

1. Run `nadia tools`, pick `🖱️  Computer Use (macOS/Windows/Linux)`.
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording** → allow your terminal (or Nadia app). `nadia computer-use doctor` will tell you which permission is missing. |
...
```
nadia -t computer_use chat
```

or add `computer_use` to your enabled toolsets in `~/.nadia/config.yaml`.

## `nadia computer-use doctor` — your first triage stop

`nadia computer-use doctor` runs cua-driver's structured
`health_report` MCP tool and prints a per-check matrix. It's the single
fastest way to find out *why* an action isn't working.

```
$ nadia computer-use doctor
...
agent is acting here." Each Nadia run declares its own cua-driver
...
Nadia intentionally keeps its skill (`skills/computer-use/SKILL.md`)
focused on the Nadia-side `computer_use` action vocabulary — the
...
These are **platform deep dives, not duplicates of the Nadia skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **Nadia
...
Nadia applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.nadia/config.yaml` if you
want every action confirmed.

## Token efficiency

Screenshots are expensive. Nadia applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  Nadia agent) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the Nadia agent itself at High integrity (launch from an
...
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. Nadia warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive Nadia from inside the RDP / console
...
```
NADIA_CUA_DRIVER_CMD=/path/to/your/cua-driver
```

Swap the backend entirely (for testing):

```
NADIA_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

### Telemetry

cua-driver ships with anonymous usage telemetry (PostHog) enabled by default
upstream. **Nadia disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) Nadia sets
...
When it's on, `nadia computer-use doctor` reports `telemetry: enabled`;
...
unreleased fix — point Nadia at a binary you built from source instead
of the published release. Nadia resolves the driver with
`shutil.which("cua-driver")` and **does not enforce
`NADIA_CUA_DRIVER_VERSION`**, so a local build (reported as
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for Nadia testing (see notes).
...
### Option B — point Nadia straight at the built binary (fastest loop)

Skip the install ceremony entirely: `cargo build` and set
`NADIA_CUA_DRIVER_CMD` to the resulting binary. Best for rapid
edit/build/test.
...
```
# Windows (.env)
NADIA_CUA_DRIVER_CMD=C:\path\to\cua\libs\cua-driver\rust\target\debug\cua-driver.exe
# macOS / Linux (.env)
NADIA_CUA_DRIVER_CMD=/path/to/cua/libs/cua-driver/rust/target/debug/cua-driver
```

### Confirm Nadia is using your build

- `nadia computer-use status` prints the resolved binary path and
  version.
- `nadia computer-use doctor` confirms the binary is reachable and
...
- **Nadia spawns its own `cua-driver mcp` child over stdio** — it does
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no Nadia
  change needed either way.
- **Local builds skip the version check.** Nadia warns when the
...
**First action when anything's off: run `nadia computer-use doctor`.**
...
Run `nadia computer-use install` to fetch the cua-driver binary, or
run `nadia tools` and enable the Computer Use toolset.
...
Wayland without an XWayland bridge. `nadia computer-use doctor` will
...
- **Nadia-side skill** — `skills/computer-use/SKILL.md` — teaches the
  Nadia `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  install` autodetects Nadia (planned follow-up), this happens
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (Nadia among them)
