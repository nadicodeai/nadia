...
Nadia can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<NADIA_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), Nadia renders the sprite at full fidelity when your
...
nadia pets install boba --select
...
| Goal | Command |
| --- | --- |
| Browse the gallery | `nadia pets list [query] [--limit N]` |
| List installed pets | `nadia pets list --installed` |
| Install a pet | `nadia pets install <slug> [--select] [--force]` |
...
| Preview/animate | `nadia pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `nadia pets off` |
| Remove an installed pet | `nadia pets remove <slug>` |
| Diagnose setup | `nadia pets doctor` |

`nadia pets show` flags:
...
Beyond installing pre-made pets from the gallery, Nadia can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `nadia pets` → the generate flow.
...
always-on-top desktop window. Out there it stays visible while Nadia is
...
Run `nadia pets doctor` — it reports:
