...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Nadia's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Nadia instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
nadia memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `nadia` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Nadia syncs them back at session init.
...
The `nadia honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `nadia memory setup honcho` (or run `nadia memory setup` and pick it from the list); the `nadia honcho` subcommand then appears on the next invocation.

```bash
nadia memory setup honcho    # Configure Honcho directly (works before activation)
nadia honcho status          # Connection status, config, and key settings
nadia honcho setup           # Redirects to `nadia memory setup` (post-activation alias)
nadia honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
nadia honcho peer            # Show or update peer names + dialectic reasoning level
nadia honcho mode            # Show or set recall mode (hybrid/context/tools)
nadia honcho tokens          # Show or set token budget for context and dialectic
nadia honcho identity        # Seed or show the AI peer's Honcho identity
nadia honcho sync            # Sync Honcho config to all existing profiles
nadia honcho peers           # Show peer identities across all profiles
nadia honcho sessions        # List known Honcho session mappings
nadia honcho map             # Map current directory to a Honcho session name
nadia honcho enable          # Enable Honcho for the active profile
nadia honcho disable         # Disable Honcho for the active profile
nadia honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `nadia honcho`

If you previously used the standalone `nadia honcho setup`:
...
No re-login or re-setup needed. Run `nadia memory setup` and select "honcho" — the wizard detects your existing config.
