...
chat completions — use your Nadia-managed provider subscription as their
...
| Use case | "Use Nadia as a chat backend" | "Use my Portal sub from another app" |
...
```bash
nadia proxy start --host 0.0.0.0 --port 8645
```
...
`nadia_cli/proxy/adapters/<provider>.py` and registering it in
