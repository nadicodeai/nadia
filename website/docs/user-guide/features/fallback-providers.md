...
| Google Gemini (OAuth) | `google-gemini-cli` | `nadia model` (Google OAuth; optional: `NADIA_GEMINI_PROJECT_ID`) |
...
When a task's provider is set to `"auto"` (the default), Nadia tries providers in order until one works:
...
If the resolved provider fails at call time, Nadia also has an internal retry: if the provider is not OpenRouter and no explicit `base_url` is set, it tries OpenRouter as a last-resort fallback.
