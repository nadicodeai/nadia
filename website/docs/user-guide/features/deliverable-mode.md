...
When Nadia Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.nadia/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.nadia/config.yaml` (switchable per session
via `/personality`).
...
If you use Nadia's kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.nadia/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Nadia Agent's deliverable
