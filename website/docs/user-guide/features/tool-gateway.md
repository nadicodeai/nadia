...
The Tool Gateway is included with every paid [NadicodeAI Portal](https://portal.nadicode.ai) subscription. It routes Nadia's tool calls — web search, image generation, text-to-speech, and cloud browser automation — through NadicodeAI-managed infrastructure, so you don't have to sign up with Firecrawl, FAL, OpenAI, Browser Use, or anyone else just to make your agent useful.

<div style={{display: 'flex', gap: '1rem', flexWrap: 'wrap', margin: '1.5rem 0'}}>
  <a href="https://portal.nadicode.ai" style={{background: 'var(--ifm-color-primary)', color: 'white', padding: '0.75rem 1.5rem', borderRadius: '6px', textDecoration: 'none', fontWeight: 'bold'}}>Start or manage subscription →</a>
</div>
...
| 🎨 | **Image generation** | Nine models under one endpoint: **FLUX 2 Klein 9B**, **FLUX 2 Pro**, **Z-Image Turbo**, **Nano Banana Pro** (Gemini 3 Pro Image), **GPT Image 1.5**, **GPT Image 2**, **Ideogram V3**, **Recraft V4 Pro**, **Qwen Image**. Pick per-generation with a flag, or let Nadia default to FLUX 2 Klein. |
...
```bash
nadia portal info        # Portal auth + Tool Gateway routing summary
nadia portal tools       # Gateway catalog with current routing per tool
nadia status             # Full system status (Tool Gateway is one section)
```
...
The Tool Gateway is a **paid-subscription** feature. Free-tier NadicodeAI accounts can use Portal for inference but don't include managed tools — [upgrade your plan](https://portal.nadicode.ai) to unlock the gateway.
...
The set evolves — `nadia tools` → Image Generation shows the current live list.

---

## Configuration reference

Most users never need to touch this — `nadia model` and `nadia tools` cover every workflow interactively. This section is for writing config.yaml directly or scripting setups.
...
```yaml
web:
  use_gateway: false   # Nadia now uses FIRECRAWL_API_KEY from .env
```

`nadia tools` automatically clears the flag when you pick a non-gateway provider, so this usually happens for you.
...
Tools routed through the gateway stop working until you renew or swap in direct API keys via `nadia tools`. Nadia shows a clear error pointing at the portal.

### Can I see usage or costs per tool?

Yes — the [NadicodeAI Portal dashboard](https://portal.nadicode.ai) breaks usage down by tool so you can see what's driving your bill.
...
No — keep them in `.env`. When `use_gateway: true`, Nadia skips direct keys and uses the gateway. Flip the flag back to `false` and your keys become the source again. The gateway isn't a lock-in.
