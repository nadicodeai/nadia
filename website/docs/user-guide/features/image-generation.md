...
description: Generate images via FAL.ai — 11 models including FLUX 2, GPT Image (1.5 & 2), Nano Banana Pro, Ideogram, Recraft V4 Pro, Krea 2, and more, selectable via `nadia tools`.
...
Nadia Agent generates images from text prompts via FAL.ai. Eleven models are supported out of the box, each with different speed, quality, and cost tradeoffs. The active model is user-configurable via `nadia tools` and persists in `config.yaml`.
...
3. **Submission** — `_submit_fal_request()` routes via direct FAL credentials or the managed Nadia gateway.
...
- **Requires FAL credentials** (direct `FAL_KEY` or Nadia Subscription)
