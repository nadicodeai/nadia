...
description: "Customize Nadia Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Nadia Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `NADIA_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Nadia is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Nadia now seeds a default `SOUL.md` automatically in:

```text
~/.nadia/SOUL.md
```

More precisely, it uses the current instance's `NADIA_HOME`, so if you run Nadia with a custom home directory, it will use:
...
- Nadia creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Nadia loads `SOUL.md` only from `NADIA_HOME`
- Nadia does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Nadia falls back to a built-in default identity
...
If Nadia loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `NADIA_HOME`, the personality belongs to the Nadia instance itself.

That also makes it easier to teach users:
- "Edit `~/.nadia/SOUL.md` to change Nadia's default personality."
...
- how Nadia should handle uncertainty, disagreement, or ambiguity
...
## What Nadia injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Nadia falls back to a built-in default identity ("You are Nadia Agent, an intelligent AI assistant created by NadicodeAI..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Nadia ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Nadia, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Nadia its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.nadia/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.nadia/SOUL.md`
...
- [SOUL.md Guide](/guides/use-soul-with-nadia)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Nadia speaks
- `display.skin` and `/skin` affect how Nadia looks in the terminal
