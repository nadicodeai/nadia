...
sidebar_position: 1
title: "Quickstart"
description: "Your first conversation with Nadia Agent — from install to chatting in under 5 minutes"
---

# Quickstart

This guide gets you from zero to a working Nadia setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Prefer to watch?

**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full [Nadia Agent Tutorials & Use Cases](https://www.youtube.com/playlist?list=PLmpUb_PWAkDxewld5ZYyKifuHxgIbiq2d) playlist.

<div style={{position: 'relative', paddingBottom: '56.25%', height: 0, overflow: 'hidden', maxWidth: '100%', marginBottom: '1.5rem'}}>
  <iframe
    style={{position: 'absolute', top: 0, left: 0, width: '100%', height: '100%'}}
    src="https://www.youtube-nocookie.com/embed/R3YOGfTBcQg"
    title="Nadia Agent Masterclass: Installation, Setup, Basic Commands"
    frameBorder="0"
...
- Setting up Nadia for a team, bot, or always-on workflow
...
|---|---|---|
| I just want Nadia working on my machine | `nadia setup` | Run a real chat and verify it responds |
| I already know my provider | `nadia model` | Save the config, then start chatting |
| I want a bot or always-on setup | `nadia gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `nadia model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `nadia model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if Nadia cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.
...
The single most important setup step. Use `nadia model` to walk through the choice interactively:

```bash
nadia model
```

:::tip Easiest path: Nadia Agents Portal
...
:::info Setup modes
On a fresh install, `nadia setup` offers three modes:

- **Quick Setup (Nadia Agents Portal)** — free OAuth login, no API keys; sets up a model plus the Tool Gateway tools. The recommended fast path.
...
Blank Slate writes an explicit `platform_toolsets.cli` list plus `agent.disabled_toolsets`, so nothing you didn't choose ever loads — not even after `nadia update`. Re-enable anything later with `nadia tools`, seed skills with `nadia skills opt-in --sync`, or tune settings with `nadia setup agent`.
:::
...
|----------|-----------|---------------|
| **Nadia Agents Portal** | Subscription-based, zero-config | OAuth login via `nadia model` |
| **OpenAI Codex** | ChatGPT OAuth, uses Codex models | Device code auth via `nadia model` |
| **Anthropic** | Claude models directly — Max plan + extra usage credits (OAuth), or API key for pay-per-token | `nadia model` → OAuth login (requires Max + extra credits), or an Anthropic API key |
...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `nadia_cli/models.py` may change between releases) | `nadia model` → MiniMax (OAuth) |
...
| **xAI Grok OAuth** | SuperGrok / Premium+ subscription, no API key needed | `nadia model` → xAI Grok OAuth |
...
| **Qwen OAuth** | Qwen Portal browser OAuth — no API key needed | `nadia model` → Qwen OAuth |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `nadia model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `nadia model` (requires `copilot` CLI + `copilot login`) |
...
Nadia Agent requires a model with at least **64,000 tokens** of context. Models with smaller windows cannot maintain enough working memory for multi-step tool-calling workflows and will be rejected at startup. Most hosted models (Claude, GPT, Gemini, Qwen, DeepSeek) meet this easily. If you're running a local model, set its context size to at least 64K (e.g. `--ctx-size 65536` for llama.cpp or `-c 65536` for Ollama).
:::

:::tip
You can switch providers at any time with `nadia model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

Nadia separates secrets from normal config:

- **Secrets and tokens** → `~/.nadia/.env`
- **Non-secret settings** → `~/.nadia/config.yaml`

The easiest way to set values correctly is through the CLI:

```bash
nadia config set model anthropic/claude-opus-4.6
nadia config set terminal.backend docker
nadia config set OPENROUTER_API_KEY sk-or-...
```
...
```bash
nadia            # classic CLI
nadia --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
Nadia ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `nadia` vs `nadia --tui`.
:::
...
```bash
nadia --continue    # Resume the most recent session
nadia -c            # Short form
```
...
```bash
nadia gateway setup    # Interactive platform configuration
```
...
- `nadia tools` — tune tool access per platform
- `nadia skills` — browse and install reusable workflows
...
```bash
nadia config set terminal.backend docker    # Docker isolation
nadia config set terminal.backend ssh       # Remote server
```

### Voice mode

```bash
# From the Nadia install directory (the curl installer placed it at
# ~/.nadia/nadia-agent on Linux/macOS or %LOCALAPPDATA%\nadia\nadia-agent on Windows):
...
Skills are on-demand instruction documents that teach Nadia how to do a specific task — deploy to Kubernetes, open a GitHub PR, fine-tune a model, search for GIFs. Each is a `SKILL.md` file with a name, a description, and a step-by-step procedure. The agent reads the short descriptions for free and only loads a skill's full content when a task actually calls for it, so adding skills doesn't bloat every request.

Nadia ships with a catalog of bundled skills already installed in `~/.nadia/skills/`. You can add more from the Skills Hub, or write your own.

**Browse and install from the hub:**

```bash
nadia skills browse                      # list everything available
nadia skills search kubernetes           # find skills by keyword
nadia skills install openai/skills/k8s   # install one (runs a security scan first)
```

The install argument is a `source/path` slug from the hub — `openai/skills/k8s` means the `k8s` skill from OpenAI's catalog. `nadia skills browse` shows the exact slugs to use.
...
/k8s                                       # load it and let Nadia ask what you need
```
...
(If you installed without `[all]`, run `cd ~/.nadia/nadia-agent && uv pip install -e ".[acp]"` first.)
...
|---|---|---|
| Nadia opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `nadia model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `nadia gateway setup` and check `nadia gateway status` |
| `nadia --continue` can't find old session | Switched profiles or session never saved | Check `nadia sessions list` and confirm you're in the right profile |
...
| `nadia doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
2. `nadia model`
3. `nadia setup`
4. `nadia sessions list`
5. `nadia --continue`
6. `nadia gateway status`
...
| Command | Description |
|---------|-------------|
| `nadia` | Start chatting |
| `nadia model` | Choose your LLM provider and model |
| `nadia tools` | Configure which tools are enabled per platform |
| `nadia setup` | Full setup wizard (configures everything at once) |
| `nadia doctor` | Diagnose issues |
| `nadia update` | Update to latest version |
| `nadia gateway` | Start the messaging gateway |
| `nadia --continue` | Resume last session |
