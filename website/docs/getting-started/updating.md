...
```bash
nadia update --check    # see if a newer release is on PyPI
nadia update            # runs pip install --upgrade nadia-agent
```

Or manually:

```bash
pip install --upgrade nadia-agent    # or: uv pip install --upgrade nadia-agent
```
...
Want to know if an update is available before pulling? Run `nadia update --check` — for git installs it fetches and compares commits against `origin/main`; for pip installs it queries PyPI for the latest release. No files are modified, no gateway is restarted. Useful in scripts and cron jobs that gate on "is there an update".
