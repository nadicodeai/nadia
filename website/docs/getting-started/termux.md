...
description: "Run Nadia Agent directly on an Android phone with Termux"
---

# Nadia on Android with Termux

This is the tested path for running Nadia Agent directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Nadia from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

Nadia now ships a Termux-aware installer path:
...
- links `nadia` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `nadia` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/nadia" "$PREFIX/bin/nadia"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `nadia` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.nadia/.env`.
...
### `nadia doctor` says ripgrep or Node is missing
