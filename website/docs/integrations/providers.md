...
| **Google Gemini (OAuth)** | `nadia model` → "Google Gemini (OAuth)" (provider: `google-gemini-cli`, free tier supported, browser PKCE login) |
...
# → done — Nadia auto-provisions your free tier on first request
```

Nadia ships Google's **public** `gemini-cli` desktop OAuth client by default —
...
- Tokens stored at `~/.nadia/auth/google_oauth.json` (chmod 0600, atomic write, cross-process `fcntl` lock)
- Automatic refresh 60 s before expiry
- Headless environments (SSH, `NADIA_HEADLESS=1`) → paste-mode fallback
...
- Responses translated back to OpenAI shape so the rest of Nadia works unchanged
...
| Workspace / Standard / Enterprise account | Set `NADIA_GEMINI_PROJECT_ID` or `GOOGLE_CLOUD_PROJECT` to your GCP project ID |
| VPC-SC-protected org | Nadia detects `SECURITY_POLICY_VIOLATED` and forces `standard-tier` automatically |
...
experience, use your own API key via the `gemini` provider instead. Nadia shows
...
```bash
NADIA_GEMINI_CLIENT_ID=your-client.apps.googleusercontent.com
NADIA_GEMINI_CLIENT_SECRET=...   # optional for Desktop clients
```
