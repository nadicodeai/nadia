...
| **Kimi / Moonshot** | `KIMI_API_KEY` in `~/.nadia/.env` (provider: `kimi-coding`) |
| **Kimi / Moonshot (China)** | `KIMI_CN_API_KEY` in `~/.nadia/.env` (provider: `kimi-coding-cn`; aliases: `kimi-cn`, `moonshot-cn`) |
| **Arcee AI** | `ARCEEAI_API_KEY` in `~/.nadia/.env` (provider: `arcee`; aliases: `arcee-ai`, `arceeai`) |
| **GMI Cloud** | `GMI_API_KEY` in `~/.nadia/.env` (provider: `gmi`; aliases: `gmi-cloud`, `gmicloud`) |
...
| **Xiaomi MiMo** | `XIAOMI_API_KEY` in `~/.nadia/.env` (provider: `xiaomi`, aliases: `mimo`, `xiaomi-mimo`) |
| **Tencent TokenHub** | `TOKENHUB_API_KEY` in `~/.nadia/.env` (provider: `tencent-tokenhub`, aliases: `tencent`, `tokenhub`, `tencentmaas`) |
| **OpenCode Zen** | `OPENCODE_ZEN_API_KEY` in `~/.nadia/.env` (provider: `opencode-zen`) |
| **OpenCode Go** | `OPENCODE_GO_API_KEY` in `~/.nadia/.env` (provider: `opencode-go`) |
...
Nadia has **two** model commands that serve different purposes:
...
Nadia supports GitHub Copilot as a first-class provider with two modes:
...
```bash
nadia chat --provider copilot --model gpt-5.4
```
...
Nadia sends a supported GitHub token (`gho_*`, `github_pat_*`, or `ghu_*`) directly to `api.githubcopilot.com` and includes Copilot-specific headers (`Editor-Version`, `Copilot-Integration-Id`, `Openai-Intent`, `x-initiator`).

On HTTP 401, Nadia now performs a one-shot credential recovery before fallback:
...
Some older community proxies use `api.github.com/copilot_internal/v2/token` exchange flows. That endpoint can be unavailable for some account types (returns 404). Nadia therefore keeps direct-token auth as the primary path and relies on runtime credential refresh + retry for robustness.
:::
...
```bash
nadia chat --provider copilot-acp --model copilot-acp
...
| `NADIA_COPILOT_ACP_COMMAND` | Override the Copilot CLI binary path (default: `copilot`) |
| `NADIA_COPILOT_ACP_ARGS` | Override ACP args (default: `--acp --stdio`) |
...
```bash
# NovitaAI Model API
nadia chat --provider novita --model moonshotai/kimi-k2.5
# Requires: NOVITA_API_KEY in ~/.nadia/.env

# z.ai / ZhipuAI GLM
nadia chat --provider zai --model glm-5
# Requires: GLM_API_KEY in ~/.nadia/.env

# Kimi / Moonshot AI (international: api.moonshot.ai)
nadia chat --provider kimi-coding --model kimi-for-coding
# Requires: KIMI_API_KEY in ~/.nadia/.env

# Kimi / Moonshot AI (China: api.moonshot.cn)
nadia chat --provider kimi-coding-cn --model kimi-k2.5
# Requires: KIMI_CN_API_KEY in ~/.nadia/.env

# MiniMax (global endpoint)
nadia chat --provider minimax --model MiniMax-M2.7
# Requires: MINIMAX_API_KEY in ~/.nadia/.env

# MiniMax (China endpoint)
nadia chat --provider minimax-cn --model MiniMax-M2.7
# Requires: MINIMAX_CN_API_KEY in ~/.nadia/.env

# Qwen Cloud / DashScope (Qwen models)
nadia chat --provider alibaba --model qwen3.5-plus
# Requires: DASHSCOPE_API_KEY in ~/.nadia/.env

# Xiaomi MiMo
nadia chat --provider xiaomi --model mimo-v2-pro
# Requires: XIAOMI_API_KEY in ~/.nadia/.env

# Tencent TokenHub (Hy3 Preview)
nadia chat --provider tencent-tokenhub --model hy3-preview
# Requires: TOKENHUB_API_KEY in ~/.nadia/.env

# Arcee AI (Trinity models)
nadia chat --provider arcee --model trinity-large-thinking
# Requires: ARCEEAI_API_KEY in ~/.nadia/.env

# GMI Cloud
# Use the exact model ID returned by GMI's /v1/models endpoint.
nadia chat --provider gmi --model zai-org/GLM-5.1-FP8
# Requires: GMI_API_KEY in ~/.nadia/.env
```
...
When using the Z.AI / GLM provider, Nadia automatically probes multiple endpoints (global, China, coding variants) to find one that accepts your API key. You don't need to set `GLM_BASE_URL` manually — the working endpoint is detected and cached automatically.
:::
...
xAI also ships a dedicated TTS endpoint (`/v1/tts`). Select **xAI TTS** in `nadia tools` → Voice & TTS, or see the [Voice & TTS](../user-guide/features/tts.md#text-to-speech) page for config.

**Retired xAI model migration (May 15, 2026):** xAI is retiring `grok-4*`, `grok-3`, `grok-code-fast-1`, and `grok-imagine-image-pro` on 2026-05-15. `nadia doctor` and `nadia chat` startup both detect any config still pointing at a retired ref and print the recommended replacement. Use `nadia migrate xai` for a one-shot config rewrite — dry-run by default, add `--apply` to write changes (a timestamped `config.yaml.bak-pre-migrate-xai-*` backup is created automatically).

```bash
nadia migrate xai          # preview replacements
nadia migrate xai --apply  # rewrite ~/.nadia/config.yaml in place
```
...
# Short alias
nadia chat --provider novita-ai --model deepseek/deepseek-v3-0324
```
...
nadia chat --provider bedrock --model us.anthropic.claude-sonnet-4-6

# Or with explicit env vars
AWS_PROFILE=myprofile AWS_REGION=us-east-1 nadia chat --provider bedrock --model us.anthropic.claude-sonnet-4-6
```
...
`VERTEX_PROJECT_ID` / `VERTEX_REGION` env vars override the `config.yaml` values. Install with `pip install 'nadia-agent[vertex]'` (or let Nadia lazy-install `google-auth` on first use). See the [Google Vertex AI guide](/guides/google-vertex) for the full walkthrough, and the [Google Gemini guide](/guides/google-gemini) for the static-API-key AI Studio path instead.
...
Set `NADIA_QWEN_BASE_URL` only if the portal endpoint relocates (default: `https://portal.qwen.ai/v1`).
...
```bash
nadia chat --provider alibaba_coding --model qwen3-coder-plus
```
...
nadia chat --provider nvidia --model nvidia/nemotron-3-super-120b-a12b
# Requires: NVIDIA_API_KEY in ~/.nadia/.env

# Local NIM endpoint — override base URL
NVIDIA_BASE_URL=http://localhost:8000/v1 nadia chat --provider nvidia --model nvidia/nemotron-3-super-120b-a12b
```
...
Nadia automatically attaches the NIM billing-origin header on every request to `build.nvidia.com` — no configuration needed. This routes consumption against the correct origin in NVIDIA's billing dashboard.
...
```bash
# GMI Cloud
nadia chat --provider gmi --model deepseek-ai/DeepSeek-V3.2
# Requires: GMI_API_KEY in ~/.nadia/.env
```
...
```bash
# StepFun
nadia chat --provider stepfun --model step-3.5-flash
# Requires: STEPFUN_API_KEY in ~/.nadia/.env
```
...
```bash
# Use any available model
nadia chat --provider huggingface --model Qwen/Qwen3.5-397B-A17B
# Requires: HF_TOKEN in ~/.nadia/.env

# Short alias
nadia chat --provider hf --model deepseek-ai/DeepSeek-V3.2
```
...
Nadia Agent works with **any OpenAI-compatible API endpoint**. If a server implements `/v1/chat/completions`, you can point Nadia at it. This means you can use local models, GPU inference servers, multi-provider routers, or any third-party API.
...
When switching providers, Nadia persists the base URL and provider to config so the change survives restarts. When switching away from a custom endpoint to a built-in provider, the stale base URL is automatically cleared.
...
Nadia Agent requires at least **64,000 tokens** of context for agent use with tools. Smaller windows are rejected at startup because the system prompt, tool schemas, and working conversation state need enough room for reliable multi-step workflows.
...
**You cannot set context length through the OpenAI-compatible API** (`/v1/chat/completions`). It must be configured server-side or via a Modelfile. This is the #1 source of confusion when integrating Ollama with tools like Nadia.
:::
...
| Flag | Purpose |
|------|---------|
| `--enable-auto-tool-choice` | Required for `tool_choice: "auto"` (the default in Nadia) |
| `--tool-call-parser <name>` | Parser for the model's tool call format |

Supported parsers: `nadia` (Qwen 2.5, Nadia 2/3), `llama3_json` (Llama 3.x), `mistral`, `deepseek_v3`, `deepseek_v31`, `xlam`, `pythonic`. Without these flags, tool calls won't work — the model will output tool calls as text.

**Qwen reasoning parsers:** Nadia preserves structured reasoning metadata such as `reasoning`, `reasoning_content`, and streamed reasoning deltas when OpenAI-compatible servers return them. That metadata is treated as reasoning/thinking trace data, not as a replacement for the assistant's visible answer. For Qwen reasoning models served by vLLM, make sure the final user-visible response still appears in `content`. If `--reasoning-parser qwen3` leaves `content` empty in your deployment, either disable that parser or pass a server-supported request option such as `chat_template_kwargs.enable_thinking: false` through `extra_body`.
...
Native tool calling support (best performance): Llama 3.x, Qwen 2.5 (including Coder), Nadia 2/3, Mistral, DeepSeek, Functionary. All other models use a generic handler that works but may be less efficient. See the [llama.cpp function calling docs](https://github.com/ggml-org/llama.cpp/blob/master/docs/function-calling.md) for the full list.
...
**Tool calling:** Supported since LM Studio 0.3.6. Models with native tool-calling training (Qwen 2.5, Llama 3.x, Mistral, Nadia) are auto-detected and shown with a tool badge. Other models use a generic fallback that may be less reliable.

---

### WSL2 Networking (Windows Users)

Since Nadia Agent requires a Unix environment, Windows users run it inside WSL2. If your model server (Ollama, LM Studio, etc.) runs on the **Windows host**, you need to bridge the network gap — WSL2 uses a virtual network adapter with its own subnet, so `localhost` inside WSL2 refers to the Linux VM, **not** the Windows host.
...
Use that IP in your Nadia config:
...
If you get a JSON response listing your models, you're good. Use that same URL as the `base_url` in your Nadia config.

---

### Troubleshooting Local Models

These issues affect **all** local inference servers when used with Nadia.

#### "Connection refused" from WSL2 to a Windows-hosted model server

If you're running Nadia inside WSL2 and your model server on the Windows host, `http://localhost:<port>` won't work in WSL2's default NAT networking mode. See [WSL2 Networking](#wsl2-networking-windows-users) above for the fix.
...
**Cause:** Context window is too small. When the conversation exceeds the context limit, most servers silently drop older messages. Nadia's system prompt + tool schemas alone can use 4k–8k tokens.

**Diagnosis:**

```bash
# Check what Nadia thinks the context is
...
Nadia auto-detects context length from your server's `/v1/models` endpoint. If the server reports a low value (or doesn't report one at all), Nadia uses the model's declared limit which may be wrong.
...
**Possible causes:**
1. **Low output cap (`max_tokens`) on the server** — SGLang defaults to 128 tokens per response. Set `--default-max-tokens` on the server or configure Nadia with `model.max_tokens` in config.yaml. Note: `max_tokens` controls response length only — it is unrelated to how long your conversation history can be (that is `context_length`).
2. **Context exhaustion** — The model filled its context window. Increase `model.context_length` or enable [context compression](/user-guide/configuration#context-compression) in Nadia.
...
**`context_length`** is the **total context window** — the combined budget for input *and* output tokens (e.g. 200,000 for Claude Opus 4.6). Nadia uses this to decide when to compress history and to validate API requests.
...
Nadia uses a multi-source resolution chain to detect the correct context window for your model and provider:
...
custom_providers:
  - name: local
    base_url: http://localhost:8080/v1
    # api_key omitted — Nadia uses "no-key-required" for keyless local servers
  - name: work
...
Some OpenAI-compatible endpoints need provider-specific request body fields. Add an `extra_body` map to the matching custom provider and Nadia will merge it into each chat-completions request for that endpoint:
...
The cloud providers listed in [Other Compatible Providers](#other-compatible-providers) all speak OpenAI's REST dialect, so they wire up the same way under `custom_providers:`. Three worked recipes follow. Each drops into `~/.nadia/config.yaml` and the matching API key goes in `~/.nadia/.env`.
...
2. Point Nadia at your instance (no API key needed):
   ```bash
   nadia config set FIRECRAWL_API_URL http://localhost:3002
   ```
...
When using OpenRouter, you can control how requests are routed across providers. Add a `provider_routing` section to `~/.nadia/config.yaml`:
...
OpenRouter ships an experimental coding-model router at `openrouter/pareto-code` that auto-routes requests to the cheapest model meeting a coding-quality bar (ranked by [Artificial Analysis](https://artificialanalysis.ai/)). Pick this model and tune the `min_coding_score` knob in `~/.nadia/config.yaml`:
...
Configure a chain of backup providers Nadia tries in order when the primary model fails (rate limits, server errors, auth failures). The canonical format is a top-level `fallback_providers:` list:
