...
The self-improving AI agent built by [NadicodeAI](https://nadicode.ai). The only agent with a built-in learning loop — it creates skills from experience, improves them during use, nudges itself to persist knowledge, and builds a deepening model of who you are across sessions.
...
After installing, run `nadia setup --portal` — one OAuth covers a model plus all four Tool Gateway tools (web search, image generation, TTS, browser). See [Nadia Agents Portal](/integrations/nadia-portal).
:::
...
| 🔌 **[MCP Integration](/user-guide/features/mcp)**                      | Connect to MCP servers, filter their tools, and extend Nadia safely  |
| 🧭 **[Use MCP with Nadia](/guides/use-mcp-with-nadia)**               | Practical MCP setup patterns, examples, and tutorials                 |
| 🎙️ **[Voice Mode](/user-guide/features/voice-mode)**                    | Real-time voice interaction in CLI, Telegram, Discord, and Discord VC |
| 🗣️ **[Use Voice Mode with Nadia](/guides/use-voice-mode-with-nadia)** | Hands-on setup and usage patterns for Nadia voice workflows          |
| 🎭 **[Personality & SOUL.md](/user-guide/features/personality)**        | Define Nadia's default voice with a global SOUL.md                    |
...
| 💡 **[Tips & Best Practices](/guides/tips)**                            | Quick wins to get the most out of Nadia                              |
...
- **Built by model trainers** — Created by [NadicodeAI](https://nadicode.ai), the lab behind Nadia, Nomos, and Psyche. Works with [Nadia Agents Portal](https://portal.nadicode.ai), [OpenRouter](https://openrouter.ai), OpenAI, or any endpoint
...
- **Full web control** — Search, extract, browse, vision, image generation, TTS — one subscription via [Nadia Agents Portal](/integrations/nadia-portal) bundles all of them
- **MCP support** — Connect to any MCP server for extended tool capabilities
- **Research-ready** — Batch processing, trajectory export, RL training with Atropos. Built by [NadicodeAI](https://nadicode.ai) — the lab behind Nadia, Nomos, and Psyche models
