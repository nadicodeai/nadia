...
sidebar_position: 1
title: "Architecture"
description: "Nadia Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Nadia Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ nadia_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # NadiaCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── nadia_state.py           # SQLite session/state database with FTS5
├── nadia_constants.py       # NADIA_HOME, profile-aware paths
...
│
├── nadia_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `nadia` subcommands (large file)
...
│   ├── skills_config.py      # nadia skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # nadia tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # nadia gateway start/stop
│
...
│   └── platforms/            # 13 adapters: telegram, discord, slack, whatsapp,
...
```text
User input → NadiaCLI.process_input()
...
Three discovery sources: `~/.nadia/plugins/` (user), `.nadia/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `nadia plugins` or `config.yaml`.

→ [Plugin Guide](/guides/build-a-nadia-plugin), [Memory Provider Plugin](./memory-provider-plugin.md)
...
Exposes Nadia as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`nadia -p <name>`) gets its own NADIA_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
