...
sidebar_position: 4
title: "Contributing"
description: "How to contribute to Nadia Agent — dev setup, code style, PR process"
---

# Contributing

Thank you for contributing to Nadia Agent! This guide covers setting up your dev environment, understanding the codebase, and getting your PR merged.
...
- Building a custom/local tool without modifying Nadia core? Start with [Build a Nadia Plugin](../guides/build-a-nadia-plugin.md)
- Building a new built-in core tool for Nadia itself? Start with [Adding Tools](./adding-tools.md)
...
The installer creates the Nadia venv, wires the `nadia` command, stamps the
install method for `nadia update`, and clones the full git project into
`$NADIA_HOME/nadia-agent` (usually `~/.nadia/nadia-agent`). That keeps your
development environment on the same layout the CLI, updater, lazy dependency
installer, gateway, and docs assume.

```bash
curl -fsSL https://raw.githubusercontent.com/nadicodeai/nadia/main/scripts/install.sh | bash
cd "${NADIA_HOME:-$HOME/.nadia}/nadia-agent"
...
Use this only if you intentionally do not want Nadia's managed install layout
(for example, a throwaway clone inside a container or CI job). If you install
this way, make sure you run the `nadia` entrypoint from this venv; running the
system `python3 -m nadia_cli.main` can pick up unrelated system Python
packages.
...
uv venv ~/.nadia/venvs/nadia-dev --python 3.11
...
```bash
mkdir -p ~/.nadia/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.nadia/config.yaml
touch ~/.nadia/.env

# Add at minimum an LLM provider key:
echo 'OPENROUTER_API_KEY=sk-or-v1-your-key' >> ~/.nadia/.env
```

### Run

```bash
# The standard installer already put `nadia` on PATH.
nadia doctor
nadia chat -q "Hello"
```

If you used the manual clone fallback, run `./nadia` from the checkout or
symlink this clone's venv explicitly:

```bash
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/nadia" ~/.local/bin/nadia
```
...
- **Profile-safe paths**: Never hardcode `~/.nadia` — use `get_nadia_home()` from `nadia_constants` for code paths and `display_nadia_home()` for user-facing messages. See [AGENTS.md](https://github.com/nadicodeai/nadia/blob/main/AGENTS.md#profiles-multi-instance-support) for full rules.
...
Nadia has terminal access. Security matters.
...
2. **Test manually**: Run `nadia` and exercise the code path you changed
...
- Use [GitHub Issues](https://github.com/nadicodeai/nadia/issues)
- Include: OS, Python version, Nadia version (`nadia version`), full error traceback
...
By contributing, you agree that your contributions will be licensed under the [MIT License](https://github.com/nadicodeai/nadia/blob/main/LICENSE).
