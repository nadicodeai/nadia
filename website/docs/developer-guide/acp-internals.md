...
The ACP adapter wraps Nadia's synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
nadia acp / nadia-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.nadia/.env
  -> configure stderr logging
  -> construct NadiaACPAgent
  -> acp.run_agent(agent, use_unstable_protocol=True)
```

The Zed ACP Registry path launches the same adapter through `uvx --from 'nadia-agent[acp]==<version>' nadia-acp`, pointed at the `nadia-agent` PyPI release.
...
- `allow_once` -> Nadia `once`
- `allow_always` -> Nadia `always`
- reject options -> Nadia `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Nadia tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["nadia-acp"])
...
Instead it reuses Nadia's runtime resolver:

- `acp_adapter/auth.py`
- `nadia_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Nadia provider/credentials. It also always advertises a terminal setup auth method (`nadia-setup`, args `--setup`) so first-run registry clients can open Nadia's interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.nadia/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `nadia-acp` toolset definition
- `nadia_cli/main.py` — `nadia acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `nadia-acp` script
