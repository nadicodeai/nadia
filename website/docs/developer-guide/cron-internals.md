...
description: "How Nadia stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `nadia_cli/cron.py` | CLI `nadia cron` subcommands |
...
Jobs are stored in `~/.nadia/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `nadia cron` commands are run or during active CLI sessions.
...
keep the process awake), it asks NadicodeAI Portal infrastructure to arm exactly **one
managed one-shot per job at that job's real next-fire time**. At fire time NadicodeAI Portal
...
The flow (the managed scheduler is provided by NadicodeAI Portal; the agent holds no
scheduler credentials):

```
create/update a cron job
  → Chronos asks NadicodeAI Portal to arm a one-shot at the job's next_run_at
      (authenticated with the agent's existing NadicodeAI Portal token)
  → at fire time NadicodeAI Portal calls the gateway: POST {callback_url}/api/cron/fire
      (authenticated with a short-lived, purpose-scoped NadicodeAI Portal-minted JWT)
...
Config (all non-secret; on hosted agents NadicodeAI Portal sets these at provision time):

| key | meaning |
|---|---|
| `cron.provider` | `chronos` to activate (empty = built-in ticker) |
| `cron.chronos.portal_url` | NadicodeAI Portal base URL (arming + the fire-token issuer) |
...
If Chronos is misconfigured or the agent isn't logged into NadicodeAI Portal,
...
agent↔NadicodeAI Portal wire contract lives in `docs/chronos-managed-cron-contract.md`.
...
```python
# ~/.nadia/scripts/check_competitors.py
...
| Local file | `local` | Save to `~/.nadia/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `nadia cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `nadia cron` CLI provides direct job management:

```bash
nadia cron list                    # Show all jobs
nadia cron create                  # Interactive job creation (alias: add)
nadia cron edit <job_id>           # Edit job configuration
nadia cron pause <job_id>          # Pause a running job
nadia cron resume <job_id>         # Resume a paused job
nadia cron run <job_id>            # Trigger immediate execution
nadia cron remove <job_id>         # Delete a job
```
