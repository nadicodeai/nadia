...
Nadia Agent uses a SQLite database (`~/.nadia/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source file: `nadia_state.py`


## Architecture Overview

```
~/.nadia/state.db (SQLite, WAL mode)
...
Multiple nadia processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.nadia/state.db
...
Default path: `~/.nadia/state.db`

This is derived from `nadia_constants.get_nadia_home()` which resolves to
`~/.nadia/` by default, or the value of `NADIA_HOME` environment variable.
