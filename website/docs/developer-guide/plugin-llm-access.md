...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept Nadia already has applies. The plugin never sees a
...
`nadia-example-plugins` (wire reference: `github.com/NadicodeAI/nadia-example-plugins/tree/main/plugin-llm-example`)
repo (companion repo for reference plugins — not bundled with
nadia-agent itself). For the async surface (`acomplete()` /
`acomplete_structured()` with `asyncio.gather()`), see
`plugin-llm-async-example` (wire reference: `github.com/NadicodeAI/nadia-example-plugins/tree/main/plugin-llm-async-example`)
in the same repo.
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — Nadia provider id (e.g. "openrouter")
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different Nadia provider
        # (must be one Nadia already knows about — same names as
        # `nadia model` and config.yaml model.provider).
        ...
        # plugin sends — Nadia does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Auth.** Pulls API keys, OAuth tokens, or refresh tokens from
  `~/.nadia/auth.json` / env, including the credential pool when
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through Nadia's usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing Nadia subsystem:
...
* Implementation: [`agent/plugin_llm.py`](https://github.com/nadicodeai/nadia/blob/main/agent/plugin_llm.py)
* Tests: [`tests/agent/test_plugin_llm.py`](https://github.com/nadicodeai/nadia/blob/main/tests/agent/test_plugin_llm.py)
* Reference plugins (companion repo):
  * `plugin-llm-example` (wire reference: `github.com/NadicodeAI/nadia-example-plugins/tree/main/plugin-llm-example`) — sync structured extraction with image input
  * `plugin-llm-async-example` (wire reference: `github.com/NadicodeAI/nadia-example-plugins/tree/main/plugin-llm-async-example`) — async with `asyncio.gather()`
