...
sidebar_position: 11
title: Model Catalog
description: Remotely-hosted manifest driving curated model picker lists for OpenRouter and Nadia Agents Portal.
---

# Model Catalog

Nadia fetches curated model lists for **OpenRouter** and **Nadia Agents Portal** from a JSON manifest hosted alongside the docs site. This lets maintainers update picker lists without shipping a new `nadia-agent` release.
...
```
https://docs.nadicode.ai/nadia/api/model-catalog.json
```
...
- **`version`** — integer schema version. Future schemas bump this; Nadia refuses manifests with versions it doesn't understand and falls back to the hardcoded snapshot.
- **`metadata`** — free-form dict at the manifest, provider, and model level. Any keys. Nadia ignores unknown fields, so you can annotate entries (`"tier": "paid"`, `"tags": [...]`, etc.) without coordinating a schema change.
- **`description`** — OpenRouter-only. Drives picker badge text (`"recommended"`, `"free"`, or empty). Nadia Agents Portal doesn't use this — free-tier gating is determined live from the Portal's pricing endpoint.
...
| When | What happens |
|---|---|
| `/model` or `nadia model` | Fetches if disk cache is stale, else uses cache |
...
Cache location: `~/.nadia/cache/model_catalog.json`.
...
# editing OPENROUTER_MODELS or _PROVIDER_MODELS["nadia"] in nadia_cli/models.py).
