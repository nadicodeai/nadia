...
description: "Authoritative reference for Nadia terminal commands and command families"
---
...
```bash
nadia [global-options] <command> [subcommand/options]
```
...
| `--profile <name>`, `-p <name>` | Select which Nadia profile to use for this invocation. Overrides the sticky default set by `nadia profile use`. |
| `--resume <session>`, `-r <session>` | Resume a previous session by ID or title. |
| `--continue [name]`, `-c [name]` | Resume the most recent session, or the most recent session matching a title. |
| `--worktree`, `-w` | Start in an isolated git worktree for parallel-agent workflows. |
| `--yolo` | Bypass dangerous-command approval prompts. |
| `--pass-session-id` | Include the session ID in the agent's system prompt. |
| `--ignore-user-config` | Ignore `~/.nadia/config.yaml` and fall back to built-in defaults. Credentials in `.env` are still loaded. |
| `--ignore-rules` | Skip auto-injection of `AGENTS.md`, `SOUL.md`, `.cursorrules`, memory, and preloaded skills. |
| `--tui` | Launch the [TUI](../user-guide/tui.md) instead of the classic CLI. Equivalent to `NADIA_TUI=1`. Always wins over `display.interface`. |
...
| `nadia migrate` | Diagnose and (optionally) rewrite `config.yaml` to replace references to retired models or deprecated settings (e.g. `migrate xai`). |
...
| `nadia security audit` | On-demand supply-chain audit (OSV.dev) for the venv, plugin requirements, and pinned MCP servers. |
| `nadia dump` | Copy-pasteable setup summary for support/debugging. |
| `nadia prompt-size` | Show a byte breakdown of the system prompt + tool schemas (skills index, memory, profile). Runs offline. |
| `nadia debug` | Debug tools — upload logs and system info for support. |
| `nadia backup` | Back up Nadia home directory to a zip file. |
| `nadia checkpoints` | Inspect / prune / clear `~/.nadia/checkpoints/` (the shadow store used by `/rollback`). Run with no args for a status overview. |
| `nadia import` | Restore a Nadia backup from a zip file. |
| `nadia logs` | View, tail, and filter agent/gateway/error log files. |
| `nadia config` | Show, edit, migrate, and query configuration files. |
| `nadia pairing` | Approve or revoke messaging pairing codes. |
| `nadia skills` | Browse, install, publish, audit, and configure skills. |
| `nadia bundles` | Group several skills under a single `/<name>` slash command. See [Skill Bundles](../user-guide/features/skills.md#skill-bundles). |
| `nadia curator` | Background skill maintenance — status, run, pause, pin. See [Curator](../user-guide/features/curator.md). |
| `nadia memory` | Configure external memory provider. Plugin-specific subcommands (e.g. `nadia honcho`) register automatically when their provider is active. |
...
| `--ignore-user-config` | Ignore `~/.nadia/config.yaml` and use built-in defaults. Credentials in `.env` are still loaded. Useful for isolated CI runs, reproducible bug reports, and third-party integrations. |
| `--ignore-rules` | Skip auto-injection of `AGENTS.md`, `SOUL.md`, `.cursorrules`, persistent memory, and preloaded skills. Combine with `--ignore-user-config` for a fully isolated run. |
| `--safe-mode` | Troubleshooting mode: disable ALL customizations — user config, rules/memory injection, plugins, and MCP servers (implies `--ignore-user-config` and `--ignore-rules`). Use to isolate whether a problem comes from your setup or from Nadia itself. |
...
```bash
nadia
nadia chat -q "Summarize the latest PRs"
nadia chat --provider openrouter --model anthropic/claude-sonnet-4.6
nadia chat --toolsets web,terminal,skills
nadia chat --quiet -q "Return only JSON"
nadia chat --worktree -q "Review this repo and open a PR"
nadia chat --ignore-user-config --ignore-rules -q "Repro without my personal setup"
nadia chat --safe-mode -q "Is this bug mine or Nadia's?"
```

### `nadia -z <prompt>` — scripted one-shot

For programmatic callers (shell scripts, CI, cron, parent processes piping in a prompt), `nadia -z` is the purest one-shot entry point: **single prompt in, final response text out, nothing else on stdout or stderr.** No banner, no spinner, no tool previews, no `Session:` line — just the agent's final reply as plain text.

```bash
nadia -z "What's the capital of France?"
# → Paris.

# Parent scripts can cleanly capture the response:
answer=$(nadia -z "summarize this" < /path/to/file.txt)
```

Per-run overrides (no mutation to `~/.nadia/config.yaml`):

| Flag | Equivalent env var | Purpose |
|---|---|---|
| `-m` / `--model <model>` | `NADIA_INFERENCE_MODEL` | Override the model for this run |
| `--provider <provider>` | _(none)_ | Override the provider for this run |

```bash
nadia -z "…" --provider openrouter --model openai/gpt-5.5
# or:
NADIA_INFERENCE_MODEL=anthropic/claude-sonnet-4.6 nadia -z "…"
```

Same agent, same tools, same skills — just strips every interactive / cosmetic layer. If you need tool output in the transcript too, use `nadia chat -q` instead; `-z` is explicitly for "I only want the final answer".

## `nadia model`

Interactive provider + model selector. **This is the command for adding new providers, setting up API keys, and running OAuth flows.** Run it from your terminal — not from inside an active Nadia chat session.
...
:::warning nadia model vs /model — know the difference
**`nadia model`** (run from your terminal, outside any Nadia session) is the **full provider setup wizard**. It can add new providers, run OAuth flows, prompt for API keys, and configure endpoints.
...
**If you need to add a new provider:** Exit your Nadia session first (`Ctrl+C` or `/quit`), then run `nadia model` from your terminal prompt.
:::
...
If you've only configured OpenRouter, `/model` will only show OpenRouter models. To add another provider (Anthropic, DeepSeek, Copilot, etc.), exit your session and run `nadia model` from the terminal.
:::
...
```bash
nadia slack manifest              # print manifest to stdout
nadia slack manifest --write      # write to ~/.nadia/slack-manifest.json
nadia slack manifest --slashes-only  # just the features.slash_commands array
```
...
|------|---------|---------|
| `--write [PATH]` | stdout | Write to a file instead of stdout. Bare `--write` writes `$NADIA_HOME/slack-manifest.json`. |
| `--name NAME` | `Nadia` | Bot display name in Slack. |
| `--description DESC` | default blurb | Bot description shown in the Slack app directory. |
| `--slashes-only` | off | Emit only `features.slash_commands` for merging into a manually-maintained manifest. |

Run `nadia slack manifest --write` again after `nadia update` to pick
up any new commands.
...
```bash
nadia send --to <target> "message text"
nadia send --to <target> --file <path>
echo "message" | nadia send --to <target>
nadia send --list [platform]
```

Send a one-shot message to a configured messaging platform without spinning up an agent or gateway loop. Reuses the gateway's already-configured credentials (`~/.nadia/.env` + `~/.nadia/config.yaml`) so ops scripts, cron jobs, CI hooks, and monitoring daemons can post status updates without reimplementing each platform's REST client.

For bot-token platforms (Telegram, Discord, Slack, Signal, SMS, WhatsApp-CloudAPI) no running gateway is required — `nadia send` talks directly to the platform's REST endpoint. Plugin platforms that need a persistent adapter still require a live gateway.
...
If neither a positional `message` argument nor `--file` is provided, `nadia send` reads from stdin when it is not a TTY. Exit codes: `0` on success, `1` on delivery/backend failure, `2` on usage errors.
...
```bash
nadia send --to telegram "MEDIA:/tmp/screenshot.png"
nadia send --to telegram "Build chart for today MEDIA:/tmp/chart.png"   # with caption
nadia send --to discord:#ops "MEDIA:/tmp/report.pdf"
```
...
```bash
nadia send --to telegram "[[as_document]] MEDIA:/tmp/screenshot.png"
```

Examples:

```bash
nadia send --to telegram "deploy finished"
echo "RAM 92%" | nadia send --to telegram:-1001234567890
nadia send --to discord:#ops --file /tmp/report.md
nadia send --to slack:#eng --subject "[CI]" --file build.log
nadia send --list                  # all platforms
nadia send --list telegram         # filter by platform
```
...
```bash
nadia secrets bitwarden <subcommand>
nadia secrets bw <subcommand>          # short alias
```

Pull API keys from an external secret manager at process startup instead of storing them in `~/.nadia/.env`. Currently supports **Bitwarden Secrets Manager**. See the full guide: [Bitwarden integration](../user-guide/secrets/bitwarden.md).
...
> Not to be confused with `nadia claw migrate` (one-shot import of OpenClaw configuration into Nadia) — `nadia migrate` is the top-level config-rewrite command.
...
| Subcommand | Description |
|------------|-------------|
| `start` | Run the proxy in the foreground. Flags: `--provider <nadia\|xai>` (default `nous`), `--host <addr>` (default `127.0.0.1`; use `0.0.0.0` to expose on LAN), `--port <int>` (default `8645`). |
...
```bash
nadia security <subcommand>
```

On-demand vulnerability scan against [OSV.dev](https://osv.dev). Covers the Nadia venv (installed PyPI distributions), Python dependencies declared by plugins under `~/.nadia/plugins/`, and pinned `npx`/`uvx` MCP servers in `config.yaml`. Does NOT scan globally-installed packages or editor/browser extensions.
...
| `--skip-venv` | off | Skip scanning the Nadia Python venv. |
| `--skip-plugins` | off | Skip scanning plugin requirements files. |
| `--skip-mcp` | off | Skip scanning pinned MCP servers in `config.yaml`. |


## `nadia login` / `nadia logout` *(Deprecated)*

:::caution
`nadia login` has been removed. Use `nadia auth` to manage OAuth credentials, `nadia model` to select a provider, or `nadia setup` for full interactive setup.
:::
...
```bash
nadia auth                                              # Interactive wizard
nadia auth list                                         # Show all pools
nadia auth list openrouter                              # Show specific provider
nadia auth add openrouter --api-key sk-or-v1-xxx        # Add API key
nadia auth add anthropic --type oauth                   # Add OAuth credential
nadia auth remove openrouter 2                          # Remove by index
nadia auth reset openrouter                             # Clear cooldowns
nadia auth status anthropic                             # Show auth status for a provider
nadia auth logout anthropic                             # Log out and clear stored auth state
nadia auth spotify                                      # Authenticate Nadia with Spotify via PKCE
```
...
```bash
nadia cron <list|create|edit|pause|resume|run|remove|status|tick>
```
...
```bash
nadia kanban [--board <slug>] <action> [options]
```

Multi-profile, multi-project collaboration board. Each install can host many boards (one per project, repo, or domain); each board is a standalone queue with its own SQLite DB and dispatcher scope. New installs start with one board called `default`, whose DB is `~/.nadia/kanban.db` for back-compat; additional boards live at `~/.nadia/kanban/boards/<slug>/kanban.db`. The gateway-embedded dispatcher sweeps every board per tick.

**Global flags (apply to every action below):**

| Flag | Purpose |
|------|---------|
| `--board <slug>` | Operate on a specific board. Defaults to the current board (set via `nadia kanban boards switch`, the `NADIA_KANBAN_BOARD` env var, or `default`). |

**This is the human / scripting surface.** Agent workers spawned by the dispatcher drive the board through a dedicated `kanban_*` [toolset](/user-guide/features/kanban#how-workers-interact-with-the-board) (`kanban_show`, `kanban_complete`, `kanban_block`, `kanban_create`, `kanban_link`, `kanban_comment`, `kanban_heartbeat`; orchestrator profiles also get `kanban_list` and `kanban_unblock`) instead of shelling to `nadia kanban`. Workers have `NADIA_KANBAN_BOARD` pinned in their env so they physically cannot see other boards.
...
| `boards switch <slug>` / `boards use` | Persist `<slug>` as the active board (writes `~/.nadia/kanban/current`). |
...
nadia kanban boards create atm10-server --name "ATM10 Server" --icon 🎮
nadia kanban --board atm10-server create "Restart server" --assignee ops

# Switch the active board for subsequent calls.
nadia kanban boards switch atm10-server
nadia kanban list                  # shows atm10-server tasks

# Archive a board (recoverable) or hard-delete it.
nadia kanban boards rm atm10-server
nadia kanban boards rm atm10-server --delete
```

Board resolution order (highest precedence first): `--board <slug>` flag → `NADIA_KANBAN_BOARD` env var → `~/.nadia/kanban/current` file → `default`.
...
```bash
nadia webhook <subscribe|list|remove|test>
```
...
### `nadia webhook subscribe`

```bash
nadia webhook subscribe <name> [options]
```
...
Subscriptions persist to `~/.nadia/webhook_subscriptions.json` and are hot-reloaded by the webhook adapter without a gateway restart.
...
```bash
nadia dump [--show-keys]
```

Outputs a compact, plain-text summary of your entire Nadia setup. Designed to be copy-pasted into Discord, GitHub issues, or Telegram when asking for support — no ANSI colors, no special formatting, just data.
...
| Section | Details |
|---------|---------|
| **Header** | Nadia version, release date, git commit hash |
| **Environment** | OS, Python version, OpenAI SDK version |
| **Identity** | Active profile name, NADIA_HOME path |
...
:::tip
`nadia dump` is specifically designed for sharing. For interactive diagnostics, use `nadia doctor`. For a visual overview, use `nadia status`.
:::

## `nadia debug`

```bash
nadia debug share [options]
```
...
```bash
nadia debug share              # Upload debug report, print URL
nadia debug share --lines 500  # Include more log lines
nadia debug share --expire 30  # Keep paste for 30 days
nadia debug share --local      # Print report to terminal (no upload)
```

## `nadia backup`

```bash
nadia backup [options]
```

Create a zip archive of your Nadia configuration, skills, sessions, and data. The backup excludes the nadia-agent codebase itself.

| Option | Description |
|--------|-------------|
| `-o`, `--output <path>` | Output path for the zip file (default: `~/nadia-backup-<timestamp>.zip`). |
| `-q`, `--quick` | Quick snapshot: only critical state files (config.yaml, state.db, .env, auth, cron jobs). Much faster than a full backup. |
| `-l`, `--label <name>` | Label for the snapshot (only used with `--quick`). |

The backup uses SQLite's `backup()` API for safe copying, so it works correctly even when Nadia is running (WAL-mode safe).

**What's excluded from the zip:**

- `*.db-wal`, `*.db-shm`, `*.db-journal` — SQLite's WAL / shared-memory / journal sidecars. The `*.db` file already got a consistent snapshot via `sqlite3.backup()`; shipping the live sidecars alongside it would let a restore see a half-committed state.
- `checkpoints/` — per-session trajectory caches. Hash-keyed and regenerated per session; wouldn't port cleanly to another install anyway.
- The `nadia-agent` code itself (this is a user-data backup, not a repo snapshot).

### Examples

```bash
nadia backup                           # Full backup to ~/nadia-backup-*.zip
nadia backup -o /tmp/nadia.zip        # Full backup to specific path
nadia backup --quick                   # Quick state-only snapshot
nadia backup --quick --label "pre-upgrade"  # Quick snapshot with label
```

## `nadia checkpoints`

```bash
nadia checkpoints [COMMAND]
```

Inspect and manage the shadow git store at `~/.nadia/checkpoints/` — the storage layer behind the in-session `/rollback` command. Safe to run any time; does not require the agent to be running.

| Subcommand | Description |
|------------|-------------|
| `status` (default) | Show total size, project count, and per-project breakdown. Bare `nadia checkpoints` is equivalent. |
...
```bash
nadia checkpoints                                  # status overview
nadia checkpoints prune --retention-days 3         # aggressive cleanup
nadia checkpoints prune --max-size-mb 200          # tighten size cap once
nadia checkpoints clear-legacy -f                  # drop v1 archive dirs
nadia checkpoints clear -f                         # wipe everything
```
...
```bash
nadia import <zipfile> [options]
```

Restore a previously created Nadia backup into your Nadia home directory. All files in the archive overwrite existing files in your Nadia home; `--force` only skips the confirmation prompt that fires when the target already has a Nadia installation.
...
### Examples
```bash
nadia import ~/nadia-backup-20260423.zip           # Prompts before overwriting existing config
nadia import ~/nadia-backup-20260423.zip --force   # Overwrite without prompting
```

## `nadia logs`

```bash
nadia logs [log_name] [options]
```

View, tail, and filter Nadia log files. All logs are stored in `~/.nadia/logs/` (or `<profile>/logs/` for non-default profiles).
...
nadia logs gateway -n 100

# Show only warnings and errors from the last hour
nadia logs --level WARNING --since 1h

# Filter by a specific session
nadia logs --session abc123

# Follow errors.log, starting from 30 minutes ago
nadia logs errors --since 30m -f
...
nadia logs --level WARNING --since 2h --session tg-12345
```
...
Nadia uses Python's `RotatingFileHandler`. Old logs are rotated automatically — look for `agent.log.1`, `agent.log.2`, etc. The `nadia logs list` subcommand shows all log files including rotated ones.


## `nadia prompt-size`

```bash
nadia prompt-size [--platform <name>] [--json]
```
...
- **Prompt tiers** — stable / context / volatile, matching how Nadia layers
...
nadia prompt-size --platform telegram

# Machine-readable output for scripts
nadia prompt-size --json
```

:::tip
The skills index and tool schemas scale with how many skills and tools you have
enabled. To shrink the prompt, disable unused toolsets (`nadia tools`) or
uninstall skills you don't need (`nadia skills`). Context files (AGENTS.md,
...
```bash
nadia pairing <list|approve|revoke|clear-pending>
```
...
```bash
nadia skills browse
nadia skills browse --source official
nadia skills search react --source skills-sh
nadia skills search https://mintlify.com/docs --source well-known
nadia skills inspect official/security/1password
nadia skills inspect skills-sh/vercel-labs/json-render/json-render-react
nadia skills install official/migration/openclaw-migration
nadia skills install skills-sh/anthropics/skills/pdf --force
nadia skills install https://sharethis.chat/SKILL.md                     # Direct URL (single-file SKILL.md)
nadia skills install https://example.com/SKILL.md --name my-skill        # Override name when frontmatter has none
nadia skills check
nadia skills update
nadia skills config
nadia skills reset google-workspace
nadia skills reset google-workspace --restore --yes
nadia skills opt-out                  # stop future bundled-skill seeding (nothing deleted)
nadia skills opt-out --remove --yes   # also delete UNMODIFIED bundled skills
nadia skills opt-in --sync            # undo: remove marker and re-seed now
```
...
- `--source well-known` lets you point Nadia at a site exposing `/.well-known/skills/index.json`.
...
```bash
nadia bundles <subcommand>
```

Skill bundles group several skills under one `/<bundle-name>` slash command. Invoking the bundle loads every referenced skill into a single combined user message. Storage: `~/.nadia/skill-bundles/<slug>.yaml`. See [Skill Bundles](../user-guide/features/skills.md#skill-bundles) for the YAML schema and behavior.
...
| `reload` | Re-scan `~/.nadia/skill-bundles/` and report added/removed bundles |

Examples:

```bash
nadia bundles create backend-dev \
...
nadia bundles list
nadia bundles show backend-dev
nadia bundles delete backend-dev
```
...
| `backup` | Take a manual tar.gz snapshot of `~/.nadia/skills/` (curator also snapshots automatically before every real run) |
| `rollback` | Restore `~/.nadia/skills/` from a snapshot (defaults to newest) |
...
On a fresh install the first scheduled pass is deferred by one full `interval_hours` (7 days by default) — the gateway will not curate immediately on the first tick after `nadia update`. Use `nadia curator run --dry-run` to preview before that happens.
...
| `add` | Pick a provider + model (same picker as `nadia model`) and append to the chain |
...
```bash
nadia hooks <subcommand>
```

Inspect shell-script hooks declared in `~/.nadia/config.yaml`, test them against synthetic payloads, and manage the first-use consent allowlist at `~/.nadia/shell-hooks-allowlist.json`.
...
When an external memory provider is active, it may register its own top-level `nadia <provider>` command for provider-specific management (e.g. `nadia honcho` when Honcho is active). Inactive providers do not expose their subcommands. Run `nadia --help` to see what's currently wired in.
:::
...
Starts Nadia as an ACP (Agent Client Protocol) stdio server for editor integration.
...
```bash
nadia mcp <subcommand>
```

Manage MCP (Model Context Protocol) server configurations and run Nadia as an MCP server.
...
See [MCP Config Reference](./mcp-config-reference.md), [Use MCP with Nadia](../guides/use-mcp-with-nadia.md), and [MCP Server Mode](../user-guide/features/mcp.md#running-nadia-as-an-mcp-server).

## `nadia plugins`

```bash
nadia plugins [subcommand]
```

Unified plugin management — general plugins, memory providers, and context engines in one place. Running `nadia plugins` with no subcommand opens a composite interactive screen with two sections:
...
See [Plugins](../user-guide/features/plugins.md) and [Build a Nadia Plugin](../guides/build-a-nadia-plugin.md).
...
`nadia computer-use install` is the stable entry point for installing the
[cua-driver](https://github.com/trycua/cua) binary used by the
`computer_use` toolset. It runs the same upstream installer that
`nadia tools` invokes when you first enable Computer Use, so it's safe
to use for re-running the install if the toolset toggle didn't trigger
it (for example, on returning-user setups).

`nadia update` automatically re-runs the upstream installer at the end
...
```bash
nadia insights [--days N] [--source platform]
```
...
```bash
nadia claw migrate [options]
```

Migrate your OpenClaw setup to Nadia. Reads from `~/.openclaw` (or a custom path) and writes to `~/.nadia`. Automatically detects legacy directory names (`~/.clawdbot`, `~/.moltbot`) and config filenames (`clawdbot.json`, `moltbot.json`).

| Option | Description |
|--------|-------------|
| `--dry-run` | Preview what would be migrated without writing anything. |
| `--preset <name>` | Migration preset: `full` (all compatible settings) or `user-data` (excludes infrastructure config). Neither preset imports secrets — pass `--migrate-secrets` explicitly. |
| `--overwrite` | Overwrite existing Nadia files on conflicts (default: refuse to apply when the plan has conflicts). |
| `--migrate-secrets` | Include API keys in migration. Required even under `--preset full`. |
| `--no-backup` | Skip the pre-migration zip snapshot of `~/.nadia/` (by default a single restore-point archive is written to `~/.nadia/backups/pre-migration-*.zip` before apply; restorable with `nadia import`). |
...
The migration covers 30+ categories across persona, memory, skills, model providers, messaging platforms, agent behavior, session policies, MCP servers, TTS, and more. Items are either **directly imported** into Nadia equivalents or **archived** for manual review.
...
nadia claw migrate --dry-run

# Full migration (all compatible settings, no secrets)
nadia claw migrate --preset full

# Full migration including API keys
nadia claw migrate --preset full --migrate-secrets

# Migrate user data only (no secrets), overwrite conflicts
nadia claw migrate --preset user-data --overwrite
...
nadia dashboard --port 8080 --no-open
...
```bash
nadia profile <subcommand>
```

Manage profiles — multiple isolated Nadia instances, each with its own config, sessions, skills, and home directory.
...
```bash
nadia profile list
nadia profile create work --clone
nadia profile use work
nadia profile alias work --name h-work
nadia profile export work -o work-backup.tar.gz
nadia profile import work-backup.tar.gz --name restored
nadia profile install github.com/user/my-distro --alias
nadia profile update work
nadia -p work chat -q "Hello from work profile"
```

## `nadia completion`

```bash
nadia completion [bash|zsh|fish]
```

Print a shell completion script to stdout. Source the output in your shell profile for tab-completion of Nadia commands, subcommands, and profile names.

Examples:

```bash
# Bash
nadia completion bash >> ~/.bashrc

# Zsh
nadia completion zsh >> ~/.zshrc

# Fish
nadia completion fish > ~/.config/fish/completions/nadia.fish
```
...
| `--backup` | Create a labeled pre-update snapshot of `NADIA_HOME` (config, auth, sessions, skills, pairing data) before pulling. Default is **off** — the previous always-backup behavior was adding minutes to every update on large homes. Flip it on permanently via `updates.pre_update_backup: true` in `config.yaml`. |
| `--yes`, `-y` | Assume yes for interactive prompts such as config migration and stash restore. API-key entry is skipped; run `nadia config migrate` separately for those. |

Additional behavior:

- **Gateway restart.** After a successful update, Nadia attempts to restart all running gateway profiles automatically so they pick up the new code. Use `nadia gateway restart` when you want to restart a gateway without applying an update.
...
- **npm lockfile churn.** Before stashing or switching branches, Nadia makes a best-effort cleanup of tracked `package-lock.json` diffs produced by npm install/build steps. Commit or manually stash intentional lockfile edits before running `nadia update`.
- **Pairing data snapshot.** Even when `--backup` is off, `nadia update` takes a lightweight snapshot of `~/.nadia/pairing/` before `git pull`. You can roll it back with `nadia backup restore --state pre-update` if a pull rewrites a file you were editing.
- **Legacy `nadia.service` warning.** If Nadia detects a pre-rename `nadia.service` systemd unit (instead of the current `nadia-gateway.service`), it prints a one-time migration hint so you can avoid flap-loop issues.
