...
This page covers all commands related to [Nadia profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `nadia profile` without a subcommand shows help.
...
```bash
nadia profile use <name>
```

Sets `<name>` as the active profile. All subsequent `nadia` commands (without `-p`) will use this profile.
...
nadia profile use default
```

## `nadia profile create`

```bash
nadia profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `nadia profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `nadia update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.nadia`), use `nadia skills opt-out` / `nadia skills opt-in`. |
...
nadia profile create work2 --clone-from work

# Clone everything from a specific profile
nadia profile create work2-backup --clone-from work --clone-all
```

## `nadia profile describe`

```bash
nadia profile describe [<name>] [options]
```
...
nadia profile describe researcher

# Set it explicitly
nadia profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
nadia profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
nadia profile describe --all --auto
```

## `nadia profile delete`

```bash
nadia profile delete <name> [options]
```
...
```bash
nadia profile delete mybot
nadia profile delete mybot --yes
```
...
```bash
nadia profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Nadia home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ nadia profile show work
Profile: work
Path:    ~/.nadia/profiles/work
...
```bash
nadia profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Nadia installation.
...
```bash
nadia profile alias work
# Creates/updates ~/.local/bin/work

nadia profile alias work --name mywork
# Creates ~/.local/bin/mywork

nadia profile alias work --remove
# Removes the wrapper script
```

## `nadia profile rename`

```bash
nadia profile rename <old-name> <new-name>
```
...
```bash
nadia profile rename mybot assistant
# ~/.nadia/profiles/mybot → ~/.nadia/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `nadia profile export`

```bash
nadia profile export <name> [options]
```
...
```bash
nadia profile export work
# Creates work.tar.gz in the current directory

nadia profile export work -o ./work-2026-03-29.tar.gz
```

## `nadia profile import`

```bash
nadia profile import <archive> [options]
```
...
```bash
nadia profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

nadia profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`nadia profile export` / `import` are still the right commands for
...
### `nadia profile install`

```bash
nadia profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `nadia -p telemetry`). |
...
nadia profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
nadia profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
nadia profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
nadia profile install ./telemetry/
```

### `nadia profile update`

```bash
nadia profile update <name> [--force-config] [--yes]
```
...
```bash
nadia profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Nadia version, author, env var requirements, the source URL/path, and
...
`nadia profile list` also shows the distribution name and version in a
`Distribution` column, and `nadia profile show <name>` / `delete <name>`
...
nadia profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
nadia profile install https://github.com/your-org/internal-assistant.git
```
...
`nadia_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
Nadia version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Nadia can clone from.
3. Tell recipients to run `nadia profile install <your-repo-url>`.
...
## `nadia -p` / `nadia --profile`

```bash
nadia -p <name> <command> [options]
nadia --profile <name> <command> [options]
```

Global flag to run any Nadia command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
nadia -p work chat -q "Check the server status"
nadia --profile dev gateway start
nadia -p personal skills list
nadia -p work config edit
```

## `nadia completion`

```bash
nadia completion <shell>
```
...
- `nadia profile <TAB>` — subcommands (list, use, create, etc.)
- `nadia profile use <TAB>` — profile names
- `nadia -p <TAB>` — profile names
