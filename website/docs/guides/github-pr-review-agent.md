...
┌───────────────────────────────────────────────────────────────────┐
│                                                                   │
│   Cron Timer  ──▶  Nadia Agent  ──▶  GitHub API  ──▶  Review     │
...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to Nadia instantly when PRs are opened or updated.
:::
...
- **Nadia Agent installed** — see the [Installation guide](/getting-started/installation)
- **Gateway running** for cron jobs:
  ```bash
  nadia gateway install   # Install as a service
  # or
...
Use `deliver: "local"` to save reviews to `~/.nadia/cron/output/`. Great for testing before wiring up notifications.
:::
...
Make sure Nadia can access GitHub. Start a chat:
...
```
Run: gh pr list --repo nadicodeai/nadia --state open --limit 3
```
...
Still in the chat, ask Nadia to review a real PR:

```
Review this pull request. Read the diff, check for bugs, security issues,
and code quality. Be specific about line numbers and quote problematic code.

Run: gh pr diff 3888 --repo nadicodeai/nadia
```
...
A skill gives Nadia consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.

```bash
mkdir -p ~/.nadia/skills/code-review
```

Create `~/.nadia/skills/code-review/SKILL.md`:
...
Verify it loaded — start `nadia` and you should see `code-review` in the skills list at startup.

---

## Step 4: Teach It Your Conventions

This is what makes the reviewer actually useful. Start a session and teach Nadia your team's standards:
...
2. Teach Nadia your conventions via memory (Step 4)
3. The more context it has about your stack, the better the reviews

### Cron job doesn't run
```bash
nadia gateway status    # Is the gateway running?
nadia cron list         # Is the job enabled?
```
...
- **[Build a Plugin](/guides/build-a-nadia-plugin)** — wrap the review logic into a shareable plugin
