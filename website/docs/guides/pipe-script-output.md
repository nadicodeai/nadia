...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `nadia send`."
---

# Pipe Script Output to Messaging Platforms

`nadia send` is a small, scriptable CLI that pushes a message to any
messaging platform Nadia is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | nadia send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `nadia
...
nadia send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
nadia send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
nadia send --list

# Filter by platform
nadia send --list telegram
```
...
Any platform Nadia ships adapters for works as a target:
`telegram`, `discord`, `slack`, `signal`, `sms`, `whatsapp`, `matrix`,
`mattermost`, `email`, and
others.
...
`nadia send` resolves the message body in this order:

1. **Positional argument** — `nadia send --to telegram "hi"`
2. **`--file PATH`** — `nadia send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | nadia send --to telegram`

When stdin is a TTY (no pipe), Nadia does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  nadia send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `nadia send` reuses your Nadia config, the same script works on
any host where Nadia is installed — no need to export bot tokens into
...
`nadia send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  nadia send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | nadia send \
...
```bash
./train.py --epochs 200 && \
  nadia send --to telegram "training done" || \
  nadia send --to telegram "training failed (exit $?)"
```
...
nadia send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(nadia send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `nadia send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `nadia send` calls
the platform's REST endpoint directly using credentials from
`~/.nadia/.env` and `~/.nadia/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `nadia gateway start` and retry.
...
# Machine-readable
nadia send --list --json
```

The listing is built from `~/.nadia/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`nadia gateway
...
| Approach | Multi-platform | Reuses Nadia creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `nadia send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`nadia send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `nadia send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `nadia send` shares with cron delivery.
