...
Copy-paste blueprints for common automation patterns. Each blueprint uses Nadia's built-in [cron scheduler](/user-guide/features/cron) for time-based triggers and [webhook platform](/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`nadia webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `nadia webhook subscribe`) |
...
```bash
nadia cron create "0 2 * * *" \
  "You are a project manager triaging the nadicodeai/nadia GitHub repo.

1. Run: gh issue list --repo nadicodeai/nadia --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
nadia webhook subscribe github-pr-review \
  --events "pull_request" \
...
```bash
nadia cron create "0 9 * * 1" \
  "Scan the nadicodeai/nadia repo for documentation drift.

1. Run: gh pr list --repo nadicodeai/nadia --state merged --json number,title,files,mergedAt --limit 30
2. Filter to PRs merged in the last 7 days
3. For each merged PR, check if it modified:
   - Tool schemas (tools/*.py) — may need docs/reference/tools-reference.md update
   - CLI commands (nadia_cli/commands.py, nadia_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (nadia_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
nadia cron create "0 6 * * *" \
  "Run a dependency security audit on the nadia-agent project.

1. cd ~/.nadia/nadia-agent && source .venv/bin/activate
...
```bash
nadia webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
nadia webhook subscribe alert-triage \
...
```python title="~/.nadia/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "Nadia-Monitor/1.0"})
...
nadia cron create "every 30m" \
  ...
  --script ~/.nadia/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
nadia cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Nadia Agent development." \
...
```bash
nadia webhook subscribe github-issues \
  --events "issues" \
...
```bash
nadia webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
nadia webhook subscribe stripe-payments \
...
```bash
nadia cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the nadia-agent codebase.
