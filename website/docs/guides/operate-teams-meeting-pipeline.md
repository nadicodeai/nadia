...
```bash
nadia teams-pipeline token-health
nadia teams-pipeline token-health --force-refresh
```
...
```bash
nadia teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
nadia teams-pipeline maintain-subscriptions
nadia teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Nadia cron (recommended if you already run the Nadia gateway)

Nadia ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.nadia/scripts/`. First create the script:

```bash
mkdir -p ~/.nadia/scripts
cat > ~/.nadia/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec nadia teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.nadia/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
nadia cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/nadia-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Nadia Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=nadia
EnvironmentFile=/etc/nadia/env
ExecStart=/usr/local/bin/nadia teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/nadia-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Nadia Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now nadia-teams-pipeline-maintain.timer
systemctl list-timers nadia-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/nadia teams-pipeline maintain-subscriptions >> /var/log/nadia/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.nadia/.env` at the top of a wrapper script that crontab calls.
...
```bash
nadia teams-pipeline subscriptions   # should show expirationDateTime advanced
nadia teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
nadia teams-pipeline list
nadia teams-pipeline list --status failed
nadia teams-pipeline show <job-id>
```

### Replay a stored job

```bash
nadia teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
nadia teams-pipeline fetch --meeting-id <meeting-id>
nadia teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `nadia teams-pipeline maintain-subscriptions --dry-run`
- inspect `nadia teams-pipeline list --status failed`
...
- run `nadia teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `nadia teams-pipeline run`
...
- [ ] `nadia teams-pipeline validate` returns an OK snapshot
- [ ] `nadia teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Nadia cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
