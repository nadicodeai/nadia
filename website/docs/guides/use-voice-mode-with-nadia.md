---
sidebar_position: 8
title: "Use Voice Mode with Nadia"
description: "A practical guide to setting up and using Nadia voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Nadia
...
:::tip
[Nadia Agents Portal](/integrations/nadia-portal) bundles both the LLM and TTS through one OAuth — voice mode works end-to-end with no extra credentials.
:::
...
- you want Nadia sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Nadia.
...
## Step 1: make sure normal Nadia works first
...
Nadia supports both local and cloud speech stacks.
...
### If you use `nadia setup`

If you choose NeuTTS in the setup wizard, Nadia checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Nadia transcribes and responds
...
- asking Nadia to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Nadia loop.

## Tuning CLI behavior

### Silence threshold

If Nadia starts/stops too aggressively, tune:
...
Nadia stays a normal chat bot, but can speak replies.
...
- you want Nadia to function like a portable research or ops assistant
...
Nadia joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Nadia detects speech boundaries
- transcripts are posted in the associated text channel
- Nadia responds in text and audio
...
1. get text Nadia working
2. install `nadia-agent[voice]`
