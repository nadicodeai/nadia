...
This tutorial walks you through setting up a Telegram bot powered by Nadia Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Nadia Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.nadia/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Nadia itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Nadia Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_nadia_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Nadia Agent. DM me for help with code, research, debugging, and more.
   ```
...
```
[Gateway] Starting Nadia Gateway...
...
# View live logs
journalctl --user -u nadia-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `nadia gateway install` to update the plist.
:::
...
```bash
nadia gateway stop && nadia gateway start
```
...
nadia pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.nadia/.env`:
...
Control how much detail the bot shows when using tools. In `~/.nadia/config.yaml`:
...
Customize how the bot communicates by editing `~/.nadia/SOUL.md`:

For a full guide, see [Use SOUL.md with Nadia](/guides/use-soul-with-nadia).
...
```bash
# From the CLI
nadia cron list          # View all scheduled jobs
nadia cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u nadia-gateway` (Linux) or `~/.nadia/logs/gateway.log` (macOS) |
| Cron job output | `~/.nadia/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.nadia/cron/jobs.json` |
| Pairing data | `~/.nadia/pairing/` |
| Session history | `~/.nadia/sessions/` |
