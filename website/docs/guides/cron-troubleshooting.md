...
description: "Diagnose and fix common Nadia cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`nadia gateway` for foreground, or `nadia gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `nadia cron tick`.
...
```bash
date
nadia cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.nadia/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.nadia/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.nadia/.env` |
...
| `local` | Write access to `~/.nadia/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `nadia cron list` for updated `last_error` field (if available).
...
Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `nadia skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-daily-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `nadia skills list`.
...
2. `~/.nadia/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `nadia cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the Nadia config directory). Verify:
```bash
ls ~/.nadia/scripts/your-script.py   # Must exist
nadia cron edit <job_id> --script ~/.nadia/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `nadia skills install <skill-name>`.
...
Jobs are stored in `~/.nadia/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.nadia/cron/jobs.json
chmod 600 ~/.nadia/cron/jobs.json   # Your user should own it
```
...
```bash
nadia cron list                    # Show all jobs, states, next_run times
nadia cron run <job_id>            # Schedule for next tick (for testing)
nadia cron edit <job_id>           # Fix configuration issues
nadia logs                         # View recent Nadia logs
nadia skills list                  # Verify installed skills
```
...
1. Run the job with `nadia cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.nadia/logs/agent.log` for scheduler messages and `~/.nadia/logs/errors.log` for warnings
3. Open an issue at [github.com/nadicodeai/nadia](https://github.com/nadicodeai/nadia) with:
