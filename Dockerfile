...
# Do not write .pyc files at runtime: /opt/nadia is immutable in the
...
# subprocesses, git, bun, etc.) that would otherwise accumulate when nadia
# ran as PID 1. See #15012. Phase 2 of the s6-overlay supervision plan
# replaces tini with s6-overlay's /init (PID 1 = s6-svscan), which reaps
# zombies non-blockingly on SIGCHLD and additionally supervises the main
# nadia process, the dashboard, and per-profile gateways.
RUN apt-get update && \
...
# ---------- s6-overlay install ----------
# s6-overlay provides supervision for the main nadia process, the dashboard,
...
RUN set -eu; \
    ...
    # 'Nadia WebUI' catalog). The image has moved to s6-overlay /init\
...
# Non-root user for runtime; UID can be overridden via NADIA_UID at runtime
RUN useradd -u 10000 -m -d /opt/data nadia
...
#
# ui-tui/packages/nadia-ink/ is copied IN FULL (not just its manifests)
...
COPY ui-tui/packages/nadia-ink/ ui-tui/packages/nadia-ink/
# apps/shared/ is copied IN FULL because web/package.json references it as a
# `file:` workspace dependency (same pattern as nadia-ink above).
...
# lock on the @nadia/ink entry, tripped the TUI launcher's
...
# for the same reason: it lazy-installs into /opt/nadia/.venv at first
...
# gives the non-root nadia user read + traverse but no write; root retains
# write so the build steps below don't need chmod u+w dances.
COPY --link --chmod=a+rX,go-w . .

# ---------- Permissions ----------
# Link nadia-agent itself (editable). Deps are already installed in the
# cached layer above; `--no-deps` makes this a fast egg-link creation with no
# resolution or downloads.
RUN uv pip install --no-cache-dir --no-deps -e "."

# Wire the exec shim and install-method stamp.  Files under /opt/nadia are
# already root-owned (COPY, uv sync, npm install all run as root) and
# read-only for the nadia user (go-w from the --chmod above).

USER root
RUN mkdir -p /opt/nadia/bin && \
    cp /opt/nadia/docker/nadia-exec-shim.sh /opt/nadia/bin/nadia && \
    chmod 0755 /opt/nadia/bin/nadia && \
    printf 'docker\n' > /opt/nadia/.install_method
# The ``.install_method`` stamp is baked next to the running code (the install
# tree), NOT into $NADIA_HOME. $NADIA_HOME (/opt/data) is a shared data
# volume that is commonly bind-mounted from the host and even shared with a
# host-side Desktop/CLI install; stamping it at boot used to clobber that
# host install's marker and wrongly block its ``nadia update``. A code-scoped
# stamp is read first by detect_install_method() and is immune to the share.
# Start as root so the s6-overlay stage2 hook can usermod/groupmod and chown
# the data volume. Each supervised service then drops to the nadia user via
# `s6-setuidgid nadia` in its run script. If NADIA_UID is unset, services
# run as the default nadia user (UID 10000).

# ---------- Bake build-time git revision ----------
# .dockerignore excludes .git, so `git rev-parse HEAD` from inside the
# container always returns nothing — meaning `nadia dump` reports
# "(unknown)" and the startup banner drops its `· upstream <sha>` suffix.
# That makes support triage from container bug reports impossible:
# we can't tell which commit the user is actually running.
#
# Fix: write the commit SHA passed via the NADIA_GIT_SHA build-arg to
# /opt/nadia/.nadia_build_sha at build time, and have
# nadia_cli/build_info.py read it at runtime.  Both `nadia dump` and
...
ARG NADIA_GIT_SHA=
RUN if [ -n "${NADIA_GIT_SHA}" ]; then \
        printf '%s\n' "${NADIA_GIT_SHA}" > /opt/nadia/.nadia_build_sha; \
    fi

# ---------- s6-overlay service wiring ----------
# Static services declared at build time: main-nadia + dashboard.
...
# fast path (`node --expose-gc /opt/nadia/ui-tui/dist/entry.js`) and skip the
...
ENV NADIA_TUI_DIR=/opt/nadia/ui-tui
ENV NADIA_HOME=/opt/data
ENV NADIA_WRITE_SAFE_ROOT=/opt/data
ENV NADIA_DISABLE_LAZY_INSTALLS=1
# The published image seals /opt/nadia (root-owned, read-only) so a runtime
...
# is seeded + chowned to the nadia user by docker/stage2-hook.sh and lives
...
ENV NADIA_LAZY_INSTALL_TARGET=/opt/data/lazy-packages

# `docker exec` privilege-drop shim. When operators run
# `docker exec <c> nadia ...` they default to root, and any file the
# command writes under $NADIA_HOME (auth.json, .env, config.yaml) ends
# up root-owned and unreadable to the supervised gateway (UID 10000).
# The shim lives at /opt/nadia/bin/nadia, sits earliest on PATH, and
# transparently re-exec's the real venv binary via `s6-setuidgid nadia`
# when invoked as root. Non-root callers (supervised processes,
# `--user nadia`, etc.) hit the short-circuit path with no overhead.
# Recursion is impossible because the shim exec's the venv binary by
# absolute path (/opt/nadia/.venv/bin/nadia). See the shim source for
# the opt-out env var (NADIA_DOCKER_EXEC_AS_ROOT=1).

# Pre-s6 entrypoint.sh did `source .venv/bin/activate` which exported
# the venv bin onto PATH; Architecture B's main-wrapper.sh does the
# same for the container's main process, but `docker exec` and our
# cont-init.d scripts don't pass through the wrapper. Expose the venv
# bin globally so `docker exec <container> nadia ...` and any
# subprocess that doesn't activate the venv first still find nadia.
#
# /opt/nadia/bin is prepended ahead of the venv so the privilege-drop
# shim wins PATH resolution. The shim's last act is to exec the venv
# binary by absolute path, so this PATH ordering is transparent to
# every other consumer.
ENV PATH="/opt/nadia/bin:/opt/nadia/.venv/bin:/opt/data/.local/bin:${PATH}"
...
#
# main-wrapper.sh handles arg routing (bare-exec vs. nadia
# subcommand vs. no-args), drops to the nadia user via s6-setuidgid,
