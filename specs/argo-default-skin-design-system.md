# Spec: Argo default skin driven by the NadicodeAI Design System
...
- **Success:** Fresh image → `argo` runs the `argo` skin by default (DS ink colors, Argo banner art, no caduceus, minimal spinner); `argo dashboard` renders in the DS theme; a DS version bump re-flows into all surfaces on the next image build; **zero `upstream/` files edited directly**; all existing gates (build, leakage, pristine, packaging-contract, dist-test) stay green.
...
_Added after the 2026-06-06 spec review. The Confirmed Intent's end-goal (all shipped surfaces present as Argo via the DS) is unchanged; delivery is sequenced because the web surface is a larger migration than first assumed (see review finding below)._

- **Phase 1 (this spec):** the **terminal CLI skin** + **`ui-tui`** — both colored from the DS DTCG tokens, Argo ASCII art replacing the caduceus, neutralized spinner, `argo` made the default skin in the image. Self-contained and brand-faithful on its own.
- **Phase 2 (deferred — separate spec/plan):** the **web dashboard**. The dashboard is not bare Tailwind — it runs **`@nous-research/ui` 0.18.2 as its design system, imported by 38 files** (`grep -rln "@nous-research/ui" upstream/web/src` → 38), wired in `upstream/web/src/index.css` (its fonts + globals + JIT `@source`). Adopting the NadicodeAI DS there is a **component-library migration** (component-API gap, Geist-vs-Collapse/Mondwest font swap, utility-class reconciliation) and also removes a Nous-branded dependency from the shipped bundle. That is its own project; the Confirmed Intent's `argo dashboard` success line moves to Phase 2.
...
- **Distribution default-skin-switch standard:** there must be ONE clearly-documented mechanism for "change a shipped default for the distribution" (the `display.skin: argo` switch is the first use). If the existing `content_edits`/patch conventions don't already document this crisply, document them.
...
1. The `argo` skin is registered as a **built-in** skin (so it is always available and selectable as the config default), introduced via a **quilt patch** to `upstream/hermes_cli/skin_engine.py`'s `_BUILTIN_SKINS` — not shipped as a drop-in `~/.argo/skins/argo.yaml` user file. Patching upstream via quilt is sanctioned; directly editing `upstream/` is not. _(Mechanism is an Open Question — see below; this is the working assumption.)_
2. The default-skin switch (`display.skin: argo`) is applied **for the distribution only** (via the rename/overlay/config layer), not by changing upstream's config default. The fork is Docker-only, so "image default" == "product default". Verified safe: `cli.py:2884` hardcodes the Hermes banner only when `skin_name == "default"`; a non-`default` skin uses `agent_name` branding instead, so switching the default to `argo` bypasses the hardcoded "⚕ NOUS HERMES" string.
3. "Generated from tokens" means a **build-time generator** that reads the DS DTCG export and emits **two** targets into `dist/argo/` during the build: (a) the Python `argo` skin color map, and (b) the `ui-tui` TypeScript theme override (`ThemeColors`). `ui-tui` themes itself independently of the Python skin engine via `upstream/ui-tui/src/theme.ts` (`DEFAULT_THEME`, currently Hermes gold `#FFD700`) and a `caduceus()` hero in `upstream/ui-tui/src/banner.ts` — both must be overridden/retired for Argo. The generator reads the DTCG tokens from the **installed** DS package (`node_modules/@nadicodeai/design-system/dist/tokens/nadicode.dtcg.json`). The generated values are what ship; a DS version bump (normal `npm` update) re-flows on the next build.
...
6. The Argo ASCII `banner_logo` + `banner_hero` (Python) and the `ui-tui` banner art are **authored once** from `logo-mark.svg` / `logo-wordmark.svg` (rendered to Rich-markup ASCII / Ink lines), colored from DS tokens. The art is a static asset; only its colors track tokens.
7. The kawaii spinner faces/verbs and the caduceus appear only in `default`/`DEFAULT_THEME`; the `argo` skin and the ui-tui theme override fully populate `branding`/`spinner`/`banner_*` so no Hermes value leaks through inheritance.
...
- **Build/dist pipeline** — `tools/build.py` assembles `dist/argo/` from: upstream → **quilt patches** (`patches/00NN-*.patch`) → **overlay copy** (`overlay/*` → `dist/argo/`, fails on collision) → rename (`argo-rename.yaml` → `overlay/hermes_cli/_rename_defaults.py`). Quilt workdir over `upstream/` with `QUILT_PATCHES`.
- **Image** — `Dockerfile` (slim + `image-full`), publishes to `ghcr.io/nadicodeai/argo`.
...
The `argo` skin maps the DS **ink surface** onto the skin's ~30 color keys; the same source tokens drive the `ui-tui` `ThemeColors`. Source values (from `nadicode.color.<name>.$value.hex`):
...
```
Build dist:        make build            # upstream + patches + overlay + rename → dist/argo/
Run tests:         make test
Lint:              make lint
...
upstream/                         → pristine Hermes fork — NEVER edit directly
  hermes_cli/skin_engine.py       → _BUILTIN_SKINS, SkinConfig (argo skin added via PATCH)
  ui-tui/src/theme.ts             → DEFAULT_THEME override target (DS hex) — via PATCH/overlay
  ui-tui/src/banner.ts            → caduceus() hero → Argo art (via PATCH)
patches/00NN-*.patch              → quilt patches (next: 0016+, e.g. argo-skin, ui-tui-ds-theme)
patches/asserts/00NN-*.txt        → per-patch assertion contracts (banner/branding invariants)
overlay/                          → files copied verbatim into dist/argo/ (no upstream collision)
  hermes_cli/                     → generated argo-skin color data (build output target)
tools/                            → build.py + new DTCG→{skin,ui-tui theme} generator
  build.py                        → assembly pipeline (generator hooked in here or Docker stage)
argo-rename.yaml                  → rename map (+ display.skin: argo default override candidate)
...
specs/argo-default-skin-design-system.md  → this spec (Phase 1)
...
"argo": {
    ...
    "branding": { "agent_name": "Argo", "response_label": " Argo ", ... },  # fully populated
    "tool_prefix": "┊",
...
// banner.ts caduceus() hero replaced with Argo mark art
```
...
- **Unit (skin engine):** `argo` skin loads/validates/resolves all color keys; `set_active_skin("argo")` succeeds; other skins' inheritance still works.
- **Default selection:** with the dist config, `get_active_skin().name == "argo"`; branding resolves to "Argo" with **no** "NOUS HERMES"/caduceus/kawaii fallback (covers the `skin_name == "default"` branch at `cli.py:2884`).
...
- **Leakage / pristine:** `make leakage-static` finds no residual Hermes-only strings in the `argo` skin / ui-tui theme / dist; `make check-upstream-pristine` confirms `upstream/` untouched; banner OSC-8 invariant + rename gates (`check_wire_identifiers`, `check_no_china_in_docs`) green.
...
- **Always:**
  ...
  - Keep every existing skin and the rename layer intact; the `argo` skin is purely additive.
...
- **Ask first:**
  - The skin-registration mechanism (built-in via patch vs. baked user-skin file) and where `display.skin: argo` is set.
  ...
  - The final Argo ASCII art (logo/hero) before it ships.
...
**Phase 1 (this spec):**
1. `docker run ghcr.io/nadicodeai/argo` (fresh) → CLI banner shows **Argo** art (no caduceus), **DS ink/dark** colors, **neutralized** spinner; `get_active_skin().name == "argo"` by default.
2. `argo --tui` (`ui-tui`) renders with DS hex colors (Ink) and Argo banner art, consistent with the terminal skin.
...
5. No "NOUS HERMES", caduceus, or kawaii/war glyph leaks through the `argo` skin or ui-tui theme on any Phase-1 surface.
...
7. `argo dashboard` renders in the NadicodeAI DS theme with `@nous-research/ui` removed from the shipped web bundle.
...
2. **Default-skin switch location:** `argo-rename.yaml`, a config-default patch, or `overlay`? (The `cli.py:2884` branch is already confirmed safe for a non-`default` name.)
...
8. **Native (non-Docker) installs:** the fork is Docker-only, but if any native install path exists, confirm whether it should also default to `argo` or remain `default`.
