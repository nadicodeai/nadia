...
from nadia_portal import portal_store
...
# ---------------------------------------------------------------------------
# CLI wiring — the plugin registers `connect` under the `portal` command group
# via ctx.register_cli_command(..., parent="portal"); the loader creates the
# leaf parser and calls add_connect_arguments() on it (see nadia_portal.register).
# ---------------------------------------------------------------------------

CONNECT_HELP = "Connect this Local Profile to a Nadia Agent in the NadicodeAI portal"
CONNECT_DESCRIPTION = (
...
def add_connect_arguments(connect_parser: Any) -> None:
    """Add the ``connect`` subcommand's arguments to an existing leaf parser."""
