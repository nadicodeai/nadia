"""Centralized logging setup for Nadia Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.nadia/logs/`` (profile-aware via ``get_nadia_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``nadia_cli.web_server``,
    ``nadia_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
# essentially always in Nadia (TUI, gateway, ``hy_memory`` server, MCP
...
from nadia_constants import get_config_path, get_nadia_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_nadia_session_injector", False):
    ...
    _session_record_factory._nadia_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``nadia logs --component``.
COMPONENT_PREFIXES = {
    ...
    # ``nadia logs --component gateway``.
    "gateway": ("gateway", "nadia_plugins", "plugins.platforms"),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("nadia_cli", "cli"),
    "cron": ("cron",),
    "gui": (
        "nadia_cli.web_server",
        "nadia_cli.pty_bridge",
        "tui_gateway",
        "uvicorn",
    ),
}
...
def setup_logging(
    *,
    ...
    """Configure the Nadia logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    nadia_home
        Override for the Nadia home directory.  Falls back to
        ``get_nadia_home()`` (profile-aware).
    log_level
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_nadia_verbose", False):
                return

    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._nadia_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the nadia group. However, both ``_open()`` (initial creation)
    ...
    def __init__(self, *args, **kwargs):
        from nadia_cli.config import is_managed
