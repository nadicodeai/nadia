/**
 * Nadia Dashboard Plugin SDK — typed contract (SPIKE)
 * ====================================================
 *
 * This is the public type surface for ``window.__NADIA_PLUGIN_SDK__`` and
 * ``window.__NADIA_PLUGINS__``, the globals the dashboard host exposes to
 ...
 * window.__NADIA_PLUGIN_SDK__`` because:
 *   1. The runtime object is assembled from many internal modules
 *      (``@/lib/api``, local UI helpers, …). Deriving the type would
 ...
 *
 * Versioning: bump ``NadiaPluginSDK["sdkVersion"]`` (and the
 ...
 *   - Ship as a published ``@nadia/dashboard-plugin-sdk`` types package, or
 ...
 *     (pulls local UI types into the contract) or is the loose
 ...
 * of calling ``fetch`` with a hand-read ``window.__NADIA_SESSION_TOKEN__``.
 */
...
// ---------------------------------------------------------------------------
// Registry surface (window.__NADIA_PLUGINS__)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// SDK surface (window.__NADIA_PLUGIN_SDK__)
// ---------------------------------------------------------------------------

export interface NadiaPluginSDK {
  ...
  /**
   * Shared UI primitives (NadicodeAI UI / shadcn). Typed permissively at the
...
declare global {
  interface Window {
    __NADIA_PLUGIN_SDK__?: NadiaPluginSDK;
    __NADIA_PLUGINS__?: PluginRegistry;
  }
}
